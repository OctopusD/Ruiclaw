...
RuiClaw can act as a WebSocket server, allowing external clients (web apps, CLIs, scripts) to interact with the agent in real time via persistent connections.
...
async def main():
    async with websockets.connect("ws://127.0.0.1:8765/?client_id=alice") as ws:
        ...
        await ws.send(json.dumps({"content": "Hello ruiclaw!"}))
...
| `trustedProxyAuth.assertionHeader` | string | — | Header injected by the identity-aware proxy after successful authentication. Routing/client metadata headers (`Host`, `Forwarded`, `X-Forwarded-*`, `X-Real-IP`, `CF-Connecting-IP`) are rejected; ruiclaw trusts the remaining header's non-empty value but does not cryptographically validate it. |
...
1. Client sends `GET {tokenIssuePath}` with `Authorization: Bearer {tokenIssueSecret}` (or `X-RuiClaw-Auth` header).
...
identity-aware reverse proxy authenticates the user before connecting to ruiclaw.
...
RuiClaw deliberately uses only `connection.remote_address` for the peer check.
It never uses `X-Forwarded-For`, `Forwarded`, `X-Real-IP`, `CF-Connecting-IP`,
or `X-Forwarded-Host` to decide whether the proxy is trusted. RuiClaw trusts the
...
untrusted clients can connect directly to the ruiclaw listener.
...
ruiclaw over the configured loopback address and supplies a non-empty assertion.
Keep ruiclaw firewalled from untrusted clients; this configuration is not a
...
`chat_id` is a *capability*: anyone holding a valid WebSocket auth credential and the chat_id can attach to that conversation and see its output. This is safe for ruiclaw's local, single-user model. Multi-tenant deployments should namespace chat_ids per user (or introduce a per-tenant auth gate) — ruiclaw does not do this today.
...
- An HTTP file server serving the ruiclaw media directory
