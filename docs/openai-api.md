# RuiClaw OpenAI-Compatible API: Run a Local Agent Behind /v1/chat/completions

ruiclaw can expose a minimal OpenAI-compatible endpoint for local integrations:
...
Run the CLI check first. If `ruiclaw agent -m "Hello!"` fails, fix provider or config setup before debugging the API server. By default, the API binds to `127.0.0.1:8900`. You can change this in `config.json`.
...
server to all interfaces with `api.host: "0.0.0.0"` or `"::"`, ruiclaw requires
...
curl http://127.0.0.1:8900/v1/models \
  -H "Authorization: Bearer $RUICLAW_API_KEY"
```
...
If `channel` points to a channel that is not enabled in your config, ruiclaw will queue the outbound event but no platform delivery will occur.
