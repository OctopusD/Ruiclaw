...
Config file: `~/.ruiclaw/config.json`
...
The generated `config.json` uses camelCase keys such as `apiKey` and `intervalS`. snake_case keys are also accepted for compatibility, but the docs prefer camelCase because that is what ruiclaw writes back to disk.
...
> [!NOTE]
> If your config file is older than the current schema, run `ruiclaw onboard --refresh`. ruiclaw adds missing default fields while preserving your existing values.
...
| Deploy the gateway | [`guides/deploy-ruiclaw-gateway.md`](./guides/deploy-ruiclaw-gateway.md) |
...
|---|---|---|---|
| Make the first model reply work | `providers.<name>.apiKey`, optional `providers.<name>.apiBase`, `modelPresets.<preset>`, `agents.defaults.modelPreset` | `ruiclaw status`, then `ruiclaw agent -m "Hello!"` | [Providers](#providers), [Model Presets](#model-presets) |
| Add fallback models | `modelPresets.<fallback>`, `agents.defaults.fallbackModels` | `ruiclaw status`, then a normal agent run | [Model Fallbacks](#model-fallbacks) |
| Keep secrets out of the config file | `${ENV_VAR}` placeholders inside any string value | Start ruiclaw from the same environment that sets the variable | [Environment Variables for Secrets](#environment-variables-for-secrets) |
| Open the bundled WebUI | `channels.websocket.enabled`, optional `channels.websocket.port`, `channels.websocket.tokenIssueSecret` | `ruiclaw webui` | [Channel Settings](#channel-settings), [WebSocket docs](./websocket.md) |
| Connect one chat app | `channels.<channel>.enabled`, channel credentials, optional pairing or `channels.<channel>.allowFrom` | `ruiclaw channels status`, then `ruiclaw gateway --verbose` | [Channel Settings](#channel-settings), [Chat Apps](./chat-apps.md) |
...
| Add external tools through MCP | `tools.mcpServers.<name>` | Start `ruiclaw gateway --verbose` and check startup/tool logs | [MCP](#mcp-model-context-protocol) |
...
| Tune request timeouts or process concurrency | `RUICLAW_STREAM_IDLE_TIMEOUT_S`, `RUICLAW_MAX_CONCURRENT_REQUESTS` | Start ruiclaw from the same environment and inspect startup/runtime logs | [Runtime Environment Variables](#runtime-environment-variables) |
| Run multiple isolated bots | separate `--config` and `--workspace` paths, plus distinct `gateway.port` or channel ports when processes run together | Use the same explicit paths with `ruiclaw status`, `agent`, `webui`, `gateway`, and `serve` | [Multiple Instances](./multiple-instances.md), [CLI Reference](./cli-reference.md) |
...
Any string value in `config.json` can use `${VAR_NAME}`. Resolution runs once at startup, in memory only — resolved values are never written back to disk, so editing config through `ruiclaw onboard` or the WebUI preserves the placeholder.

If a referenced variable is unset, ruiclaw fails fast and reports the exact config field
and variable name without echoing the field value. Run `ruiclaw status` with the same
...
Pick whatever fits your deployment — ruiclaw only reads `os.environ` at startup, so any mechanism that populates the process environment works.

**systemd** — use `EnvironmentFile=` in the service unit to load variables from a file that only the deploying user can read:

```ini
# /etc/systemd/system/ruiclaw.service (excerpt)
[Service]
EnvironmentFile=/home/youruser/ruiclaw_secrets.env
User=ruiclaw
ExecStart=...
```

```bash
# /home/youruser/ruiclaw_secrets.env (mode 600, owned by youruser)
...
```bash
docker run --rm --env-file=./ruiclaw.env \
  -v ~/.ruiclaw:/home/ruiclaw/.ruiclaw \
...
op run --env-file=.env.tpl -- ruiclaw agent

# pass (passwordstore.org)
ANTHROPIC_API_KEY="$(pass show api/anthropic)" ruiclaw agent

# Bitwarden
ANTHROPIC_API_KEY="$(bw get password api/anthropic)" ruiclaw agent
```

## Runtime Environment Variables

These variables are process-level switches. Set them in the same terminal, service unit, container, or supervisor that starts ruiclaw.

### Runtime controls

| Variable | Default | Description |
|----------|---------|-------------|
| `RUICLAW_MAX_CONCURRENT_REQUESTS` | Unlimited | Maximum concurrently running inbound agent requests. Set a positive integer to apply a cap; unset, `0`, or a negative value means unlimited. |
| `RUICLAW_LLM_TIMEOUT_S` | Unused | Model calls use streaming idle timeouts instead of a fixed total duration. Use `RUICLAW_STREAM_IDLE_TIMEOUT_S` to control stalled requests. |
| `RUICLAW_STREAM_IDLE_TIMEOUT_S` | `90` | Maximum idle wait, in seconds, for model streams, including internal tasks such as Dream, memory archiving, title generation, and Heartbeat evaluation. Each stream event renews the wait, including reasoning and tool-call deltas. Invalid or non-positive values are ignored; values above `3600` are clamped. |
| `RUICLAW_OPENAI_COMPAT_TIMEOUT_S` | `120` | HTTP request timeout, in seconds, for direct non-streaming OpenAI-compatible provider calls. Streaming calls use `RUICLAW_STREAM_IDLE_TIMEOUT_S`. Invalid or non-positive values are ignored. |
| `RUICLAW_WORKSPACE_SANDBOX_ENFORCED` | unset | Marks that an external workspace sandbox is already enforced. Truthy values (`1`, `true`, `yes`, `on`, `enabled`) use `RUICLAW_WORKSPACE_SANDBOX_PROVIDER` as the label; any other non-false value is treated as the provider name. |
| `RUICLAW_WORKSPACE_SANDBOX_PROVIDER` | `unknown` | Display label for the external workspace sandbox when `RUICLAW_WORKSPACE_SANDBOX_ENFORCED` is truthy, for example `macos_app_sandbox` or `bwrap`. |
| `RUICLAW_SANDBOX_ENFORCED` | unset | Legacy compatibility alias for `RUICLAW_WORKSPACE_SANDBOX_ENFORCED`. |
| `RUICLAW_TMUX_SOCKET_DIR` | `${TMPDIR:-/tmp}/ruiclaw-tmux-sockets` | Socket directory used by the bundled `tmux` skill scripts. |

### Installer, build, and WebUI development

| Variable | Default | Description |
|----------|---------|-------------|
| `RUICLAW_BIN_DIR` | `$HOME/.local/bin` | Installer launcher directory on macOS/Linux. |
| `RUICLAW_VENV` | `$HOME/.ruiclaw/venv` | Managed virtual environment path used by the installer fallback. |
| `RUICLAW_SKIP_WIZARD` | unset | Set to `1` to skip automatic WebUI or wizard setup after one-command install. |
| `RUICLAW_SKIP_WEBUI_BUILD` | unset | Set to `1` to skip bundling the WebUI during package builds. |
| `RUICLAW_FORCE_WEBUI_BUILD` | unset | Set to `1` to rebuild the bundled WebUI even when `ruiclaw/web/dist/index.html` already exists. |
| `RUICLAW_EXTRAS` | unset | Docker build argument containing comma-separated Python extras such as `bedrock`. |
| `RUICLAW_CHANNELS` | `whatsapp` | Docker build argument containing comma-separated channels whose manifest dependencies are preinstalled. |
| `RUICLAW_API_URL` | `http://127.0.0.1:8765` | Gateway target for the Vite WebUI dev server proxy. |

Internal variables such as `RUICLAW_RESTART_*` and `RUICLAW_PATH_*` are set by ruiclaw itself and are not a supported user configuration surface.

## Langfuse Observability

ruiclaw can trace OpenAI-compatible provider calls through Langfuse's OpenAI SDK wrapper. This is configured with environment variables, not `config.json`.

Install the optional package in the same Python environment that runs ruiclaw:

```bash
ruiclaw plugins enable langfuse
```

Set Langfuse credentials before starting `ruiclaw agent`, `ruiclaw gateway`, or `ruiclaw serve`:
...
When `LANGFUSE_SECRET_KEY` is set and the `langfuse` package is installed, ruiclaw uses `langfuse.openai.AsyncOpenAI` for OpenAI-compatible providers so model requests are sent to Langfuse in the background. If the secret key is set but `langfuse` is missing, ruiclaw logs a warning and falls back to the regular OpenAI client.
...
Tracing covers the providers that go through ruiclaw's OpenAI-compatible client path. Native providers that do not use that client may not produce Langfuse OpenAI-wrapper traces.
...
> - **MiniMax Coding Plan**: Exclusive discount links for the ruiclaw community: [Overseas](https://platform.minimax.io/subscribe/coding-plan?code=9txpdXw04g&source=link) · [Mainland China](https://platform.minimaxi.com/subscribe/token-plan?code=GILTJpMTqZ&source=link)
> - **MiniMax (Mainland China)**: If your API key is from MiniMax's mainland China platform (minimaxi.com), set `"apiBase": "https://api.minimaxi.com/v1"` in your minimax provider config.
> - **MiniMax thinking mode**: `providers.minimaxAnthropic` is the config block for `reasoningEffort` / thinking mode. MiniMax exposes that capability through its Anthropic-compatible endpoint, so ruiclaw keeps it as a separate provider instead of guessing MiniMax-specific thinking parameters on the generic OpenAI-compatible `minimax` endpoint. It uses the same `MINIMAX_API_KEY`. Default Anthropic-compatible base URL: `https://api.minimax.io/anthropic`; for mainland China use `https://api.minimaxi.com/anthropic`.
> - **Kimi Coding Plan**: Use `providers.kimiCoding` with `provider: "kimi_coding"` for Kimi's dedicated Anthropic Messages API endpoint. The endpoint requires a Claude-compatible `User-Agent`; ruiclaw sends `claude-code/0.1.0` by default, and you can override it with `extraHeaders.User-Agent` if your account requires a different value.
...
| `volcengine` | LLM (VolcEngine, pay-per-use) | [Coding Plan](https://www.volcengine.com/activity/codingplan?utm_campaign=ruiclaw&utm_content=ruiclaw&utm_medium=devrel&utm_source=OWO&utm_term=ruiclaw) · [volcengine.com](https://www.volcengine.com) |
| `volcengine_coding_plan` | LLM (VolcEngine Coding Plan subscription endpoint) | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=ruiclaw&utm_content=ruiclaw&utm_medium=devrel&utm_source=OWO&utm_term=ruiclaw) |
| `byteplus` | LLM (VolcEngine international, pay-per-use) | [Coding Plan](https://www.byteplus.com/en/activity/codingplan?utm_campaign=ruiclaw&utm_content=ruiclaw&utm_medium=devrel&utm_source=OWO&utm_term=ruiclaw) · [byteplus.com](https://www.byteplus.com) |
| `byteplus_coding_plan` | LLM (BytePlus Coding Plan subscription endpoint) | [byteplus.com](https://www.byteplus.com/en/activity/codingplan?utm_campaign=ruiclaw&utm_content=ruiclaw&utm_medium=devrel&utm_source=OWO&utm_term=ruiclaw) |
...
| `moonshot` | LLM (Moonshot/Kimi) | [platform.kimi.com](https://platform.kimi.com?aff=ruiclaw) |
| `kimi_coding` | LLM (Kimi Coding Plan, Anthropic Messages API) | [platform.kimi.com](https://platform.kimi.com?aff=ruiclaw) |
...
| `openai_codex` | LLM (Codex, OAuth) | `ruiclaw provider login openai-codex --set-main` |
| `xai_grok` | LLM (Grok, OAuth) | `ruiclaw provider login xai-grok --set-main` |
| `github_copilot` | LLM (GitHub Copilot, OAuth) | `ruiclaw provider login github-copilot` |
| `qianfan` | LLM (Baidu Qianfan) | [cloud.baidu.com](https://cloud.baidu.com/doc/qianfan/s/Hmh4suq26) |

<details>
<summary><b>OpenAI</b></summary>

By default, OpenAI uses `apiType: "auto"`: ruiclaw calls Chat Completions normally and routes GPT-5/o-series or explicit `reasoningEffort` requests through the Responses API when useful. You can force a specific API surface:
...
`extraBody` follows the selected OpenAI API surface. With Chat Completions, ruiclaw passes
...
API body shape; ruiclaw merges ordinary top-level fields into the Responses request body, appends
...
fields. A hosted search tool replaces ruiclaw's same-name local `web_search` function for that
...
ruiclaw preserves Responses conversation state automatically for OpenAI Responses, OpenAI Codex, Azure OpenAI, DeepSeek V4, and compatible GitHub Copilot models.
...
> `apiBase` remains mandatory in both modes — it's your Azure resource endpoint and cannot be inferred. If neither `apiKey` is set nor `azure-identity` is installed, the provider raises a clear error pointing you at `ruiclaw plugins enable azure`.
...
```bash
ruiclaw plugins enable bedrock
```

> [!NOTE]
> If you configured Bedrock before `boto3` became an optional dependency, run
> `ruiclaw plugins enable bedrock` after upgrading. Otherwise the provider will
...
You can also set `providers.bedrock.apiKey` to a Bedrock API key; ruiclaw exports it as `AWS_BEARER_TOKEN_BEDROCK` for the AWS SDK.
...
Claude Opus 4.7 does not accept `temperature`, `top_p`, or `top_k`; ruiclaw omits `temperature` automatically for this model. If `reasoningEffort` is set to `low`, `medium`, `high`, `max`, or `adaptive`, ruiclaw sends Bedrock's adaptive thinking parameter.
...
Use Bedrock model IDs or inference profile IDs with a `bedrock/` prefix in ruiclaw config. ruiclaw removes the prefix before calling AWS.
...
Model-specific fields can be supplied with `extraBody`; ruiclaw merges it into Converse `additionalModelRequestFields`:
...
Current scope: ruiclaw passes `messages`, `system`, `inferenceConfig`, `toolConfig`, and `additionalModelRequestFields`. Bedrock Prompt Management, Guardrails, `serviceTier`, and other top-level Converse options are not first-class config fields yet.
...
```bash
ruiclaw agent -m "Reply with one short sentence."
```
...
```bash
ruiclaw provider login openai-codex --set-main
```
...
from that response; if discovery is unavailable, ruiclaw keeps a small built-in
...
```bash
ruiclaw provider login xai-grok --set-main
ruiclaw agent -m "Hello from Grok."
```
...
This is xAI subscription OAuth, not X Developer OAuth. ruiclaw follows the
...
`~/.ruiclaw/auth/xai.json`), separately from Grok Build so rotating refresh
...
GitHub Copilot uses OAuth instead of API keys. Requires a [GitHub account with a plan](https://github.com/features/copilot/plans) configured. No `providers.github_copilot` block is needed in `config.json`; `ruiclaw provider login` stores the OAuth session outside config.

After login, the WebUI loads the account-specific Copilot model catalog online.
Only models compatible with ruiclaw's current chat-completions or Responses
transport are shown.

For GitHub Enterprise / Copilot for Business, set the endpoint overrides you need before login:
```bash
export RUICLAW_GITHUB_COPILOT_CLIENT_ID="your-enterprise-client-id"
export RUICLAW_GITHUB_DEVICE_CODE_URL="https://ghe.example/login/device/code"
export RUICLAW_GITHUB_ACCESS_TOKEN_URL="https://ghe.example/login/oauth/access_token"
export RUICLAW_GITHUB_USER_URL="https://api.ghe.example/user"
export RUICLAW_COPILOT_TOKEN_URL="https://api.ghe.example/copilot_internal/v2/token"
export RUICLAW_COPILOT_BASE_URL="https://copilot-api.ghe.example"
```

**1. Login:**
```bash
ruiclaw provider login github-copilot
```

**2. Set model** (merge into `~/.ruiclaw/config.json`):
```json
{
  "modelPresets": {
    "copilot": {
...
ruiclaw agent -c ~/.ruiclaw-telegram/config.json -m "Hello!"

# One-off workspace override on top of that config
ruiclaw agent -c ~/.ruiclaw-telegram/config.json -w /tmp/ruiclaw-telegram-test -m "Hello!"
```
...
OpenCode Zen and OpenCode Go are available through ruiclaw's built-in
...
| Provider | Default API base | Model prefix accepted by ruiclaw |
|----------|------------------|-----------------------------------|
...
provider-specific model endpoints, and `chat/completions`. ruiclaw's OpenCode
...
LongCat is available through ruiclaw's built-in OpenAI-compatible provider flow. The default API base already points to `https://api.longcat.chat/openai/v1`, so you usually only need to set `apiKey`.
...
Ant Ling is available through ruiclaw's built-in OpenAI-compatible provider flow. The default API base points to `https://api.ant-ling.com/v1`, so you usually only need to set `apiKey`.
...
Some OpenAI-compatible gateways expose request-body extensions such as vLLM guided decoding or local sampling controls. Put those under `extraBody`; ruiclaw merges them into the chat-completions request body after its provider defaults:
...
If a custom OpenAI-compatible endpoint exposes a provider-specific thinking toggle, set `thinkingStyle` so ruiclaw can translate `reasoningEffort` into the right request body. Supported styles are `thinking_type` (`{"thinking":{"type":"enabled"}}`), `enable_thinking` (`{"enable_thinking": true}`), and `reasoning_split` (`{"reasoning_split": true}`):
...
**2. Add to config** (partial — merge into `~/.ruiclaw/config.json`):
```json
{
  "providers": {
    "ollama": {
...
[Atomic Chat](https://atomic.chat/) is a local-first desktop app that exposes an **OpenAI-compatible** HTTP API (default `http://localhost:1337/v1`). This setup applies when you want to run ruiclaw against a model on your own machine instead of a hosted API provider.
...
**3. Add to config** (partial — merge into `~/.ruiclaw/config.json`):
...
String entries are preset names, not raw model names. In the example above, `"deep"` means `modelPresets.deep`; ruiclaw will not interpret it as a provider model ID. Changing a preset updates both `/model <preset>` switching and any fallback chain that references it.
...
Failover normally runs when the primary provider returns a fallbackable model/provider error before any answer text has been streamed. Stream-stall timeouts are the recovery exception: if the provider already emitted partial answer text and then stalls, ruiclaw closes the current stream segment and retries/fails over in a new segment. Typical fallback cases include timeouts, connection errors, 5xx server errors, 429 rate limits, overloads, authentication/permission failures such as invalid or expired credentials, and quota/balance exhaustion. It does not run for malformed requests, content filtering/refusals, or context-length/message-format errors.

If fallback candidates use smaller `contextWindowTokens` values, ruiclaw builds context using the smallest window in the active chain so every candidate can receive the same prompt.
...
Global settings that apply to all channels. Configure under the `channels` section in `~/.ruiclaw/config.json`:
...
When a channel `send()` raises, ruiclaw retries at the channel-manager layer. By default, `channels.sendMaxRetries` is `3`, and that count includes the initial send.
...
>
> If a channel is completely unreachable, ruiclaw cannot notify the user through that same channel. Watch logs for `Failed to send to {channel} after N attempts` to spot persistent delivery failures.

## Web Tools

ruiclaw incorporates basic tools for accessing the web. These include searching via APIs, and fetching arbitrary web pages in Markdown format. They are enabled by default, and can be configured in `~/.ruiclaw/config.json` under `tools.web`.
...
ruiclaw supports multiple web search providers. Configure in `~/.ruiclaw/config.json` under `tools.web.search`.
...
ruiclaw by default uses [Jina Reader](https://jina.ai/reader/), a third-party API, to convert arbitrary pages into Markdown format for easy digestion by the LLM, with a local fallback based on [readability-lxml](https://github.com/buriy/python-readability) if the former fails.
...
ruiclaw supports [MCP](https://modelcontextprotocol.io/) — connect external tool servers and use them as native agent tools.
...
ruiclaw opens the server's authorization page and handles the callback through
...
registration data are stored in the ruiclaw data directory under
...
ruiclaw can register the matching HTTPS callback and finish automatically. A
loopback WebUI may use HTTP. When a remote WebUI is served over plain HTTP,
ruiclaw instead registers a localhost callback and asks you to paste the complete
...
`enabledTools` accepts either the raw MCP tool name (for example `read_file`) or the wrapped ruiclaw tool name (for example `mcp_filesystem_write_file`).
...
> [!TIP]
> For production deployments, set both `"restrictToWorkspace": true` and `"tools.exec.sandbox"` (`"bwrap"` on Linux, `"seatbelt"` on macOS) in your config. `restrictToWorkspace` enables ruiclaw's application-level workspace guards; `tools.exec.sandbox` provides process-level isolation for shell commands.
...
> workspace, that project becomes the normal file and shell boundary. RuiClaw
...
| `tools.webuiAllowRemotePackageInstall` | `false` | When `false`, the WebUI can install missing optional packages only from a browser opened on the same machine as ruiclaw. Set to `true` only when a trusted remote admin is allowed to install Python packages into this ruiclaw environment. |
...
The gateway can run a protected heartbeat cron job that periodically checks `HEARTBEAT.md` in the active workspace. This is enabled by default when you run `ruiclaw gateway`.
...
By default, ruiclaw allows four subagents to run at the same time. Additional subagents wait for capacity instead of being rejected. Lower the limit if a local model server cannot hold multiple KV caches, or raise it when the provider can handle more parallel work:
...
When a user is idle for longer than a configured threshold, ruiclaw **proactively** compresses the older part of the session context into a summary while keeping a recent legal suffix of live messages. This reduces token cost and first-token latency when the user returns — instead of re-processing a long stale context with an expired KV cache, the model receives a compact summary, the most recent live context, and fresh input.
...
By default, ruiclaw uses `UTC` for runtime time context. If you want the agent to think in your local time, set `agents.defaults.timezone` to a valid [IANA timezone name](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones):
...
By default, each channel × chat ID combination gets its own session. If you use ruiclaw across multiple channels (e.g. Telegram + Discord + CLI) and want them to share the same conversation, enable `unifiedSession`:
...
ruiclaw ships with built-in skills, and your workspace can also define custom skills under `skills/`. If you want to hide specific skills from the agent, set `agents.defaults.disabledSkills` to a list of skill directory names:
...
Enabled plugins run as the ruiclaw user; permissions are descriptive, not an OS sandbox. The optional `extensions.dev.ruiclaw.logo` accepts a contained PNG, JPEG, or WebP up to 256 KiB.
