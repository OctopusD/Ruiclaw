...
Use this page when you want to understand ruiclaw before changing advanced settings. It explains the moving parts without requiring you to read the source first.

If you want source-file ownership and extension points, read [`architecture.md`](./architecture.md) after this page.

## Runtime Shape

ruiclaw has one small core loop and several ways to enter it:
...
The simplest path is `ruiclaw agent -m "Hello!"`: one inbound message goes through the agent loop and prints the reply in your terminal. The long-running path is `ruiclaw gateway`: channels receive messages from chat apps or the WebUI, publish them to the same agent loop, and send replies back to the originating channel.

## Config vs Workspace

The default instance lives under `~/.ruiclaw/`:

| Path | Meaning |
|---|---|
| `~/.ruiclaw/config.json` | Instance configuration: providers, model defaults, channels, tools, gateway, API, and runtime options |
| `~/.ruiclaw/workspace/` | Agent workspace: memory, heartbeat tasks, cron jobs, skills, and generated artifacts |
| `~/.ruiclaw/sessions/<workspace-id>/` | Session history stored outside the agent-accessible workspace; the opaque ID follows workspace moves |

You can override both with command flags:

```bash
ruiclaw onboard --config ./bot-a/config.json --workspace ./bot-a/workspace
...
The config file controls what ruiclaw may use. The workspace is where ruiclaw keeps state for that instance.
...
`config.json` accepts both camelCase and snake_case keys. The docs use camelCase because ruiclaw writes config back to disk with camelCase aliases, for example `apiKey`, `modelPresets`, `intervalS`, and `maxToolResultChars`.

Most examples are partial snippets. Merge them into the existing file created by `ruiclaw onboard`; do not replace the whole file unless you want to reset the instance.
...
|---|---|---|
| CLI one-shot | `ruiclaw agent -m "..."` | First-run checks, scripts, and quick local questions |
| CLI interactive | `ruiclaw agent` | Terminal chat with persistent session history |
| Gateway | `ruiclaw gateway` | Chat apps, WebUI, heartbeat, Dream, and long-running service mode |
| OpenAI-compatible API | `ruiclaw serve` | Programmatic access through `/v1/chat/completions` |
| WebUI | `ruiclaw webui` | Prepare the local WebUI, start the gateway, and open the browser workbench |
...
1. If the active preset provider or implicit default provider is not `"auto"`, ruiclaw uses that provider.
2. If provider is `"auto"`, ruiclaw tries to infer the provider from the model name, configured API keys, local provider base URLs, or gateway providers.
...
ruiclaw uses two related stores:
...
The configured workspace contains a `.ruiclaw/workspace-id` file. It contains only an
opaque random identifier—never conversation content or credentials. Keep it with workspace
backups: it lets ruiclaw find the same external session namespace after the workspace is
...
Agent Plugins are ruiclaw's common package and activation boundary for
...
When `ruiclaw gateway` starts, it runs workspace-scoped automations and
registers system jobs:

- `dream`, when `agents.defaults.dream.enabled` is true;
- `heartbeat`, when `gateway.heartbeat.enabled` is true.

Heartbeat reads `<workspace>/HEARTBEAT.md`. If the file has tasks under `## Active Tasks`, ruiclaw executes them and sends only useful/actionable results to the most recently active chat target. Routine "nothing changed" results are suppressed.
...
`ruiclaw trigger <id> "<message>"` when a local script or external service wants
ruiclaw to respond in that session. Webhook servers, third-party auth, and
event-to-message formatting stay outside ruiclaw. Trigger deliveries are stored
