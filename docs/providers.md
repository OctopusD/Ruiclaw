...
The docs show concrete provider names so the JSON is copyable, not because ruiclaw ranks providers. Start from the service or endpoint you actually control:
...
| An OAuth-based account | Run the matching `ruiclaw provider login ...` command, then select that provider explicitly in a preset. |
| No provider yet | Pick one outside ruiclaw based on account access, pricing, regional availability, privacy requirements, and the model IDs you need. Then come back with its key and model ID. |
...
The provider config gives ruiclaw credentials and endpoint details. The model preset names the provider/model pair. The agent defaults choose which named preset to use for normal turns. Replace the example provider and model together; mixing an API key from one provider with a model ID from another is the most common first-run failure.
...
|---|---|---|
| `provider` | `modelPresets.<name>.provider` | Which ruiclaw provider adapter should send the request. |
...
You usually omit `apiBase` for hosted built-in providers such as OpenRouter, Anthropic direct, OpenAI direct, Groq, or Bedrock because ruiclaw knows their default endpoints. Set `apiBase` for `custom`, local OpenAI-compatible servers, provider proxies, regional endpoints, or subscription endpoints. Include the API version path when the endpoint requires it, for example `https://api.example.com/v1` or `http://localhost:11434/v1`.

Use `proxy` when one provider must send HTTP traffic through a proxy without changing process-wide `HTTP_PROXY` / `HTTPS_PROXY`. This is supported for providers that use ruiclaw's OpenAI-compatible client, including `openai`, `custom`, named custom providers, OpenRouter-style gateways, local OpenAI-compatible servers, and similar registry entries. It is also supported for `openai_codex` and `xai_grok`, including OAuth token exchange/refresh and model requests. Native provider backends such as `anthropic`, `bedrock`, `azure_openai`, and `github_copilot` reject `proxy`; use their endpoint-specific configuration instead.
...
appended to ruiclaw's generated functions. This keeps unrelated local tools such as `write_file`
...
RuiClaw sends the model ID unchanged, including its provider prefix. Use
...
`opencode-go/<model-id>` for Go. ruiclaw accepts those prefixes and strips them
...
Custom provider keys are treated as direct OpenAI-compatible providers. `apiBase` is required because ruiclaw cannot know the endpoint URL. `apiKey` is optional for local servers or private proxies that do not require one. Choose a name that does not conflict with a built-in provider name or alias, such as `openai`, `openai-codex`, `github-copilot`, or `lm-studio`. Do not set `apiType` on custom provider keys; `apiType` is only for `providers.openai`.

If your custom endpoint documents a nonstandard thinking toggle, set `providers.<name>.thinkingStyle` to `thinking_type`, `enable_thinking`, or `reasoning_split`; ruiclaw then maps `reasoningEffort` onto that provider-specific request body. Leave it unset for ordinary OpenAI-compatible endpoints.
...
Use an inference-enabled model ID exactly as ModelScope publishes it (usually `Namespace/model-name`). The default base URL is `https://api-inference.modelscope.cn/v1`; override `providers.modelscope.apiBase` only if your account routes through a different host. Chat model IDs may optionally be prefixed with `modelscope/`; ruiclaw strips that routing prefix before sending the request.
...
Start Ollama separately, then point ruiclaw at the OpenAI-compatible endpoint.
...
cache reuse, diagnose the rendered template before changing ruiclaw's context or
memory settings. The
...
model catalog, so newly available subscription models appear without a ruiclaw
release. Online metadata is cached and enriched with ruiclaw's curated labels;
if xAI is temporarily unavailable, ruiclaw uses the last successful catalog or
...
without invoking a local ruiclaw tool. Credentials are stored under the
active instance's `auth/xai.json` (normally `~/.ruiclaw/auth/xai.json`), not in
...
xAI may change that upstream contract independently of ruiclaw.

For GitHub Copilot:

```bash
ruiclaw provider login github-copilot --set-main
```

The WebUI reads the models enabled for the signed-in Copilot account. ruiclaw
...
String entries in `fallbackModels` are preset names, not raw model names. ruiclaw tries them in order after the active preset. Each fallback preset uses its own `provider`, `model`, `maxTokens`, `contextWindowTokens`, `temperature`, and optional `reasoningEffort`.
...
`fallbackModels` belongs under `agents.defaults`, not inside each preset. If fallback candidates use smaller context windows, ruiclaw builds context using the smallest window in the active chain so every candidate can receive the same prompt. See [`configuration.md#model-fallbacks`](./configuration.md#model-fallbacks) for failure conditions.
...
If `ruiclaw agent -m "Hello!"` fails:
