...
Legacy `tools.myEnabled` / `tools.mySet` keys are auto-migrated on load, and rewritten in-place the next time `ruiclaw onboard` refreshes the config.
...
```text
my(action="set", key="current_project", value="ruiclaw")
