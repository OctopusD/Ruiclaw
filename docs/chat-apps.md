...
Connect ruiclaw to Telegram, Discord, Slack, WeChat, Email, Mattermost, and
...
| Platform | Guide |
|---|---|
| Telegram | [Build a Telegram AI Agent with ruiclaw](./guides/telegram-ai-agent.md) |
| Discord | [Build a Discord AI Agent with ruiclaw](./guides/discord-ai-agent.md) |
| Slack | [Build a Slack AI Agent with ruiclaw](./guides/slack-ai-agent.md) |
| Feishu | [Build a Feishu AI Agent with ruiclaw](./guides/feishu-ai-agent.md) |
| WhatsApp | [Build a WhatsApp AI Agent with ruiclaw](./guides/whatsapp-ai-agent.md) |
| WeChat | [Build a WeChat AI Agent with ruiclaw](./guides/wechat-ai-agent.md) |
| QQ | [Build a QQ AI Agent with ruiclaw](./guides/qq-ai-agent.md) |
| Email | [Build an Email AI Agent with ruiclaw](./guides/email-ai-agent.md) |
| Mattermost | [Build a Mattermost AI Agent with ruiclaw](./guides/mattermost-ai-agent.md) |
...
If that fails, fix installation, config, provider, or model setup first with [`quick-start.md`](./quick-start.md), [`providers.md`](./providers.md), and [`troubleshooting.md`](./troubleshooting.md). Chat apps require `ruiclaw gateway` to stay running after the channel is configured.
...
5. Let ruiclaw install the optional channel support when prompted.
...
Optional package installation is available to a same-machine WebUI by default. Remote browser clients cannot change the Python environment unless an administrator explicitly enables that capability. Run `ruiclaw plugins enable <channel>` locally when the guided install is unavailable.
...
> enable the channel in the same Python environment so ruiclaw installs its
> manifest-declared dependencies:
>
> ```bash
> ruiclaw plugins enable <channel>
> ```
>
> Replace `<channel>` with names such as `telegram`, `slack`, `feishu`,
> `dingtalk`, `matrix`, `qq`, `napcat`, `weixin`, `wecom`, or `msteams`.
> To turn a channel off later, run `ruiclaw plugins disable <channel>`.
> ruiclaw keeps the saved settings, but stops loading that channel after the
> next restart.

## Manual Setup Pattern

Most examples below are snippets to merge into `~/.ruiclaw/config.json`. When a snippet includes `allowFrom`, it is showing a static allowlist. For pairing-based access on supported channels, omit `allowFrom`; Slack and Mattermost also need `dm.policy` set to `"allowlist"` for DMs to issue pairing codes.
...
3. Merge that platform's JSON snippet into `~/.ruiclaw/config.json`.
...
6. Check that ruiclaw can see the configured channel:
...
If `ruiclaw channels status` does not show the channel as enabled, the config snippet is in the wrong place, the channel name is misspelled, or the config file you edited is not the one ruiclaw is reading. If the channel is enabled but messages do not arrive, run `ruiclaw gateway --verbose` and compare the platform-side credentials, event permissions, and allow lists.
...
| **WhatsApp** | QR code scan (`ruiclaw channels login whatsapp`) |
| **WeChat (Weixin)** | QR code scan (`ruiclaw channels login weixin`) |
| **Feishu** | QR code scan (`ruiclaw channels login feishu`) or App ID + App Secret |
...
2. Run `ruiclaw webui`, then open **Settings → Channels → Telegram**.
...
The configuration badge means ruiclaw found a saved token. The live connection
...
```bash
ruiclaw plugins enable telegram
```
...
Telegram uses long polling by default. To receive updates through a webhook, expose a public HTTPS URL that forwards to ruiclaw's local listener and set `mode` to `webhook`:
...
> `webhookSecretToken` is required in webhook mode. Do not expose the local webhook listener directly to the public internet without a reverse proxy or tunnel in front of it. TLS/Host policy is handled by your proxy; ruiclaw only listens on `webhookListenHost:webhookListenPort` and validates Telegram's webhook secret token. `webhookMaxConnections` defaults to `4`; ruiclaw still serializes Telegram updates per conversation before forwarding them to the agent.
>
> `webhookUrl` is the public HTTPS URL registered with Telegram. `webhookPath` is the local path ruiclaw listens on. They often use the same path, but may differ when a reverse proxy or tunnel rewrites the request path.
...
```bash
ruiclaw plugins enable mochat
```

Without these dependencies, Mochat still works through HTTP polling.

**1. Ask ruiclaw to set up Mochat for you**

Simply send this message to ruiclaw (replace `xxx@xxx` with your real email):

```
Read https://raw.githubusercontent.com/HKUDS/MoChat/refs/heads/main/skills/ruiclaw/skill.md and register on MoChat. My Email account is xxx@xxx Bind me as your owner and DM me on MoChat.
```

ruiclaw will automatically register, configure `~/.ruiclaw/config.json`, and connect to Mochat.
...
That's it — ruiclaw handles the rest!
...
If you prefer to configure manually, add the following to `~/.ruiclaw/config.json`:
...
- You need:
  - `userId` (example: `@ruiclaw:matrix.org`)
  - `password`
...
```json
{
  "channels": {
    "whatsapp": {
      "databasePath": "~/.ruiclaw/whatsapp-auth/neonize.db"
    }
  }
}
```
...
- Re-run `ruiclaw channels login whatsapp`; old Baileys bridge auth data is not reused by neonize.
...
Modern WhatsApp can deliver a sender's LID instead of their phone number. ruiclaw
...
```bash
ruiclaw plugins enable feishu
ruiclaw channels login feishu
# Use --force to create/sign in with a new bot
```

Open the printed URL or scan the QR code with Feishu/Lark on your phone. If the optional `qrcode` package is installed, ruiclaw shows a terminal QR code; otherwise it prints the login URL. ruiclaw writes `appId`, `appSecret`, `domain`, and `enabled` under `channels.feishu` in the active config file. Use `--config <path>` to update a non-default config.
...
- **Permissions**:
  ...
  - **Streaming replies** (default in ruiclaw): add **`cardkit:card:write`** (often labeled **Create and update cards** in the Feishu developer console). Required for CardKit entities and streamed assistant text. Older apps may not have it yet — open **Permission management**, enable the scope, then **publish** a new app version if the console requires it.
  - If you **cannot** add `cardkit:card:write`, set `"streaming": false` under `channels.feishu` (see below). The bot still works; replies use normal interactive cards without token-by-token streaming.
- **Events**: Add `im.message.receive_v1` (receive messages)
  - Select **Long Connection** mode (requires running ruiclaw first to establish connection)
...
> `allowFrom`: Add your open_id (find it in ruiclaw logs when you message the bot). Use `["*"]` to allow all users.
...
> - `allowFrom`: Add your openid (find it in ruiclaw logs when you message the bot). Use `["*"]` for public access.
...
```bash
ruiclaw plugins enable dingtalk
```
...
> `files:read` is required to read files users send to ruiclaw. `files:write` is required for ruiclaw to send images, videos, and other file uploads. If you add either scope later, reinstall the Slack app to the workspace and restart ruiclaw so it uses the updated bot token.
...
Give ruiclaw its own email account. It polls **IMAP** for incoming mail and replies via **SMTP** — like a personal email assistant.

**1. Get credentials (Gmail example)**
- Create a dedicated Gmail account for your bot (e.g. `my-ruiclaw@gmail.com`)
...
> - `allowFrom`: Add the sender ID you see in ruiclaw logs for your WeChat account. Use `["*"]` to allow all users.
> - `token`: Optional. If omitted, log in interactively and ruiclaw will save the token for you.
> - `routeTag`: Optional. When your upstream Weixin deployment requires request routing, ruiclaw will send it as the `SKRouteTag` header.
> - `stateDir`: Optional. Defaults to ruiclaw's runtime directory for Weixin state.
...
```bash
ruiclaw channels login weixin --force
```
...
```bash
ruiclaw plugins enable msteams
```
...
> - `mentionOnlyResponse` controls what RuiClaw receives when a user sends only a bot mention (`<at>RuiClaw</at>`). Set to `""` to ignore mention-only messages.
