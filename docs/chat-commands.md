...
When someone sends a DM to the bot and isn't on the allowlist — whether it's a new user or an existing user on a new channel — ruiclaw automatically replies with a **pairing code** (like `ABCD-EFGH`) that expires in 10 minutes. To grant them access:
...
Replace `"Review PR #4502"` with the message you want ruiclaw to receive. The
trigger is bound to the session where it was created, so the message goes back
to that same chat. Keep `ruiclaw gateway` running so trigger messages can be
...
```bash
printf '%s\n' "Review the latest failed CI job" | ruiclaw trigger trg_8K4P2Q9X
```

If an external webhook should wake ruiclaw up, run your own small webhook
service and have it call the trigger command after it builds the final message:

```bash
ruiclaw trigger <trigger-id> "<message>"
```

If you run multiple ruiclaw instances, pass the same config or workspace
...
Periodic background checks are driven by `HEARTBEAT.md` in your workspace (`~/.ruiclaw/workspace/HEARTBEAT.md`). When `ruiclaw gateway` starts, it registers a protected heartbeat cron job by default. Every 30 minutes, that job checks the file; if it finds tasks under `## Active Tasks`, the agent executes them and delivers only results that pass the notification gate to your most recently active chat channel. If there are no active tasks, or the result is routine with nothing useful to report, the heartbeat is skipped silently.
...
**Setup:** edit `~/.ruiclaw/workspace/HEARTBEAT.md` (created automatically by `ruiclaw onboard`):
...
You can change the interval or disable the built-in heartbeat in `~/.ruiclaw/config.json`:
...
> **Note:** The gateway must be running (`ruiclaw gateway`) and you must have chatted with the bot at least once so it knows which channel to deliver to.
