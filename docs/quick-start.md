...
This guide has one goal: get a normal ruiclaw reply in your browser. Do not add chat apps, MCP servers, fallback models, or deployment until this path works.
...
These repository docs describe `main`, which can be newer than the released package. The installer below installs the latest stable release. Check `ruiclaw --version` and use the [matching stable guide](https://ruiclaw.wiki/docs/latest/getting-started/quick-start) if its setup screens differ from this page.
...
The recommended installer keeps ruiclaw out of the system Python environment. On a fresh local desktop, it starts the WebUI when installation finishes.
...
The installer chooses an active virtual environment, `uv`, `pipx`, or a managed environment under `~/.ruiclaw/venv`. It installs the stable PyPI release. At the end it prints the exact command it used to run ruiclaw; if `ruiclaw` is not on `PATH`, reuse that full command in the examples below.
...
| Path | Purpose |
|---|---|
| `~/.ruiclaw/config.json` | Provider, model, WebUI, channel, tool, and runtime settings |
| `~/.ruiclaw/workspace/` | Memory, skills, automations, and generated files |
| `~/.ruiclaw/sessions/<workspace-id>/` | Recent session history stored outside the workspace; the ID remains stable across workspace moves |
...
If the installer-started WebUI is no longer running, run `ruiclaw webui` again. Leave that launcher open; the first-run WebUI is bound to localhost, so other devices on your network cannot reach it.
...
Any normal assistant answer is success. It proves that ruiclaw can load the config, reach the selected model, use the workspace, and serve the browser UI.
...
```bash
ruiclaw gateway --background
ruiclaw gateway status
```

Use `ruiclaw gateway logs`, `restart`, and `stop` to manage that background gateway.
...
universal fallback). While ruiclaw is working, `Enter` sends immediately, `Tab` waits until the
...
If pip reports `externally-managed-environment`, use the recommended installer, `uv tool install ruiclaw-ai`, `pipx install ruiclaw-ai`, or create a virtual environment. Do not force a system-wide install.
...
install keeps Python pointed at the checkout; `ruiclaw` runs `tui/` with Bun, and
`ruiclaw webui` automatically rebuilds `webui/` when its bundled assets are stale. All normal
commands remain the same as a stable install. For development details, follow
[`../CONTRIBUTING.md`](../CONTRIBUTING.md).

If the package is installed but the shell cannot find `ruiclaw`, use the runner that owns the installation. The recommended installer prints the exact command to reuse. Common forms are:

```bash
uv tool run --from ruiclaw-ai ruiclaw --version
pipx run --spec ruiclaw-ai ruiclaw --version
~/.ruiclaw/venv/bin/python -m ruiclaw --version
```

On Windows, the managed-environment form is `& "$HOME\.ruiclaw\venv\Scripts\python.exe" -m ruiclaw --version`. Replace `--version` with `webui`, `onboard --wizard`, or any other arguments you need. Use plain `python -m ruiclaw` only when that Python executable belongs to the environment where ruiclaw was installed.

## Manual Configuration Fallback

Use this only when the wizard is unavailable or you intentionally manage JSON. First run `ruiclaw onboard`, then merge a provider and a named model preset into `~/.ruiclaw/config.json`.
...
# Or one of these
uv tool upgrade ruiclaw-ai
pipx upgrade ruiclaw-ai
python -m pip install -U ruiclaw-ai
```
...
dependencies/assets when launched. Then check `ruiclaw --version`. Run
`ruiclaw onboard --refresh` when you want to add newly introduced default fields while preserving
existing settings.
...
| Symptom | First check |
|---|---|
| `ruiclaw: command not found` | Reuse the installer command or method-specific runner described under [Other Install Methods](#other-install-methods) |
...
| WebUI works but a chat app does not | Check **Settings → Channels**, then run `ruiclaw channels status` |
