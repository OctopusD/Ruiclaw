...
Most examples below are snippets to merge into `~/.ruiclaw/config.json`. Keep any existing sections you still need, and replace placeholder keys such as `${OPENROUTER_API_KEY}` with environment-variable references or real values only on your own machine.
...
| An OpenAI-compatible `/v1` endpoint that is not a named ruiclaw provider | [Custom OpenAI-Compatible Provider](#recipe-custom-openai-compatible-provider) | `apiBase`, optional API key, and the model ID served by that endpoint |
...
| A working agent and a Langfuse project | [Langfuse Tracing](#recipe-langfuse-tracing) | Langfuse env vars in the same process environment that starts ruiclaw |

## How to Use a Recipe

1. Install ruiclaw and run `ruiclaw onboard` once so `~/.ruiclaw/config.json` exists. Use `ruiclaw onboard --wizard` if you prefer prompts over hand-editing JSON.
2. Put secrets in environment variables when possible.
3. Merge the recipe snippet into `~/.ruiclaw/config.json`.
4. Run `ruiclaw status`.
5. Run `ruiclaw agent -m "Hello!"`.
...
Environment variables set this way apply only to the current terminal. For long-running services such as systemd, Docker, LaunchAgent, or a remote shell, set the variables in that service environment before starting ruiclaw.
...
If this fails with `401` or `unauthorized`, check that `OPENROUTER_API_KEY` is visible in the same terminal or service that starts ruiclaw. If it fails with `model not found`, choose a model ID that OpenRouter lists for your account.
...
and `opencode_go` providers in ruiclaw use the OpenAI-compatible
...
```bash
OPENAI_API_KEY="sk-..." ruiclaw agent -m "Hello!"
```

If your shell cannot use inline environment variables, set `OPENAI_API_KEY` first and then run `ruiclaw agent -m "Hello!"`. If the provider rejects `apiType`, remove `apiType` unless you are using a documented OpenAI-specific mode.
...
```bash
ANTHROPIC_API_KEY="sk-ant-..." ruiclaw agent -m "Hello!"
```
...
This recipe applies when your key comes from Kimi's Coding Plan endpoint. RuiClaw uses a dedicated `kimi_coding` provider for this Anthropic Messages API endpoint; do not configure it as a generic `custom` provider.
...
The default base URL is `https://api.kimi.com/coding/v1`. This endpoint requires a Claude-compatible `User-Agent`; ruiclaw sends `claude-code/0.1.0` by default. If your account requires a different value, override it with `providers.kimiCoding.extraHeaders.User-Agent`.

## Recipe: Custom OpenAI-Compatible Provider

This recipe applies to an OpenAI-compatible service that is not a named ruiclaw provider.
...
Verify the endpoint before blaming ruiclaw:
...
If direct Ollama responses are fast but tool-using ruiclaw turns repeatedly evaluate
...
`fallbackModels` belongs under `agents.defaults`. String entries are preset names, not raw model names. ruiclaw tries the active preset first, then the fallback presets in order.

Keep fallback candidates realistic. If the local fallback has a smaller context window, ruiclaw must build context that fits the smallest window in the active chain.
...
Set the environment variables before starting ruiclaw:
...
| Langfuse shows no traces | Env vars are missing, `langfuse` is not installed in the active Python environment, or the provider path is native | Run `python -m pip show langfuse` and restart ruiclaw from the same environment |
