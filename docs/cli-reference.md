...
| Goal | Command | Notes |
|---|---|---|
| Check the install | `ruiclaw --version` | If this fails, try `python -m ruiclaw --version` |
| Create or refresh config | `ruiclaw onboard` | Creates `~/.ruiclaw/config.json` and `~/.ruiclaw/workspace/` |
| Refresh config non-interactively | `ruiclaw onboard --refresh` | Preserves existing values and adds missing default fields without prompting |
| Use guided setup | `ruiclaw onboard --wizard` | Best when you prefer prompts over hand-editing JSON |
| Open the browser workbench | `ruiclaw webui` | Prepares local WebUI settings, starts the gateway, and opens the browser |
| Check readiness without calling a model | `ruiclaw status` | Summarizes config/workspace and validates the active provider/model configuration |
| Send one test message | `ruiclaw -m "Hello!"` | First proof that install, config, provider, model, and workspace all work |
| Chat in the terminal | `ruiclaw` | Interactive local chat; `ruiclaw agent` remains an explicit alias |
| Run the gateway directly | `ruiclaw gateway` | Service/ops command for WebUI, chat apps, cron, and heartbeat |
| Deliver a local trigger | `ruiclaw trigger <id> "message"` | Created first with `/trigger <name>` in the target chat/session |
| Serve an OpenAI-compatible API | `ruiclaw serve` | Starts `/v1/chat/completions`, `/v1/models`, and `/health` |
| Check chat channel setup | `ruiclaw channels status` | Useful before starting `ruiclaw gateway` |
| Manage optional features | `ruiclaw plugins list` | Shows channels and optional capabilities you can turn on |
| Log in to QR/OAuth-style channels | `ruiclaw channels login <channel>` | Used by channels such as WhatsApp and WeChat |
| Log in to OAuth model providers | `ruiclaw provider login <provider>` | Used by OpenAI Codex, xAI subscription, and GitHub Copilot providers |
...
ruiclaw --help
ruiclaw --version
python -m ruiclaw --help
python -m ruiclaw --version
```

`python -m ruiclaw ...` is useful when the package is installed but the `ruiclaw` script is not on `PATH`.

### Coexisting with RuiClaw Desktop

The Python package and RuiClaw Desktop keep separate runtimes, configuration,
workspaces, and state. When a running Desktop release publishes its private
terminal-access descriptor, an interactive bare `ruiclaw` or `ruiclaw webui`
...
Choosing Desktop for `ruiclaw webui` opens its already-running browser workbench
and exits; closing the browser never stops the Desktop gateway. Choosing Desktop
for bare `ruiclaw` opens the terminal UI against that same running backend when
the Desktop host, gateway and terminal client support terminal protocol 1. Older
versions fail explicitly with update guidance. Use explicit `ruiclaw agent` for
...
under `RuiClawBrowserHandoff-v1`, with an explicit current-user-only protected ACL.
...
Any explicit subcommand or option—including `ruiclaw agent`,
`ruiclaw webui --no-open`, config/workspace selectors, help, version, completion,
...
Desktop-only distributions use the narrow `ruiclaw-desktop-tui` entrypoint inside
...
```bash
ruiclaw agent --config ./bot-a/config.json --workspace ./bot-a/workspace -m "Hello"
ruiclaw gateway --config ./bot-a/config.json --workspace ./bot-a/workspace
ruiclaw serve --config ./bot-a/config.json --workspace ./bot-a/workspace
```

Use `--verbose` on long-running processes when you need startup or runtime logs:

```bash
ruiclaw gateway --verbose
ruiclaw serve --verbose
```

Long-running commands keep working until you stop them. Press `Ctrl+C` in that terminal
to stop foreground `ruiclaw gateway` or `ruiclaw serve`. If you started the gateway
with `--background`, use `ruiclaw gateway stop`.

## Setup

| Command | Description |
|---|---|
| `ruiclaw onboard` | Initialize or refresh the default config and workspace |
| `ruiclaw onboard --refresh` | Refresh an existing config without prompting, preserving existing values |
| `ruiclaw onboard --wizard` | Use the interactive setup wizard |
| `ruiclaw onboard --config <path> --workspace <path>` | Initialize or refresh a specific instance |

Default paths:

| Path | Default |
|---|---|
| Config | `~/.ruiclaw/config.json` |
| Workspace | `~/.ruiclaw/workspace/` |

## Status

| Command | Description |
|---|---|
| `ruiclaw status` | Summarize the default config/workspace and check Agent provider/model readiness |
| `ruiclaw status --config <path>` | Check a specific config file |
| `ruiclaw status --workspace <path>` | Show status with a workspace override |

Status does not send a model request. On success, run the printed
`ruiclaw agent -m "Hello!"` command to verify network access and credentials. On failure,
follow the printed WebUI **Settings → Models** or `ruiclaw onboard --wizard` route.

## Agent CLI

| Command | Description |
|---|---|
| `ruiclaw -m "Hello!"` | Send one message and exit |
| `ruiclaw` | Start interactive terminal chat |
| `ruiclaw --session <id>` | Use a WebSocket session key; add `--classic` for another channel |
| `ruiclaw --workspace <path>` | Override workspace |
| `ruiclaw --config <path>` | Use a specific config file |
| `ruiclaw --classic` | Use the classic Python prompt instead of the native terminal UI |
| `ruiclaw --theme auto\|dark\|light` | Auto-detect the terminal appearance or force a TUI palette |
| `ruiclaw --no-markdown` | Use the classic prompt and print plain text instead of Markdown |
| `ruiclaw --logs` | Use the classic prompt and show runtime logs while chatting |
...
ready-to-run `ruiclaw agent --session ...` command for the current session.

## Session Storage and Rollback

Session JSONL files live under `<config-dir>/sessions/<workspace-id>/`, outside the
agent-readable workspace. On the first upgraded start, ruiclaw safely migrates existing
`<workspace>/sessions/*.jsonl` files after verifying an atomic copy. Stop every old ruiclaw
process that uses the workspace before upgrading; old and new binaries must not write the
same session concurrently.

To prepare a downgrade, stop ruiclaw and copy the current sessions back to the path understood
by older releases:

```bash
ruiclaw sessions restore-workspace --config ./bot-a/config.json --workspace ./bot-a/workspace
```
...
Interactive mode uses ruiclaw's native TypeScript terminal UI. It talks to the same local gateway as the WebUI, so streaming, tool progress, and WebSocket sessions share one protocol instead of maintaining a second agent loop. If no gateway is running, either client starts it on demand. The TUI paints immediately while the local gateway starts, then obtains fresh bootstrap credentials and connects in the background. Exiting one TUI or WebUI launcher releases only that client; the last interactive launcher stops the on-demand gateway. A small gateway watchdog also reclaims an on-demand process if its last client crashes. `/detach` promotes the shared gateway to persistent background mode before closing the TUI, so active agent work continues without a connected client. An explicit `ruiclaw gateway --background` starts or promotes the gateway the same way before opening a client. `ruiclaw gateway restart` restarts a detached gateway without changing that lifetime; restart an attached foreground gateway in its owning terminal. `ruiclaw gateway stop` ends either mode.
...
| Command | Description |
|---|---|
| `ruiclaw webui` | Create config/workspace if needed, enable the local WebUI channel after confirmation, start the gateway, open `http://127.0.0.1:8765`, and follow new gateway logs |
| `ruiclaw webui --background` | Deprecated; prints the equivalent explicit `ruiclaw gateway --background` command and exits |
| `ruiclaw webui --dev` | Start the gateway and Vite together at `http://127.0.0.1:5173`, with live frontend updates |
| `ruiclaw webui --no-open` | Prepare and start the WebUI without opening a browser |
| `ruiclaw webui --port <port>` | Set the WebUI/WebSocket port |
| `ruiclaw webui --gateway-port <port>` | Override the gateway health port |
| `ruiclaw webui --yes` | Apply safe localhost WebUI defaults without confirmation; configure provider credentials in **Settings → Models** |
...
owned only by `ruiclaw gateway --background`; `ruiclaw webui --background` prints migration
...
`ruiclaw gateway` starts enabled chat channels, WebUI/WebSocket when configured, cron-backed system jobs, Dream, heartbeat, and the health endpoint. Most local browser users should start with `ruiclaw webui`; use `gateway` directly for service management, chat app operation, and advanced deployment. By default it runs in the foreground, which keeps existing scripts and terminal workflows unchanged. Use `--background` when you want a local macOS, Linux, or Windows process that you can manage from the CLI.

| Command | Description |
|---|---|
| `ruiclaw gateway` | Start the gateway in the foreground with config defaults |
| `ruiclaw gateway --verbose` | Show verbose runtime output |
| `ruiclaw gateway --port <port>` | Override `gateway.port` for the health endpoint |
| `ruiclaw gateway --workspace <path>` | Override workspace |
| `ruiclaw gateway --config <path>` | Use a specific config file |
| `ruiclaw gateway --background` | Start the gateway as a background process |
| `ruiclaw gateway status` | Show PID, foreground/background launch mode, explicit/on-demand lifetime, live client count, state, and logs |
| `ruiclaw gateway logs --no-follow` | Print recent background gateway logs and exit |
| `ruiclaw gateway logs` | Follow background gateway logs |
| `ruiclaw gateway restart` | Restart the recorded background gateway with the current config |
| `ruiclaw gateway stop` | Stop the recorded background gateway |
| `ruiclaw gateway install-service` | Install a systemd user service or macOS LaunchAgent |
| `ruiclaw gateway install-service --dry-run` | Preview the generated service file and system commands |
| `ruiclaw gateway uninstall-service` | Remove the installed system service |

For custom instances, pass the same selector flags to management commands:

```bash
ruiclaw gateway --background --config ./bot-a/config.json --workspace ./bot-a/workspace
ruiclaw gateway status --config ./bot-a/config.json --workspace ./bot-a/workspace
ruiclaw gateway stop --config ./bot-a/config.json --workspace ./bot-a/workspace
ruiclaw gateway install-service --config ./bot-a/config.json --workspace ./bot-a/workspace --name bot-a
```
...
`ruiclaw trigger` delivers one local message to a trigger that was created from
a chat/session with `/trigger <name>`.

```bash
ruiclaw trigger trg_8K4P2Q9X "Review PR #4502"
```

Keep `ruiclaw gateway` running so the message can be delivered to the linked
chat/session. The message is recorded as an automation turn in that session,
not as a normal chat message typed by the user.

The command writes to a workspace-local durable queue. If `ruiclaw gateway` is
...
```bash
generate-report | ruiclaw trigger trg_8K4P2Q9X
```
...
| Command | Description |
|---|---|
| `ruiclaw trigger <id> "message"` | Deliver one message through a trigger |
| `ruiclaw trigger <id>` | Read the message from stdin |
| `ruiclaw trigger --config <path> <id> "message"` | Use the workspace from a specific config |
| `ruiclaw trigger --workspace <path> <id> "message"` | Use a specific workspace |
...
call this CLI after it decides what message ruiclaw should receive.
...
| Command | Description |
|---|---|
| `ruiclaw serve` | Start `/v1/chat/completions`, `/v1/models`, and `/health` |
| `ruiclaw serve --host <host>` | Override API bind host |
| `ruiclaw serve --port <port>` | Override API port |
| `ruiclaw serve --timeout <seconds>` | Override per-request timeout |
| `ruiclaw serve --verbose` | Show runtime logs |
| `ruiclaw serve --workspace <path>` | Override workspace |
| `ruiclaw serve --config <path>` | Use a specific config file |
...
| Command | Description |
|---|---|
| `ruiclaw status` | Inspect the default instance |
| `ruiclaw status --config <path>` | Inspect a specific config |
| `ruiclaw status --config <path> --workspace <path>` | Inspect a specific config with a workspace override |

## Channels

| Command | Description |
|---|---|
| `ruiclaw channels status` | Show configured channel status |
| `ruiclaw channels status --config <path>` | Show channel status for a specific config |
| `ruiclaw channels login <channel>` | Run interactive login for supported channels |
| `ruiclaw channels login <channel> --force` | Re-authenticate even if credentials already exist |
| `ruiclaw channels login <channel> --config <path>` | Use a specific config file |
| `ruiclaw plugins list --config <path>` | Show plugin/channel enabled state for a specific config |
...
Use these commands when you want ruiclaw to add or remove a built-in capability
...
ruiclaw runtime support packages, not the user-invokable tools shown in WebUI
Apps. They cannot be attached to a chat turn with `@`.

| Feature name | What it enables |
|---|---|
| `api` | Dependencies required by the OpenAI-compatible `ruiclaw serve` process |
...
| Command | Description |
|---|---|
| `ruiclaw plugins list` | Show available channels and optional capabilities |
| `ruiclaw plugins enable <name>` | Install missing support and enable the feature or channel |
| `ruiclaw plugins enable <name> --logs` | Show package install logs while enabling |
| `ruiclaw plugins disable <channel>` | Turn off a channel without deleting its saved settings |
| `ruiclaw plugins list --config <path>` | Read a specific config file |
| `ruiclaw plugins enable <name> --config <path>` | Update a specific config file |
| `ruiclaw plugins disable <channel> --config <path>` | Turn off a channel in a specific config file |

Document and PDF reading are included in the standard installation. The old
`ruiclaw plugins enable documents` and `ruiclaw plugins enable pdf` commands
...
| Command | Description |
|---|---|
| `ruiclaw provider login openai-codex --set-main` | Authenticate Codex and select its current default model |
| `ruiclaw provider login xai-grok --set-main` | Authenticate an eligible X Premium / Grok subscription and select Grok 4.6; hosted X Search is enabled for models that advertise support |
| `ruiclaw provider login github-copilot --set-main` | Authenticate GitHub Copilot and select its current default model |
| `ruiclaw provider logout openai-codex` | Remove OpenAI Codex OAuth state |
| `ruiclaw provider logout xai-grok --config <path>` | Remove the selected ruiclaw instance's xAI OAuth state |
| `ruiclaw provider logout github-copilot` | Remove GitHub Copilot OAuth state |
