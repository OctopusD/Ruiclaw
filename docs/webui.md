# RuiClaw WebUI: Browser Workbench for Self-Hosted AI Agents

<!-- Meta description: Run ruiclaw from a browser WebUI with persistent and temporary chats, visible tool activity, workspace controls, Apps, skill discovery, settings, and Automations. -->

The WebUI is ruiclaw's browser workbench for persistent topics, grouped
...
The published `ruiclaw-ai` wheel already includes the WebUI bundle. You only need
...
`ruiclaw webui` creates the config/workspace when needed, enables the local
...
`ruiclaw webui --background` is retained only to print migration guidance. This keeps one
...
interactive launcher stops an on-demand gateway. `ruiclaw gateway --background` makes the
gateway persistent; manage it with `ruiclaw gateway status`, `ruiclaw gateway
logs`, `ruiclaw gateway restart`, and `ruiclaw gateway stop`.
...
When ruiclaw compacts context, the timeline shows its progress and outcome.
...
Use a temporary chat for a conversation that should not be added to ruiclaw's
...
- **CLI Apps** are local command-line adapters that ruiclaw runs on your
...
Apps intentionally does not list ruiclaw runtime support packages such as
...
included in ruiclaw and activate automatically when a file is attached. The
equivalent CLI for optional integrations remains `ruiclaw plugins`. See
[`cli-reference.md`](./cli-reference.md#optional-features).
That command manages ruiclaw runtime extras, not Agent Plugin packages.
...
extraction tools without requiring an API key. This does not replace ruiclaw's
...
It is an optional integration and does not replace ruiclaw's built-in web search
...
the topic or channel where they are supposed to run so ruiclaw keeps the
...
- Local triggers, created with `/trigger <name>`, run when you call a local
  command such as `ruiclaw trigger trg_8K4P2Q9X "Review PR #4502"`.
...
because ruiclaw would not know where to deliver the scheduled turn. Recreate it
...
message. Use the copied `ruiclaw trigger ...` command and replace `"message"`
...
skills. Actions that install missing ruiclaw support packages or third-party
...
trusted to change ruiclaw's Python environment and install workspace skill
...
1. `ruiclaw agent -m "Hello!"` works in the same Python environment.
2. `~/.ruiclaw/config.json` does not explicitly set `channels.websocket.enabled` to `false`.
3. `ruiclaw gateway` is still running.
