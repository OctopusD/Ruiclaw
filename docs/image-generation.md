...
ruiclaw can generate and edit images through the `generate_image` tool. Enable the tool in WebUI Settings, then ask for an image normally in chat; the agent decides when to call it and can keep iterating on generated images in the same conversation.
...
> [!TIP]
> Prefer environment variables for API keys. ruiclaw resolves `${VAR_NAME}` values from the environment at startup.
...
| `tools.imageGeneration.saveDir` | string | `"generated"` | Relative directory under ruiclaw's media directory for generated artifacts |
...
For providers that return image URLs, direct downloads use DNS pinning. When an explicit provider `proxy` is configured, ruiclaw rejects malformed URLs and locally identifiable private/internal targets on the initial URL and every redirect. Hostnames unavailable to local DNS are delegated to that trusted proxy, which owns final DNS resolution and network egress. Process-wide proxy environment variables are not used for these downloads.
...
For compatibility with the default ruiclaw setting, custom maps `defaultImageSize: "1K"` to `1024x1024`. Other explicit size hints are passed through unchanged.

### AIHubMix

AIHubMix `gpt-image-2-free` is supported through AIHubMix's unified predictions API. Internally ruiclaw calls:
...
ruiclaw supports two Gemini image generation model families via Google's Generative Language API:
...
Zhipu (智谱) `glm-image` model supports text-to-image generation. The API returns temporary image URLs (valid for 30 days); ruiclaw downloads and re-encodes them as base64 data URLs.
...
Generated images are stored under the active ruiclaw instance's media directory:

```text
~/.ruiclaw/media/generated/YYYY-MM-DD/img_<id>.<ext>
~/.ruiclaw/media/generated/YYYY-MM-DD/img_<id>.json
```
...
```text
A minimal app icon for ruiclaw: friendly robot head, rounded square, soft blue and white palette, clean vector style, no text
```
...
| AIHubMix says `Incorrect model ID` | Use `model: "gpt-image-2-free"`; ruiclaw expands it to the required `openai/gpt-image-2-free` model path internally |
| Generation times out | Try a smaller/default image size, set AIHubMix `extraBody.quality` to `"low"`, or retry later |
| Reference image rejected | Reference image paths must be inside the workspace or ruiclaw media directory and must be valid image files |
