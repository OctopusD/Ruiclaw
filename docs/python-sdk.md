# RuiClaw Python SDK: Run an AI Agent from Python

Use ruiclaw as a Python library. The SDK gives you the same agent runtime used
...
- ruiclaw SDK runs an agent around a model.
...
Install and configure ruiclaw first. If you have not done that yet, follow the
...
`RuiClaw.from_config()` reuses your normal `~/.ruiclaw/config.json` and
`~/.ruiclaw/workspace/`. Provider, model, tools, memory, and session behavior
...
`ruiclaw status` should show the config path, workspace path, active model or
...
from ruiclaw import RuiClaw


async def main() -> None:
    async with RuiClaw.from_config() as bot:
...
```python
from ruiclaw import STREAM_EVENT_TEXT_DELTA
...
Save this as `sdk_demo.py` after `ruiclaw agent -m "Hello!"` works:
...
async def main() -> None:
    prompt = " ".join(sys.argv[1:]) or "Explain what ruiclaw is in one paragraph."
    ...
    async with RuiClaw.from_config() as bot:
        ...
        async for event in bot.stream(prompt, session_key=session_key):
            if event.type == STREAM_EVENT_TEXT_DELTA:
                ...
                raise RuntimeError(event.error or "ruiclaw run failed")
...
workspace: /Users/alice/.ruiclaw/workspace
...
This script shows the usual production shape: create one `RuiClaw`, choose a
...
| Concept | Meaning |
|---------|---------|
| `RuiClaw` | The SDK object that owns one configured agent runtime. |
...
| Memory | Long-term memory files managed by ruiclaw. |
...
1. Create a `RuiClaw` from config.
...
ruiclaw has two programming surfaces:

| Use | Choose | Why |
|-----|--------|-----|
| Python code running in the same process as ruiclaw | Python SDK | Direct access to `RunResult`, sessions, memory, runtime helpers, hooks, and stream events. |
...
process isolation, or need to call ruiclaw from a non-Python service.
...
async with RuiClaw.from_config(workspace="/my/project") as bot:
    result = await bot.run("Explain the project structure")
```

Use a custom config when you run multiple ruiclaw instances or test an isolated
setup:

```python
async with RuiClaw.from_config(
...
The config controls what ruiclaw may use. The workspace is where ruiclaw keeps
...
```python
bot = RuiClaw.from_config(model="openai/gpt-4.1")
```
...
```python
bot = RuiClaw.from_config(model_preset="fast")
...
provider and model work with `ruiclaw agent -m "Hello!"` first.
...
become ruiclaw session history. Ingesting a transcript does not call the model,
...
metrics, tracing, or output post-processing without modifying ruiclaw internals:

```python
from ruiclaw.agent import AgentHook, AgentHookContext
...
### `RuiClaw.from_config(config_path=None, *, workspace=None, model=None, model_preset=None)`

Create a `RuiClaw` instance from a config file.

| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `config_path` | `str \| Path \| None` | `None` | Path to `config.json`. Defaults to `~/.ruiclaw/config.json`. |
...
ruiclaw agent loop. A context provider receives a `RequestContext` before each
model turn and may return one or more `RuntimeContextBlock` values. Use
`attributes` for caller-owned routing data; ruiclaw keeps it separate from
...
```python
from ruiclaw.agent import AgentHook
