# AI Agent Memory in ruiclaw

This page explains how ruiclaw implements long-term AI agent memory: session
history, compressed archives, durable knowledge files, Dream consolidation, and
Git-backed memory changes.

ruiclaw's memory is built on a simple belief: memory should feel alive, but it should not feel chaotic.

Good memory is not a pile of notes. It is a quiet system of attention. It notices what is worth keeping, lets go of what no longer needs the spotlight, and turns lived experience into something calm, durable, and useful.

That is the shape of memory in ruiclaw.

## The Design

ruiclaw does not treat memory as one giant file.
...
Memory moves through ruiclaw in two stages.
...
This is why ruiclaw's memory is not just archival. It is interpretive.

## The Files

In this page, `workspace` means the configured **agent workspace** (the default
is `~/.ruiclaw/workspace/`, or the path passed with `--workspace`). Selecting a
...
- `SOUL.md` remembers how ruiclaw should sound.
...
After Dream changes long-term memory files, ruiclaw can record that change with `GitStore`.
...
Dream decides what to keep, update, or forget using ruiclaw's built-in memory instructions. Most users can leave this alone.
...
To return to ruiclaw's default behavior, delete `prompts/dream.md` or leave it empty.

Each workspace has its own guide. Changing this file does not affect other ruiclaw workspaces.
