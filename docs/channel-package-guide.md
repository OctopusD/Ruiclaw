...
Use this guide to add a self-contained channel package to the ruiclaw repository. A channel is part of ruiclaw when its package lives at `ruiclaw/channels/<channel>/`; there is no separate external channel-plugin path.

> **Breaking change:** ruiclaw no longer discovers the `ruiclaw.channels` Python entry-point group. Move an entry-point implementation into `ruiclaw/channels/<channel>/` with a package-owned manifest, runtime, tests, and optional WebUI contribution.

## How It Works

When `ruiclaw gateway` starts, ruiclaw scans the packages under `ruiclaw/channels/` and loads each dependency-free `ChannelPlugin` descriptor from `manifest.py`.
...
```python
# ruiclaw/channels/webhook/__init__.py
"""Webhook channel package."""
```

```python
# ruiclaw/channels/webhook/manifest.py
from ruiclaw.channels.contracts import ChannelFieldSpec, ChannelSetupSpec
...
```python
# ruiclaw/channels/webhook/runtime.py
import asyncio
...
from ruiclaw.config.schema import Base
...
```bash
ruiclaw plugins list      # verify the channel package appears as "webhook"
ruiclaw onboard           # add default config for detected channels
```

Edit `~/.ruiclaw/config.json`:
...
Every channel is a self-contained package at `ruiclaw/channels/<channel>/`; channel-specific runtime code, setup metadata, tests, WebUI structure, components, and translations stay under that directory.

### Package Layout

```text
ruiclaw/channels/<channel>/
...
Do not add a runtime module directly under `ruiclaw/channels/`, create a parallel manifest tree, or add a central per-channel UI catalog. If existing channel files move, use `git mv` so history remains traceable.

### Manifest and Runtime Boundary

`manifest.py` exports a typed `ChannelPlugin` whose `runtime` target is an absolute import target, such as `ruiclaw.channels.telegram.runtime:TelegramChannel`; using `f"{__package__}.runtime:TelegramChannel"` keeps it package-owned without repeating the package path. Discovery imports the manifest before it knows whether the optional platform dependency is installed, so `manifest.py` must not import `runtime.py` or any platform SDK. Import runtime symbols from `runtime.py` explicitly; `__init__.py` remains an inert package marker.
...
Use the small constructors in [`ruiclaw/channels/_manifest.py`](../ruiclaw/channels/_manifest.py) for declarative field and requirement definitions. Use [`ruiclaw/channels/dingtalk/manifest.py`](../ruiclaw/channels/dingtalk/manifest.py) as a compact single-instance example and [`ruiclaw/channels/feishu/`](../ruiclaw/channels/feishu/) as a multi-instance example.
...
```json
{
  "description": "Use ruiclaw from Example chats.",
...
- The locale registry discovers only `ruiclaw/channels/*/webui/locales/*.json` and must not import the UI registry, React, or TSX.
...
Put channel-specific Python tests in `ruiclaw/channels/<channel>/tests/`. Keep only shared registry, manager, base-class, and cross-channel contract tests in `tests/channels/`. Release builds exclude package-local tests while the repository test configuration discovers both trees.

For a focused channel change, run the smallest relevant set:

```bash
uv run pytest ruiclaw/channels/<channel>/tests -q
...
```bash
ruiclaw channels login <channel_name>
ruiclaw channels login <channel_name> --force  # re-authenticate
```
...
```python
from ruiclaw.channels.contracts import ChannelFieldSpec, ChannelSetupSpec, SetupRequirement
from ruiclaw.channels.plugin import ChannelPlugin
...
from ruiclaw.channels.contracts import ChannelInstanceSpec, ChannelManagementSpec
...
Return a concrete iterable or generator from the adapter's `instance_specs()`; ruiclaw materializes and validates it before constructing any runtime. Raise an exception for malformed persisted data rather than silently changing instance identity. Keep network-backed metadata refresh behind the runtime's `refresh_feature_metadata()` so feature GET requests remain dependency-free and read-only.
...
Besides normal assistant text, ruiclaw can emit low-emphasis trace blocks. These are intended for UI affordances like status rows, collapsible "used tools" groups, or reasoning/thinking blocks. Platforms that do not have a good place for them can ignore them safely.
...
```python
from ruiclaw.bus.outbound_events import ProgressEvent
...
Channel runtimes use Pydantic config models by subclassing `Base` from `ruiclaw.config.schema`.

### Pattern

1. Define a Pydantic model inheriting from `ruiclaw.config.schema.Base`:
...
`ruiclaw onboard` reads the descriptor without importing the runtime. Put writable defaults in `ChannelSetupSpec`:
...
| What | Format | Example |
|------|--------|---------|
| Package directory | `ruiclaw/channels/{name}` | `ruiclaw/channels/webhook` |
| Manifest name | `{name}` | `webhook` |
| Config section | `channels.{name}` | `channels.webhook` |
| Runtime import | `ruiclaw.channels.{name}.runtime` | `ruiclaw.channels.webhook.runtime` |
...
ruiclaw plugins list    # should show the package as "webhook"
ruiclaw plugins enable webhook
ruiclaw gateway         # test end-to-end
```

## Verify

```bash
$ ruiclaw plugins list
