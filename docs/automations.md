...
<!-- Meta description: Create, run, and manage ruiclaw scheduled automations, local triggers, and heartbeat-backed background checks. -->

Automations are agent turns that run later in a linked topic. Use them
when ruiclaw should do work without someone actively typing: reminders,
...
result should appear. That lets ruiclaw keep the right session history,
...
|---|---|---|---|
| Scheduled automation | Time, interval, or cron expression | Recurring reminders, scheduled summaries, one-time future tasks | Ask ruiclaw in the target topic to schedule it with the `cron` tool |
| Local trigger | A local `ruiclaw trigger ...` command | CI jobs, webhooks, shell scripts, generated reports | `/trigger <name>` in the target topic |
...
Keep `ruiclaw gateway` running. The gateway owns background delivery for chat
...
local trigger messages. If you run multiple ruiclaw instances, pass the matching
`--config` or `--workspace` option to `ruiclaw trigger`.

Create each automation from the target topic. An automation without a linked
topic cannot be enabled or run from the WebUI because ruiclaw would not know
where to deliver the turn.

## Scheduled Automations

Scheduled automations are created by the agent's `cron` tool. In practice, ask
ruiclaw from the target chat or WebUI topic:
...
`America/Vancouver`; otherwise ruiclaw uses the runtime default timezone.
...
specific ruiclaw session later.
...
ruiclaw replies with a trigger ID and a command shaped like:

```bash
ruiclaw trigger trg_8K4P2Q9X "Review PR #4502"
```

Replace the quoted text with the message ruiclaw should receive. For generated
...
```bash
ruiclaw trigger --config ./bot-a/config.json trg_8K4P2Q9X "Nightly report"
ruiclaw trigger --workspace ./bot-a/workspace trg_8K4P2Q9X "Nightly report"
```

ruiclaw does not provide a built-in public webhook receiver for local triggers.
If GitHub, CI, or another external system should wake ruiclaw, run your own
small webhook service and have it call `ruiclaw trigger` after it builds the
final message.
...
Heartbeat is enabled by default when `ruiclaw gateway` starts. Configure it in
...
message. Copy the `ruiclaw trigger ...` command from the WebUI and replace
...
```bash
ruiclaw trigger <trigger-id> "Build failed on main. Inspect the logs and suggest the next fix."
```

For a local report script:

```bash
generate-report | ruiclaw trigger <trigger-id>
```

## Troubleshooting

If an automation does not run, check that `ruiclaw gateway` is running, the
...
- [`cli-reference.md#local-triggers`](./cli-reference.md#local-triggers) for `ruiclaw trigger`
