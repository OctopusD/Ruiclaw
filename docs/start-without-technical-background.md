...
This walkthrough is for people who have not used a terminal, API key, or JSON config file before. The goal is only to get one reply in a browser. You do not need to understand ruiclaw's architecture or edit its config by hand.
...
| WebUI | The local browser page where you use ruiclaw |
...
ruiclaw does not create an AI provider account for you. Before setup, have these details nearby:
...
The installer downloads the stable ruiclaw package into an isolated Python environment. On a fresh local desktop, it then starts the WebUI and opens your browser. This can take a few minutes on the first run. Keep the terminal open. It prints the exact command used to run ruiclaw; if `ruiclaw` is not found later, reuse that whole command instead of switching to a different Python command.
...
If the installer finishes without opening the browser and `ruiclaw` is available, run:

```bash
ruiclaw webui
```

If the terminal cannot find `ruiclaw`, take the exact command printed by the installer and replace its final arguments with `webui`. That command may begin with `uv tool run`, `pipx run`, or the full path to ruiclaw's private Python environment.

On SSH, a computer without a desktop, an existing configuration, or an older ruiclaw release, the installer may open the terminal wizard instead. Choose **Quick Start** there and follow its prompts.
...
| Schedule a reminder or recurring task | Ask ruiclaw in the target chat, then manage it in **Automations** |
...
Some runtime changes ask you to restart ruiclaw. Use the restart action shown by the WebUI, or return to the terminal, press `Ctrl+C`, and run `ruiclaw webui` again.
...
|---|---|
| `ruiclaw: command not found` | Reuse the exact ruiclaw command printed by the installer; it points to the isolated environment that contains the package |
...
| Browser opens but messages fail | Test `ruiclaw agent -m "Hello!"` to separate a model problem from a WebUI problem |
| A change was saved but nothing changed | Restart ruiclaw so the running process reloads the config |

If you ask for help, include your operating system, `ruiclaw --version`, `ruiclaw status`, the exact command, and the exact error. Remove every API key, bot token, password, OAuth token, and private account ID first.
...
Leave that launcher open while you use ruiclaw. Pressing `Ctrl+C` disconnects it; the shared gateway stops when it was the last local WebUI or TUI client. After the normal foreground start and model setup work, use `ruiclaw gateway --background` when you want the gateway to stay online with no clients; then manage it with `ruiclaw gateway status`, `logs`, `restart`, and `stop`.
