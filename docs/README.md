...
Repository docs follow the current source tree and can be newer than the latest package release. For published release docs, visit [ruiclaw.wiki](https://ruiclaw.wiki/docs/latest/getting-started/ruiclaw-overview).
...
| You are comfortable running commands | [Install and Quick Start](./quick-start.md) | `ruiclaw status` is healthy and the WebUI or CLI can get one reply |
...
1. Install ruiclaw.
2. Let the installer open `ruiclaw webui` on a fresh local desktop.
3. Configure a provider and model in **Settings → Models**.
4. Send `Hello!` before configuring anything else.

Most people do not need to edit JSON for the first run. The WebUI handles the initial provider, model, and local browser settings. SSH, headless, existing-config, and older-release installs retain `ruiclaw onboard --wizard` as a terminal fallback. After the WebUI opens, use **Settings** for models and built-in capabilities, **Settings → Channels** for chat apps, and **Apps** for Agent Plugins, CLI Apps, and MCP integrations.
...
| Run ruiclaw continuously | [Deployment](./deployment.md) |
| Run separate bots or workspaces | [Multiple Instances](./multiple-instances.md) |
| Call ruiclaw from Python | [Python SDK](./python-sdk.md) |
...
Configuration examples are usually snippets to merge into `~/.ruiclaw/config.json`, not complete replacement files. The docs use camelCase because ruiclaw writes config that way. Keep real API keys, bot tokens, and passwords out of issues and public logs.

## Extend or Contribute

These pages explain implementation and extension points. You do not need them to install or operate ruiclaw.
...
If a command or screen no longer matches these docs, please [open an issue](https://github.com/HKUDS/nanobot/issues) with your ruiclaw version, operating system, and the page that needs correction.
