...
This page collects contributor-facing notes for extending ruiclaw. User-facing setup and runtime options live in [`configuration.md`](./configuration.md).

## Adding an LLM Provider

ruiclaw uses the provider registry in `ruiclaw/providers/registry.py` as the source of truth for LLM provider metadata. Most OpenAI-compatible providers need only two changes.
...
2. Add a field to `ProvidersConfig` in `ruiclaw/config/schema.py`:
...
- `ruiclaw/audio/transcription_registry.py` owns provider names, aliases, default models, and adapter loading.
- `ruiclaw/providers/transcription.py` owns provider-specific HTTP behavior.
...
2. Add a `ProviderSpec` in `ruiclaw/providers/registry.py`.
...
3. Add an adapter class in `ruiclaw/providers/transcription.py`.
...
4. Register the adapter in `ruiclaw/audio/transcription_registry.py`.

```python
TranscriptionProviderSpec(
    name="my_stt",
    default_model="my-default-stt-model",
    adapter="ruiclaw.providers.transcription:MySTTTranscriptionProvider",
    aliases=("mystt",),
)
```
