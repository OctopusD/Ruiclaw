...
Use this page after `ruiclaw agent -m "Hello!"` works locally. Deployment keeps long-running surfaces online: WebUI, chat apps, heartbeat, Dream, cron jobs, and channel connections.
...
|---|---|
| `ruiclaw status` shows the expected config and workspace | Confirms the process will read the instance you meant to run |
| `ruiclaw agent -m "Hello!"` works | Proves install, config, provider, model, and workspace writes before adding a service layer |
...
| Ports are planned | Gateway health defaults to local-only `127.0.0.1:18790`; WebUI/WebSocket defaults to `8765`; `ruiclaw serve` defaults to `8900` |
| Logs are easy to reach | Use `docker compose logs`, `journalctl`, LaunchAgent log files, or `ruiclaw gateway --verbose` while diagnosing startup |
...
|---|---|---|---|
| Render | One-click hosted gateway and WebUI | Persistent disk at `/home/ruiclaw/.ruiclaw` | [Deploy to Render](#render) |
| Docker Compose | Repeatable container runs on Linux servers or workstations | Bind-mount `~/.ruiclaw` to `/home/ruiclaw/.ruiclaw` | `docker compose run --rm ruiclaw-cli agent -m "Hello!"` |
| Docker CLI | Manual container testing or small one-off hosts | Bind-mount `~/.ruiclaw` to `/home/ruiclaw/.ruiclaw` | `docker run -v ~/.ruiclaw:/home/ruiclaw/.ruiclaw --rm ruiclaw status` |
| systemd user service | Linux user-level gateway that restarts automatically | Host user's `~/.ruiclaw` unless you pass explicit paths | `systemctl --user status ruiclaw-gateway` |
| macOS LaunchAgent | macOS gateway that starts after login | Host user's `~/.ruiclaw` unless the plist passes explicit paths | `launchctl list | grep ai.ruiclaw.gateway` |

## Render

Run ruiclaw online without managing a server. The blueprint deploys the gateway and bundled WebUI together, with a persistent disk so sessions, memory, and chat history survive restarts.

> [!IMPORTANT]
> This setup requires a paid Render service because persistent disks are not available on the free tier. During setup, provide `ANTHROPIC_API_KEY` and set `RUICLAW_WEB_TOKEN` to a strong private password (for example, generate one with `openssl rand -hex 32`).
...
2. Enter your `ANTHROPIC_API_KEY`. Set `RUICLAW_WEB_TOKEN` to a new random value and save it in your password manager; this is the password for the public WebUI.
3. Create the Blueprint and wait for the service status to become **Live**. The first build can take several minutes.
4. Open the generated `onrender.com` URL. The **Authentication required** page means the gateway is running: enter the same `RUICLAW_WEB_TOKEN` value to open the WebUI.

The model API key is used by ruiclaw to call Anthropic. The Web token only protects access to this deployment; do not share it in issues, screenshots, or chat.
...
> [!TIP]
> The `-v ~/.ruiclaw:/home/ruiclaw/.ruiclaw` flag mounts your local config directory into the container, so your config and workspace persist across container restarts.
> The container runs as the non-root user `ruiclaw` (UID 1000) and reads config from `/home/ruiclaw/.ruiclaw`. Always mount your host config directory to `/home/ruiclaw/.ruiclaw`, not `/root/.ruiclaw`.
> If you get **Permission denied**, fix ownership on the host first: `sudo chown -R 1000:1000 ~/.ruiclaw`, or pass `--user $(id -u):$(id -g)` to match your host UID. Podman users can use `--userns=keep-id` instead.
>
> [!IMPORTANT]
> Official Docker usage currently means building from this repository with the included `Dockerfile`. Docker Hub images under third-party namespaces are not maintained or verified by HKUDS/nanobot; do not mount API keys or bot tokens into them unless you trust the publisher.

> [!IMPORTANT]
> The gateway and WebSocket channel default to `host: "127.0.0.1"` in `config.json` (set in `ruiclaw/config/schema.py`). Docker `-p` port forwarding cannot reach a container's loopback interface, so for the host or LAN to reach the exposed ports you must set both binds to `0.0.0.0` in `~/.ruiclaw/config.json` before starting the container. To serve the bundled WebUI from Docker, bind the WebSocket channel externally and protect bootstrap with `tokenIssueSecret`:
>
> ```json
> {
...
For a local `cloudflared` process in front of ruiclaw, Cloudflare Access can
...
```json
{
  ...
  "channels": {
    "websocket": {
      "host": "127.0.0.1",
      "port": 8765,
      "publicWsUrl": "wss://ruiclaw.example.com/",
      "trustedProxyAuth": {
...
are rejected as `assertionHeader` values. RuiClaw trusts the assertion but does
not cryptographically validate the JWT, so configure the tunnel and Access
policy carefully and do not expose the ruiclaw listener directly to untrusted
...
a comma-separated `RUICLAW_CHANNELS` build argument:

```bash
RUICLAW_CHANNELS=telegram,slack docker compose build
```

The image keeps ruiclaw in a virtual environment owned by its built-in non-root
...
with `RUICLAW_CHANNELS` keeps that installation reproducible instead of relying
...
```bash
docker compose run --rm ruiclaw-cli onboard   # first-time setup
vim ~/.ruiclaw/config.json                     # add API keys
docker compose up -d ruiclaw-gateway           # start gateway
```

```bash
docker compose run --rm ruiclaw-cli agent -m "Hello!"   # run CLI
docker compose logs -f ruiclaw-gateway                   # view logs
...
`"tools.exec.sandbox": "bwrap"` in `~/.ruiclaw/config.json`, add the bwrap
override file when starting containers:

```bash
docker compose -f docker-compose.yml -f docker-compose.bwrap.yml up -d ruiclaw-gateway
docker compose -f docker-compose.yml -f docker-compose.bwrap.yml run --rm ruiclaw-cli agent -m "Hello!"
```
...
```bash
# Build the image
docker build -t ruiclaw .

# Or preinstall a regular Python extra such as Bedrock support
docker build --build-arg RUICLAW_EXTRAS=bedrock -t ruiclaw .

# Or preinstall dependencies for a specific set of channels
docker build --build-arg RUICLAW_CHANNELS=telegram,slack -t ruiclaw .

# Initialize config (first time only)
docker run -v ~/.ruiclaw:/home/ruiclaw/.ruiclaw --rm ruiclaw onboard

# Edit config on host to add API keys
vim ~/.ruiclaw/config.json
...
docker run -v ~/.ruiclaw:/home/ruiclaw/.ruiclaw --rm ruiclaw agent -m "Hello!"
docker run -v ~/.ruiclaw:/home/ruiclaw/.ruiclaw --rm ruiclaw status
```
...
```bash
ruiclaw gateway install-service --manager systemd --dry-run
```

Install, enable, and start it:

```bash
ruiclaw gateway install-service --manager systemd
```

For a custom instance, pass the same config/workspace selector you use to run the gateway:

```bash
ruiclaw gateway install-service \
  --manager systemd \
  --name ruiclaw-telegram \
  --config ~/.ruiclaw-telegram/config.json \
...
```bash
systemctl --user status ruiclaw-gateway        # check status
systemctl --user restart ruiclaw-gateway       # restart after config changes
journalctl --user -u ruiclaw-gateway -f        # follow logs
ruiclaw gateway uninstall-service --manager systemd
```

The installer writes `~/.config/systemd/user/ruiclaw-gateway.service`, runs
`systemctl --user daemon-reload`, enables the unit, and restarts it. It uses the
current Python executable with `python -m ruiclaw gateway --foreground`, so the
service runs in the same environment you used to install ruiclaw.
...
Use a LaunchAgent when you want `ruiclaw gateway` to stay online after you log in, without keeping a terminal open.

Preview the generated plist first:

```bash
ruiclaw gateway install-service --manager launchd --dry-run
```

Install, load, enable, and start it:

```bash
ruiclaw gateway install-service --manager launchd
```
...
```bash
launchctl list | grep ai.ruiclaw.gateway
launchctl kickstart -k gui/$(id -u)/ai.ruiclaw.gateway
ruiclaw gateway uninstall-service --manager launchd
```

The installer writes `~/Library/LaunchAgents/ai.ruiclaw.gateway.plist`, uses the
current Python executable with `python -m ruiclaw gateway --foreground`, and
writes LaunchAgent logs under `~/.ruiclaw/logs/`.

> **Note:** if startup fails with "address already in use", stop the manually started `ruiclaw gateway` process first.
