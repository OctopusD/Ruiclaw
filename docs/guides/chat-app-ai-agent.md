# How to Connect an AI Agent to Chat Apps with ruiclaw

ruiclaw can run as a self-hosted chatbot or AI agent in Telegram, Discord,
...
The `ruiclaw webui` command already runs the gateway. For a chat-only or server deployment, start it directly:
...
- If `ruiclaw channels status` does not show the channel, the config key or
...
- If messages do not arrive, run `ruiclaw gateway --verbose` and compare
