# How to Configure an OpenAI-Compatible Provider in ruiclaw

ruiclaw can call OpenAI-compatible model providers by configuring an `apiBase`,
...
- one successful `ruiclaw agent` run
...
Verify the endpoint responds before debugging ruiclaw:
...
Merge this into `~/.ruiclaw/config.json`:
...
- Do not point ruiclaw at untrusted proxy endpoints for private workspaces.

## Troubleshooting

- If `curl /models` fails, fix the provider endpoint before changing ruiclaw.
- If ruiclaw says the model is unknown, check the model ID expected by the
  provider.
- If auth fails, confirm whether the provider wants Bearer auth and whether the
  key is present in the environment that starts ruiclaw.
