# How to Improve Ollama Tool-Calling Prompt Cache Reuse in ruiclaw

Some Ollama model templates move or remove tool definitions as a conversation
switches between user, assistant, and tool messages. ruiclaw can send a correct
...
`llama3.1:8b` tag with a prefix-stable tool template. It does not modify ruiclaw or
...
- a ruiclaw model preset that uses the derived tag
...
- ruiclaw becomes slow after the model calls a tool;
...
```bash
ruiclaw agent --session cli:ollama-cache-check \
...
ruiclaw appends an assistant tool call and its result, that user message is no
...
This is a model-template behavior. At the API boundary, ruiclaw continues to append
...
## Select the derived model in ruiclaw

Merge this preset into `~/.ruiclaw/config.json` and select it:
...
```bash
ruiclaw status
ruiclaw agent --session cli:ollama-stable-check \
  --message "Use the exec tool to calculate 2+2, then answer"
ruiclaw agent --session cli:ollama-stable-check \
  --message "Use the exec tool to calculate 4+7, then answer"
```
