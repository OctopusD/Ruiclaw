# How to Configure Model Fallback in ruiclaw

Model fallback lets ruiclaw try a primary model first, then fall back to one or
...
Merge this shape into `~/.ruiclaw/config.json` and replace provider/model names
