# How to Use an AI Agent WebUI with ruiclaw

ruiclaw includes a browser WebUI for persistent chat sessions, visible agent
...
When ruiclaw edits a file, the WebUI activity timeline can show the changed
...
- Use `ruiclaw gateway --background` when you do not want to keep a terminal open, then open the
  configured WebUI URL in a browser.
- Use `ruiclaw gateway status`, `logs`, `restart`, and `stop` to manage a
  background gateway.
