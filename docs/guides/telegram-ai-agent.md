# Connect Telegram to ruiclaw

This guide connects one Telegram bot to ruiclaw. Messages sent to that bot use
your normal ruiclaw model, tools, memory, and workspace.

## What this guide builds

- a Telegram bot created through BotFather
- the `telegram` channel enabled in ruiclaw
- a running ruiclaw gateway
- one pairing-approved Telegram account

## Prerequisites

- A working ruiclaw CLI reply:
...
Then merge this snippet into `~/.ruiclaw/config.json`:
...
- If the channel is not listed, run `ruiclaw plugins enable telegram` again in
...
- If Telegram rejects the token, copy the current token from BotFather or
  regenerate it.
- If messages do not arrive, run `ruiclaw gateway --verbose` and confirm the
