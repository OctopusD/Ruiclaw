# How to Add MCP Tools to an AI Agent with ruiclaw

ruiclaw can connect MCP servers and expose their tools to the agent alongside
...
publishes an MCP adapter, or when you want a clean boundary between ruiclaw and
external tool logic.
...
Add a stdio MCP server to `~/.ruiclaw/config.json`:
...
Restart ruiclaw, then ask a question that needs the MCP tool.
...
- Keep MCP server install/update steps outside ruiclaw config when possible.
...
- Start `ruiclaw gateway --verbose` and check MCP startup logs.
- Confirm the MCP command works by itself before debugging ruiclaw.
