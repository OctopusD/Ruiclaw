# Build a Feishu AI Agent with ruiclaw

This guide connects ruiclaw to Feishu or Lark through the `feishu` channel. The
channel uses a WebSocket long connection, so the first setup does not require a
public webhook URL.

## What this guide builds

- a Feishu/Lark bot app connected to ruiclaw
...
- Permission to run `ruiclaw gateway` continuously.
...
Open the printed URL or scan the QR code. ruiclaw writes the generated `appId`,
...
shape into `~/.ruiclaw/config.json`:
...
- If no messages arrive, check Feishu/Lark event permissions, Long Connection
  mode, and `ruiclaw gateway --verbose`.
