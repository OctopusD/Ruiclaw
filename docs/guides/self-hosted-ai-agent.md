# How to Run a Self-Hosted AI Agent with ruiclaw

This guide sets up ruiclaw as a self-hosted AI agent runtime on your own
machine or server. The result is a gateway process that can serve the WebUI,
chat apps, automations, and API integrations.

## What you will build

- a ruiclaw config and workspace under your control
- a model provider connected through `config.json`
- a long-running `ruiclaw gateway`
...
Or connect a channel in `~/.ruiclaw/config.json`, then keep the same gateway
...
- Run `ruiclaw status` with the same `--config` and `--workspace` flags used by
  the service.
- Run `ruiclaw gateway --verbose` while debugging channel startup.
