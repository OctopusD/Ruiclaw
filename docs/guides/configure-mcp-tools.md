# How to Configure MCP Tools in ruiclaw

This guide adds an MCP server to ruiclaw so the agent can use external tools
through the Model Context Protocol.

## What you will build

- a working ruiclaw agent
- one MCP integration configured through Apps or `~/.ruiclaw/config.json`
...
you want external tools to be managed outside ruiclaw core.
...
1. Run `ruiclaw webui` and open **Apps**.
2. Choose a known MCP server preset, or add a custom stdio, HTTP, or SSE server.
   ...
   **Connect**. Approve access in the browser window. HTTPS and localhost WebUIs
   ...
   localhost callback URL from the browser address bar and paste it into ruiclaw.
...
For manual or deployment-managed config, add this to `~/.ruiclaw/config.json`:
...
Restart ruiclaw and ask a question that requires the MCP tool.
...
- HTTP/SSE MCP uses ruiclaw's SSRF guard, including OAuth discovery, registration,
  token exchange, and redirects.
- OAuth credentials live in the ruiclaw data directory, not in `config.json`.
...
- Run the MCP command outside ruiclaw first.
- Start `ruiclaw gateway --verbose` and inspect tool registration logs.
