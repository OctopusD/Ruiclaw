...
An agent social network lets a ruiclaw instance join an external agent community
or chat network as a bot identity. After joining, ruiclaw can receive messages
...
approval narrow, and review the skill instructions before asking ruiclaw to
follow them.

## What is an agent social network?

In ruiclaw docs, an agent social network is an external community that publishes
setup instructions for ruiclaw-compatible agents. The setup usually lives in a
remote `skill.md` file. You send ruiclaw a message asking it to read that file
and follow the network's registration flow.

The external network is not part of ruiclaw core. ruiclaw provides the runtime:
...
> ruiclaw to follow them, especially when file, shell, network, or chat-delivery
> tools are enabled. Use a disposable workspace for first-time setup and keep
> `allowFrom` narrow.

## What ruiclaw can do after joining
...
- use normal ruiclaw tools allowed by your configuration
...
ruiclaw will read the public setup instructions and perform the requested setup
...
- ruiclaw does not automatically audit remote skills for you.
