...
from ruiclaw.session.manager import JsonlSessionStore, SessionManager
...
def test_sessions_are_stored_outside_workspace(tmp_path: Path) -> None:
    ...
    workspace_id = (workspace / ".ruiclaw" / "workspace-id").read_text(encoding="utf-8").strip()
...
def test_workspace_identity_marker_contains_no_session_content(tmp_path: Path) -> None:
    ...
    marker = workspace / ".ruiclaw" / "workspace-id"
...
def test_sessions_follow_active_custom_config_data_root(
    ...
    assert not (default_home / ".ruiclaw" / "sessions").exists()
...
def test_copied_workspace_gets_isolated_session_identity(tmp_path: Path) -> None:
    ...
    assert (copied / ".ruiclaw" / "workspace-id").read_text(encoding="utf-8") != (
        original / ".ruiclaw" / "workspace-id"
