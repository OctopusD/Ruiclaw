...
class TestSaveFsync:
    ...
    def test_save_ignores_unsupported_directory_fsync(
        ...
        with (
            ...
            patch("ruiclaw.session.manager.os.open", return_value=directory_fd) as open_dir,
            patch(
                "ruiclaw.session.manager.os.fsync",
                side_effect=[None, OSError(errno.EINVAL, "Invalid argument")],
            ),
            patch("ruiclaw.session.manager.os.close") as close_dir,
        ):
            manager.save(session, fsync=True)
    ...
    def test_save_propagates_other_directory_fsync_errors(
        ...
        with (
            manager.locked_session_files(),
            patch("ruiclaw.session.manager.os.open", return_value=directory_fd),
            patch(
                "ruiclaw.session.manager.os.fsync",
