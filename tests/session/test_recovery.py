...
from ruiclaw.bus.outbound_events import RecoveryStateEvent, SessionUpdatedEvent
from ruiclaw.bus.queue import MessageBus
from ruiclaw.bus.runtime_events import PARENT_RUN_ID_METADATA_KEY, RUN_ID_METADATA_KEY
...
from ruiclaw.webui import session_list_index, transcript
...
async def test_stale_incomplete_transcript_waits_for_confirmation(tmp_path: Path, monkeypatch) -> None:
    ...
    _persist(sessions, session)
    monkeypatch.setattr(
        "ruiclaw.webui.transcript.has_unfinished_transcript_tail",
        lambda _key: True,
    )
