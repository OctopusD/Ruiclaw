...
from ruiclaw.apps.cli.service import CliAppManager
...
def test_subprocess_env_excludes_api_keys_on_windows(monkeypatch, tmp_path) -> None:
    monkeypatch.setattr("ruiclaw.apps.cli.service.sys.platform", "win32")
    monkeypatch.setenv("OPENAI_API_KEY", "sk-should-not-leak")
    monkeypatch.setenv("ANTHROPIC_API_KEY", "sk-ant-leak")
...
def test_run_passes_filtered_env(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("ruiclaw.apps.cli.service.shutil.which", lambda entry: "/bin/echo")
