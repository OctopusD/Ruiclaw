...
from ruiclaw.gateway.runtime import monitor_gateway_clients
from ruiclaw.process_runtime import process_is_running
...
def test_default_instance_preserves_released_gateway_paths() -> None:
    config_path = Path.home() / ".ruiclaw" / "config.json"
...
def test_stop_reaps_an_owned_child_without_consuming_the_shutdown_timeout(
    ...
    monkeypatch.setattr(runtime, "_process_identity", lambda _pid: 12345)
    monkeypatch.setattr(
        "ruiclaw.process_runtime.os.getpgid",
        lambda _pid: process.pid,
        raising=False,
    )
    monkeypatch.setattr(
        "ruiclaw.process_runtime.os.killpg",
...
def test_windows_process_probe_never_sends_ctrl_c(monkeypatch):
    monkeypatch.setattr(
        "ruiclaw.process_runtime._windows_process_identity",
        lambda pid: "created-at" if pid == 12345 else None,
    )
    monkeypatch.setattr(
        "ruiclaw.process_runtime.os.kill",
...
def test_windows_host_probe_stays_safe_when_target_platform_is_posix(monkeypatch):
    monkeypatch.setattr("ruiclaw.process_runtime._platform_name", lambda: "Windows")
    monkeypatch.setattr(
        "ruiclaw.process_runtime._windows_process_identity",
        lambda pid: "created-at" if pid == 12345 else None,
    )
...
def test_posix_process_probe_treats_a_zombie_as_stopped(monkeypatch):
    monkeypatch.setattr("ruiclaw.process_runtime._platform_name", lambda: "Darwin")
    monkeypatch.setattr("ruiclaw.process_runtime.os.kill", lambda *_args: None)
    monkeypatch.setattr(
        "ruiclaw.process_runtime.subprocess.run",
...
def test_status_distinguishes_live_process_from_degraded_gateway_readiness(
    ...
    monkeypatch.setattr(
        "ruiclaw.gateway.runtime._gateway_health_ready",
...
def test_posix_process_identity_includes_start_time_and_accepts_legacy_state(
    ...
    runtime = GatewayRuntime(paths=_paths(tmp_path), platform_name="Linux")
    monkeypatch.setattr("ruiclaw.process_runtime._platform_name", lambda: "Linux")
    monkeypatch.setattr("ruiclaw.process_runtime.os.getpgid", lambda _pid: 42, raising=False)
...
def test_darwin_process_identity_is_locale_independent(tmp_path, monkeypatch):
    ...
    started_at = int(time.mktime((2026, 8, 18, 2, 17, 54, -1, -1, -1)))
    monkeypatch.setattr(
        "ruiclaw.process_runtime._darwin_process_birth",
...
def test_terminate_posix_tolerates_process_group_disappearing_before_sigkill(
    ...
    monkeypatch.setattr("ruiclaw.process_runtime.os.killpg", fake_killpg, raising=False)
