...
from ruiclaw.gateway import GatewayStartOptions
from ruiclaw.gateway.service import GatewayServiceInstaller, GatewayServiceOptions
...
def test_systemd_install_dry_run_renders_user_unit(tmp_path):
    ...
    result = installer.install(
        GatewayServiceOptions(
            start=GatewayStartOptions(
                ...
                config_path="/tmp/ruiclaw/config.json",
            ),
    ...
    assert result.path == tmp_path / ".config/systemd/user/ruiclaw-gateway.service"
    assert ("systemctl", "--user", "daemon-reload") in result.commands
    assert ("systemctl", "--user", "enable", "ruiclaw-gateway.service") in result.commands
    assert ("systemctl", "--user", "restart", "ruiclaw-gateway.service") in result.commands
    assert result.content is not None
    assert 'WorkingDirectory="/tmp/ruiclaw workspace"' in result.content
    assert 'ExecStart=/venv/bin/python -m ruiclaw gateway --foreground --port 18790 --verbose' in result.content
    assert '--workspace "/tmp/ruiclaw workspace" --config /tmp/ruiclaw/config.json' in result.content
...
def test_launchd_install_dry_run_renders_plist(tmp_path):
    ...
    result = installer.install(
        GatewayServiceOptions(
            start=GatewayStartOptions(
                port=18791,
                workspace="/Users/test/.ruiclaw/workspace",
                config_path="/Users/test/.ruiclaw/config.json",
            ),
    ...
    assert result.path == tmp_path / "Library/LaunchAgents/ai.ruiclaw.gateway.plist"
    assert result.content is not None
    payload = plistlib.loads(result.content.encode("utf-8"))
    assert payload["Label"] == "ai.ruiclaw.gateway"
    assert payload["ProgramArguments"] == [
        ...
        "18791",
        "--workspace",
        "/Users/test/.ruiclaw/workspace",
        "--config",
        "/Users/test/.ruiclaw/config.json",
    ]
...
def test_uninstall_systemd_removes_unit_and_reloads(tmp_path):
    ...
    unit = tmp_path / ".config/systemd/user/ruiclaw-gateway.service"
    ...
    assert commands == [
        ["systemctl", "--user", "disable", "--now", "ruiclaw-gateway.service"],
        ["systemctl", "--user", "daemon-reload"],
    ]
