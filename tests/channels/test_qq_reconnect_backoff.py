...
def _make_channel():
    ...
    from ruiclaw.channels.qq.runtime import QQChannel, QQConfig
...
def _make_bot(channel):
    from ruiclaw.channels.qq.runtime import _make_bot_class
...
async def test_bot_connect_dns_error_accounts_for_sdk_pacing():
    ...
    with (
        patch("ruiclaw.channels.qq.runtime.BotWebSocket") as mock_ws_cls,
        patch("ruiclaw.channels.qq.runtime.time.monotonic", side_effect=lambda: clock),
...
async def test_bot_connect_dns_error_no_traceback(capsys):
    ...
    with (
        patch(
            "ruiclaw.channels.qq.runtime.BotWebSocket"
        ) as mock_ws_cls,
...
async def test_bot_connect_backoff_doubles_and_caps():
    from ruiclaw.channels.qq.runtime import _RECONNECT_BACKOFF_MAX
...
async def test_bot_connect_success_resets_backoff():
    ...
    with patch("ruiclaw.channels.qq.runtime.BotWebSocket") as mock_ws_cls:
        mock_client = MagicMock()
...
def test_is_network_error_classification():
    from ruiclaw.channels.qq.runtime import _is_network_error
