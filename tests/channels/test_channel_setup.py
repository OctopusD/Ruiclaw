...
import ruiclaw.channels._setup as channel_setup_module
import ruiclaw.channels.registry as registry_module
...
from ruiclaw.channels.registry import channel_default_enabled, discover_plugins
...
INTERNAL_CHANNEL_FIELDS = {
    ...
    # ruiclaw WebUI owns this transport and intentionally has no channel dialog.
    "websocket": {
        "allowFrom",
        "host",
        "maxMessageBytes",
...
def test_every_channel_is_a_self_contained_package() -> None:
    ...
    for name in EXPECTED_CHANNELS:
        ...
        assert plugin.runtime.startswith(f"ruiclaw.channels.{name}.runtime:")
...
def test_channel_manifests_only_import_contract_modules() -> None:
    channel_dir = Path(channel_setup_module.__file__).parent
    allowed_imports = {
        "ruiclaw.channels._manifest",
        "ruiclaw.channels.contracts",
        "ruiclaw.channels.plugin",
    }
    ...
    for name in EXPECTED_CHANNELS:
        ...
        allowed_channel_imports = {
            ...
            if module.startswith(f"ruiclaw.channels.{name}.")
...
def test_feishu_package_manifest_owns_runtime_and_webui_metadata() -> None:
    ...
    assert plugin is not None
    assert plugin.runtime == "ruiclaw.channels.feishu.runtime:FeishuChannel"
    assert plugin.dependencies == ("lark-oapi>=1.5.0,<2.0.0",)
    assert plugin.connector == "ruiclaw.channels.feishu.connect:FeishuConnectStore"
...
def test_weixin_package_manifest_owns_runtime_and_webui_metadata() -> None:
    ...
    assert plugin is not None
    assert plugin.runtime == "ruiclaw.channels.weixin.runtime:WeixinChannel"
    assert plugin.dependencies == ("qrcode[pil]>=8.0", "pycryptodome>=3.20.0")
    assert plugin.connector == "ruiclaw.channels.weixin.connect:WeixinConnectStore"
    assert plugin.webui == "webui/index.tsx"
...
def test_whatsapp_package_manifest_owns_browser_connector() -> None:
    ...
    assert plugin is not None
    assert plugin.connector == "ruiclaw.channels.whatsapp.connect:WhatsAppConnectStore"
    assert plugin.webui == "webui/index.tsx"
...
for name in {sorted(EXPECTED_CHANNELS)!r}:
    plugin = load_channel_package(name)
    assert plugin is not None
    assert f"ruiclaw.channels.{{name}}.runtime" not in sys.modules
"""
