...
from ruiclaw.bus.events import OutboundMessage
from ruiclaw.bus.outbound_events import (
    StreamDeltaEvent,
...
from ruiclaw.channels.contracts import (
    ChannelFieldSpec,
    ChannelInstanceSpec,
...
from ruiclaw.channels.manager import ORIGIN_REPLY_FINGERPRINTS_MAX_SIZE, ChannelManager
from ruiclaw.channels.plugin import ChannelPlugin, load_channel_package
from ruiclaw.config.loader import load_config, save_config
from ruiclaw.config.schema import ChannelsConfig, Config
from ruiclaw.providers.transcription import GroqTranscriptionProvider as _GroqProvider
from ruiclaw.providers.transcription import OpenAITranscriptionProvider as _OpenAIProvider
from ruiclaw.utils.restart import RestartNotice
...
def _stub_channel_registry(
    ...
    *plugins: ChannelPlugin,
    ...
    monkeypatch.setattr("ruiclaw.channels.registry.discover_plugins", discover)
...
def _stub_channel_packages(
    ...
    *names: str,
    ...
    from ruiclaw.channels.plugin import load_channel_package
...
def _stub_optional_feature_cli(
    ...
    *,
    ...
    monkeypatch.setattr("ruiclaw.optional_features.optional_dependency_groups", lambda: extras)
    monkeypatch.setattr("ruiclaw.optional_features.extra_installed", lambda _name, _deps: installed)
    if commands is not None:
        monkeypatch.setattr(
            "ruiclaw.optional_features.run_install_command",
...
def test_special_setup_validation_is_owned_by_channel_package(name: str):
    ...
    assert plugin.setup.validator.__module__ == f"ruiclaw.channels.{name}.validation"
...
def test_interactive_connector_is_owned_by_channel_package(name: str):
    ...
    assert plugin.connector.startswith(f"ruiclaw.channels.{name}.")
    assert plugin.load_connector().__class__.__module__ == f"ruiclaw.channels.{name}.connect"
...
def test_channel_manager_loads_descriptor_but_not_disabled_runtime(monkeypatch):
    ...
    monkeypatch.setattr(
        "ruiclaw.channels.registry._channel_package_names",
        lambda: ["fakeplugin"],
    )
    monkeypatch.setattr(
        "ruiclaw.channels.registry.load_channel_package",
...
def test_feature_payload_uses_unified_instance_activation(monkeypatch):
    from ruiclaw.optional_features import optional_features_payload
    ...
    _stub_channel_registry(monkeypatch, _channel_plugin(_FakeMultiChannel))
    monkeypatch.setattr("ruiclaw.optional_features.optional_dependency_groups", lambda: {})
...
def test_multi_plugin_action_defaults_to_default_instance(
    ...
    from ruiclaw.webui.ruiclaw_features_api import ruiclaw_features_action
    ...
    disabled = ruiclaw_features_action("disable", {"name": ["managedmulti"]})
...
async def test_single_channel_enable_applies_defaults_before_hot_reload(
    ...
    payload = ruiclaw_features_action("enable", {"name": ["singleplugin"]})
...
def test_discover_plugins_loads_package_descriptors():
    from ruiclaw.channels.registry import discover_plugins

    plugin = _channel_plugin(_FakeLine)
    with (
        patch("ruiclaw.channels.registry._channel_package_names", return_value=["line"]),
        patch("ruiclaw.channels.registry.load_channel_package", return_value=plugin),
    ):
...
def test_plugin_setup_contract_drives_save_and_validation(
    ...
    from ruiclaw.channels.validation import validate_channel_config
    from ruiclaw.config import loader
    from ruiclaw.webui.settings_routes import WebUISettingsRouter
    from ruiclaw.webui.settings_services import WebUISettingsServices
...
def test_webui_save_rejects_duplicate_feishu_ids_without_writing(monkeypatch, tmp_path):
    from ruiclaw.config import loader
    from ruiclaw.webui.settings_api import WebUISettingsError
    from ruiclaw.webui.settings_routes import WebUISettingsRouter
    from ruiclaw.webui.settings_services import WebUISettingsServices
...
def test_discover_plugins_skips_names_outside_enabled_set():
    ...
    with (
        patch("ruiclaw.channels.registry._channel_package_names", return_value=["disabled"]),
        patch("ruiclaw.channels.registry.load_channel_package", side_effect=_load_disabled),
    ):
...
def test_discover_plugins_handles_load_error():
    ...
    with (
        patch("ruiclaw.channels.registry._channel_package_names", return_value=["broken"]),
        patch("ruiclaw.channels.registry.load_channel_package", side_effect=_boom),
    ):
        result = discover_plugins()
...
def test_discover_all_includes_available_channel_packages():
    from ruiclaw.channels.registry import discover_all, discover_plugins
...
def test_discover_enabled_imports_only_enabled_packages():
    from ruiclaw.channels.registry import discover_enabled
...
def test_discover_enabled_warns_for_enabled_package_import_errors():
    ...
    with patch("ruiclaw.channels.registry.logger.warning") as warning:
...
def test_manager_loads_plugin_from_dict_config(monkeypatch):
    ...
    from ruiclaw.channels.manager import ChannelManager
...
def test_manager_installs_manifest_dependencies_before_loading_enabled_channel(monkeypatch):
    from ruiclaw.optional_features import InstallResult
    ...
    monkeypatch.setattr("ruiclaw.optional_features.extra_installed", extra_installed)
    monkeypatch.setattr("ruiclaw.optional_features.install_extra", install_extra)
    config = Config.model_validate({
        "channels": {
...
def test_manager_reports_dependency_install_failure_as_runtime_failure(monkeypatch):
    ...
    _stub_channel_registry(monkeypatch, plugin)
    monkeypatch.setattr(
        "ruiclaw.optional_features.extra_installed",
        lambda _name, _dependencies: False,
    )
    monkeypatch.setattr(
        "ruiclaw.optional_features.install_extra",
...
def test_manager_loads_websocket_from_default_config():
    ...
    with patch("ruiclaw.channels.registry.discover_plugins", return_value={"websocket": plugin}):
...
async def test_base_channel_reads_current_transcription_config_each_call(
    ...
    from ruiclaw.providers import transcription as transcription_mod
    ...
    save_config(config, config_path)
    monkeypatch.setattr("ruiclaw.config.loader._current_config_path", config_path)
...
async def test_base_channel_respects_disabled_transcription_config(
    ...
    with patch("ruiclaw.providers.transcription.GroqTranscriptionProvider") as provider:
...
def test_openai_transcription_provider_honors_api_base_argument():
    from ruiclaw.providers.transcription import OpenAITranscriptionProvider
...
async def test_transcription_provider_includes_language(tmp_path, provider_cls, language):
    ...
    with patch("ruiclaw.providers.transcription.httpx.AsyncClient", return_value=_stub_async_client(captured)):
...
def test_channels_login_uses_discovered_plugin_class(monkeypatch):
    ...
    monkeypatch.setattr("ruiclaw.config.loader.load_config", lambda config_path=None: Config())
    monkeypatch.setattr(
        "ruiclaw.channels.registry.discover_all",
...
def test_plugins_enable_extra_without_channel_only_installs(monkeypatch, tmp_path):
    ...
    log_flags: list[bool] = []
    config_path = tmp_path / "config.json"
    original_set_logs = cli_commands._set_ruiclaw_logs
    ...
    monkeypatch.setattr("ruiclaw.cli.commands._set_ruiclaw_logs", _set_logs)
...
def test_plugins_enable_logs_option_enables_ruiclaw_logs(monkeypatch, tmp_path):
    from typer.testing import CliRunner
...
def test_enable_optional_feature_blocks_install_when_disallowed(monkeypatch, tmp_path):
    from ruiclaw.optional_features import OptionalFeatureError, enable_optional_feature
    ...
    monkeypatch.setattr("ruiclaw.optional_features.extra_installed", lambda _name, _deps: False)
...
def test_enable_optional_feature_skips_install_when_dependency_present(
    ...
    from ruiclaw.optional_features import InstallResult, enable_optional_feature
    ...
    monkeypatch.setattr("ruiclaw.optional_features.extra_installed", lambda _name, _deps: True)
    ...
    monkeypatch.setattr("ruiclaw.optional_features.install_extra", _install_extra)
...
def test_enable_optional_feature_lazy_reader_does_not_require_restart(monkeypatch, tmp_path):
    from ruiclaw.optional_features import enable_optional_feature
    ...
    assert payload["last_action"]["message"] == "Feature 'documents' is included with ruiclaw"


def test_enable_optional_feature_reports_install_failure(monkeypatch, tmp_path):
    from ruiclaw.optional_features import (
        InstallResult,
...
def test_install_only_adds_channel_support_without_enabling_it(monkeypatch, tmp_path):
    ...
    payload = ruiclaw_features_action(
        "enable",
...
def test_disable_optional_feature_rejects_unknown_features_and_non_channels(
    ...
    from ruiclaw.optional_features import OptionalFeatureError, disable_optional_feature
...
def test_disable_optional_feature_writes_channel_disabled(monkeypatch, tmp_path):
    from ruiclaw.optional_features import disable_optional_feature
...
def test_feishu_enable_rejects_duplicate_instance_ids_without_writing(tmp_path):
    from ruiclaw.channels.registry import load_channel_plugin
    from ruiclaw.optional_features import OptionalFeatureError, set_channel_config_enabled
...
def test_live_runtime_status_overrides_enabled_configuration_for_webui():
    from ruiclaw.optional_features import with_channel_runtime_status
...
def test_package_manifest_metadata_drives_optional_feature_payload(monkeypatch):
    ...
    monkeypatch.setattr("ruiclaw.optional_features.extra_installed", record_extra)
...
def test_optional_features_payload_lists_feishu_instances(monkeypatch):
    ...
    assert [
        ...
        ("default", "ruiclaw", "Voraflare Bot", "https://example.com/bot.png", True),
...
def test_optional_features_payload_does_not_refresh_saved_feishu_identity(monkeypatch, tmp_path):
    from ruiclaw.channels.feishu import runtime as feishu_module
    from ruiclaw.config import loader
    from ruiclaw.optional_features import optional_features_payload
    ...
    assert instance["display_name"] == "ruiclaw"
...
def test_optional_features_payload_preserves_legacy_flat_feishu_config(monkeypatch, tmp_path):
    ...
    assert payload["features"][0]["instances"][0]["display_name"] == "ruiclaw"
    assert config_path.read_text(encoding="utf-8") == before
...
def test_enable_uses_uv_when_tool_environment_has_no_pip(
    ...
    from ruiclaw import optional_features
...
def test_enable_bootstraps_pip_with_ensurepip(monkeypatch):
    ...
    assert calls == [
        [sys.executable, "-m", "pip", "install", "ruiclaw-ai[bedrock]"],
...
def test_channel_default_config():
    ...
    from ruiclaw.channels.dingtalk.runtime import DingTalkChannel
...
async def test_send_with_retry_retries_on_failure():
    ...
    with patch("ruiclaw.channels.manager.asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
        await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_send_with_retry_no_retry_when_max_is_zero():
    ...
    with patch("ruiclaw.channels.manager.asyncio.sleep", new_callable=AsyncMock):
        await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_send_with_retry_propagates_cancelled_error_during_sleep():
    ...
    with patch("ruiclaw.channels.manager.asyncio.sleep", side_effect=cancel_during_sleep):
        with pytest.raises(asyncio.CancelledError):
            await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_notify_restart_done_waits_until_channel_starts():
    ...
    with patch("ruiclaw.channels.manager.consume_restart_notice_from_env", return_value=notice):
...
async def test_restart_notice_retries_until_running_channel_accepts_delivery():
    ...
    with patch("ruiclaw.channels.manager._SEND_RETRY_DELAYS", (0,)):
