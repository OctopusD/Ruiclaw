...
from ruiclaw.agent import plugins as agent_plugins
from ruiclaw.agent.skills import SkillsLoader
from ruiclaw.apps.cli.service import CliAppError, CliAppManager, CliAppsRuntimeConfig
...
def test_installed_payload_enriches_apps_from_cached_catalog(
    ...
    monkeypatch.setattr(
        "ruiclaw.apps.cli.service.shutil.which",
...
def test_payload_includes_ruiclaw_extension_registry(tmp_path: Path) -> None:
    manager = _manager(tmp_path)
    _write_cache(manager._cache_path("harness"), {"meta": {"updated": "2026-04-16"}, "clis": []})
    ...
    assert app["manifest"]["source"] == "ruiclaw-extension"
    assert app["manifest"]["trust"]["registry"] == "ruiclaw-extension"
...
def test_optional_extension_registry_failure_does_not_break_payload(
    ...
    monkeypatch.setattr("ruiclaw.apps.cli.service.httpx.get", fail_get)
...
def test_run_argv_logs_command_exit_and_output(
    ...
    from ruiclaw.apps.cli import service as cli_service
...
def test_install_records_entry_point_path_and_pip_distribution(
    ...
    monkeypatch.setattr(
        "ruiclaw.apps.cli.service.importlib_metadata.distributions",
        lambda: [
            SimpleNamespace(
                entry_points=[
...
def test_fetch_skill_content_allows_cli_anything_raw_skill_url(
    ...
    monkeypatch.setattr("ruiclaw.apps.cli.service.httpx.get", fake_get)
...
def test_fetch_skill_content_uses_extension_raw_base_for_relative_skills(
    ...
    content = manager._fetch_skill_content({
        "name": "hyperframes",
        "skill_md": "skills/hyperframes/SKILL.md",
        "_raw_base": "https://raw.githubusercontent.com/Re-bin/ruiclaw-extension/main",
    })

    assert content and "# HyperFrames" in content
    assert seen == [
        "https://raw.githubusercontent.com/Re-bin/ruiclaw-extension/main/skills/hyperframes/SKILL.md"
    ]
...
def test_migrated_cli_app_skill_keeps_legacy_identity_alias(
    ...
    data_dir = tmp_path / "data"
    monkeypatch.setattr(
        "ruiclaw.apps.cli.service.get_runtime_subdir",
...
def test_run_installed_cli_uses_argv_without_shell(
    ...
    monkeypatch.setattr("ruiclaw.apps.cli.service.subprocess.run", fake_run)
    manager._save_installed(
        {
            "gimp": {
                "version": "1.0.0",
