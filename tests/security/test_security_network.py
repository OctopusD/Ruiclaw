"""Tests for ruiclaw.security.network — SSRF protection and internal URL detection."""
...
from ruiclaw.security.network import (
...
def test_blocks_private_ipv4(ip: str, label: str):
    with patch("ruiclaw.security.network.socket.getaddrinfo", _fake_resolve("evil.com", [ip])):
...
def test_blocks_ipv6_loopback():
    ...
    with patch("ruiclaw.security.network.socket.getaddrinfo", _resolver):
...
def test_blocks_ipv6_mapped_loopback():
    ...
    with patch("ruiclaw.security.network.socket.getaddrinfo", _fake_resolve_v6("evil.com", ["::ffff:127.0.0.1"])):
        ok, err = validate_url_target("http://evil.com/")
        assert not ok
...
def test_blocks_ipv6_mapped_metadata():
    ...
    with patch("ruiclaw.security.network.socket.getaddrinfo", _fake_resolve_v6("evil.com", ["::ffff:169.254.169.254"])):
        ok, err = validate_url_target("http://evil.com/")
        assert not ok
...
def test_blocks_ipv6_mapped_rfc1918():
    ...
    with patch("ruiclaw.security.network.socket.getaddrinfo", _fake_resolve_v6("evil.com", ["::ffff:10.0.0.1"])):
        ok, err = validate_url_target("http://evil.com/")
        assert not ok
...
def test_blocks_sampled_addresses_from_internal_networks():
    ...
    for idx, ip in enumerate(samples):
        ...
        with patch("ruiclaw.security.network.socket.getaddrinfo", resolver(host, [ip])):
...
def test_allows_public_ipv6():
    ...
    with patch("ruiclaw.security.network.socket.getaddrinfo", _fake_resolve_v6("example.com", ["2606:4700::6810:84e5"])):
...
def test_allows_public_ip():
    with patch("ruiclaw.security.network.socket.getaddrinfo", _fake_resolve("example.com", ["93.184.216.34"])):
...
def test_resolve_url_target_only_delegates_dns_to_trusted_proxy(
    ...
    with patch(
        "ruiclaw.security.network.socket.getaddrinfo",
...
def test_pin_resolved_url_dns_prevents_second_resolution_rebind():
    ...
    with patch("ruiclaw.security.network.socket.getaddrinfo", _rebinding_resolver):
...
def test_allows_normal_https():
    with patch("ruiclaw.security.network.socket.getaddrinfo", _fake_resolve("github.com", ["140.82.121.3"])):
...
def test_detects_curl_metadata():
    with patch("ruiclaw.security.network.socket.getaddrinfo", _fake_resolve("169.254.169.254", ["169.254.169.254"])):
        assert contains_internal_url('curl -s http://169.254.169.254/computeMetadata/v1/')


def test_detects_wget_localhost():
    with patch("ruiclaw.security.network.socket.getaddrinfo", _fake_resolve("localhost", ["127.0.0.1"])):
...
def test_loopback_exception_rejects_public_name_resolving_to_loopback():
    with patch("ruiclaw.security.network.socket.getaddrinfo", _fake_resolve("example.com", ["127.0.0.1"])):
...
def test_blocks_cgnat_by_default():
    ...
    with patch("ruiclaw.security.network.socket.getaddrinfo", _fake_resolve("ts.local", ["100.100.1.1"])):
...
def test_whitelist_does_not_affect_other_blocked():
    ...
    configure_ssrf_whitelist(["100.64.0.0/10"])
    try:
        with patch("ruiclaw.security.network.socket.getaddrinfo", _fake_resolve("evil.com", ["10.0.0.1"])):
...
def test_whitelist_allows_ipv6_mapped_cgnat():
    ...
    configure_ssrf_whitelist(["100.64.0.0/10"])
    try:
        with patch("ruiclaw.security.network.socket.getaddrinfo", _fake_resolve_v6("ts.local", ["::ffff:100.100.1.1"])):
            ok, err = validate_url_target("http://ts.local/api")
