...
from ruiclaw.providers.xai_grok_provider import (
...
def _mock_token(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "ruiclaw.providers.xai_grok_provider.get_xai_oauth_token",
...
def _mock_model_capabilities(
    ...
    *,
    ...
    monkeypatch.setattr(
        "ruiclaw.providers.xai_grok_provider.get_xai_grok_model_catalog",
        fake_catalog,
    )
...
async def test_provider_retries_stream_eof_before_returning_content(monkeypatch) -> None:
    ...
    monkeypatch.setattr("ruiclaw.providers.xai_grok_provider._request_xai", fake_request)
...
async def test_provider_injects_hosted_x_search_and_required_proxy_headers(monkeypatch) -> None:
    ...
    assert headers["x-grok-client-identifier"] == "ruiclaw"
...
async def test_second_401_is_non_retryable_and_prompts_reauthentication(monkeypatch) -> None:
    ...
    async def always_unauthorized(*_args, **_kwargs):
        raise _XAIHTTPError(
            "xAI rejected the login. Sign in again with `ruiclaw provider login xai-grok`.",
            status_code=401,
            should_retry=False,
        )

    monkeypatch.setattr(
        "ruiclaw.providers.xai_grok_provider._request_xai",
        always_unauthorized,
    )
    provider = XAIGrokProvider()
    ...
    assert "ruiclaw provider login xai-grok" in (response.content or "")
...
async def test_raw_response_request_streams_text_usage_and_inline_citations(monkeypatch) -> None:
    ...
    monkeypatch.setattr("ruiclaw.providers.xai_grok_provider.httpx.AsyncClient", fake_client)
    deltas: list[str] = []
...
def test_client_version_rejection_explains_update_and_preserves_body() -> None:
    ...
    assert "xAI requires a newer Grok client version. Update ruiclaw and try again." in str(error)
