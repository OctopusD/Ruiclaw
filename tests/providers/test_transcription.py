...
from ruiclaw.audio.transcription import (
...
from ruiclaw.providers.transcription import (
...
def test_transcription_registry_lists_providers_and_aliases() -> None:
    ...
    assert siliconflow.adapter == "ruiclaw.providers.transcription:OpenAITranscriptionProvider"
...
async def test_transcribe_audio_file_routes_openrouter_provider(audio_file: Path) -> None:
    ...
    with patch("ruiclaw.providers.transcription.OpenRouterTranscriptionProvider", StubOpenRouter):
...
async def test_transcribe_audio_file_routes_xiaomi_mimo_provider(audio_file: Path) -> None:
    ...
    with patch("ruiclaw.providers.transcription.XiaomiMiMoTranscriptionProvider", StubXiaomiMiMo):
        result = await transcribe_audio_file(audio_file, config)
...
async def test_transcribe_audio_file_routes_assemblyai_provider(audio_file: Path) -> None:
    ...
    with patch("ruiclaw.providers.transcription.AssemblyAITranscriptionProvider", StubAssemblyAI):
        result = await transcribe_audio_file(audio_file, config)
