...
import ruiclaw.providers.base as provider_base
from ruiclaw.config.schema import Config
from ruiclaw.providers.factory import make_provider
from ruiclaw.providers.openai_codex_provider import (
    OpenAICodexProvider,
...
from ruiclaw.providers.openai_responses import (
...
def _mock_codex_token(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "ruiclaw.providers.openai_codex_provider.get_codex_token",
        fake_token,
    )
...
async def test_codex_provider_reuses_tls_context_for_concurrent_requests(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "ruiclaw.providers.openai_codex_provider.httpx.create_ssl_context",
        fake_create_ssl_context,
    )
    monkeypatch.setattr("ruiclaw.providers.openai_codex_provider._request_codex", fake_request)
...
def _capture_codex_warnings(monkeypatch: pytest.MonkeyPatch) -> _WarningCaptureLogger:
    ...
    monkeypatch.setattr("ruiclaw.providers.openai_codex_provider.logger", capture)
    return capture
...
async def test_codex_request_non_200_populates_http_metadata(monkeypatch) -> None:
    ...
    monkeypatch.setattr("ruiclaw.providers.openai_codex_provider.httpx.AsyncClient", fake_client)
...
async def test_codex_request_honors_stream_idle_timeout_env(monkeypatch) -> None:
    """RUICLAW_STREAM_IDLE_TIMEOUT_S overrides the default Codex stream timeout."""
    monkeypatch.setenv("RUICLAW_STREAM_IDLE_TIMEOUT_S", "5")
    original_client = httpx.AsyncClient
    seen: dict[str, int] = {}
...
async def test_codex_omits_prompt_cache_key_without_session_id(monkeypatch) -> None:
    ...
    await provider.chat(
        [
            {"role": "system", "content": "You are ruiclaw."},
...
async def test_codex_provider_passes_proxy_to_oauth_and_response_request(monkeypatch) -> None:
    ...
    monkeypatch.setattr("ruiclaw.providers.openai_codex_provider.get_codex_token", fake_token)
    monkeypatch.setattr("ruiclaw.providers.openai_codex_provider._request_codex", fake_request)
...
async def test_codex_replayed_tool_turn_omits_server_item_ids(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "ruiclaw.providers.openai_codex_provider._request_codex",
        fake_request,
    )
