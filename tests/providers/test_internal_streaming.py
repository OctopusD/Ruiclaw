...
from ruiclaw.agent.memory import MemoryArchiver, MemoryStore
...
async def make_provider(monkeypatch):
    monkeypatch.setenv("RUICLAW_STREAM_IDLE_TIMEOUT_S", "0.3")
    clients = []
...
async def test_runner_streams_past_old_wall_limit_with_optional_ui(
    ...
    monkeypatch.setenv("RUICLAW_LLM_TIMEOUT_S", "0.01")
...
async def test_chat_only_provider_still_has_a_timeout(monkeypatch, finalize):
    monkeypatch.setenv("RUICLAW_STREAM_IDLE_TIMEOUT_S", "0.05")
