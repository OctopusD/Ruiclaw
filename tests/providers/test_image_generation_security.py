...
from ruiclaw.providers import image_generation
from ruiclaw.providers.image_generation import ImageGenerationError, _download_image_data_url
...
async def test_generated_image_download_revalidates_redirects(monkeypatch) -> None:
    ...
    monkeypatch.setattr("ruiclaw.security.network.socket.getaddrinfo", resolve_test_hosts)
    requested: list[str] = []
...
async def test_generated_image_download_delegates_unresolved_host_to_provider_proxy(
    ...
    monkeypatch.setattr("ruiclaw.security.network.socket.getaddrinfo", fail_local_dns)
