...
def generated_image_downloads(monkeypatch) -> list[tuple[str, str | None]]:
    ...
    monkeypatch.setattr(
        "ruiclaw.providers.image_generation._download_image_data_url",
        download,
    )
    return downloads
...
async def test_aihubmix_image_generation_payload_and_response() -> None:
    ...
    client = AIHubMixImageGenerationClient(
        ...
        extra_headers={"APP-Code": "ruiclaw"},
    ...
    assert call["headers"]["APP-Code"] == "ruiclaw"
    assert call["json"] == {
        "input": {
            "prompt": "draw a logo",
            "n": 1,
...
async def test_codex_proxy_applies_to_oauth_and_http(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "ruiclaw.providers.image_generation.httpx.AsyncClient",
        FakeAsyncClient,
    )
...
def _modelscope_fast_poll(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "ruiclaw.providers.image_generation._MODELSCOPE_POLL_INTERVAL_S", 0.0
    )
...
async def test_modelscope_image_generation_poll_timeout(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "ruiclaw.providers.image_generation._MODELSCOPE_POLL_MAX_ATTEMPTS", 3
    )
    submit = FakeResponse({"task_id": "t1"})
