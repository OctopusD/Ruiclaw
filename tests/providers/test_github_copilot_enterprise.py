...
from ruiclaw.providers import github_copilot_provider as gc


def test_resolve_falls_back_to_default_without_env(monkeypatch):
    monkeypatch.delenv("RUICLAW_COPILOT_BASE_URL", raising=False)
    assert gc._resolve("RUICLAW_COPILOT_BASE_URL", gc.DEFAULT_COPILOT_BASE_URL) == (
        gc.DEFAULT_COPILOT_BASE_URL
    )


def test_resolve_uses_env_override_and_strips(monkeypatch):
    monkeypatch.setenv("RUICLAW_COPILOT_TOKEN_URL", "  https://api.acme.ghe.com/copilot_internal/v2/token  ")
    assert gc._resolve("RUICLAW_COPILOT_TOKEN_URL", gc.DEFAULT_COPILOT_TOKEN_URL) == (
        "https://api.acme.ghe.com/copilot_internal/v2/token"
    )


def test_blank_env_override_falls_back_to_default(monkeypatch):
    monkeypatch.setenv("RUICLAW_COPILOT_BASE_URL", "   ")
    assert gc._resolve("RUICLAW_COPILOT_BASE_URL", gc.DEFAULT_COPILOT_BASE_URL) == (
        gc.DEFAULT_COPILOT_BASE_URL
    )
...
def test_provider_api_base_honors_env_override(monkeypatch):
    monkeypatch.setenv("RUICLAW_COPILOT_BASE_URL", "https://copilot-api.acme.ghe.com")
...
def test_login_uses_enterprise_endpoint_overrides(monkeypatch):
    monkeypatch.setenv("RUICLAW_GITHUB_COPILOT_CLIENT_ID", "enterprise-client-id")
    monkeypatch.setenv("RUICLAW_GITHUB_DEVICE_CODE_URL", "https://ghe.example/login/device/code")
    monkeypatch.setenv(
        "RUICLAW_GITHUB_ACCESS_TOKEN_URL",
        "https://ghe.example/login/oauth/access_token",
    )
    monkeypatch.setenv("RUICLAW_GITHUB_USER_URL", "https://api.ghe.example/user")
