...
from ruiclaw.providers import openai_compat_provider
...
async def test_openai_compat_provider_sets_timeout_on_local_http_client() -> None:
    ...
    with (
        patch("ruiclaw.providers.openai_compat_provider.AsyncOpenAI") as mock_async_openai,
        patch(
            "httpx.AsyncClient",
...
async def test_openai_compat_provider_timeout_can_be_overridden_by_env(monkeypatch) -> None:
    monkeypatch.setenv("RUICLAW_OPENAI_COMPAT_TIMEOUT_S", "45")
...
async def test_missing_langfuse_warning_recommends_plugin_command(monkeypatch) -> None:
    ...
    with (
        ...
        patch("ruiclaw.providers.openai_compat_provider.logger.warning") as mock_warning,
    ):
        provider = OpenAICompatProvider(api_key="test-key", api_base="https://example.com/v1")
        await provider._ensure_client()
