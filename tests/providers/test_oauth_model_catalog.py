...
from ruiclaw.providers.openai_codex_provider import DEFAULT_OPENAI_CODEX_MODELS_URL
from ruiclaw.providers.registry import ProviderModelSpec
from ruiclaw.providers.xai_grok_provider import DEFAULT_XAI_GROK_MODELS_URL
from ruiclaw.providers.xai_oauth import XAIToken
...
def test_xai_catalog_fetches_remote_models_and_reuses_capability_metadata(
    ...
    monkeypatch.setattr(
        "ruiclaw.providers.xai_grok_provider.get_xai_oauth_storage_path",
        lambda: tmp_path / "auth" / "xai.json",
    )
    monkeypatch.setattr(
        "ruiclaw.providers.xai_grok_provider.get_xai_oauth_login_status",
        lambda: token,
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr("ruiclaw.providers.xai_grok_provider.httpx.Client", fake_client)
...
def test_openai_codex_catalog_uses_account_catalog_and_filters_hidden_models(
    ...
    monkeypatch.setattr(
        "ruiclaw.providers.openai_codex_provider.FileTokenStorage",
    ...
    monkeypatch.setattr("ruiclaw.providers.openai_codex_provider.httpx.Client", fake_client)
...
def test_github_copilot_catalog_only_lists_compatible_chat_models(
    ...
    monkeypatch.setattr(
        "ruiclaw.providers.github_copilot_provider.get_storage",
        lambda: Storage(),
    )
    monkeypatch.setattr("ruiclaw.providers.github_copilot_provider.httpx.Client", fake_client)
