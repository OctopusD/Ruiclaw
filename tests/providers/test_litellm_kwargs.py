...
from ruiclaw.providers.base import ProviderCallContext
...
async def test_openai_compat_chat_stream_forwards_reasoning_deltas_deepseek_style() -> None:
    ...
    with patch("ruiclaw.providers.openai_compat_provider.AsyncOpenAI") as mock_openai:
...
async def test_deepseek_v4_pro_uses_responses_api() -> None:
    ...
    with patch("ruiclaw.providers.openai_compat_provider.AsyncOpenAI") as mock_client_class:
...
async def test_openrouter_sets_default_attribution_headers() -> None:
    spec = find_by_name("openrouter")
    with patch("ruiclaw.providers.openai_compat_provider.AsyncOpenAI") as mock_client_cls:
        provider = OpenAICompatProvider(
    ...
    assert headers["HTTP-Referer"] == "https://github.com/OctopusD/Ruiclaw"
    assert headers["X-OpenRouter-Title"] == "ruiclaw"
...
async def test_openrouter_user_headers_override_default_attribution() -> None:
    ...
    with patch("ruiclaw.providers.openai_compat_provider.AsyncOpenAI") as mock_client_cls:
        provider = OpenAICompatProvider(
            ...
            default_model="anthropic/claude-sonnet-4-5",
            extra_headers={
                "HTTP-Referer": "https://ruiclaw.ai",
                "X-OpenRouter-Title": "RuiClaw Pro",
    ...
    headers = mock_client_cls.call_args.kwargs["default_headers"]
    assert headers["HTTP-Referer"] == "https://ruiclaw.ai"
    assert headers["X-OpenRouter-Title"] == "RuiClaw Pro"
    assert headers["X-OpenRouter-Categories"] == "cli-agent,personal-agent"
...
async def test_openai_compat_stream_watchdog_returns_error_on_stall(monkeypatch) -> None:
    monkeypatch.setenv("RUICLAW_STREAM_IDLE_TIMEOUT_S", "0.01")
