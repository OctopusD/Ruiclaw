...
from ruiclaw.providers.openai_responses.converters import (
    convert_messages,
    convert_tools,
    convert_user_message,
    split_tool_call_id,
)
from ruiclaw.providers.openai_responses.parsing import (
...
from ruiclaw.providers.openai_responses.state import (
...
class TestParseResponseOutput:
    ...
    def test_malformed_tool_arguments_logged(self):
        ...
        with patch("ruiclaw.providers.openai_responses.parsing.logger") as mock_logger:
            result = parse_response_output(resp)
    ...
    async def test_hosted_web_search_lifecycle_is_streamed_as_tool_progress(self):
        ...
        search_done = SimpleNamespace(
            ...
            action=SimpleNamespace(
                type="search",
                queries=["ruiclaw DeepSeek", "ruiclaw latest release"],
                sources=[
                    SimpleNamespace(
        ...
        assert tool_events == [
            ...
            },
            {
                ...
                "name": "web_search",
                "arguments": {
                    "query": "ruiclaw DeepSeek · ruiclaw latest release",
                },
                "result": {
                    "status": "completed",
                    "sources": [{
