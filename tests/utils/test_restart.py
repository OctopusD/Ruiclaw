...
def test_set_and_consume_restart_notice_env_roundtrip(monkeypatch):
    monkeypatch.delenv("RUICLAW_RESTART_NOTIFY_CHANNEL", raising=False)
    monkeypatch.delenv("RUICLAW_RESTART_NOTIFY_CHAT_ID", raising=False)
    monkeypatch.delenv("RUICLAW_RESTART_NOTIFY_METADATA", raising=False)
    monkeypatch.delenv("RUICLAW_RESTART_STARTED_AT", raising=False)
    ...
    assert "RUICLAW_RESTART_NOTIFY_CHANNEL" not in os.environ
    assert "RUICLAW_RESTART_NOTIFY_CHAT_ID" not in os.environ
    assert "RUICLAW_RESTART_NOTIFY_METADATA" not in os.environ
    assert "RUICLAW_RESTART_STARTED_AT" not in os.environ
...
def test_restart_notice_clears_stale_metadata(monkeypatch):
    monkeypatch.setenv("RUICLAW_RESTART_NOTIFY_METADATA", '{"stale": true}')
    set_restart_notice_to_env(channel="cli", chat_id="direct")
    assert "RUICLAW_RESTART_NOTIFY_METADATA" not in os.environ


def test_format_restart_completed_message_with_elapsed(monkeypatch):
    monkeypatch.setattr("ruiclaw.utils.restart.time.time", lambda: 102.0)
