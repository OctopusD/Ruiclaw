...
import ruiclaw.webui.transcript as transcript_module
...
def test_append_stamps_created_at_ms(tmp_path, monkeypatch) -> None:
    monkeypatch.setattr("ruiclaw.config.paths.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr("ruiclaw.webui.transcript.time.time", lambda: 1_700_000_000.0)
...
def _force_small_transcript_budget(monkeypatch, *, limit: int = 520, target: int = 260) -> None:
    monkeypatch.setattr("ruiclaw.webui.transcript._MAX_TRANSCRIPT_FILE_BYTES", limit)
    monkeypatch.setattr("ruiclaw.webui.transcript._ACTIVE_TRANSCRIPT_ROTATE_BYTES", limit)
    monkeypatch.setattr("ruiclaw.webui.transcript._TARGET_ACTIVE_TRANSCRIPT_BYTES", target)
...
def test_delete_webui_transcript_removes_segments(tmp_path, monkeypatch) -> None:
    from ruiclaw.webui.thread_disk import webui_thread_file_path
    from ruiclaw.webui.transcript import delete_webui_transcript, webui_transcript_path
