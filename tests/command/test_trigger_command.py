...
from ruiclaw.command.builtin import build_help_text, register_builtin_commands
...
async def test_trigger_command_creates_session_bound_local_trigger(tmp_path: Path) -> None:
    ...
    msg = InboundMessage(
        ...
        sender_id="user",
        chat_id="chat-1",
        content="/trigger@ruiclaw_bot PR review",
        metadata={"webui": True},
    )
    ctx = CommandContext(
        msg=msg,
        ...
        raw="/trigger@ruiclaw_bot PR review",
        loop=loop,
    )

    assert router.is_dispatchable_command("/trigger@ruiclaw_bot PR review") is True
    ...
    assert f"ruiclaw trigger {trigger.id} \"message\"" in response.content
