...
from ruiclaw.agent.context import RUICLAW_CONTEXT_SOURCE_WEIGHTS_META
...
from ruiclaw.providers.base import GenerationSettings, LLMResponse, ProviderConversationState
...
@pytest.mark.asyncio
@pytest.mark.parametrize("command", ["/compact", " /COMPACT@ruiclaw "])
...
async def test_checkpoint_continues_through_reloaded_session(loop, trigger, summary) -> None:
    ...
    assert sent[0]["content"] == loop.context.build_system_prompt(
        channel="cli", session_summary=expected_summary,
    )
    source_weights = sent[0]["_meta"][RUICLAW_CONTEXT_SOURCE_WEIGHTS_META]
    assert source_weights["bootstrap"] > 0
    assert source_weights["skills"] > 0
...
async def test_compact_during_active_turn_waits_for_the_session_lock(loop) -> None:
    key = "websocket:test"
    msg = InboundMessage(
        channel="websocket", sender_id="user", chat_id="test", content="/COMPACT@ruiclaw",
    )
