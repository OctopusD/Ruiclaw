...
from ruiclaw.webui.http_utils import http_json_response
from ruiclaw.webui.mcp_presets_api import custom_mcp_action
...
def _mutation_request(path: str, payload: dict[str, object]) -> SimpleNamespace:
    ...
    request._ruiclaw_webui_mutation_request = True
    request._ruiclaw_webui_mutation_payload = payload
    request._ruiclaw_trusted_proxy_authenticated = True
    return request
...
async def test_usage_query_runs_off_the_event_loop(monkeypatch) -> None:
    ...
    monkeypatch.setattr("ruiclaw.webui.settings_routes.settings_usage_payload", usage_payload)
...
async def test_mcp_reload_callback_is_bounded(
    ...
    monkeypatch.setattr(
        "ruiclaw.webui.settings_routes._MCP_RELOAD_TIMEOUT_SECONDS",
        0.01,
    )
...
async def test_mcp_oauth_start_uses_gateway_callback_and_requires_api_auth(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "ruiclaw.webui.settings_routes.ensure_mcp_oauth_server",
...
async def test_oauth_completion_reads_websocket_payload(
    ...
    monkeypatch.setattr("ruiclaw.webui.settings_routes.complete_oauth_provider", complete)
    router = _router()
    request = _mutation_request(
...
async def test_runtime_config_mutation_routes_refresh_live_runtime(
    ...
    monkeypatch.setattr(f"ruiclaw.webui.settings_routes.{function_name}", mutate)
...
async def test_model_update_route_forwards_session_rename_dependency(monkeypatch) -> None:
    ...
    monkeypatch.setattr("ruiclaw.webui.settings_routes.update_model_configuration", update)
        ...
        (
            {
                ...
                "latestVersion": "1.3.0",
                "pypiUrl": "https://pypi.org/project/ruiclaw-ai/",
            },
            {
                "updateAvailable": {
                    "currentVersion": "1.2.0",
...
async def test_version_check_route_returns_stable_payload(
    ...
    monkeypatch.setattr(
        "ruiclaw.webui.settings_routes.check_for_update",
        lambda: update_info,
    )
...
async def test_version_check_route_enforces_auth_and_bounds_failures(
    ...
    monkeypatch.setattr("ruiclaw.webui.settings_routes.check_for_update", check)
    request = SimpleNamespace(path="/api/settings/version-check", headers=Headers())
...
async def test_image_switch_round_trip_clears_restart(tmp_path, monkeypatch, enabled):
    from ruiclaw.config.loader import save_config
