...
def test_skill_detail_exposes_copyable_install_commands(tmp_path: Path) -> None:
    _write_skill(
        ...
        "custom-skill",
        metadata=(
            'metadata: {"ruiclaw":{"requires":{"bins":["demo"]},'
...
def test_set_webui_skill_enabled_persists_and_updates_runtime(
    ...
    config = _config()
    saved: list[object] = []
    monkeypatch.setattr("ruiclaw.webui.skills_api.load_config", lambda _path=None: config)
    monkeypatch.setattr(
        "ruiclaw.webui.skills_api.save_config",
...
def test_delete_webui_skill_restores_directory_when_config_save_fails(
    ...
    monkeypatch.setattr("ruiclaw.webui.skills_api.save_config", fail_save)
    disabled = {"custom-skill"}
