...
from ruiclaw.config.schema import Config, InlineFallbackConfig, ModelPresetConfig
from ruiclaw.llm_usage import get_llm_usage_store
from ruiclaw.llm_usage.models import LLMCallRecord
from ruiclaw.providers.base import LLMUsage
from ruiclaw.providers.oauth_model_catalog import OAuthModelCatalogSnapshot
from ruiclaw.providers.registry import ProviderModelSpec, find_by_name
...
from ruiclaw.webui.settings_services import WebUIOAuthFlowRegistry
...
def test_settings_payload_propagates_preset_resolution_failure(
    ...
    config = Config()
    monkeypatch.setattr("ruiclaw.webui.settings_api.load_config", lambda: config)
    monkeypatch.setattr(
        Config,
        "resolve_preset",
...
def test_settings_payload_includes_versioned_docs(
    ...
    monkeypatch.setattr("ruiclaw.webui.settings_api.__version__", "0.2.3")

    payload = settings_payload()

    assert payload["docs"] == {
        "version": "0.2.3",
        "base_url": "https://ruiclaw.wiki/docs/0.2.3",
        "chat_apps_url": "https://ruiclaw.wiki/docs/0.2.3/getting-started/chat-apps",
        "latest_url": "https://ruiclaw.wiki/docs/latest",
    }
...
def test_update_model_configuration_rolls_back_sessions_when_config_save_fails(
    ...
    monkeypatch.setattr("ruiclaw.webui.settings_api._save_settings_config", fail_save)
...
def test_update_agent_settings_marks_timezone_as_manual(
    ...
    monkeypatch.setattr(
        "ruiclaw.config.timezone.get_localzone_name",
...
def test_settings_payload_includes_oauth_provider_status(
    ...
    monkeypatch.setattr("ruiclaw.webui.settings_api._oauth_provider_status", fake_oauth_status)
...
def test_xai_grok_status_accepts_refreshable_login(
    ...
    monkeypatch.setattr(
        "ruiclaw.providers.xai_oauth.get_xai_oauth_login_status",
        lambda: token,
    )
...
def test_openai_codex_oauth_login_passes_configured_proxy(
    ...
    monkeypatch.setattr(
        "ruiclaw.providers.openai_codex_oauth.start_openai_codex_oauth_login",
        fake_start,
    )
    ...
    monkeypatch.setattr(
        "ruiclaw.providers.openai_codex_oauth.complete_openai_codex_oauth_login",
        fake_complete,
    )
    monkeypatch.setattr(
        "ruiclaw.webui.settings_api.settings_payload",
...
def test_openai_codex_oauth_login_reports_missing_oauth_cli_kit(
    ...
    def fake_import(name, *args, **kwargs):
        if name == "ruiclaw.providers.openai_codex_oauth":
...
def test_github_copilot_oauth_login_reports_missing_oauth_cli_kit(
    ...
    def fake_import(name, *args, **kwargs):
        if name == "ruiclaw.providers.github_copilot_provider":
            raise ImportError("missing")
        return real_import(name, *args, **kwargs)
...
def test_xai_grok_login_starts_fresh_browser_flow_with_proxy(
    ...
    monkeypatch.setattr("ruiclaw.providers.xai_oauth.start_xai_oauth_login", fake_start)
    ...
    monkeypatch.setattr(
        "ruiclaw.providers.xai_oauth.complete_xai_oauth_login",
        fake_complete,
    )
    monkeypatch.setattr(
        "ruiclaw.webui.settings_api.settings_payload",
...
def test_provider_models_payload_fetches_openai_compatible_models(
    ...
    monkeypatch.setattr("ruiclaw.webui.settings_api.httpx.get", fake_get)
...
def test_provider_models_payload_returns_online_openai_codex_models(
    ...
    monkeypatch.setattr(
        "ruiclaw.webui.settings_models.get_oauth_model_catalog",
...
def test_azure_openai_spec_no_longer_requires_api_key() -> None:
    ...
    from ruiclaw.webui.settings_api import _provider_requires_api_key
