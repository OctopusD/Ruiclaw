...
from ruiclaw.agent.plugins import AGENT_PLUGIN_MCP_SCHEMA, AGENT_PLUGIN_SCHEMA
from ruiclaw.agent.tools.mcp_oauth import MCPOAuthStorage, mcp_oauth_has_credentials
...
def test_test_mcp_preset_reports_missing_dependency(
    ...
    _use_config(tmp_path, monkeypatch)
    mcp_presets_action("enable", {"name": ["playwright"]})
    monkeypatch.setattr("ruiclaw.webui.mcp_presets_api.shutil.which", lambda _command: None)
...
def test_test_mcp_preset_connects_and_reports_tools(
    ...
    monkeypatch.setattr("ruiclaw.agent.tools.mcp.connect_mcp_servers", fake_connect)
...
def test_normalize_mcp_mentions_uses_explicit_gateway_config(
    ...
    monkeypatch.setattr("ruiclaw.config.loader._current_config_path", default_path)
    custom_mcp_action(
        "custom",
        {
            "name": ["gateway-docs"],
