...
async def test_webui_transcribe_audio_rejects_oversized_audio(
    ...
    save_config(config, config_path)
    monkeypatch.setattr("ruiclaw.config.loader._current_config_path", config_path)
    monkeypatch.setattr("ruiclaw.audio.transcription.get_media_dir", lambda _channel=None: tmp_path)
...
async def test_webui_transcribe_audio_returns_text_and_removes_temp_file(
    ...
    monkeypatch.setattr("ruiclaw.config.loader._current_config_path", config_path)
    monkeypatch.setattr(
        "ruiclaw.audio.transcription.get_media_dir",
    ...
    monkeypatch.setattr(
        "ruiclaw.audio.transcription.transcribe_audio_file",
