...
from ruiclaw.events import ContextCompactionEvent, RecoveryStateEvent, RetryStatusEvent
...
def test_wire_fixtures_are_real_python_projections(monkeypatch):
    monkeypatch.setattr("ruiclaw.webui.outbound_wire.time.time", lambda: 100.0)
...
def test_tui_source_archive_preserves_shared_module_import_path():
    ...
    with tarfile.open(fileobj=io.BytesIO(module._source_archive(ROOT / "tui"))) as archive:
        ...
        assert "ruiclaw-tui-source/tui/src/protocol.ts" in names
        assert "ruiclaw-tui-source/packages/client-events/notifications.ts" in names
        assert "ruiclaw-tui-source/tui/bun.lock" in names
        assert "ruiclaw-tui-source/LICENSE" in names
