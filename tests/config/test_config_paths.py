...
def test_runtime_dirs_follow_config_path(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("ruiclaw.config.paths.get_config_path", lambda: config_file)
...
def test_shared_and_legacy_paths_remain_global() -> None:
    assert get_cli_history_path() == Path.home() / ".ruiclaw" / "history" / "cli_history"
    assert get_legacy_sessions_dir() == Path.home() / ".ruiclaw" / "sessions"


def test_workspace_path_is_explicitly_resolved() -> None:
    assert get_workspace_path() == Path.home() / ".ruiclaw" / "workspace"
...
def test_is_default_workspace_distinguishes_default_and_custom_paths() -> None:
    ...
    assert is_default_workspace(Path.home() / ".ruiclaw" / "workspace") is True
