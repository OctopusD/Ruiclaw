...
from ruiclaw.agent.tools import web as web_module
from ruiclaw.agent.tools.web import WebFetchTool, _get_with_safe_redirects
from ruiclaw.config.schema import WebFetchConfig
from ruiclaw.security.network import PinnedDNSAsyncTransport
...
def _patch_web_fetch_fake_client(monkeypatch: pytest.MonkeyPatch) -> list[dict]:
    ...
    monkeypatch.setattr(
        "ruiclaw.security.network.httpx.AsyncHTTPTransport",
...
async def test_web_fetch_blocks_private_ip():
    tool = WebFetchTool()
    with patch("ruiclaw.security.network.socket.getaddrinfo", _fake_resolve_private):
...
async def test_web_fetch_blocks_localhost():
    ...
    with patch("ruiclaw.security.network.socket.getaddrinfo", _resolve_localhost):
...
async def test_web_fetch_result_contains_untrusted_flag(monkeypatch: pytest.MonkeyPatch):
    ...
    with patch("ruiclaw.security.network.socket.getaddrinfo", _fake_resolve_public):
...
async def test_safe_redirect_requests_use_independent_pinned_dns_concurrently(monkeypatch):
    ...
    monkeypatch.setattr("ruiclaw.security.network.socket.getaddrinfo", _rebinding_resolver)
...
async def test_web_fetch_can_skip_jina_and_use_custom_user_agent(monkeypatch):
    ...
    monkeypatch.setattr("ruiclaw.agent.tools.web.httpx.AsyncClient", FakeClient)
    monkeypatch.setattr(web_module, "_pinned_dns_transport", lambda: object())
...
async def test_web_fetch_blocks_private_redirect_before_readability_request(monkeypatch):
    ...
    with patch("ruiclaw.security.network.socket.getaddrinfo", resolve_public_start_only):
...
async def test_web_fetch_blocks_private_redirect_before_returning_image(monkeypatch):
    ...
    monkeypatch.setattr("ruiclaw.agent.tools.web.httpx.AsyncClient", TransportAsyncClient)
    monkeypatch.setattr(web_module, "_pinned_dns_transport", lambda: object())
