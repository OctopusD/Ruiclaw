...
from ruiclaw.agent.tools.search import FindFilesTool, GrepTool
...
async def test_web_search_tool_refreshes_dynamic_config_loader(monkeypatch) -> None:
    ...
    assert await tool.execute("ruiclaw") == "duckduckgo:ruiclaw:3"
...
async def test_grep_searches_xlsx_beyond_attachment_preview_limit(
    ...
    from ruiclaw.utils import document as document_utils
