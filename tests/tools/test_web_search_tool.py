...
from ruiclaw.agent.tools.registry import is_tool_error_result
from ruiclaw.agent.tools.web import WebSearchTool
from ruiclaw.config.schema import WebSearchConfig
...
async def test_brave_search(monkeypatch):
    async def mock_get(self, url, **kw):
        ...
        assert kw["headers"]["User-Agent"] == "ruiclaw-search-test"
        return _response(json={
            "web": {"results": [{"title": "RuiClaw", "url": "https://example.com", "description": "AI assistant"}]}
        })

    monkeypatch.setattr(httpx.AsyncClient, "get", mock_get)
    tool = _tool(provider="brave", api_key="brave-key", user_agent="ruiclaw-search-test")
    result = await tool.execute(query="ruiclaw", count=1)
    assert "RuiClaw" in result
...
async def test_brave_search_retries_rate_limit_once(monkeypatch):
    ...
    monkeypatch.setattr("ruiclaw.agent.tools.web.asyncio.sleep", mock_sleep)
    monkeypatch.setattr(httpx.AsyncClient, "get", mock_get)
...
async def test_tavily_search(monkeypatch):
    ...
    tool = _tool(provider="tavily", api_key="tavily-key", user_agent="ruiclaw-search-test")
...
async def test_keenable_search(monkeypatch):
    async def mock_post(self, url, **kw):
        ...
        assert kw["headers"]["User-Agent"] == "ruiclaw-search-test"
        assert kw["headers"]["X-Keenable-Title"] == "ruiclaw"
        return _response(json={
            "results": [{"title": "Keen", "url": "https://keenable.ai", "description": "short", "snippet": "longer excerpt"}]
        })

    monkeypatch.setattr(httpx.AsyncClient, "post", mock_post)
    tool = _tool(provider="keenable", api_key="keen-key", user_agent="ruiclaw-search-test")
...
async def test_serper_search(monkeypatch):
    ...
    monkeypatch.setattr(httpx.AsyncClient, "post", mock_post)
    tool = _tool(provider="serper", api_key="serper-key", user_agent="ruiclaw-search-test")
...
async def test_anysearch_search(monkeypatch):
    async def mock_post(self, url, **kw):
        ...
        assert kw["headers"]["X-Anysearch-Client"] == "ruiclaw/1.0.0"
        assert kw["headers"]["User-Agent"] == "ruiclaw-search-test"
    ...
    monkeypatch.setattr(httpx.AsyncClient, "post", mock_post)
    tool = _tool(provider="anysearch", api_key="anysearch-key", user_agent="ruiclaw-search-test")
...
async def test_anysearch_without_api_key_uses_anonymous_quota(monkeypatch):
    ...
    monkeypatch.setattr(httpx.AsyncClient, "post", mock_post)
    monkeypatch.delenv("ANYSEARCH_API_KEY", raising=False)
    tool = _tool(provider="anysearch", api_key="", user_agent="ruiclaw-search-test")
    result = await tool.execute(query="anysearch", count=1)
...
async def test_bocha_search(monkeypatch):
    ...
    monkeypatch.setattr(httpx.AsyncClient, "post", mock_post)
    tool = _tool(provider="bocha", api_key="bocha-key", user_agent="ruiclaw-search-test")
...
async def test_volcengine_search(monkeypatch):
    async def mock_post(self, url, **kw):
        ...
        assert kw["headers"]["X-Traffic-Tag"] == "ruiclaw"
        assert kw["headers"]["User-Agent"] == "ruiclaw-search-test"
        assert kw["json"] == {
            "Query": "北京周边游",
            "SearchType": "web",
    ...
    monkeypatch.setattr(httpx.AsyncClient, "post", mock_post)
    tool = _tool(provider="volcengine", api_key="volc-key", user_agent="ruiclaw-search-test")
...
async def test_searxng_search(monkeypatch):
    ...
    monkeypatch.setattr(httpx.AsyncClient, "get", mock_get)
    tool = _tool(provider="searxng", base_url="https://searx.example", user_agent="ruiclaw-search-test")
    result = await tool.execute(query="test")
...
async def test_duckduckgo_search(monkeypatch):
    ...
    monkeypatch.setattr("ruiclaw.agent.tools.web.DDGS", MockDDGS, raising=False)
    import ruiclaw.agent.tools.web as web_mod
...
async def test_jina_search(monkeypatch):
    ...
    monkeypatch.setattr(httpx.AsyncClient, "get", mock_get)
    tool = _tool(provider="jina", api_key="jina-key", user_agent="ruiclaw-search-test")
    result = await tool.execute(query="test")
...
async def test_kagi_search(monkeypatch):
    ...
    monkeypatch.setattr(httpx.AsyncClient, "post", mock_post)
    tool = _tool(provider="kagi", api_key="kagi-key", user_agent="ruiclaw-search-test")
...
async def test_exa_search(monkeypatch):
    ...
    monkeypatch.setattr(httpx.AsyncClient, "post", mock_post)
    tool = _tool(provider="exa", api_key="exa-key", user_agent="ruiclaw-search-test")
    result = await tool.execute(query="test", count=2)
...
async def test_olostep_package_missing_returns_install_hint(monkeypatch):
    ...
    assert result == (
        "Error: Olostep support is not installed. Run `ruiclaw plugins enable olostep`."
    )
