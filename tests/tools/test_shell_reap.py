...
from ruiclaw.agent.tools.exec_session import _ExecSession
from ruiclaw.agent.tools.shell import ExecTool, _reap_pid
...
def test_reap_pid_noops_without_waitpid():
    ...
    with patch("ruiclaw.agent.tools.shell.os") as mock_os:
...
async def test_kill_process_skips_kill_when_already_exited():
    ...
    with patch("ruiclaw.agent.tools.shell._reap_pid") as reap:
...
async def test_execute_reaps_after_normal_completion():
    ...
    with (
        ...
        patch("ruiclaw.agent.tools.shell._reap_pid") as reap,
    ):
        tool = ExecTool()
...
async def test_execute_exception_after_success_does_not_raise_on_dead_process():
    ...
    with (
        ...
        patch("ruiclaw.agent.tools.shell._reap_pid") as reap,
        patch(
            "ruiclaw.agent.tools.shell.clamp_session_int",
...
async def test_kill_process_tree_targets_group_after_root_exits():
    ...
    with (
        ...
        patch("ruiclaw.agent.tools.shell.os.killpg", create=True) as kill_group,
        patch("ruiclaw.agent.tools.shell.signal.SIGKILL", 9, create=True),
        patch("ruiclaw.agent.tools.shell._reap_pid") as reap,
    ):
