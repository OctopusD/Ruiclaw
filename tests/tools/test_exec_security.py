...
async def test_exec_blocks_wget_localhost():
    tool = ExecTool(restrict_to_workspace=True)
    with patch("ruiclaw.security.network.socket.getaddrinfo", _fake_resolve_localhost):
...
# --- #2989: block writes to ruiclaw internal state files -----------------
...
def test_exec_allows_absolute_path_inside_sandbox_ro_bind(tmp_path, monkeypatch, backend):
    ...
    monkeypatch.setattr("ruiclaw.agent.tools.shell._IS_WINDOWS", False)
    tool = ExecTool(
        working_dir=str(workspace),
        restrict_to_workspace=True,
        sandbox=backend,
...
def test_exec_allows_workspace_paths_from_subdirectory(tmp_path):
    ...
    commands in ``~/.ruiclaw/workspace/obsidian_notes`` reference paths
