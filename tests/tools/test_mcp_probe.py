...
from ruiclaw.agent.tools import mcp as mcp_mod
from ruiclaw.agent.tools.mcp import _probe_http_url, connect_mcp_servers
from ruiclaw.agent.tools.registry import ToolRegistry
from ruiclaw.config.schema import MCPServerConfig
from ruiclaw.security.network import configure_ssrf_whitelist
...
async def test_probe_uses_default_port_for_http(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        "ruiclaw.agent.tools.mcp.resolve_url_target",
    ...
    monkeypatch.setattr("ruiclaw.agent.tools.mcp.asyncio.open_connection", _open_connection)
...
async def test_probe_tries_next_validated_ip_when_first_is_unreachable(monkeypatch):
    ...
    monkeypatch.setattr("ruiclaw.security.network.socket.getaddrinfo", _resolver)
    monkeypatch.setattr("ruiclaw.agent.tools.mcp.asyncio.open_connection", _open_connection)
...
async def test_connect_skips_unreachable_streamable_http():
    ...
    with patch("ruiclaw.agent.tools.mcp._probe_http_url", _unreachable):
...
async def test_probe_not_called_for_stdio():
    ...
    with patch("ruiclaw.agent.tools.mcp._probe_http_url", _spy_probe):
        cfg = MagicMock()
        cfg.type = "stdio"
        cfg.url = None
