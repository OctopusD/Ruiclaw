...
from ruiclaw.agent.tools.web import WebFetchTool, _validate_url
...
def _patched_web_fetch():
    with (
        patch("ruiclaw.security.network.socket.getaddrinfo", _fake_resolve_public),
        patch("ruiclaw.agent.tools.web.httpx.AsyncClient", FakeClient),
        patch("ruiclaw.agent.tools.web._pinned_dns_transport", lambda: object()),
    ):
        yield
