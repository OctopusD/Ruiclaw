...
from ruiclaw.agent.tools import (
    ArraySchema,
    IntegerSchema,
    ObjectSchema,
    Schema,
...
def test_exec_extract_absolute_paths_captures_home_paths() -> None:
    cmd = "cat ~/.ruiclaw/config.json > ~/out.txt"
    paths = ExecTool._extract_absolute_paths(cmd)
    assert "~/.ruiclaw/config.json" in paths
    assert "~/out.txt" in paths


def test_exec_extract_absolute_paths_captures_paths_after_equals() -> None:
    cmd = "curl --output=/etc/passwd --config=~/.ruiclaw/config.json --user-home=~root"
    paths = ExecTool._extract_absolute_paths(cmd)
...
def test_exec_extract_absolute_paths_captures_quoted_paths() -> None:
    cmd = 'cat "/tmp/data.txt" "~/.ruiclaw/config.json"'
    paths = ExecTool._extract_absolute_paths(cmd)
    assert "/tmp/data.txt" in paths
    assert "~/.ruiclaw/config.json" in paths
...
def test_exec_guard_blocks_home_path_outside_workspace(tmp_path) -> None:
    tool = ExecTool(restrict_to_workspace=True)
    error = tool._guard_command("cat ~/.ruiclaw/config.json", str(tmp_path))
...
def test_exec_guard_blocks_equals_home_path_outside_workspace(tmp_path) -> None:
    tool = ExecTool(restrict_to_workspace=True)
    error = tool._guard_command("cat --config=~/.ruiclaw/config.json", str(tmp_path))
    assert error is not None
    assert error.startswith(
        "Error: Command blocked by safety guard (path outside working dir)"
    )
...
def test_exec_guard_allows_media_path_outside_workspace(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("ruiclaw.agent.tools.shell.get_media_dir", lambda: media_dir)
...
def test_exec_guard_blocks_windows_drive_root_outside_workspace(monkeypatch) -> None:
    import ruiclaw.agent.tools.shell as shell_mod
