...
import ruiclaw.agent.tools.mcp as mcp_mod
from ruiclaw.agent.tools.mcp import (
    MCPPromptWrapper,
    MCPProvider,
    MCPResourceWrapper,
    MCPToolWrapper,
...
async def test_saved_oauth_http_403_projects_failed_runtime_without_details(
    ...
    oauth_mod = ModuleType("ruiclaw.agent.tools.mcp_oauth")
    ...
    monkeypatch.setitem(sys.modules, "ruiclaw.agent.tools.mcp_oauth", oauth_mod)
...
async def test_connect_mcp_servers_enabled_tools_warns_on_unknown_entries(
    ...
    monkeypatch.setattr("ruiclaw.agent.tools.mcp.logger.warning", _warning)
...
async def test_connect_mcp_servers_passes_stdio_cwd(
    ...
    registry = ToolRegistry()
    stacks = await connect_mcp_servers(
        {"test": MCPServerConfig(command="fake", cwd="/tmp/ruiclaw-mcp-test")},
        registry,
    )
    for stack in stacks.values():
        await stack.aclose()

    assert captured["cwd"] == "/tmp/ruiclaw-mcp-test"
