...
from ruiclaw.agent.tools.exec_session import ExecSessionManager, ExecSessionTool
from ruiclaw.agent.tools.shell import ExecTool
...
class TestBuildEnvUnix:
    ...
    def test_expected_keys(self):
        with patch("ruiclaw.agent.tools.shell._IS_WINDOWS", False):
    ...
    def test_secrets_excluded(self, monkeypatch):
        ...
        monkeypatch.setenv("RUICLAW_TOKEN", "tok-secret")
        with patch("ruiclaw.agent.tools.shell._IS_WINDOWS", False):
            env = ExecTool()._build_env()
        assert "OPENAI_API_KEY" not in env
        assert "RUICLAW_TOKEN" not in env
        for v in env.values():
...
class TestBuildEnvWindows:
    ...
    def test_expected_keys(self):
        with patch("ruiclaw.agent.tools.shell._IS_WINDOWS", True):
            env = ExecTool()._build_env()
    ...
    def test_path_has_sensible_default(self):
        with (
            patch("ruiclaw.agent.tools.shell._IS_WINDOWS", True),
...
class TestSpawnUnix:
    ...
    async def test_uses_bash(self):
        with (
            patch("ruiclaw.agent.tools.shell._IS_WINDOWS", False),
...
class TestSpawnWindows:
    ...
    async def test_job_assignment_failure_kills_suspended_process(self):
        ...
        with (
            patch("ruiclaw.agent.tools.shell._IS_WINDOWS", True),
            patch("ruiclaw.agent.tools.shell.sys", MagicMock(platform="win32")),
            patch("asyncio.create_subprocess_exec", new_callable=AsyncMock) as mock_exec,
    ...
    async def test_prefers_pwsh_when_available(self):
        ...
        with (
            patch("ruiclaw.agent.tools.shell._IS_WINDOWS", True),
            patch("ruiclaw.agent.tools.shell.shutil.which", side_effect=fake_which),
            patch("asyncio.create_subprocess_exec", new_callable=AsyncMock) as mock_exec,
        ):
            mock_exec.return_value = AsyncMock()
            await ExecTool._spawn("dir", r"C:\work", env)
...
class TestPathAppendPlatform:
    ...
    async def test_unix_uses_env_var_in_fixed_export(self):
        ...
        with (
            patch("ruiclaw.agent.tools.shell._IS_WINDOWS", False),
            patch("ruiclaw.agent.tools.shell.os.pathsep", ":"),
        ...
        assert captured_cmd == 'export PATH="$PATH:$RUICLAW_PATH_APPEND"; ls'
        assert captured_env["RUICLAW_PATH_APPEND"] == "/opt/bin; echo INJECTED"
    ...
    async def test_unix_path_prepend_uses_env_var_in_fixed_export(self):
        ...
        assert captured_cmd == 'export PATH="$RUICLAW_PATH_PREPEND:$PATH"; python --version'
        assert captured_env["RUICLAW_PATH_PREPEND"] == "/venv/bin; echo INJECTED"
        assert "INJECTED" not in captured_cmd
    ...
    async def test_unix_path_prepend_and_append_order(self):
        ...
        assert captured_cmd == (
            'export PATH="$RUICLAW_PATH_PREPEND:$PATH:$RUICLAW_PATH_APPEND"; python --version'
        )
        assert captured_env["RUICLAW_PATH_PREPEND"] == "/venv/bin"
        assert captured_env["RUICLAW_PATH_APPEND"] == "/usr/sbin"
...
class TestSandboxPlatform:
    ...
    async def test_sandbox_applied_on_unix(self, backend):
        ...
        with (
            patch("ruiclaw.agent.tools.shell._IS_WINDOWS", False),
            patch("ruiclaw.agent.tools.shell.wrap_command", return_value=f"{backend} -- sh -c ls") as mock_wrap,
            patch.object(ExecTool, "_spawn", return_value=mock_proc) as mock_spawn,
            patch.object(ExecTool, "_guard_command", return_value=None),
        ):
