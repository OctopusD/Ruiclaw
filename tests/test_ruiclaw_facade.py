"""Tests for the RuiClaw programmatic facade."""
...
from ruiclaw.utils.llm_runtime import runtime_from_provider_snapshot
...
def test_from_config_missing_file():
    with pytest.raises(FileNotFoundError):
        RuiClaw.from_config("/nonexistent/config.json")
...
def test_from_config_missing_env_reports_explicit_config_path(
    ...
    name = "RUICLAW_TEST_SDK_MISSING_KEY"
    ...
    with pytest.raises(ConfigLoadError) as exc_info:
        RuiClaw.from_config(config_path)
...
def test_from_config_creates_instance(tmp_path):
    ...
    bot = RuiClaw.from_config(config_path, workspace=workspace)
...
def test_from_config_composes_configured_mcp_outside_agent_loop(tmp_path):
    ...
    bot = RuiClaw.from_config(config_path, workspace=tmp_path)
...
def test_from_config_accepts_default_model_preset(tmp_path):
    ...
    bot = RuiClaw.from_config(config_path, workspace=tmp_path, model_preset="fast")
...
def test_from_config_default_path():
    ...
    with patch("ruiclaw.config.loader.load_config") as mock_load, \
         patch("ruiclaw.providers.factory.make_provider") as mock_prov:
...
async def test_run_with_hooks(tmp_path):
    from ruiclaw.agent.hook import AgentHook, AgentHookContext, SDKCaptureHook
    from ruiclaw.bus.events import OutboundMessage
...
async def test_run_hooks_restored_on_error(tmp_path):
    ...
    from ruiclaw.agent.hook import AgentHook
...
def test_workspace_override(tmp_path):
    ...
    bot = RuiClaw.from_config(config_path, workspace=custom_ws)
...
def test_request_context_preserves_legacy_positional_arguments(tmp_path):
    from ruiclaw.agent.tools.context import RequestContext
...
async def test_run_exposes_attributes_to_context_provider_without_persisting_them(tmp_path):
    ...
    from ruiclaw.providers.base import LLMResponse
    ...
    assert isinstance(seen[0].run_id, str) and seen[0].run_id
    assert seen[0].parent_run_id is None
    run_dir = tmp_path / ".ruiclaw" / "runs" / seen[0].run_id
    assert (run_dir / "manifest.json").exists()
    assert (run_dir / "events.jsonl").exists()
    report = json.loads((run_dir / "report.json").read_text(encoding="utf-8"))
    assert report["status"] == "succeeded"
    assert report["model_calls"] == 1
    assert report["context"]["requests"] == 1
    source_tokens = report["context"]["max_source_tokens"]
    assert source_tokens["bootstrap"] > 0
    assert source_tokens["user_input"] > 0
    assert sum(source_tokens.values()) == report["context"]["max_total_tokens"]
    event_types = {
        json.loads(line)["event_type"]
        for line in (run_dir / "events.jsonl").read_text(encoding="utf-8").splitlines()
    }
    assert "model_context_built" in event_types
...
@pytest.mark.asyncio
async def test_run_ledger_failure_does_not_change_agent_result(tmp_path, monkeypatch):
    from ruiclaw.agent.loop import AgentLoop
    from ruiclaw.bus.queue import MessageBus
    from ruiclaw.observability.run_ledger import RunLedger
    from ruiclaw.providers.base import LLMResponse

    provider = _fake_provider("test-model")
    provider.chat_stream_with_retry = AsyncMock(return_value=LLMResponse(content="done"))

    async def fail_start(_ledger: RunLedger) -> None:
        raise OSError("audit disk unavailable")

    monkeypatch.setattr(RunLedger, "start", fail_start)
    bot = RuiClaw(AgentLoop(
        bus=MessageBus(),
        provider=provider,
        workspace=tmp_path,
    ...
    result = await bot.run("hi", session_key="sdk:audit-failure")
...
async def test_persisted_turn_callback_is_best_effort_and_reads_display_safe_session(tmp_path):
    from ruiclaw import SessionTurnPersisted
    from ruiclaw.agent.loop import AgentLoop
    from ruiclaw.bus.queue import MessageBus
    from ruiclaw.providers.base import LLMResponse
...
async def test_ephemeral_run_does_not_invoke_persisted_turn_callback(tmp_path):
    ...
    assert seen == []
    assert bot.sessions.list() == []
    assert not (tmp_path / ".ruiclaw" / "runs").exists()


def test_import_from_top_level():
    import ruiclaw

    assert ruiclaw.RuiClaw is RuiClaw
    assert ruiclaw.RequestContext.__name__ == "RequestContext"
    assert ruiclaw.RuntimeContextBlock.__name__ == "RuntimeContextBlock"
    assert ruiclaw.RuntimeContextProvider is not None
    assert ruiclaw.SessionTurnPersisted.__name__ == "SessionTurnPersisted"
    assert ruiclaw.RunResult is RunResult
    assert ruiclaw.RunStream is RunStream
    assert ruiclaw.SessionInfo is SessionInfo
    assert ruiclaw.SessionSnapshot is SessionSnapshot
    assert ruiclaw.StreamEvent is StreamEvent
    assert ruiclaw.StreamEventType is StreamEventType
    assert ruiclaw.STREAM_EVENT_TEXT_DELTA == STREAM_EVENT_TEXT_DELTA
    assert ruiclaw.STREAM_EVENT_RUN_COMPLETED == STREAM_EVENT_RUN_COMPLETED
    assert ruiclaw.STREAM_EVENT_TYPES == STREAM_EVENT_TYPES
...
async def test_run_populates_observability_fields(tmp_path):
    from ruiclaw.agent.hook import AgentRunHookContext
    from ruiclaw.bus.events import OutboundMessage
...
async def test_sdk_capture_prefers_run_level_snapshot():
    from ruiclaw.agent.hook import AgentHookContext, AgentRunHookContext, SDKCaptureHook
    from ruiclaw.providers.base import ToolCallRequest
