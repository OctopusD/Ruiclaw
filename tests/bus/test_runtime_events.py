...
@pytest.mark.asyncio
async def test_runtime_event_publisher_carries_run_identity() -> None:
    bus = MessageBus()
    ...
    bus.subscribe(seen.append, SessionTurnStarted)

    await publisher.session_turn_started(
        msg,
        "websocket:chat-a",
        turn_id="turn-1",
        run_id="run-2",
    ...
    assert seen[0].context.turn_id == "turn-1"
    assert seen[0].context.run_id == "run-2"
    assert seen[0].context.parent_run_id == "run-1"
