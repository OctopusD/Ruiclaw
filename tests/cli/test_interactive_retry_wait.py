...
from ruiclaw.bus.outbound_events import ProgressEvent, RetryWaitEvent
...
async def test_interactive_retry_wait_is_rendered_as_progress_even_when_progress_disabled():
    ...
    with patch("ruiclaw.cli.terminal._print_interactive_progress_line", side_effect=fake_print):
...
async def test_reasoning_displayed_when_show_reasoning_enabled():
    ...
    with patch("ruiclaw.cli.terminal._print_cli_reasoning", side_effect=lambda t, th, r=None: calls.append(t)):
...
async def test_reasoning_hidden_when_show_reasoning_disabled():
    ...
    with patch("ruiclaw.cli.terminal._print_cli_reasoning") as mock_reasoning:
        handled = await terminal._maybe_print_interactive_progress(msg, None, channels_config)
...
async def test_reasoning_shown_when_send_progress_disabled():
    ...
    with patch(
        "ruiclaw.cli.terminal._print_cli_reasoning",
