...
from ruiclaw.cli.commands import SafeFileHistory as LegacySafeFileHistory
from ruiclaw.cli.terminal import SafeFileHistory, _sanitize_surrogates
...
class TestSanitizeSurrogates:
    ...
    def test_emoji_already_correct(self):
        ...
        assert _sanitize_surrogates("hello 🐈 ruiclaw") == "hello 🐈 ruiclaw"
...
class TestSafeFileHistory:
    ...
    def test_emoji_preserved(self, tmp_path):
        hist = SafeFileHistory(str(tmp_path / "history"))
        hist.store_string("hello 🐈 ruiclaw")
        entries = list(hist.load_history_strings())
        assert entries[0] == "hello 🐈 ruiclaw"
