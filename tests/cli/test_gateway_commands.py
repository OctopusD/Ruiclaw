...
from ruiclaw.cli.gateway import _resolved_config_selector, create_gateway_app
...
from ruiclaw.gateway.service import GatewayServiceOptions, GatewayServiceResult
...
class FakeServiceInstaller:
    ...
    def install(self, options: GatewayServiceOptions, *, dry_run: bool) -> GatewayServiceResult:
        ...
        return GatewayServiceResult(
            ...
            "systemd",
            self.tmp_path / "ruiclaw-gateway.service",
            (("systemctl", "--user", "daemon-reload"),),
            "[Unit]\nDescription=RuiClaw Gateway\n",
        )
...
def test_gateway_restart_does_not_create_a_persistent_gateway(tmp_path):
    ...
    assert "ruiclaw gateway --background" in result.stdout
