...
from ruiclaw.cli import entry
from ruiclaw.cli.entry import _agent_invocation_args, _native_tui_candidate
...
def test_root_shell_completion_keeps_root_subcommands() -> None:
    ...
    env.update(
        {
            "_RUICLAW_COMPLETE": "complete_bash",
            "COMP_WORDS": "ruiclaw ",
            "COMP_CWORD": "1",
        }
    )
    script = (
        "import sys; "
        "from ruiclaw.cli.entry import main; "
        "sys.argv = ['ruiclaw']; "
        "main()"
    )
...
def test_root_alias_dispatches_the_shared_agent_command(monkeypatch) -> None:
    ...
    monkeypatch.setattr(entry.sys, "argv", ["ruiclaw", "-m", "hello"])
    monkeypatch.setattr(
        entry,
...
def test_desktop_handled_bare_invocation_does_not_start_python(monkeypatch) -> None:
    ...
    monkeypatch.setattr(entry.sys, "argv", ["ruiclaw"])
