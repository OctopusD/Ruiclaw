...
from ruiclaw.cli import stream as stream_mod
from ruiclaw.cli import terminal
...
def mock_prompt_session():
    ...
    with patch("ruiclaw.cli.terminal._prompt_session", mock_session), \
         patch("ruiclaw.cli.terminal.patch_stdout"):
        yield mock_session
...
def test_init_prompt_session_creates_session():
    ...
    with patch("ruiclaw.cli.terminal.PromptSession") as mock_session_cls, \
         patch("ruiclaw.cli.terminal.FileHistory"), \
...
async def test_print_interactive_progress_line_pauses_spinner_before_printing():
    ...
    with patch("ruiclaw.cli.terminal._print_interactive_line", side_effect=fake_print):
        thinking = stream_mod.ThinkingSpinner(console=mock_console)
        with thinking:
