...
from ruiclaw.cli.process_identity import named_executable, set_cli_process_identity
...
@pytest.mark.parametrize(
    ...
    [
        (["agent"], "ruiclaw-agent"),
        (["gateway", "--background"], "ruiclaw-gateway"),
        (["webui"], "ruiclaw-webui"),
        (["status"], "ruiclaw"),
        ([], "ruiclaw"),
    ],
...
def test_cli_process_identity_uses_product_and_role(
    ...
    titles: list[str] = []
    monkeypatch.setattr("ruiclaw.cli.process_identity.os.name", "posix")
    monkeypatch.setattr("ruiclaw.cli.process_identity._set_process_title", titles.append)
...
def test_cli_process_identity_keeps_windows_launcher_name(
    ...
    titles: list[str] = []
    monkeypatch.setattr("ruiclaw.cli.process_identity.os.name", "nt")
    monkeypatch.setattr("ruiclaw.cli.process_identity._set_process_title", titles.append)
...
def test_legacy_console_entrypoint_still_sets_subcommand_identity(
    ...
    monkeypatch.setattr("ruiclaw.cli.commands.set_cli_process_identity", commands.append)
...
def test_legacy_console_entrypoint_routes_bare_command_to_agent(
    ...
    monkeypatch.setattr("ruiclaw.cli.commands.set_cli_process_identity", identities.append)
    monkeypatch.setattr(
        "ruiclaw.cli.entry._run_agent",
...
def test_named_executable_creates_stable_role_symlink(
    ...
    first = Path(
        named_executable(executable.as_posix(), name="ruiclaw-tui", directory=tmp_path / "run")
    )
    second = Path(
        named_executable(executable.as_posix(), name="ruiclaw-tui", directory=tmp_path / "run")
    )
...
def test_named_executable_uses_original_on_windows(
    ...
    assert (
        named_executable("bun.exe", name="ruiclaw-tui", directory=tmp_path / "run")
        == "bun.exe"
    )
