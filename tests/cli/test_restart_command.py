...
from ruiclaw.providers.base import LLMResponse, LLMUsage
...
class TestRestartCommand:
    ...
    async def test_restart_sends_message_and_calls_execv(self):
        from ruiclaw.command.builtin import cmd_restart
        ...
        with patch.dict(os.environ, {}, clear=False), \
             patch("ruiclaw.command.builtin.asyncio", new=fake_asyncio), \
             patch("ruiclaw.command.builtin.os.execv") as mock_execv:
    ...
    async def test_restart_windows_auto_spawns_and_exits(self):
        ...
        with patch("ruiclaw.command.builtin.asyncio", new=fake_asyncio), \
             patch("ruiclaw.command.builtin.sys.platform", "win32"), \
             patch("ruiclaw.command.builtin.subprocess.CREATE_NEW_PROCESS_GROUP", 512, create=True), \
             patch("ruiclaw.command.builtin.subprocess.Popen") as mock_popen, \
             patch("ruiclaw.command.builtin.os._exit") as mock_exit, \
             patch("ruiclaw.command.builtin.os.execv") as mock_execv:
            await cmd_restart(ctx)
            await scheduled[0]

        mock_popen.assert_called_once_with(
            [sys.executable, "-m", "ruiclaw"] + sys.argv[1:],
            creationflags=512,
        )
    ...
    async def test_restart_intercepted_in_run_loop(self):
        ...
        with patch.object(loop, "_dispatch", new_callable=AsyncMock) as mock_dispatch, \
             patch("ruiclaw.command.builtin.asyncio", new=fake_asyncio), \
             patch("ruiclaw.command.builtin.os.execv"):
    ...
    async def test_status_intercepted_in_run_loop(self):
        ...
        with patch.object(loop, "_dispatch", new_callable=AsyncMock) as mock_dispatch:
            ...
            mock_dispatch.assert_not_called()
            assert "ruiclaw" in out.content.lower() or "Model" in out.content
    ...
    async def test_run_agent_loop_estimates_usage_when_provider_omits_it(self, monkeypatch):
        loop, _bus = _make_loop()
        monkeypatch.setattr(
            "ruiclaw.agent.runner.estimate_prompt_tokens_chain",
            lambda *_args, **_kwargs: (123, "test"),
        )
        monkeypatch.setattr(
            "ruiclaw.agent.runner.estimate_message_tokens",
            lambda _message: 7,
        )
