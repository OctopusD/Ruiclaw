...
import ruiclaw.cli.tui_launcher as tui_launcher
from ruiclaw.cli.agent import agent
from ruiclaw.cli.tui_launcher import (
    TuiSessionError,
    TuiUnavailableError,
...
from ruiclaw.config.schema import Config, ModelPresetConfig
...
def test_launcher_passes_the_canonical_model_preset_to_the_tui(
    ...
    monkeypatch.setattr("ruiclaw.cli.tui_launcher.resolve_tui_command", lambda: ["ruiclaw-tui"])
    ...
    monkeypatch.setattr("ruiclaw.cli.tui_launcher._ensure_gateway", ensure_gateway)
    ...
    def popen(command: list[str], *, env: dict[str, str]) -> FakeProcess:
        assert command == ["ruiclaw-tui"]
        captured.update(env)
        events.append("spawned")
        return FakeProcess()

    monkeypatch.setattr("ruiclaw.cli.tui_launcher.subprocess.Popen", popen)
    ...
    assert result == 0
    assert captured["RUICLAW_TUI_MODEL"] == "openai/gpt-5.6"
    assert captured["RUICLAW_TUI_MODEL_PRESET"] == "Deep Research"
    assert captured["RUICLAW_TUI_WORKSPACE"] == str(Path.cwd().resolve())
    assert captured["RUICLAW_TUI_BOOTSTRAP_URL"] == (
        "http://127.0.0.1:8765/webui/bootstrap"
    )
    assert captured["RUICLAW_TUI_HEALTH_URL"] == "http://127.0.0.1:18790/health"
    assert captured["RUICLAW_TUI_BOOTSTRAP_SECRET"] == "bootstrap-secret"
    assert "RUICLAW_TUI_WS_URL" not in captured
    assert "RUICLAW_TUI_API_TOKEN" not in captured
    assert "RUICLAW_TUI_CHAT_ID" not in captured
    assert "RUICLAW_TUI_STATE_PATH" not in captured
...
def test_launcher_terminates_the_tui_when_gateway_start_fails(
    ...
    monkeypatch.setattr("ruiclaw.cli.tui_launcher.resolve_tui_command", lambda: ["ruiclaw-tui"])
    monkeypatch.setattr(
        "ruiclaw.cli.tui_launcher.subprocess.Popen",
        lambda *args, **kwargs: FakeProcess(),
    )
    monkeypatch.setattr("ruiclaw.cli.tui_launcher._ensure_gateway", fail_gateway)
...
def test_launcher_keeps_the_tui_alive_while_an_existing_gateway_recovers(
    ...
    monkeypatch.setattr("ruiclaw.gateway.GatewayRuntime", FakeRuntime)
    monkeypatch.setattr("ruiclaw.cli.tui_launcher.resolve_tui_command", lambda: ["ruiclaw-tui"])
    ...
    monkeypatch.setattr(
        "ruiclaw.cli.tui_launcher._webui_endpoint_reachable",
...
def test_launcher_promotes_the_gateway_when_the_tui_detaches(
    ...
    monkeypatch.setattr("ruiclaw.cli.tui_launcher.subprocess.Popen", popen)
    monkeypatch.setattr(
        "ruiclaw.cli.tui_launcher._ensure_gateway",
    ...
    assert captured["RUICLAW_TUI_GATEWAY_STOP_COMMAND"] == (
        f"ruiclaw gateway stop --config '{config_path}' --workspace '{workspace.resolve()}'"
    )
...
def test_explicit_tui_binary_must_exist(
    ...
    monkeypatch.setenv("RUICLAW_TUI_BIN", str(missing))
...
def test_windows_arm64_fails_instead_of_using_the_classic_prompt(
    ...
    monkeypatch.delenv("RUICLAW_TUI_BIN", raising=False)
    monkeypatch.setattr("ruiclaw.cli.tui_launcher.platform.system", lambda: "Windows")
    monkeypatch.setattr("ruiclaw.cli.tui_launcher.platform.machine", lambda: "ARM64")
...
def test_source_checkout_does_not_fall_back_to_a_release_tui_without_bun(
    ...
    monkeypatch.delenv("RUICLAW_TUI_BIN", raising=False)
    monkeypatch.setattr(
        "ruiclaw.cli.tui_launcher._source_checkout_tui_dir",
        lambda: source_dir,
    )
    monkeypatch.setattr("ruiclaw.cli.tui_launcher.shutil.which", lambda _name: None)
    monkeypatch.setattr(
        "ruiclaw.cli.tui_launcher._download_release_tui",
...
def test_source_checkout_requires_project_and_tui_markers(
    ...
    module_path = project_root / "ruiclaw" / "cli" / "tui_launcher.py"
...
def test_interactive_agent_uses_native_tui(
    ...
    monkeypatch.setattr("ruiclaw.cli.agent._load_runtime_config", lambda *_args: config)
    monkeypatch.setattr("ruiclaw.cli.tui_launcher.launch_tui", launch)
    monkeypatch.setattr("ruiclaw.config.loader.get_config_path", lambda: config_path)
    monkeypatch.setattr("ruiclaw.cli.agent.sys.stdin", SimpleNamespace(isatty=lambda: True))
    monkeypatch.setattr("ruiclaw.cli.agent.sys.stdout", SimpleNamespace(isatty=lambda: True))
...
def test_interactive_agent_does_not_silently_fall_back(
    ...
    monkeypatch.setattr("ruiclaw.cli.agent._load_runtime_config", lambda *_args: config)
    monkeypatch.setattr("ruiclaw.cli.agent.console.print", lambda value: output.append(value))
    monkeypatch.setattr("ruiclaw.cli.tui_launcher.launch_tui", unavailable)
    monkeypatch.setattr("ruiclaw.config.loader.get_config_path", lambda: tmp_path / "config.json")
    monkeypatch.setattr("ruiclaw.cli.agent.sys.stdin", SimpleNamespace(isatty=lambda: True))
    monkeypatch.setattr("ruiclaw.cli.agent.sys.stdout", SimpleNamespace(isatty=lambda: True))
    ...
    assert output == [
        ...
        "[dim]Use `ruiclaw agent --classic` only if you want the old prompt.[/dim]",
    ]
...
def test_default_agent_does_not_fall_back_outside_a_terminal(
    ...
    monkeypatch.setattr("ruiclaw.cli.agent._load_runtime_config", lambda *_args: Config())
    monkeypatch.setattr("ruiclaw.cli.agent.sys.stdin", SimpleNamespace(isatty=lambda: False))
    monkeypatch.setattr("ruiclaw.cli.agent.sys.stdout", SimpleNamespace(isatty=lambda: True))
...
def test_source_checkout_refreshes_locked_tui_dependencies(
    ...
    monkeypatch.setattr("ruiclaw.cli.tui_launcher.subprocess.run", install)
    monkeypatch.setattr(
        "ruiclaw.cli.tui_launcher.named_executable",
...
def test_source_checkout_fails_when_locked_dependencies_cannot_be_refreshed(
    ...
    source_dir.mkdir()
    monkeypatch.setattr(
        "ruiclaw.cli.tui_launcher.subprocess.run",
...
def test_release_tui_is_verified_and_cached(
    ...
    monkeypatch.setattr("ruiclaw.cli.tui_launcher.__version__", "9.9.9")
    monkeypatch.setattr("ruiclaw.cli.tui_launcher.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr("ruiclaw.cli.tui_launcher._read_release_asset", read_asset)
...
def test_release_tui_rejects_bad_checksum(
    ...
    monkeypatch.setattr("ruiclaw.cli.tui_launcher.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "ruiclaw.cli.tui_launcher._read_release_asset",
...
def test_gateway_reuse_requires_the_matching_managed_instance(
    ...
    monkeypatch.setattr("ruiclaw.gateway.GatewayRuntime", FakeRuntime)
    monkeypatch.setattr("ruiclaw.cli.tui_launcher._webui_endpoint_reachable", lambda _url: True)

    with pytest.raises(TuiUnavailableError, match="different ruiclaw instance"):
        _ensure_gateway(
            config,
            config_path=tmp_path / "config.json",
            workspace_override=str(workspace),
...
def test_gateway_reuses_the_matching_managed_instance(
    ...
    monkeypatch.setattr("ruiclaw.cli.tui_launcher._webui_endpoint_reachable", lambda _url: True)
    monkeypatch.setattr(
        "ruiclaw.cli.tui_launcher._gateway_health_ready",
