...
from ruiclaw.cli import desktop_target, desktop_tui, tui_launcher
from ruiclaw.cli.desktop_target import DesktopReply, DesktopTargetError
...
def test_launcher_has_no_gateway_ownership_and_no_credentials_in_child_env(monkeypatch):
    ...
    monkeypatch.setenv("RUICLAW_TUI_GATEWAY_STOP_COMMAND", "forbidden stop")
    monkeypatch.setenv("RUICLAW_TUI_BOOTSTRAP_SECRET", "unrelated-bootstrap")
    monkeypatch.setenv("RUICLAW_TUI_WS_URL", "ws://unrelated")
    monkeypatch.delenv("RUICLAW_DESKTOP_CLIENT_CACHE", raising=False)
    calls = []
    process = MagicMock()
    ...
    def probe(args, **kw):
        ...
        assert not any(key.startswith("RUICLAW_TUI_") for key in kw["env"])
    ...
    env = options["env"]
    assert set(key for key in env if key.startswith("RUICLAW_TUI_")) == {
        "RUICLAW_TUI_DESKTOP_TARGET", "RUICLAW_TUI_DESKTOP_RESOLVER",
    }
    assert "synthetic" not in json.dumps((args, options))
    assert "unrelated-bootstrap" not in json.dumps((args, options))
    assert json.loads(env["RUICLAW_TUI_DESKTOP_TARGET"])["gatewayId"] == GATEWAY_ID
    resolver = json.loads(env["RUICLAW_TUI_DESKTOP_RESOLVER"])
...
import os, sys
keys = {key for key in os.environ if key.startswith('RUICLAW_TUI_')}
if sys.argv[-1] == '--desktop-protocol':
    ...
    assert keys == {'RUICLAW_TUI_DESKTOP_TARGET', 'RUICLAW_TUI_DESKTOP_RESOLVER'}
    sys.exit(90)
"""]
    monkeypatch.setattr(tui_launcher, "resolve_tui_command", lambda: command)
    monkeypatch.delenv("RUICLAW_DESKTOP_CLIENT_CACHE", raising=False)
    monkeypatch.setenv("RUICLAW_TUI_BOOTSTRAP_URL", "http://127.0.0.1:1/webui/bootstrap")
    monkeypatch.setenv("RUICLAW_TUI_BOOTSTRAP_SECRET", "unrelated-bootstrap")
    monkeypatch.setenv("RUICLAW_TUI_DESKTOP_RESOLVER", "unrelated-resolver")
...
def test_desktop_only_entry_never_forwards_unknown_commands(args, monkeypatch):
    monkeypatch.setattr(sys, "argv", ["ruiclaw-desktop-tui", *args])
...
def test_desktop_cache_avoids_default_python_data_root(monkeypatch, tmp_path):
    monkeypatch.setenv("RUICLAW_DESKTOP_CLIENT_CACHE", str(tmp_path))
    observed = []
    def resolve(**kwargs):
