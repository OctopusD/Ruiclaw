...
from ruiclaw.agent.memory import MemoryStore
from ruiclaw.agent.tools.registry import ToolRegistry
from ruiclaw.agent.turn_delivery import TurnDeliveryFactory
from ruiclaw.bus.events import InboundMessage, OutboundMessage
from ruiclaw.bus.queue import MessageBus
from ruiclaw.cli import commands as cli_commands
from ruiclaw.cli import gateway_runtime as cli_gateway_runtime
from ruiclaw.cli import provider as provider_commands
from ruiclaw.cli import terminal as cli_terminal
from ruiclaw.cli import webui as cli_webui
from ruiclaw.cli import webui_support as cli_webui_support
from ruiclaw.cli.commands import app
from ruiclaw.config.schema import Config
from ruiclaw.cron.service import CronJobSkippedError
from ruiclaw.cron.session_turns import CRON_DEFER_UNTIL_IDLE_META, CRON_TRIGGER_META
from ruiclaw.cron.types import CronJob, CronPayload
from ruiclaw.cron.webui_metadata import cron_proactive_delivery_metadata
from ruiclaw.providers.factory import ProviderSnapshot, make_provider, provider_signature
from ruiclaw.providers.openai_codex_provider import _strip_model_prefix
from ruiclaw.providers.registry import find_by_name
from ruiclaw.providers.unconfigured_provider import UnconfiguredProvider
from ruiclaw.session.webui_turns import WebuiTurnRoutePolicy
from ruiclaw.webui.dev import WebUIDevError
from ruiclaw.webui.metadata import (
...
def mock_paths():
    ...
    with patch("ruiclaw.config.loader.get_config_path") as mock_cp, \
         patch("ruiclaw.config.loader.save_config") as mock_sc, \
         patch("ruiclaw.config.loader.load_config") as mock_lc, \
         patch("ruiclaw.cli.commands.get_workspace_path") as mock_ws:
...
def test_onboard_fresh_install(mock_paths):
    ...
    assert "RuiClaw is ready" in result.stdout
...
def test_onboard_recommends_webui(mock_paths):
    ...
    assert result.exit_code == 0
    assert "✓ RuiClaw is ready. Run: ruiclaw webui" in result.stdout
...
def test_onboard_interactive_discard_does_not_save_or_create_workspace(mock_paths, monkeypatch):
    ...
    from ruiclaw.cli.onboard import OnboardResult

    monkeypatch.setattr(
        "ruiclaw.cli.onboard.run_onboard",
...
def test_onboard_uses_explicit_config_and_workspace_paths(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("ruiclaw.channels.registry.discover_all", lambda: {})
    ...
    assert f'ruiclaw webui -c "{resolved_config}"' in result.stdout
...
def test_plugins_list_uses_explicit_config(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr(
        "ruiclaw.channels.registry.discover_plugins",
    ...
    monkeypatch.setattr(
        "ruiclaw.optional_features.optional_dependency_groups",
        lambda: {},
    )
...
def test_provider_logout_xai_grok_removes_instance_credentials(tmp_path, monkeypatch):
    ...
    lock_path.write_text("", encoding="utf-8")
    monkeypatch.setattr(
        "ruiclaw.providers.xai_oauth.get_xai_oauth_storage_path",
        lambda: token_path,
    )
...
def test_provider_logout_xai_grok_uses_explicit_config_path(tmp_path, monkeypatch):
    from ruiclaw.config import loader
...
def test_provider_logout_paths_resolve_to_expected_files():
    ...
    from ruiclaw.providers.github_copilot_provider import get_storage
    from ruiclaw.providers.xai_oauth import get_xai_oauth_storage_path
...
def test_provider_login_openai_codex_handles_missing_oauth_symbol(monkeypatch):
    ...
    assert result.exit_code == 1
    assert (
        "This ruiclaw installation is missing the required oauth-cli-kit package. "
        "Reinstall or upgrade ruiclaw-ai using the same installation method."
...
def test_provider_login_openai_codex_passes_configured_proxy(monkeypatch):
    ...
    monkeypatch.setattr(
        "ruiclaw.config.loader.load_config",
...
def test_provider_login_xai_grok_runs_browser_flow_with_configured_proxy(monkeypatch):
    ...
    monkeypatch.setattr(
        "ruiclaw.providers.xai_oauth.get_xai_oauth_token",
    ...
    monkeypatch.setattr("ruiclaw.providers.xai_oauth.login_xai_oauth", fake_login)
...
def test_openai_compat_provider_passes_model_through():
    from ruiclaw.providers.openai_compat_provider import OpenAICompatProvider

    with patch("ruiclaw.providers.openai_compat_provider.AsyncOpenAI"):
...
def test_github_copilot_provider_strips_prefixed_model_name():
    from ruiclaw.providers.github_copilot_provider import GitHubCopilotProvider
...
async def test_github_copilot_provider_refreshes_client_api_key_before_chat():
    ...
    with patch("ruiclaw.providers.openai_compat_provider.AsyncOpenAI", return_value=mock_client):
...
def test_make_provider_passes_extra_headers_to_custom_provider():
    ...
    with patch("ruiclaw.providers.openai_compat_provider.AsyncOpenAI") as mock_async_openai:
        provider = make_provider(config)
...
def mock_agent_runtime(tmp_path):
    ...
    with patch("ruiclaw.config.loader.load_config", return_value=config) as mock_load_config, \
         patch("ruiclaw.config.loader.resolve_config_env_vars", side_effect=lambda c: c), \
         patch("ruiclaw.cli.agent.sync_workspace_templates") as mock_sync_templates, \
         patch("ruiclaw.providers.factory.make_provider", return_value=_fake_provider()), \
         patch("ruiclaw.cli.terminal._print_agent_response") as mock_print_response, \
         patch("ruiclaw.bus.queue.MessageBus"), \
         patch("ruiclaw.cron.service.CronService"), \
         patch("ruiclaw.cli.agent.AgentLoop.from_config") as mock_from_config:
...
def test_agent_config_sets_active_path(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "ruiclaw.config.loader.set_config_path",
        lambda path: seen.__setitem__("config_path", path),
    )
    monkeypatch.setattr("ruiclaw.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("ruiclaw.cli.agent.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("ruiclaw.providers.factory.make_provider", lambda _config: _fake_provider())
    monkeypatch.setattr("ruiclaw.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("ruiclaw.cron.service.CronService", lambda _store: object())
    ...
    monkeypatch.setattr("ruiclaw.cli.agent.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("ruiclaw.cli.terminal._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_agent_uses_workspace_directory_for_cron_store(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("ruiclaw.config.loader.set_config_path", lambda _path: None)
    monkeypatch.setattr("ruiclaw.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("ruiclaw.cli.agent.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("ruiclaw.providers.factory.make_provider", lambda _config: _fake_provider())
    monkeypatch.setattr("ruiclaw.bus.queue.MessageBus", lambda: object())
    ...
    monkeypatch.setattr("ruiclaw.cron.service.CronService", _FakeCron)
    monkeypatch.setattr("ruiclaw.cli.agent.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("ruiclaw.cli.terminal._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_agent_workspace_override_does_not_migrate_legacy_cron(
    ...
    monkeypatch.setattr("ruiclaw.providers.factory.make_provider", lambda _config: _fake_provider())
    monkeypatch.setattr("ruiclaw.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("ruiclaw.config.paths.get_cron_dir", lambda: legacy_dir)
...
def test_agent_custom_config_workspace_does_not_migrate_legacy_cron(
    ...
    monkeypatch.setattr("ruiclaw.cli.agent.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr(
        "ruiclaw.cli.terminal._print_agent_response", lambda *_args, **_kwargs: None
    )
...
def test_heartbeat_has_active_tasks(content, expected):
    from ruiclaw.cli.gateway_runtime import _heartbeat_has_active_tasks
...
def test_heartbeat_skips_bundled_template():
    from ruiclaw.cli.gateway_runtime import _heartbeat_has_active_tasks
    from ruiclaw.utils.helpers import load_bundled_template
...
def test_heartbeat_target_skips_archived_webui_sessions():
    from ruiclaw.cli.gateway_runtime import _pick_heartbeat_target_from_sessions
...
def test_heartbeat_target_uses_last_channel_for_unified_session():
    from ruiclaw.cli.gateway_runtime import _pick_heartbeat_target_from_sessions
    from ruiclaw.session.keys import LAST_CHANNEL_METADATA_KEY, UNIFIED_SESSION_KEY
...
def test_heartbeat_target_rejects_unroutable_unified_metadata(metadata):
    from ruiclaw.cli.gateway_runtime import _pick_heartbeat_target_from_sessions
    from ruiclaw.session.keys import UNIFIED_SESSION_KEY
...
def _patch_webui_provider_ready(monkeypatch) -> None:
    monkeypatch.setattr(
        "ruiclaw.providers.factory.validate_provider_setup",
        lambda _config: None,
    )


def _patch_gateway_ports_free(monkeypatch) -> None:
    monkeypatch.setattr("ruiclaw.cli.webui._gateway_health_ready", lambda *_a, **_kw: False)
    monkeypatch.setattr("ruiclaw.cli.webui._tcp_endpoint_reachable", lambda *_a, **_kw: False)
    monkeypatch.setattr("ruiclaw.cli.webui._webui_endpoint_reachable", lambda *_a, **_kw: False)
    monkeypatch.setattr(
        "ruiclaw.cli.gateway_runtime._tcp_endpoint_reachable",
        lambda *_a, **_kw: False,
    )
    monkeypatch.setattr(
        "ruiclaw.cli.gateway_runtime._webui_endpoint_reachable",
        lambda *_a, **_kw: False,
    )


def _record_gateway_lease_release(monkeypatch, captured: dict[str, object]) -> None:
    from ruiclaw.gateway import GatewayClientLease
...
def _patch_webui_managed_gateway(
    ...
    from ruiclaw.gateway import GatewayStatus, RuntimeResult
    ...
    monkeypatch.setattr("ruiclaw.gateway.GatewayRuntime", _FakeRuntime)
    monkeypatch.setattr(
        "ruiclaw.cli.webui._prepare_webui_bundle_for_gateway",
        lambda *_args, **_kwargs: None,
    )
    monkeypatch.setattr(
        "ruiclaw.cli.webui._attach_to_background_gateway",
...
def _patch_cli_command_runtime(
    ...
    *,
    ...
    monkeypatch.setattr("ruiclaw.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("ruiclaw.config.loader.resolve_config_env_vars", lambda c: c)
    monkeypatch.setattr(
        "ruiclaw.cli.commands.sync_workspace_templates",
        sync_templates or (lambda _path: None),
    )
    monkeypatch.setattr(
        "ruiclaw.cli.webui.sync_workspace_templates",
        sync_templates or (lambda _path: None),
    )
    monkeypatch.setattr(
        "ruiclaw.cli.gateway_runtime.sync_workspace_templates",
        sync_templates or (lambda _path: None),
    )
    monkeypatch.setattr(
        "ruiclaw.providers.factory.make_provider",
        provider_factory,
    )
    monkeypatch.setattr(
        "ruiclaw.providers.factory.build_provider_snapshot",
        lambda _config: _test_provider_snapshot(provider_factory(_config), _config),
    )
    monkeypatch.setattr(
        "ruiclaw.providers.factory.load_provider_snapshot",
        lambda _config_path=None: _test_provider_snapshot(provider_factory(config), config),
    )
    monkeypatch.setattr(
        "ruiclaw.cli.webui_support._provider_setup_error",
        lambda _config: None,
    )
    _patch_gateway_ports_free(monkeypatch)

    if message_bus is not None:
        monkeypatch.setattr("ruiclaw.bus.queue.MessageBus", message_bus)
    if session_manager is not None:
        monkeypatch.setattr("ruiclaw.session.manager.SessionManager", session_manager)
    if cron_service is not None:
        monkeypatch.setattr("ruiclaw.cron.service.CronService", cron_service)
    if get_cron_dir is not None:
        monkeypatch.setattr("ruiclaw.config.paths.get_cron_dir", get_cron_dir)
...
def test_heartbeat_empty_response_is_not_evaluated(
    ...
    monkeypatch.setattr("ruiclaw.cli.gateway_runtime.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("ruiclaw.channels.manager.ChannelManager", _FakeChannelManager)
    monkeypatch.setattr("ruiclaw.cli.gateway_runtime.read_webui_sidebar_state", lambda: {})
    monkeypatch.setattr("ruiclaw.cli.gateway_runtime.evaluate_response", _unexpected_evaluator)
...
def test_webui_background_points_to_the_single_persistent_gateway_command(
    ...
    assert "ruiclaw gateway --background --config" in compact_output
...
def test_webui_dev_starts_vite_sidecar_and_gateway(monkeypatch, tmp_path: Path) -> None:
    ...
    _patch_webui_provider_ready(monkeypatch)
    _patch_gateway_ports_free(monkeypatch)
    monkeypatch.setattr("ruiclaw.cli.webui.sync_workspace_templates", lambda _path: None)
    ...
    monkeypatch.setattr("ruiclaw.cli.webui.run_webui_dev_server", fake_dev_server)
    _patch_webui_managed_gateway(monkeypatch, seen)
    monkeypatch.setattr(
        "ruiclaw.cli.webui._prepare_webui_bundle_for_gateway",
    ...
    monkeypatch.setattr(
        "ruiclaw.cli.webui._open_webui_browser",
...
def test_webui_dev_waits_for_external_gateway_via_health_endpoint(monkeypatch) -> None:
    ...
    monkeypatch.setattr("ruiclaw.cli.webui._gateway_health_ready", fake_health)
    monkeypatch.setattr(
        "ruiclaw.cli.webui._webui_endpoint_reachable",
...
def test_webui_resumes_first_run_without_provider_setup(
    ...
    monkeypatch.setattr(
        "ruiclaw.cli.webui._provider_setup_error",
        lambda _config: "No API key configured for provider 'custom'.",
    )
    _patch_gateway_ports_free(monkeypatch)
    monkeypatch.setattr("ruiclaw.cli.webui.sync_workspace_templates", lambda _path: None)
...
def test_webui_missing_runtime_env_fails_before_starting_gateway(
    ...
    config_file = tmp_path / "config.json"
    missing_env = "RUICLAW_TEST_MISSING_WEBUI_SECRET"
    ...
    assert "ruiclaw status --config" in result.stdout
...
def test_webui_foreground_attaches_to_existing_managed_gateway(monkeypatch, tmp_path: Path) -> None:
    ...
    _patch_webui_provider_ready(monkeypatch)
    monkeypatch.setattr("ruiclaw.cli.webui.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("ruiclaw.cli.webui._gateway_health_ready", lambda *_args, **_kwargs: True)
    monkeypatch.setattr(
        "ruiclaw.cli.webui_support._gateway_health_ready",
    ...
    monkeypatch.setattr("ruiclaw.cli.webui._webui_endpoint_reachable", lambda *_args, **_kwargs: True)
    monkeypatch.setattr(
        "ruiclaw.cli.webui._open_webui_browser",
...
def test_webui_foreground_does_not_claim_unmanaged_gateway(monkeypatch, tmp_path: Path) -> None:
    ...
    _patch_webui_provider_ready(monkeypatch)
    monkeypatch.setattr("ruiclaw.cli.webui.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("ruiclaw.cli.webui._gateway_health_ready", lambda *_args: True)
    monkeypatch.setattr("ruiclaw.cli.webui._webui_endpoint_reachable", lambda *_args: True)
    monkeypatch.setattr("ruiclaw.cli.webui._open_webui_browser", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(
        "ruiclaw.cli.webui._attach_to_background_gateway",
...
def test_webui_foreground_refuses_occupied_webui_port(monkeypatch, tmp_path: Path) -> None:
    ...
    _patch_webui_provider_ready(monkeypatch)
    monkeypatch.setattr("ruiclaw.cli.webui.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("ruiclaw.cli.webui._gateway_health_ready", lambda *_args, **_kwargs: False)
    monkeypatch.setattr(
        "ruiclaw.cli.webui_support._gateway_health_ready",
        lambda *_args, **_kwargs: False,
    )
    monkeypatch.setattr("ruiclaw.cli.webui._webui_endpoint_reachable", lambda *_args, **_kwargs: True)
    monkeypatch.setattr("ruiclaw.cli.webui._tcp_endpoint_reachable", lambda *_args, **_kwargs: False)
    result = runner.invoke(app, ["webui", "--config", str(config_file), "--yes"])

    assert result.exit_code == 1
    assert "ruiclaw cannot start because one of its local ports is already in use" in result.stdout
...
def test_webui_foreground_reports_an_existing_gateway_without_leaking_secret(
    ...
    monkeypatch.setattr("ruiclaw.cli.webui._webui_endpoint_reachable", lambda *_args, **_kwargs: False)
...
def _patch_serve_runtime(monkeypatch, config: Config, seen: dict[str, object]) -> None:
    ...
    monkeypatch.setattr("ruiclaw.cli.commands.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("ruiclaw.api.server.create_app", _fake_create_app)
...
def test_gateway_unbound_agent_cron_is_skipped(
    ...
    monkeypatch.setattr("ruiclaw.config.loader.set_config_path", lambda _path: None)
    monkeypatch.setattr("ruiclaw.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("ruiclaw.cli.gateway_runtime.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("ruiclaw.providers.factory.make_provider", lambda _config: provider)
    monkeypatch.setattr("ruiclaw.cli.webui_support._provider_setup_error", lambda _config: None)
    _patch_gateway_ports_free(monkeypatch)
    monkeypatch.setattr(
        "ruiclaw.providers.factory.build_provider_snapshot",
    ...
    monkeypatch.setattr("ruiclaw.bus.queue.MessageBus", lambda: bus)
    ...
    monkeypatch.setattr("ruiclaw.session.manager.SessionManager", _FakeSessionManager)
    ...
    monkeypatch.setattr("ruiclaw.cron.service.CronService", _FakeCron)
    monkeypatch.setattr("ruiclaw.cli.gateway_runtime.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("ruiclaw.channels.manager.ChannelManager", _StopAfterCronSetup)
    monkeypatch.setattr(
        "ruiclaw.cli.gateway_runtime.evaluate_response",
...
def test_gateway_local_trigger_queue_submits_agent_turns(
    ...
    monkeypatch.setattr("ruiclaw.channels.manager.ChannelManager", _FakeChannelManager)
    monkeypatch.setattr(
        "ruiclaw.triggers.local_runner.run_local_trigger_queue",
...
def test_migrate_cron_store_moves_legacy_file(tmp_path: Path) -> None:
    ...
    from ruiclaw.cli.runtime_config import _migrate_cron_store
    ...
    with patch("ruiclaw.config.paths.get_cron_dir", return_value=legacy_dir):
...
def test_gateway_health_endpoint_binds_and_serves_expected_responses(
    ...
    monkeypatch.setattr("ruiclaw.cli.gateway_runtime.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("ruiclaw.channels.manager.ChannelManager", _FakeChannelManager)
    monkeypatch.setattr("ruiclaw.cron.service.CronService", _FakeCronService)
...
def test_gateway_agent_task_owns_initial_mcp_provider_close(
    ...
    monkeypatch.setattr("ruiclaw.cli.gateway_runtime.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("ruiclaw.cli.gateway_runtime.MCPProvider", _FakeMCPProvider)
    monkeypatch.setattr("ruiclaw.channels.manager.ChannelManager", _FakeChannelManager)
    monkeypatch.setattr("ruiclaw.cron.service.CronService", _FakeCronService)
    monkeypatch.setattr("asyncio.start_server", _fake_start_server)
...
def test_gateway_shutdown_event_exits_forever_runtime_tasks(
    ...
    monkeypatch.setattr("asyncio.start_server", _fake_start_server)
    monkeypatch.setattr(
        "ruiclaw.cli.gateway_runtime._install_gateway_shutdown_handlers",
...
def test_trigger_cli_queues_message_in_workspace(
    ...
    from ruiclaw.triggers.local_store import LocalTriggerStore
