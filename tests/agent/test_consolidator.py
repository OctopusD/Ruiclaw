...
from ruiclaw.agent.memory import (
    _HISTORY_ENTRY_HARD_CAP,
    Consolidator,
    MemoryStore,
)
from ruiclaw.events import AgentEvent, ContextCompactionEvent, EventSink
...
class TestConsolidatorSummarize:
    def test_format_messages_keeps_media_only_user_turn(self):
        path = "/home/user/.ruiclaw/media/websocket/clip.mp4"
...
class TestConsolidatorArchiveErrorHandling:
    ...
    async def test_archive_propagates_template_failure_without_raw_archive(
        ...
        consolidator.store.raw_archive = MagicMock()
        monkeypatch.setattr(
            "ruiclaw.agent.memory.render_template",
