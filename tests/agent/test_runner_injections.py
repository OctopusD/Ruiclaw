...
from ruiclaw.agent.context import TranscriptInput
from ruiclaw.agent.tools.context import RequestContext
from ruiclaw.config.schema import AgentDefaults
from ruiclaw.providers.base import LLMResponse, ToolCallRequest
...
def _make_loop(tmp_path, *, recovery_admission=None):
    ...
    with patch("ruiclaw.agent.loop.ContextBuilder"), \
         patch("ruiclaw.agent.loop.SessionManager"), \
         patch("ruiclaw.agent.loop.SubagentManager") as mock_sub_mgr:
...
async def test_drain_injections_returns_empty_when_no_callback():
    ...
    from ruiclaw.agent.runner import AgentRunner
...
async def test_drain_injections_passes_limit_to_callback_when_supported():
    ...
    from ruiclaw.agent.runner import _MAX_INJECTIONS_PER_TURN, AgentRunner
    from ruiclaw.bus.events import InboundMessage
...
def test_runner_append_keeps_recovery_followups_separate() -> None:
    ...
    from ruiclaw.agent.runner import AgentRunner
    from ruiclaw.session.recovery import PENDING_FOLLOWUP_ID_KEY
...
def test_model_request_merge_preserves_runtime_markers_with_media() -> None:
    from ruiclaw.agent.context_governance import ContextGovernor
    from ruiclaw.agent.runner import AgentRunner
    from ruiclaw.runtime_context import (
...
async def test_injection_cycles_capped_at_max():
    ...
    from ruiclaw.agent.runner import _MAX_INJECTION_CYCLES, AgentRunner
    from ruiclaw.bus.events import InboundMessage
...
async def test_cron_turn_deferred_while_session_active(tmp_path):
    ...
    from ruiclaw.bus.events import InboundMessage
    from ruiclaw.cron.session_turns import (
...
async def test_local_trigger_turn_deferred_while_session_active(tmp_path):
    ...
    from ruiclaw.bus.events import InboundMessage
    from ruiclaw.triggers.local_session_turns import LOCAL_TRIGGER_META
...
async def test_submitted_cron_turn_reports_pending_until_completed(tmp_path):
    ...
    from ruiclaw.cron.session_turns import CRON_TRIGGER_META
...
async def test_local_trigger_turn_cancellation_reports_agent_failure(tmp_path):
    ...
    from ruiclaw.agent.automation_turns import AutomationTurnError
    from ruiclaw.bus.events import InboundMessage
    from ruiclaw.triggers.local_session_turns import LOCAL_TRIGGER_META
...
async def test_pending_queue_preserves_overflow_for_next_injection_cycle(tmp_path):
    ...
    from ruiclaw.agent.loop import AgentLoop
    from ruiclaw.agent.runner import _MAX_INJECTIONS_PER_TURN
    from ruiclaw.bus.events import InboundMessage
    from ruiclaw.bus.queue import MessageBus
...
async def test_pending_queue_overflow_keeps_websocket_followup_durable(tmp_path):
    ...
    from ruiclaw.session.recovery import pending_followups
...
async def test_drain_injections_on_empty_final_response():
    ...
    from ruiclaw.agent.runner import _MAX_EMPTY_RETRIES, AgentRunner
    from ruiclaw.bus.events import InboundMessage
