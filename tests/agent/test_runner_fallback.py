...
from ruiclaw.events import RetryStatusEvent
...
from ruiclaw.providers.openai_responses import resolve_compact_threshold
...
def test_fallback_models_accept_preset_refs_and_inline_configs() -> None:
    from ruiclaw.config.schema import Config, InlineFallbackConfig
...
async def test_fallback_preserves_the_operation_event_sink():
    from ruiclaw.events import EventSink
...
def test_provider_signature_tracks_fallback_presets_and_provider_config() -> None:
    from ruiclaw.config.schema import Config
    from ruiclaw.providers.factory import provider_signature
...
def test_provider_snapshot_uses_smallest_fallback_context_window() -> None:
    from ruiclaw.config.schema import Config
    from ruiclaw.providers.factory import build_provider_snapshot
...
class TestRetryBeforeFailover:
    ...
    async def test_scoped_retry_events_preserve_fallback_chain_ownership(self, stream):
        from ruiclaw.events import EventSink, RetryWaitEvent
        ...
        with patch("ruiclaw.providers.base.asyncio.sleep", new_callable=AsyncMock):
            result = await call(
    ...
    async def test_open_primary_circuit_remains_retryable_when_factory_fails(self) -> None:
        ...
        with (
            patch("ruiclaw.providers.fallback_provider.time.monotonic", return_value=100.0),
            patch("ruiclaw.providers.base.asyncio.sleep", new_callable=AsyncMock),
        ):
            result = await provider.chat_with_retry(
                [{"role": "user", "content": "hi"}],
                retry_mode="persistent",
