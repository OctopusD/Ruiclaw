...
def test_bootstrap_files_are_backed_by_templates() -> None:
    template_dir = pkg_files("ruiclaw") / "templates"
...
def test_execution_rules_reach_existing_workspace_soul(tmp_path) -> None:
    ...
    workspace = _make_workspace(tmp_path)
    legacy_soul = (
        pkg_files("ruiclaw") / "templates" / "legacy" / "SOUL.md"
...
def test_default_soul_template_keeps_execution_policy_in_tool_contract() -> None:
    ...
    soul = (pkg_files("ruiclaw") / "templates" / "SOUL.md").read_text(encoding="utf-8")
    contract = (
        pkg_files("ruiclaw") / "templates" / "agent" / "tool_contract.md"
    ).read_text(encoding="utf-8")
...
def test_template_memory_md_is_skipped(tmp_path) -> None:
    ...
    assert "This file is automatically updated by ruiclaw" not in prompt
