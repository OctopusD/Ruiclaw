...
from ruiclaw.events import ContextCompactionEvent, EventSink
...
from ruiclaw.session.summary import SUMMARY_CONTINUATION_TEXT
...
async def test_runner_drops_resumable_provider_state_when_request_is_fitted(monkeypatch):
    ...
    monkeypatch.setattr(
        "ruiclaw.agent.context_governance.estimate_message_tokens",
...
async def test_changed_messages_use_local_estimate_after_reported_usage(monkeypatch):
    ...
    assert messages == []
    assert state.last_request_compacted is True
...
async def test_runner_counts_resumed_provider_state_before_dispatch(monkeypatch):
    ...
    monkeypatch.setattr(
        "ruiclaw.providers.conversation_state.estimate_prompt_tokens_chain",
        lambda *_args, **_kwargs: (100, "test-counter"),
    )
...
def test_snip_history_reserves_budget_for_tool_definitions(monkeypatch):
    ...
    monkeypatch.setattr("ruiclaw.agent.context_governance.estimate_prompt_tokens_chain", _estimate)
    token_sizes = {
        "system": 50,
        "old user": 200,
        "old assistant": 200,
