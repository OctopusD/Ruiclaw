...
async def test_runner_persists_large_tool_results_for_follow_up_calls(tmp_path):
    ...
    persisted_path = tmp_path / ".ruiclaw" / "tool-results" / "test_runner" / "call_big.txt"
    ...
    readback = await ReadFileTool(workspace=tmp_path).execute(
        path=".ruiclaw/tool-results/test_runner/call_big.txt"
    )
...
def test_persist_tool_result_prunes_old_session_buckets(tmp_path):
    ...
    root = tmp_path / ".ruiclaw" / "tool-results"
...
def test_persist_tool_result_logs_cleanup_failures(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "ruiclaw.utils.helpers._cleanup_tool_result_buckets",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(OSError("busy")),
    )
    monkeypatch.setattr(
        "ruiclaw.utils.helpers.logger.exception",
...
async def test_read_file_result_is_not_offloaded(tmp_path):
    ...
    offload_dir = tmp_path / ".ruiclaw" / "tool-results"
...
async def test_runner_keeps_going_when_tool_result_persistence_fails():
    ...
    runner = AgentRunner()
    with patch(
        "ruiclaw.agent.context_governance.maybe_persist_tool_result",
...
async def test_mixed_tool_text_survives_model_save_replay(tmp_path):
    ...
    readback = await reader.execute(path=".ruiclaw/tool-results/mixed/mixed_text_0.txt")
