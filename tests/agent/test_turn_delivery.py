...
from ruiclaw.bus.outbound_events import ContextCompactionEvent
from ruiclaw.bus.queue import MessageBus
from ruiclaw.bus.runtime_events import TurnCompleted
from ruiclaw.events import RetryStatusEvent
from ruiclaw.providers.base import LLMProvider, ProviderCallContext
...
@pytest.mark.asyncio
async def test_delivery_publishes_one_bound_run_identity() -> None:
    bus = MessageBus()
    seen: list[object] = []
    ...
    delivery = TurnDeliveryFactory(bus).create(msg, msg.session_key)
    delivery.bind_run(turn_id="turn-1", run_id="run-2", parent_run_id="run-1")

    await delivery.started()
    await delivery.running(started_at=10.0)
    await delivery.complete(None, publish_completion=True)

    contexts = [event.context for event in seen]
    assert len(contexts) == 3
    assert {(context.turn_id, context.run_id, context.parent_run_id) for context in contexts} == {
        ("turn-1", "run-2", "run-1")
    }
