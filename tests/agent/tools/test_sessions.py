...
from ruiclaw.agent.tools.sessions import ReadSessionTool, SearchSessionsTool
from ruiclaw.runtime_context import RuntimeContextBlock, append_runtime_context
...
async def test_search_sessions_reads_the_full_webui_transcript_after_compaction(
    ...
    monkeypatch.setattr("ruiclaw.webui.transcript.get_webui_dir", lambda: webui_dir)
    monkeypatch.setattr("ruiclaw.webui.session_list_index.get_webui_dir", lambda: webui_dir)
    manager = SessionManager(tmp_path)
    _save_session(
        manager,
        "websocket:history",
