...
from ruiclaw.agent.tools.web import WebSearchConfig, WebToolsConfig
...
class TestSubagentHookStatus:
    ...
    async def test_after_iteration_updates_status(self):
        ...
        from ruiclaw.agent.subagent import SubagentStatus, _SubagentHook
    ...
    async def test_after_iteration_no_status_is_noop(self):
        ...
        from ruiclaw.agent.hook import AgentHookContext
        from ruiclaw.agent.subagent import _SubagentHook
...
class TestScratchpadInspection:
    ...
    async def test_inspect_scratchpad_string(self):
        tool = _make_tool()
        await tool.execute(action="set", key="current_project", value="ruiclaw")
...
class TestRequestContext:
    ...
    def test_audit_reads_bound_session(self):
        ...
        with patch("ruiclaw.agent.tools.self.logger.info") as info:
            with request_context(ctx):
