...
async def test_terminal_drain_timeout(tmp_path):
    ...
    with patch("ruiclaw.agent.loop.asyncio.wait_for", side_effect=_timeout):
...
async def test_terminal_drain_reuses_one_timeout_budget(tmp_path):
    ...
    with (
        patch("ruiclaw.agent.loop.asyncio.get_running_loop", return_value=fake_loop),
        patch("ruiclaw.agent.loop.asyncio.wait_for", side_effect=_deliver),
    ):
