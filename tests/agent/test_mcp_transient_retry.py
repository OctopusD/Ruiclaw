...
async def test_tool_retries_on_transient_error():
    ...
    with patch("ruiclaw.agent.tools.mcp.asyncio.sleep", new_callable=AsyncMock):
...
async def test_tool_does_not_retry_on_cancelled_error():
    ...
    with patch("ruiclaw.agent.tools.mcp.asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
        output = await wrapper.execute()
