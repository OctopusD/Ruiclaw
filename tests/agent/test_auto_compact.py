...
from ruiclaw.command import CommandContext
from ruiclaw.config.schema import AgentDefaults, Config
from ruiclaw.events import NO_EVENTS
...
class TestIdleScanThrottling:
    ...
    def test_configured_idle_scan_interval_throttles_checks(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("ruiclaw.agent.loop.time.monotonic", lambda: next(ticks))
    ...
    def test_zero_idle_scan_interval_checks_every_tick(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("ruiclaw.agent.loop.time.monotonic", lambda: 1_000.0)
