...
from ruiclaw.agent.context import RUICLAW_CONTEXT_SOURCE_META, ContextBuilder, TranscriptInput
from ruiclaw.agent.loop import AgentLoop
from ruiclaw.agent.runner import AgentRunResult
from ruiclaw.agent.tools.context import RequestContext, request_context
...
from ruiclaw.cron.session_turns import CRON_HISTORY_META, CRON_TRIGGER_META
from ruiclaw.providers.base import LLMProvider, LLMResponse, LLMUsage, ProviderConversationState
...
from ruiclaw.session.goal_state import GOAL_STATE_KEY
from ruiclaw.session.keys import (
...
from ruiclaw.session.turn_continuation import (
...
def test_build_and_save_preserves_user_text_containing_goal_guidance_tag(tmp_path: Path) -> None:
    ...
    assert messages[-1]["_meta"][RUICLAW_CONTEXT_SOURCE_META] == "user_input"
...
async def test_stop_preserves_runtime_checkpoint_for_next_turn(tmp_path: Path) -> None:
    from ruiclaw.command.builtin import cmd_stop
