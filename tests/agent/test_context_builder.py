...
from ruiclaw.runtime_context import RuntimeContextBlock
...
class TestIsTemplateContent:
    ...
    def test_content_matching_template(self):
        ...
        tpl = pkg_files("ruiclaw") / "templates" / "memory" / "MEMORY.md"
        if not tpl.is_file():
...
class TestBundledToolContract:
    def test_tool_contract_balances_general_and_coding_workflows(self):
        ...
        tpl = pkg_files("ruiclaw") / "templates" / "agent" / "tool_contract.md"
...
class TestBuildMessages:
    ...
    def test_public_builder_preserves_assistant_role_compatibility(self, tmp_path):
        from ruiclaw.agent import ContextBuilder as PublicContextBuilder
    ...
    def test_transcript_marks_context_sources_without_copying_content(self, tmp_path):
        builder = _builder(tmp_path)
        messages = builder.build_transcript(TranscriptInput(
            history=[{"role": "assistant", "content": "previous result"}],
            current_message="new question",
        ))

        system_meta = messages[0]["_meta"]
        assert system_meta[RUICLAW_CONTEXT_SOURCE_WEIGHTS_META]["bootstrap"] > 0
        assert "new question" not in str(system_meta)
