...
from ruiclaw.agent.tools.file_state import FileStateStore
from ruiclaw.bus.events import InboundMessage
from ruiclaw.bus.queue import MessageBus
from ruiclaw.command.builtin import cmd_new, register_builtin_commands
from ruiclaw.command.router import CommandContext, CommandRouter
from ruiclaw.config.schema import AgentDefaults, Config
...
def _make_loop(tmp_path: Path, unified_session: bool = False) -> AgentLoop:
    ...
    with patch("ruiclaw.agent.loop.SessionManager"), \
...
class TestUnifiedSessionConfig:
    ...
    def test_onboard_generated_config_contains_unified_session(self, tmp_path: Path):
        """save_config() writes 'unifiedSession' into config.json (simulates ruiclaw onboard)."""
