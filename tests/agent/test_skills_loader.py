"""Tests for ruiclaw.agent.skills.SkillsLoader."""
...
def _write_skill(
    ...
    *,
    ...
    """Create ``base / name / SKILL.md`` with optional ruiclaw metadata JSON."""
    skill_dir = base / name
    skill_dir.mkdir(parents=True)
    lines = ["---"]
    if metadata_json is not None:
        payload = json.dumps({"ruiclaw": metadata_json}, separators=(",", ":"))
...
def test_list_skills_filter_unavailable_excludes_unmet_bin_requirement(
    ...
    _write_skill(
        skills_root,
        "needs_bin",
        metadata_json={"requires": {"bins": ["ruiclaw_test_fake_binary"]}},
    )
    builtin = tmp_path / "builtin"
    builtin.mkdir()

    def fake_which(cmd: str) -> str | None:
        if cmd == "ruiclaw_test_fake_binary":
            return None
        return "/usr/bin/true"

    monkeypatch.setattr("ruiclaw.agent.skills.shutil.which", fake_which)
...
def test_list_skills_filter_unavailable_includes_when_bin_requirement_met(
    ...
    def fake_which(cmd: str) -> str | None:
        if cmd == "ruiclaw_test_fake_binary":
            return "/fake/ruiclaw_test_fake_binary"
        return None
...
def test_list_skills_filter_unavailable_false_keeps_unmet_requirements(
    ...
    monkeypatch.setattr("ruiclaw.agent.skills.shutil.which", lambda _cmd: None)
...
def test_list_skills_filter_unavailable_excludes_unmet_env_requirement(
    ...
    _write_skill(
        skills_root,
        "needs_env",
        metadata_json={"requires": {"env": ["RUICLAW_SKILLS_TEST_ENV_VAR"]}},
    )
    builtin = tmp_path / "builtin"
    builtin.mkdir()

    monkeypatch.delenv("RUICLAW_SKILLS_TEST_ENV_VAR", raising=False)
...
def test_list_skills_openclaw_metadata_parsed_for_requirements(
    ...
    oc_payload = json.dumps({"openclaw": {"requires": {"bins": ["ruiclaw_oc_bin"]}}}, separators=(",", ":"))
    ...
    monkeypatch.setattr(
        "ruiclaw.agent.skills.shutil.which",
        lambda cmd: "/x" if cmd == "ruiclaw_oc_bin" else None,
    )
    entries = loader.list_skills(filter_unavailable=True)
    assert entries == [
...
def test_get_skill_metadata_handles_yaml_types(tmp_path: Path) -> None:
    ...
    skill_dir.mkdir(parents=True)
    payload = json.dumps({"ruiclaw": {"requires": {"bins": ["gh"]}, "always": True}}, separators=(",", ":"))
    skill_path = skill_dir / "SKILL.md"
    skill_path.write_text(
        "---\n"
        "name: typed\n"
