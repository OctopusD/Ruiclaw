...
from ruiclaw.session.turn_continuation import INTERNAL_CONTINUATION_META
...
def test_loop_from_config_uses_caller_owned_registry(tmp_path: Path) -> None:
    ...
    config.agents.defaults.max_concurrent_tools = 2
    config.agents.defaults.tool_timeout_seconds = 7.5
    ...
    assert loop.max_concurrent_tools == 2
    assert loop.tool_timeout_seconds == 7.5
