...
def _source_archive(root: Path) -> bytes:
    ...
    with tarfile.open(fileobj=output, mode="w:gz", format=tarfile.PAX_FORMAT) as archive:
        for relative in included:
            ...
            archive.add(path, arcname=Path("ruiclaw-tui-source/tui") / relative)
        archive.add(root.parent / "packages/client-events",
                    arcname="ruiclaw-tui-source/packages/client-events")
        archive.add(root.parent / "LICENSE", arcname="ruiclaw-tui-source/LICENSE")
...
def main() -> None:
    ...
    asset = f"ruiclaw-tui-{target}{extension}"
    dist = root / "dist"
    ...
    files = {
        ...
        "ruiclaw-tui-source.tar.gz": _source_archive(root),
    }
