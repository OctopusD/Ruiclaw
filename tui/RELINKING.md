...
The ruiclaw TUI release archive contains the executable, its complete JavaScript/TypeScript
...
- ruiclaw TUI: the `tui/` and `packages/client-events/` directories in the GitHub release tag that contains this archive
...
`ruiclaw-tui-source.tar.gz` from the release archive and run the resulting Bun executable:

```bash
cd ruiclaw-tui-source/tui
