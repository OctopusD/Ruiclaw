...
function source(value: unknown = connection, exitCode = 0) {
  ...
  return desktopConnectionSource({
    RUICLAW_TUI_DESKTOP_RESOLVER: JSON.stringify([process.execPath, "-e", script]),
    RUICLAW_TUI_DESKTOP_TARGET: JSON.stringify({ instanceId, gatewayId }),
  })!
}
...
describe("Desktop credential resolver", () => {
  ...
  test("fails closed for incomplete launcher metadata", () => {
    expect(() => desktopConnectionSource({ RUICLAW_TUI_DESKTOP_TARGET: "{}" })).toThrow("Invalid Desktop")
  })
