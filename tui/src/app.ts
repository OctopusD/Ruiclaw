...
const COMPOSER_PLACEHOLDER = "Ask ruiclaw anything"
...
function connectionStatusText(
  ...
  if (status === "unavailable") {
    return info?.health === "degraded"
      ? "Still getting ready…"
      : "RuiClaw is taking longer to respond…"
  }
  if (status === "error") return "RuiClaw unavailable · restart ruiclaw"
...
export function sessionExitMessage(chatId: string): string {
  ...
  return `Resume with: ruiclaw agent --session ${sessionId}\n`
}
...
export class RuiClawTui {
  ...
  private constructor(
    ...
    this.client = client || new RuiClawClient({
      ...(options.resolveConnection ? {
        ...
        reconnect: false,
        targetEndpoint: "RuiClaw Desktop",
  ...
  static async create(options: AppOptions): Promise<RuiClawTui> {
    ...
    return RuiClawTui.mount(renderer, options, undefined, undefined, host)
  }
  ...
  private handleStatus(
    ...
    if (this.options.desktopGatewayId && status === "error") {
      this.connectionMessage = "Desktop disconnected or incompatible · exit and run ruiclaw to reconnect"
    }
    if (status === "connected") {
      this.ready = false
