...
const options: AppOptions = {
  ...
  model: "test/model",
  modelPreset: "default",
  workspace: "/tmp/ruiclaw-workspace",
  version: "test",
...
test("formats a reusable session ID after exit", () => {
  expect(sessionExitMessage("resume-chat")).toBe(
    "Resume with: ruiclaw agent --session websocket:resume-chat\n",
  )
})
...
const mount = (setup: TestRendererSetup, sent: string[] = []) => RuiClawTui.mount(
  setup.renderer,
  options,
  client(sent),
...
describe("RuiClawTui layout", () => {
  ...
  test("reflows a single retained layout across terminal resizes", async () => {
    ...
    for (const [width, height] of [[100, 30], [56, 18], [118, 36]] as const) {
      ...
      expect(occurrences(frame, "Ask ruiclaw anything")).toBe(1)
  ...
  test("clears the placeholder on the first typed character", async () => {
    ...
    const composer = (app as unknown as { composer: TextareaRenderable }).composer
    await setup.renderOnce()
    expect(setup.captureCharFrame()).toContain("Ask ruiclaw anything")
    ...
    expect(frame).not.toContain("Ask ruiclaw anything")
    expect(frame).not.toContain("buAsk ruiclaw anything")

    setup.mockInput.pressBackspace()
    setup.mockInput.pressBackspace()
    await setup.flush()
    expect(composer.placeholder).toBe("Ask ruiclaw anything")
    expect(setup.captureCharFrame()).toContain("Ask ruiclaw anything")
  })
  ...
  test("switches and creates gateway chats without replacing core slash commands", async () => {
    ...
    const app = RuiClawTui.mount(
      setup.renderer,
      { ...options, apiUrl: "http://ruiclaw.test", apiToken: "secret" },
      transport,
      new MockTreeSitterClient({ autoResolveTimeout: 0 }),
    )
    app.accept({ event: "attached", chat_id: "chat" })
  ...
  test("refreshes expired API credentials before opening sessions", async () => {
    ...
    const app = RuiClawTui.mount(
      ...
      {
        ...options,
        bootstrapUrl: "http://ruiclaw.test/webui/bootstrap",
        bootstrapSecret: "bootstrap-secret",
        apiUrl: "http://ruiclaw.test",
  ...
  test("opens composer runtime controls by mouse and applies canonical choices", async () => {
    ...
    try {
      ...
      expect(scopes).toEqual([{
        project_path: "/tmp/ruiclaw-workspace",
        access_mode: "full",
  ...
  test("switches sessions only through the sessions command", async () => {
    ...
    try {
      ...
      expect(titleItems.some((item) => item.id === "ruiclaw-tui-title-text")).toBe(false)
  ...
  test("shows compact session context without exposing private reasoning", async () => {
    ...
    try {
      ...
      expect(occurrences(compact, "Ask ruiclaw anything")).toBe(1)
  ...
  test("loads earlier transcript pages in place when PageUp reaches the top", async () => {
    ...
    const app = RuiClawTui.mount(
      setup.renderer,
      { ...options, apiUrl: "http://ruiclaw.test", apiToken: "secret", chatId: "chat" },
      client(),
      new MockTreeSitterClient({ autoResolveTimeout: 0 }),
    )
  ...
  test("survives rapid narrow resizes with long CJK and code", async () => {
    ...
    app.accept({
      ...
      text: [
        ...
        "https://ruiclaw.test/a-very-long-unbroken-path-that-must-not-break-the-layout",
        "```ts",
        "const greeting = '你好，ruiclaw'",
        "```",
      ].join("\n\n"),
    })
    app.accept({ event: "stream_end", chat_id: "chat" })
  ...
  test("inherits the host background after long output fills the viewport", async () => {
    ...
    app.accept({
      ...
      text: Array.from({ length: 80 }, (_, index) => (
        `### Section ${index + 1}\n中文长回答、**bold** and [link](https://ruiclaw.test/${index + 1})`
      )).join("\n\n"),
    })
    app.accept({ event: "stream_end", chat_id: "chat" })
    app.accept({ event: "turn_end", chat_id: "chat" })
  ...
  test("distinguishes the composer with a quiet focus edge", async () => {
    ...
    const composerLine = setup.captureCharFrame().split("\n")
      .find((line) => line.includes("Ask ruiclaw anything")) || ""
  ...
  test("uses asymmetric roles instead of chat bubbles", async () => {
    ...
    const headerLine = frame.split("\n").find((line) => line.includes(">_  ruiclaw")) || ""
  ...
  test("replaces streamed drafts with canonical stream-end text", async () => {
    ...
    app.accept({
      event: "stream_end",
      chat_id: "chat",
      text: "canonical https://ruiclaw.test/signed/current",
      resuming: true,
      merge_next: true,
    })
    ...
    app.accept({
      event: "stream_end",
      chat_id: "chat",
      text: "final https://ruiclaw.test/signed/current tail",
    })
    app.accept({ event: "turn_end", chat_id: "chat" })
    await setup.flush()
    const frame = setup.captureCharFrame()

    expect(frame).toContain("final https://ruiclaw.test/signed/current tail")
    expect(frame).not.toContain("canonical https://ruiclaw.test/signed/current")
  ...
  test("animates one stable status line while the agent works", async () => {
    ...
    app.accept({ event: "turn_end", chat_id: "chat" })
    await setup.flush()
    expect(ui.composer.placeholder).toBe("Ask ruiclaw anything")
  })
  ...
  test("groups consecutive file activity only in the collapsed preview", async () => {
    ...
    app.accept({
      ...
      tool_events: Array.from({ length: 6 }, (_, index) => ({
        ...
        call_id: `read-${index}`,
        name: "read_file",
        arguments: { path: `/tmp/ruiclaw-workspace/src/file-${index}.ts` },
      })),
    })
    await setup.renderOnce()
    let frame = setup.captureCharFrame()
  ...
  test("reconciles active state from attach hydration after reconnect", async () => {
    ...
    expect(occurrences(restored, ">_  ruiclaw")).toBe(1)
    app.accept({
      event: "goal_status",
      chat_id: "chat",
      status: "running",
  ...
  test("shows actionable connection states without implementation details", async () => {
    ...
    expect(ui.status.plainText).toBe("RuiClaw is taking longer to respond…")
    expect(ui.status.plainText).not.toContain("Unable")
    ...
    expect(ui.status.plainText).toBe("RuiClaw unavailable · restart ruiclaw")
  ...
  test("deduplicates compaction history against events queued during hydration", async () => {
    ...
    const app = RuiClawTui.mount(
      setup.renderer,
      { ...options, apiUrl: "http://ruiclaw.test", apiToken: "token", chatId: "chat" },
      client(),
      new MockTreeSitterClient({ autoResolveTimeout: 0 }),
    )
    try {
...
describe("RuiClawTui with a Herdr pane title reporter", () => {
  test("keeps the full terminal experience while reporting task titles", async () => {
    ...
    expect(activeFrame).toContain(">_  ruiclaw")
...
if (process.platform !== "win32") {
  test("restores the terminal after SIGTERM", async () => {
    const child = Bun.spawn(["bun", "src/index.ts"], {
      ...
      env: {
        ...process.env,
        RUICLAW_TUI_WS_URL: "ws://127.0.0.1:9/ws",
        RUICLAW_TUI_API_URL: "",
        RUICLAW_TUI_API_TOKEN: "",
        RUICLAW_TUI_CHAT_ID: "resume-chat",
        RUICLAW_TUI_MODEL: "test/model",
        RUICLAW_TUI_WORKSPACE: "/tmp/ruiclaw-test",
        RUICLAW_TUI_VERSION: "test",
        RUICLAW_TUI_ACCESS: "workspace access",
        RUICLAW_TUI_THEME: "dark",
      },
      stdout: "pipe",
      stderr: "pipe",
    })
