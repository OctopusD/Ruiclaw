...
export class DiffViewer {
  ...
  private render(): void {
    ...
    if (text) {
      this.scroll.add(new DiffRenderable(this.renderer, {
        id: `ruiclaw-tui-diff-${this.selected}`,
        diff: text,
        width: "100%",
        height: "auto",
        flexShrink: 0,
  ...
  private text(content: string, tone: "muted" | "error" | "accent"): TextRenderable {
    return new TextRenderable(this.renderer, {
      id: `ruiclaw-tui-diff-note-${this.selected}-${tone}`,
      content,
      width: "100%",
      minHeight: 1,
      fg: this.theme[tone],
