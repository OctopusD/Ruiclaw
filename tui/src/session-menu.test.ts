...
describe("SessionMenu", () => {
  ...
  test("shows compact workspace names only when they distinguish sessions", async () => {
    ...
    await setup.renderOnce()
    expect(setup.captureCharFrame()).not.toContain("ruiclaw · Codex")
    ...
    await setup.renderOnce()
    const frame = setup.captureCharFrame()
    expect(frame).toContain("ruiclaw · Codex")
    ...
    menu.open([
      {
        ...scoped[0]!,
        workspaceScope: {
          project_path: "/work/frontend/ruiclaw",
          project_name: "ruiclaw",
          access_mode: "restricted",
        },
      },
      {
        ...scoped[1]!,
        workspaceScope: {
          project_path: "/work/backend/ruiclaw",
          project_name: "ruiclaw",
          access_mode: "restricted",
        },
      },
    ...
    expect(duplicates).toContain("frontend/ruiclaw")
    expect(duplicates).toContain("backend/ruiclaw")
  })
