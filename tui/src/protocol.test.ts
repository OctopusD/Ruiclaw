...
describe("Desktop attach-only protocol", () => {
  ...
  async function fixture(run: (client: RuiClawClient, socket: FakeSocket, statuses: ConnectionStatus[], attempts: () => number) => Promise<void>) {
    ...
    const client = new RuiClawClient({
...
describe("gateway protocol", () => {
  test("bootstraps fresh websocket and API credentials", async () => {
    ...
    globalThis.fetch = (async (_input: string | URL | Request, init?: RequestInit) => {
      ...
      return new Response(JSON.stringify({
        ws_url: "ws://ruiclaw.test/ws?mode=local",
        token: "socket token",
        api_token: "api-token",
      }))
    }) as typeof fetch
    ...
    try {
      const connection = await fetchGatewayConnection(
        "http://ruiclaw.test/webui/bootstrap",
        "bootstrap-secret",
        "http://ruiclaw.test",
        "tui-42",
      )
      expect(headers?.get("X-RuiClaw-Auth")).toBe("bootstrap-secret")
      expect(connection).toEqual({
        wsUrl: "ws://ruiclaw.test/ws?mode=local&token=socket+token&client_id=tui-42",
    ...
    try {
      await expect(fetchSlashCommands(
        ...
        async (rejectedApiToken) => {
          ...
          return { apiUrl: "http://ruiclaw.test", apiToken: "fresh-api-token" }
        },
  ...
  test("waits for bootstrap before opening the websocket", async () => {
    ...
    try {
      ...
      resolveConnection?.({
        wsUrl: "ws://ruiclaw.test/ws?token=fresh",
        apiUrl: "http://ruiclaw.test",
        apiToken: "fresh-api-token",
      })
      await Bun.sleep(1)
      expect(requestedUrl).toBe("ws://ruiclaw.test/ws?token=fresh")
    ...
    try {
      const client = new RuiClawClient({
        resolveConnection: async () => {
          ...
          return {
            wsUrl: "ws://ruiclaw.test/ws?token=second",
            apiUrl: "http://ruiclaw.test",
      ...
      expect(requestedUrl).toBe("ws://ruiclaw.test/ws?token=second")
      client.close()
    } finally {
      Object.defineProperty(globalThis, "WebSocket", { configurable: true, value: original })
    }
  ...
  test("loads runtime model and access controls from canonical APIs", async () => {
    ...
    try {
      expect(await fetchRuntimeControls("http://ruiclaw.test", "secret")).toEqual({
  ...
  test("reattaches the same generated chat after a transient disconnect", async () => {
    ...
    try {
      ...
      expect(reconnecting).toMatchObject({
        ...
        info: { endpoint: "ruiclaw.test", attempt: 1 },
      })
      sockets[1]?.emit("open")
  ...
  test("reports when the bounded history snapshot omits earlier turns", async () => {
    ...
    try {
      const history = await fetchHistory("http://ruiclaw.test", "token", "chat", "newer-page")
  ...
  test("loads compaction history as activity rows", async () => {
    ...
    try {
      const history = await fetchHistory("http://ruiclaw.test", "token", "chat")
  ...
  test("loads the explainable session context projection", async () => {
    ...
    try {
      expect(await fetchSessionContext("http://ruiclaw.test", "secret", "chat")).toEqual({
        totalMessages: 24,
        archivedMessages: 16,
        replayMessages: 10,
  ...
  test("loads the gateway-owned slash command catalog", async () => {
    ...
    try {
      expect(await fetchSlashCommands("http://ruiclaw.test", "secret")).toEqual([{
        command: "/history",
        title: "History",
        description: "Show recent messages",
        argHint: "[n]",
  ...
  test("loads only enabled and available skills", async () => {
    ...
    try {
      expect(await fetchAvailableSkills("http://ruiclaw.test", "secret")).toEqual([{
        name: "verify",
        description: "Verify public behavior",
        source: "builtin",
      }])
  ...
  test("loads and normalizes WebUI sessions", async () => {
    ...
    try {
      expect(await fetchSessions("http://ruiclaw.test", "secret")).toEqual([{
        chatId: "chat-1",
  ...
  test("combines installed tools and saved sessions in one mention namespace", async () => {
    ...
    try {
      expect(await fetchMentionCandidates("http://ruiclaw.test", "secret")).toEqual([
        {
          kind: "cli",
          name: "github",
          displayName: "GitHub",
