...
The terminal UI is a TypeScript client for ruiclaw's existing WebSocket gateway. It owns presentation and input only; the Python gateway remains the single implementation of sessions, the agent loop, tools, memory, and security policy.
...
When Herdr supplies `HERDR_ENV=1` and `HERDR_PANE_ID`, ruiclaw keeps the same full-screen layout, controls, and navigation available in any other terminal. Its only host-specific behavior is reporting the latest user task as the Herdr pane title through the supported pane CLI. Creating a new chat, switching to a chat without a task, and exiting the TUI clear that title. RuiClaw does not report agent lifecycle, session, model, Git branch, workspace, or action metadata to Herdr.
...
composer; ruiclaw sends the original text unchanged. Press `Ctrl+V` or `Alt+V` while the composer
...
While ruiclaw is working, the composer prompt becomes
...
same gateway metadata used by the WebUI. While ruiclaw is working, `Enter` sends immediately,
...
`/new-chat` preserves the current conversation and starts another one; ruiclaw's existing `/new`
...
`ruiclaw agent --session ...` command that resumes the current session. When earlier transcript
...
`ruiclaw agent --classic --session <channel:id>` to resume a session owned by another channel;
