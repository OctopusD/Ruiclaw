...
def _isolate_ruiclaw_log_activation() -> Iterator[None]:
...
def _isolate_sessions_root(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Iterator[None]:
    ...
    monkeypatch.setattr(
        "ruiclaw.session.manager.get_runtime_subdir",
        runtime_subdir,
    )
    monkeypatch.setattr(
        "ruiclaw.session.manager.get_legacy_sessions_dir",
        lambda: legacy_root,
    )
    yield
...
def _isolate_pairing_store(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "ruiclaw.pairing.store._store_path",
        lambda: pairing_path,
    )
