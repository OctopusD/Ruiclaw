...
function guardWebuiEntryChunk(): Plugin {
  return {
    name: "ruiclaw-guard-webui-entry-chunk",
    apply: "build",
...
export function gzipWebuiAssets(): Plugin {
  return {
    name: "ruiclaw-gzip-webui-assets",
    apply: "build",
...
export default defineConfig(({ mode }) => {
  ...
  const target = env.RUICLAW_API_URL ?? "http://127.0.0.1:8765";
  const hmrPath = "/__ruiclaw_vite_hmr";
  ...
  return {
    ...
    build: {
      outDir: path.resolve(__dirname, "../ruiclaw/web/dist"),
      emptyOutDir: true,
    ...
    server: {
      ...
      // Keep Vite's HMR socket on a dedicated path. RuiClaw's app WebSocket is
