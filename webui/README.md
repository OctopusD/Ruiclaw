# ruiclaw WebUI Source

This directory contains the React/TypeScript source for the ruiclaw WebUI. If
you installed `ruiclaw-ai` from PyPI and only want to use the bundled browser UI,
...
ruiclaw/web/dist/      build output served by the gateway
```

## Develop the WebUI (Vite HMR)

### 1. Install ruiclaw from source
...
```bash
RUICLAW_API_URL=http://127.0.0.1:9000 bun run dev
```
...
```bash
cd webui
bun run build          # writes to ../ruiclaw/web/dist
```
