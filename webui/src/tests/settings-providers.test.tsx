...
describe("Settings providers", () => {
  ...
  it("signs in to the xAI Grok provider", async () => {
    ...
    expect(
      screen.getByText(
        "Complete sign-in in your browser. If ruiclaw does not connect automatically, paste the authorization code below.",
      ),
    ).toBeInTheDocument();
  ...
  it("polls local OpenAI Codex sign-in until the loopback callback completes", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Sign in" }));
    const dialog = (await screen.findByText("Complete sign-in in your browser. If ruiclaw does not connect automatically, copy the full localhost callback URL from the address bar and paste it below.")).closest('[role="dialog"]') as HTMLElement;
    ...
    expect(
      within(dialog).getByText(
        "Complete sign-in in your browser. If ruiclaw does not connect automatically, copy the full localhost callback URL from the address bar and paste it below.",
      ),
    ).toBeInTheDocument();
  ...
  it("completes remote OpenAI Codex sign-in with the full callback URL", async () => {
    ...
    try {
      ...
      expect(
        screen.getByText(
          "Sign in through this browser, then paste the full localhost callback URL back into ruiclaw.",
        ),
      ).toBeInTheDocument();
