...
vi.mock("@/lib/ruiclaw-client", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@/lib/ruiclaw-client")>();
  class MockClient {
  ...
  return { ...actual, RuiClawClient: MockClient };
});
...
describe("App layout", () => {
  beforeEach(async () => {
    ...
    Reflect.deleteProperty(window, "ruiclawHost");
    setNavigatorPlatform("Linux x86_64");
    localStorage.removeItem("ruiclaw-webui.sidebar");
    localStorage.removeItem("ruiclaw-webui.sidebar.width");
    localStorage.removeItem("ruiclaw-webui.sidebar.completed-runs.v1");
    localStorage.removeItem("ruiclaw-webui.sidebar.session-updates.v1");
    localStorage.removeItem("ruiclaw-webui.collapsed-pane-groups.v1");
    localStorage.removeItem("ruiclaw-webui.restartStartedAt");
    localStorage.removeItem("ruiclaw-webui.restartRoute");
  ...
  it("opens Channels below Automations and supports its keyboard shortcut", async () => {
    mockFetchRoutes({
      ...
      "/api/settings/ruiclaw-features": { features: [], enabled_count: 0 },
    });
    render(<App />);
    await waitFor(() => expect(connectSpy).toHaveBeenCalled());
    const sidebar = screen.getByRole("navigation", { name: "Sidebar navigation" });
    ...
    expect(document.title).toBe("Channels · RuiClaw");
  ...
  it("creates a new temporary chat from the hero each time", async () => {
    ...
    await waitFor(() => expect(document.title).toBe("first private message · RuiClaw"));
    expect(screen.queryByRole("button", { name: "Temporary chat" })).not.toBeInTheDocument();
  ...
  it("restores the standalone Channels page from a legacy restart route", async () => {
    localStorage.setItem("ruiclaw-webui.restartStartedAt", String(Date.now()));
    localStorage.setItem("ruiclaw-webui.restartRoute", "#/settings?section=channels");
    window.history.replaceState(null, "", "/#/new");
    mockFetchRoutes({
      "/api/settings": baseSettingsPayload(),
      "/api/settings/ruiclaw-features": {
        features: [{
          name: "websocket",
  ...
  it("opens Skills from the main sidebar", async () => {
    ...
    expect(document.title).toBe("Skills · RuiClaw");
  ...
  it("opens Automations from the main sidebar", async () => {
    ...
    expect(document.title).toBe("Automations · RuiClaw");
  ...
  it("localizes the Automations surface", async () => {
    ...
    expect(document.title).toBe("自动任务 · RuiClaw");
  });
  ...
  it("resizes the sidebar, collapses at the drag threshold and restores its saved width", async () => {
    ...
    expect(sidebar).toHaveStyle({ width: "360px" });
    expect(localStorage.getItem("ruiclaw-webui.sidebar.width")).toBe("360");
  ...
  it("uses native chrome when the host bridge overrides browser gateway metadata", async () => {
    Reflect.set(window, "ruiclawHost", { pickFolder: vi.fn() });
    vi.mocked(fetchBootstrap).mockResolvedValue({
      token: "tok",
      api_token: "api-tok",
      ws_path: "/",
  ...
  it("restores sidebar run indicators after a page reload", async () => {
    ...
    localStorage.setItem(
      "ruiclaw-webui.sidebar.session-updates.v1",
  ...
  it("restores the active chat from the URL hash after a page reload", async () => {
    ...
    await waitFor(() => expect(connectSpy).toHaveBeenCalled());
    await waitFor(() => expect(document.title).toBe("Active after reload · RuiClaw"));
    const sidebar = screen.getByRole("navigation", { name: "Sidebar navigation" });
    expect(
  ...
  it("keeps the backend timezone without writing settings on mount", async () => {
    ...
    expect(
      system.queryByText("Restart RuiClaw to apply runtime changes."),
    ).not.toBeInTheDocument();
  });
  ...
  it("transitions between Apps and Skills without replacing the sidebar", async () => {
    ...
    expect(screen.queryByText("Add tools to ruiclaw, then @ them in chat.")).not.toBeInTheDocument();
    expect(screen.getByRole("navigation", { name: "Sidebar navigation" })).toBeInTheDocument();
    expect(screen.queryByRole("navigation", { name: "Settings sections" })).not.toBeInTheDocument();
    ...
    expect(document.title).toBe("Apps · RuiClaw");
  ...
  it("returns from settings to the blank start page when no session was active", async () => {
    ...
    const sidebar = screen.getByRole("navigation", { name: "Sidebar navigation" });
    fireEvent.click(within(sidebar).getByRole("button", { name: "New topic" }));
    await waitFor(() => expect(document.title).toBe("RuiClaw"));
