...
describe("ThreadComposer", () => {
  ...
  it("keeps project selection as a compact composer dropdown", async () => {
    ...
    const onWorkspaceScopeChange = vi.fn();
    const defaultScope = {
      project_path: "/Users/test/.ruiclaw/workspace",
  ...
  it.each([
    ["Windows", "D:\\Users\\test\\.ruiclaw\\workspace", "D:\\path\\to\\project"],
    ["macOS", "/Users/test/.ruiclaw/workspace", "/Users/name/project"],
    ["Linux", "/home/test/.ruiclaw/workspace", "/home/name/project"],
  ...
  it("uses the native folder picker for project selection on native host", async () => {
    ...
    Object.defineProperty(window, "ruiclawHost", {
      configurable: true,
      value: {
  ...
  it("prioritizes stop as an immediate slash action while streaming", () => {
    ...
    expect(input).toHaveValue("");
    expect(window.localStorage.getItem("ruiclaw.webui.slashCommandRecents")).toBeNull();
  });

  it("orders recent slash commands first for the blank slash menu", () => {
    window.localStorage.setItem("ruiclaw.webui.slashCommandRecents", JSON.stringify(["/history"]));
    render(
      <ThreadComposer
        onSend={vi.fn()}
        placeholder="Type your message..."
  ...
  it("attaches persisted sessions only through the shared mention palette", () => {
    ...
    expect(screen.getByRole("group", { name: "RuiClaw conversations" })).toBeInTheDocument();
  ...
  it("keeps temporary chat guidance in memory only", async () => {
    ...
    expect(
      window.localStorage.getItem(
        "ruiclaw.webui.composerQueuedGuidance.v1:temporary-private",
      ),
    ).toBeNull();
