...
describe("AgentActivityCluster", () => {
  ...
  it.each(["diff", "collapsed_diff"] as const)(
    ...
    (fileEditDisplayMode) => {
      localStorage.setItem(
        "ruiclaw-webui.settings-preferences",
      ...
      try {
        ...
        localStorage.removeItem("ruiclaw-webui.settings-preferences");
      }
    },
  );
  ...
  it("keeps CLI rows in chronological trace order", () => {
        ...
        messages={[
          {
            ...
            role: "tool",
            kind: "trace",
            content: 'web_search({"query":"ruiclaw architecture"})',
            traces: ['web_search({"query":"ruiclaw architecture"})'],
            createdAt: 1,
          },
          {
            id: "t-cli",
    ...
    const searchRow = screen.getByText("Searched ruiclaw architecture").closest('[data-testid="activity-step"]');
  ...
  it("renders hosted X search as an explicit search activity", () => {
    const line = 'x_search({"query":"ruiclaw oauth"})';
    render(
      <AgentActivityCluster
        messages={[{
          id: "t-x-search",
    ...
    expect(screen.getByText("Searched X · ruiclaw oauth")).toBeInTheDocument();
  ...
  it("groups repeated searches over internal tool results without exposing raw paths", () => {
    ...
    const firstPath = "/Users/test/.ruiclaw/workspace/.ruiclaw/tool-results/websocket_session/call_first-result.txt";
    const secondPath = "/Users/test/.ruiclaw/workspace/.ruiclaw/tool-results/websocket_session/call_second-result.txt";
    const traces = [
      ...
      <AgentActivityCluster
        messages={activityMessages("", {
          ...
          fileEdits: [{
            call_id: "call-write",
            tool: "write_file",
            path: "/Users/renxubin/.ruiclaw/workspace/agent-research-video/composition.html",
            phase: "error",
            added: 0,
            deleted: 0,
            approximate: false,
  ...
  it("keeps image generation status to one activity line", () => {
    const message: UIMessage = {
      ...
      role: "tool",
      kind: "trace",
      content: 'generate_image({"prompt":"an orange ruiclaw on a desk","aspect_ratio":"4:3"})',
      traces: ['generate_image({"prompt":"an orange ruiclaw on a desk","aspect_ratio":"4:3"})'],
      toolEvents: [{
        phase: "start",
        call_id: "image-call",
        name: "generate_image",
        arguments: { prompt: "an orange ruiclaw on a desk", aspect_ratio: "4:3" },
      }],
      createdAt: 1,
    };
    const { rerender } = render(
