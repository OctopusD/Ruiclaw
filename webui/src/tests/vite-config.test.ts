...
describe("gzipWebuiAssets", () => {
  it("compresses finalized files after Rollup writes the bundle", () => {
    ...
    const outputDir = mkdtempSync(path.join(tmpdir(), "ruiclaw-vite-gzip-"));
    try {
...
describe("webuiManualChunk", () => {
  it("groups common and channel translations into one request per locale", () => {
    ...
    expect(webuiManualChunk("/repo/ruiclaw/channels/slack/webui/locales/en.json")).toBe("locale-en");
    expect(webuiManualChunk("/repo/ruiclaw/channels/slack/webui/locales/zh-CN.json")).toBe("locale-zh-CN");
