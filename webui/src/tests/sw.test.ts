...
const ORIGIN = "https://ruiclaw.test";
const CACHE_NAME = "ruiclaw-static-v2";
const ICON_CACHE_NAME = "ruiclaw-icons-v1";
...
describe("service worker", () => {
  ...
  it("removes stale cache names and prunes unreferenced static entries on activate", async () => {
    ...
    expect(sw.deletedCacheNames).toEqual(["ruiclaw-static-v1", "ruiclaw-icons-v0"]);
  ...
  it("keeps un-hashed brand assets on the network-first path", async () => {
    const sw = loadSw();
    const iconUrl = `${ORIGIN}/brand/ruiclaw_icon_192.png`;
  ...
  it("clones network responses before yielding their body to the browser", async () => {
    const sw = loadSw();
    const request = new Request(`${ORIGIN}/brand/ruiclaw_icon_192.png`);
