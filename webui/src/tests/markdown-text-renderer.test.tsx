...
describe("MarkdownTextRenderer", () => {
  ...
  it("renders local file links as previewable file references", () => {
    ...
    render(
      <MarkdownTextRenderer onOpenFilePreview={onOpenFilePreview}>
        {"Edited [hook.py](/Users/test/project/ruiclaw/agent/hook.py:12)"}
      </MarkdownTextRenderer>,
    );
    ...
    expect(reference).toHaveAttribute(
      "aria-label",
      "/Users/test/project/ruiclaw/agent/hook.py",
    );
  ...
  it("does not wrap complete fenced code blocks in an extra pre", () => {
    const { container } = render(
      <MarkdownTextRenderer highlightCode={false}>
        {"当前目录:\n\n```text\n/Users/renxubin/.ruiclaw/workspace\n```"}
      </MarkdownTextRenderer>,
    );

    expect(screen.getByText("/Users/renxubin/.ruiclaw/workspace")).toBeInTheDocument();
  ...
  it("renders markdown videos as inline players", () => {
    render(<MarkdownTextRenderer>![ruiclaw-intro.mp4](/api/media/sig/video)</MarkdownTextRenderer>);

    const video = screen.getByLabelText("Video attachment: ruiclaw-intro.mp4");
    expect(video.tagName).toBe("VIDEO");
    expect(video).toHaveAttribute("src", "/api/media/sig/video");
    expect(video).toHaveAttribute("controls");
    expect(screen.queryByRole("img", { name: "ruiclaw-intro.mp4" })).not.toBeInTheDocument();
  });
  ...
  it("renders GFM tables in a responsive data surface", () => {
    ...
    expect(screen.getByRole("table")).toHaveTextContent("ruiclaw");
  ...
  it("does not create one DOM node per CJK character for long responses", () => {
    ...
    expect(container.querySelector("[data-sd-animate]")).not.toBeInTheDocument();
    expect(container.querySelector("[data-ruiclaw-stream-unit]")).not.toBeInTheDocument();
  });
