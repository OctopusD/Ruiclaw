...
import type { RuiClawFeatureInfo, RuiClawFeaturesPayload } from "@/lib/types";
import {
...
function feature(enabled: boolean, instances = false): RuiClawFeatureInfo {
  return {
    name: "test-chat",
...
function payload(channel: RuiClawFeatureInfo): RuiClawFeaturesPayload {
  return { features: [channel], enabled_count: Number(channel.enabled) };
}

function renderChannel(channel: RuiClawFeatureInfo) {
  vi.stubGlobal("fetch", vi.fn(async (input: RequestInfo | URL) => {
    const url = String(input);
    if (url === "/api/settings") return jsonResponse(settingsPayload());
    if (url === "/api/settings/ruiclaw-features") return jsonResponse(payload(channel));
...
describe("optimistic channel switches", () => {
  ...
  for (const surface of ["catalog", "setup", "instance"] as const) {
    it.each([
      ...
      await act(async () => {
        if (succeeds) {
          mutation.resolve(surface === "setup" && !initial ? { ruiclaw_features: result } : result);
        } else {
