    ...
    render(
      <ChatList
        sessions={[session({
          ...
          title: "Root topic",
          workspaceScope: {
            project_path: "/Users/me/ruiclaw",
            project_name: "ruiclaw",
  ...
  it("groups WebUI chats by workspace project while preserving in-project sorting and activity", () => {
    const sessions = [
      ...
      session({
        ...
        workspaceScope: {
          project_path: "/Users/me/ruiclaw-bench",
          project_name: "ruiclaw-bench",
          access_mode: "full",
        },
      }),
    ];
    ...
    const ruiclawSection = screen.getByRole("region", { name: "ruiclaw" });
    const ruiclawText = ruiclawSection.textContent ?? "";
    const projectSurface = ruiclawSection.querySelector(
      "[data-sidebar-project-surface]",
    );

    expect(screen.getByRole("region", { name: "ruiclaw-bench" })).toBeInTheDocument();
    expect(projectSurface).toHaveClass("ms-4");
    expect(within(ruiclawSection).getByText("Alpha task")).toBeInTheDocument();
    expect(
      within(ruiclawSection)
        .getByText("@mira")
        .closest("[data-sidebar-session-handle]")
        ?.parentElement,
    ).toHaveClass("items-center");
    expect(within(ruiclawSection).getByText("Zeta task")).toBeInTheDocument();
    expect(ruiclawText.indexOf("Alpha task")).toBeLessThan(ruiclawText.indexOf("Zeta task"));
    expect(within(ruiclawSection).getByLabelText("Agent running")).toBeInTheDocument();
  ...
  it("keeps default workspace topics in the Topics section instead of a project folder", () => {
    const sessions = [
      session({
        ...
        updatedAt: "2026-05-21T10:00:00Z",
        workspaceScope: {
          project_path: "/Users/me/.ruiclaw/workspace",
    ...
    render(
      <ChatList
        ...
        onRequestRename={vi.fn()}
        onToggleArchive={vi.fn()}
        defaultWorkspacePath="/Users/me/.ruiclaw/workspace"
        showTimestamps
      />,
    );

    expect(screen.getByText("Projects")).toBeInTheDocument();
    expect(screen.getByRole("region", { name: "ruiclaw" })).toBeInTheDocument();
  ...
  it("can collapse a project group and keeps project rename separate from chat titles", async () => {
    ...
    render(
      <ChatList
        ...
        projectNameOverrides={{ "/Users/me/ruiclaw": "Photos" }}
        collapsedGroups={{ "project:/Users/me/ruiclaw": true }}
      />,
    );
    ...
    expect(onToggleGroup).toHaveBeenCalledWith("project:/Users/me/ruiclaw");
    ...
    expect(onNewChatInProject).toHaveBeenCalledWith("/Users/me/ruiclaw", "Photos");
    expect(onToggleGroup).toHaveBeenCalledTimes(1);

    fireEvent.contextMenu(projectButton);
    fireEvent.click(await screen.findByRole("menuitem", { name: "Rename" }));

    expect(onRequestRenameProject).toHaveBeenCalledWith("/Users/me/ruiclaw", "Photos");
  });
