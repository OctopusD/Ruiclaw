...
describe("webui API helpers", () => {
  ...
  it("fetches RuiClaw run lists and percent-encoded details", async () => {
    vi.mocked(fetch)
      ...
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ summary: { run_id: "run/1" }, events: [] }),
      } as Response);

    await listRuns("tok", 25);
    await getRun("tok", "run/1");

    expect(fetch).toHaveBeenNthCalledWith(
      1,
      "/api/webui/runs?limit=25",
      expect.objectContaining({ headers: { Authorization: "Bearer tok" } }),
    );
    expect(fetch).toHaveBeenNthCalledWith(
      2,
      "/api/webui/runs/run%2F1",
      expect.objectContaining({ headers: { Authorization: "Bearer tok" } }),
    );
  });
  ...
  it("reports HTML API fallbacks as gateway mismatch errors", async () => {
    ...
    await expect(fetchApiService("tok")).rejects.toMatchObject({
      status: 200,
      message: "Gateway returned WebUI HTML instead of JSON. Restart ruiclaw gateway and try again.",
    });
  });
  ...
  it("reads and toggles ruiclaw optional features", async () => {
    vi.mocked(fetch).mockResolvedValueOnce({
      ok: true,
      json: async () => ({
        features: [],
    ...
    await expect(fetchRuiClawFeatures("tok")).resolves.toMatchObject({ features: [] });
    expect(fetch).toHaveBeenCalledWith(
    ...
    await enableRuiClawFeature(mutationTransport, "matrix");
    expect(requestMutation).toHaveBeenLastCalledWith(
    ...
    await enableRuiClawFeature(mutationTransport, "whatsapp", { installOnly: true });
    expect(requestMutation).toHaveBeenLastCalledWith(
      "settings.feature.enable",
    ...
    await disableRuiClawFeature(mutationTransport, "matrix");
    expect(requestMutation).toHaveBeenLastCalledWith(
  ...
  it("reads and writes persisted sidebar state", async () => {
    const state = {
      ...
      project_name_overrides: { "/Users/me/ruiclaw": "Core" },
      tags_by_key: {},
      collapsed_groups: {},
      view: {
