...
/**
 * Minimal fake WebSocket implementing the subset RuiClawClient touches.
...
describe("RuiClawClient", () => {
  ...
  it("can swap the socket factory when the runtime URL changes", () => {
    ...
    client.close();
    client.updateUrl("ruiclaw-host://engine/", hostFactory);
    client.connect();

    expect(hostFactory).toHaveBeenCalledWith("ruiclaw-host://engine/");
    expect(lastSocket().url).toBe("host:ruiclaw-host://engine/");
  });
  ...
  it("uses the host socket bridge for native host URLs", async () => {
    ...
    expect(openSocket).toHaveBeenCalledWith("ruiclaw-host://engine/");
