...
function wrap(client: ReturnType<typeof fakeClient>["client"]) {
  return function Wrapper({ children }: { children: ReactNode }) {
    return (
      <ClientProvider
        client={client as unknown as import("@/lib/ruiclaw-client").RuiClawClient}
        token="tok"
      >
        {children}
      </ClientProvider>
...
describe("useRuiClawStream", () => {
  ...
  it("keeps a hydrated terminal compaction when a queued start arrives", () => {
    ...
    const { result } = renderHook(() => useRuiClawStream("chat", initial), {
      wrapper: wrap(fake.client),
    });
  ...
  it("batches answer deltas into one animation-frame update", async () => {
    ...
    const { result } = renderHook(() => useRuiClawStream("chat-batch", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("coalesces hidden-tab deltas without scheduling paint frames", () => {
    ...
    try {
      const fake = fakeClient();
      const { result } = renderHook(
        () => useRuiClawStream("chat-background", EMPTY_MESSAGES),
        { wrapper: wrap(fake.client) },
      );
  ...
  it("buffers a hidden chat view and flushes its ordered deltas on return", () => {
    ...
    let visible = false;
    const { result, rerender, unmount } = renderHook(() => useRuiClawStream("hidden-view", EMPTY_MESSAGES), {
  ...
  it("keeps the turn pending on disconnect without breaking a resumed stream", async () => {
    const fake = fakeClient();
    const { result } = renderHook(
      () => useRuiClawStream("chat-reconnect", EMPTY_MESSAGES),
      { wrapper: wrap(fake.client) },
    );
  ...
  it("clears stale stream state when the transport resets a run", async () => {
    const fake = fakeClient();
    const { result } = renderHook(
      () => useRuiClawStream("chat-reconnect-reset", EMPTY_MESSAGES),
      { wrapper: wrap(fake.client) },
    );
  ...
  it("flushes pending delta text before turn_end finalizes the turn", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-flush", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("stamps provider usage and latest context on the completed answer", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-usage", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("exposes typed recovery state and validates actions with its recovery id", async () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-recovery", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("does not let historical recovered state clear a later active turn", () => {
    const fake = fakeClient();
    const { result } = renderHook(
      () => useRuiClawStream("chat-recovered-history", EMPTY_MESSAGES),
      { wrapper: wrap(fake.client) },
    );
  ...
  it("preserves proactive automation source metadata on complete assistant messages", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-cron", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("preserves proactive automation source metadata on streamed assistant messages", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-cron-stream", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("preserves proactive automation source metadata on stream_end final text", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-cron-stream-end", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
    const source = { kind: "cron", label: "Repo check" };
  ...
  it("does not start streaming from completed trailing activity after an answer", () => {
    ...
    const { result } = renderHook(
      () => useRuiClawStream("chat-cron-done", initialMessages),
      { wrapper: wrap(fake.client) },
    );
  ...
  it("drops pending stream work when switching chats", async () => {
    ...
    const { result, rerender } = renderHook(
      ({ chatId }: { chatId: string }) => useRuiClawStream(chatId, EMPTY_MESSAGES),
      {
        wrapper: wrap(fake.client),
  ...
  it("starts in streaming mode when history shows pending tool calls", () => {
    ...
    const { result } = renderHook(
      () => useRuiClawStream("chat-p", initialMessages, true),
      {
        wrapper: wrap(fake.client),
      },
    );
  ...
  it("collapses consecutive tool_hint frames into one trace row", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-t", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("treats progress with arbitrary agent_ui like ordinary trace text", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-au", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
    act(() => {
      fake.emit("chat-au", {
  ...
  it("renders live tool traces from structured tool events", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-tool-events", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("dedupes finish-phase tool events after their start trace", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-tool-finish", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("replaces a hosted search placeholder when its query arrives", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-hosted-search", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("keeps phase updates when a tool event trace line is deduped", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-tool-phase", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("renders live file_edit events as their own activity trace", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-file-edit", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("replaces matching write_file tool events with live file edit activity", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-file-edit-events", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("keeps live file edits separate from mixed non-file tool traces", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-file-edit-mixed-tools", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("keeps every file from one apply_patch call", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-apply-patch-many", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("upgrades pending file_edit placeholders when the path arrives", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-file-edit-pending", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("merges file_edit updates after interleaved progress events", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-file-edit-progress", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("keeps interrupted pre-tool text as assistant output before activity", async () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-stream-segments", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("does not replace interrupted pre-tool text with final stream_end text", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-stream-end-final", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("splits live assistant output around tool hints without moving it into reasoning", async () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-live-segments", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("opens a new activity segment for reasoning after file edit activity", async () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-file-segments", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("keeps file edit blocks ordered across a new reasoning phase", async () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-file-order", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("accumulates reasoning_delta chunks on a placeholder until reasoning_end", async () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-r", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("absorbs a streaming reasoning placeholder into the answer turn that follows", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-r2", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("ignores empty reasoning_delta frames", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-r3", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("treats legacy kind=reasoning messages as a complete delta + end pair", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-r4", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("starts a new Thought block when reasoning arrives after visible output", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-r5", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("stamps completed live Thought blocks with their own latency", async () => {
    ...
    try {
      const fake = fakeClient();
      const { result } = renderHook(() => useRuiClawStream("chat-r5-lat", EMPTY_MESSAGES), {
        wrapper: wrap(fake.client),
      });
      await act(async () => {});
  ...
  it("keeps alternating reasoning and answer deltas in separate ordered blocks", async () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-r5b", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("does not attach a new turn's reasoning across the latest user boundary", async () => {
    ...
    const { result } = renderHook(
      () => useRuiClawStream("chat-r6", initialMessages),
      { wrapper: wrap(fake.client) },
    );
  ...
  it("does not attach reasoning across a tool trace boundary", async () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-r7", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("preserves closed reasoning slices when the tool trace is unavailable", async () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-r8", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("keeps tool-call reasoning before the matching live tool trace", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-tool-reasoning", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("absorbs non-streamed final answers into the preceding reasoning placeholder", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-final-reasoning", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("preserves reasoning-only output when a turn ends without an answer", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-empty-thinking", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("keeps earlier reasoning before sending the next user turn", () => {
    ...
    const { result } = renderHook(
      () => useRuiClawStream("chat-stale-thinking", initialMessages),
      { wrapper: wrap(fake.client) },
    );
  ...
  it("returns the submitted turn identity used by the optimistic row and wire frame", () => {
    const fake = fakeClient();
    const { result } = renderHook(
      () => useRuiClawStream("chat-submitted-turn", EMPTY_MESSAGES),
      { wrapper: wrap(fake.client) },
    );
  ...
  it("marks an optimistic turn accepted when its acknowledgement arrives", () => {
    const fake = fakeClient();
    const { result } = renderHook(
      () => useRuiClawStream("chat-accept-one", EMPTY_MESSAGES),
      { wrapper: wrap(fake.client) },
    );
    let submitted: ReturnType<typeof result.current.send> = null;
    act(() => {
  ...
  it("projects a user turn submitted from another attached client exactly once", () => {
    const fake = fakeClient();
    const { result } = renderHook(
      () => useRuiClawStream("chat-shared", EMPTY_MESSAGES),
      { wrapper: wrap(fake.client) },
    );
  ...
  it("projects a cross-session input with its public handle exactly once", () => {
    const fake = fakeClient();
    const { result } = renderHook(
      () => useRuiClawStream("chat-target", EMPTY_MESSAGES),
      { wrapper: wrap(fake.client) },
    );
    const event: InboundEvent = {
      event: "user_message",
  ...
  it("marks only the optimistic turn named by a correlated rejection as failed", () => {
    const fake = fakeClient();
    const { result } = renderHook(
      () => useRuiClawStream("chat-reject-one", EMPTY_MESSAGES),
      { wrapper: wrap(fake.client) },
    );
  ...
  it("falls back to the previous running turn when the newer turn is rejected", () => {
    const fake = fakeClient();
    const { result } = renderHook(
      () => useRuiClawStream("chat-reject-new", EMPTY_MESSAGES),
      { wrapper: wrap(fake.client) },
    );
    let first: ReturnType<typeof result.current.send> = null;
    let second: ReturnType<typeof result.current.send> = null;
  ...
  it("ends the spinner and drops pending stream work when the only turn is rejected", async () => {
    const fake = fakeClient();
    const { result } = renderHook(
      () => useRuiClawStream("chat-reject-only", EMPTY_MESSAGES),
      { wrapper: wrap(fake.client) },
    );
    let submitted: ReturnType<typeof result.current.send> = null;
    act(() => {
  ...
  it("applies a correlated rejection replayed through the chat event queue", () => {
    const fake = fakeClient();
    const { result } = renderHook(
      () => useRuiClawStream("chat-replayed-reject", EMPTY_MESSAGES),
      { wrapper: wrap(fake.client) },
    );
    let submitted: ReturnType<typeof result.current.send> = null;
    act(() => {
  ...
  it("does not show or apply an error correlated to another chat", () => {
    const fake = fakeClient();
    const { result } = renderHook(
      () => useRuiClawStream("chat-visible", EMPTY_MESSAGES),
      { wrapper: wrap(fake.client) },
    );
    let submitted: ReturnType<typeof result.current.send> = null;
    act(() => {
  ...
  it("shows an uncorrelated 1009 fault without rolling back the current turn", () => {
    const fake = fakeClient();
    const { result } = renderHook(
      () => useRuiClawStream("chat-generic-1009", EMPTY_MESSAGES),
      { wrapper: wrap(fake.client) },
    );
    act(() => {
  ...
  it("marks rejected side-channel guidance failed without stopping the main run", () => {
    const fake = fakeClient();
    const { result } = renderHook(
      () => useRuiClawStream("chat-side-reject", EMPTY_MESSAGES),
      { wrapper: wrap(fake.client) },
    );
  ...
  it("adds optimistic user file attachments as media", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-file-send", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
    const attachment = {
      media: {
  ...
  it("inlines quoted context into the optimistic and outbound user message", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-quote", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("attaches assistant media_urls to complete messages", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-m", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("keeps assistant html media as a file attachment", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-html-media", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("infers assistant svg media as an image attachment", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-svg-media", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("corrects explicit image media when the name is a non-image file", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-mislabelled-html", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("suppresses redundant stream confirmation after assistant media", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-img-result", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("stops the active turn without adding a user slash command bubble", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-stop", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("does not mark side-channel slash commands as streaming", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-status", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("finalizes active streaming before turn-ending side-channel commands", async () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-new", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("keeps the turn active after stream_end while side-channel replies arrive", () => {
    ...
    try {
      const fake = fakeClient();
      const { result } = renderHook(() => useRuiClawStream("chat-status-loop", EMPTY_MESSAGES), {
        wrapper: wrap(fake.client),
      });
  ...
  it("keeps guided output in place while the active turn resumes", async () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-guide", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("keeps length-recovery segments in one assistant message", async () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-length", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("keeps streaming alive across stream_end when tool activity follows", async () => {
    ...
    const { result } = renderHook(() => useRuiClawStream("chat-s", EMPTY_MESSAGES, false, onTurnEnd), {
      wrapper: wrap(fake.client),
    });
  ...
  it("replaces streamed content with final stream_end text when provided", async () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-stream-final", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("creates an assistant bubble from final stream_end text without prior delta", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-stream-end-only", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("stamps completion time and latency on the last assistant bubble from turn_end", () => {
    ...
    try {
      const fake = fakeClient();
      const { result } = renderHook(() => useRuiClawStream("chat-lat", EMPTY_MESSAGES), {
        wrapper: wrap(fake.client),
      });
  ...
  it("tracks goal_status running and clears on idle", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-g", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
  ...
  it("clears retry status and exposes a terminal model failure at turn end", () => {
    ...
    const fake = fakeClient();
    const { result } = renderHook(() => useRuiClawStream("chat-retry", EMPTY_MESSAGES), {
      wrapper: wrap(fake.client),
    });
...
describe("live/replay projection before canonical-event revision migration", () => {
  it.each(PROJECTION_FIXTURE_CASES)("matches the shared $name fixture", (fixtureCase) => {
    ...
    try {
      const fake = fakeClient();
      const { result } = renderHook(
        () => useRuiClawStream(fixtureCase.chat_id, fixtureCase.initial_messages),
        { wrapper: wrap(fake.client) },
      );
  ...
  it.each([true, false])("keeps continuation reasoning before its answer across frames (text snapshots: %s)", async (snapshots) => {
    ...
    const fake = fakeClient();
    const { result } = renderHook(
      () => useRuiClawStream(fixture.chat_id, fixture.initial_messages),
      { wrapper: wrap(fake.client) },
    );
