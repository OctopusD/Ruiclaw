...
const WS_OPEN = 1;
const WS_CLOSING = 2;
const HOST_SOCKET_URL_PREFIX = "ruiclaw-host://";
 ...
 * - **Production**: off unless ``localStorage.setItem('ruiclaw_debug_ws','1')`` (or ``true``).
 * - **Silence anywhere**: ``localStorage.setItem('ruiclaw_debug_ws','0')`` (or ``false`` / ``off``).
...
function wsInboundDebugEnabled(): boolean {
  ...
  try {
    ...
    const raw = ls?.getItem("ruiclaw_debug_ws")?.trim().toLowerCase() ?? "";
...
export interface RuiClawClientOptions {
  url: string;
  reconnect?: boolean;
...
export class RuiClawClient {
  ...
  constructor(private options: RuiClawClientOptions) {
  ...
  reconcileCanonicalCompletion(
    ...
    while (fences.size > RuiClawClient.COMPLETED_TURN_FENCE_MAX) {
  ...
  private handleMessage(ev: MessageEvent): void {
    ...
    try {
      ...
      if (wsInboundDebugEnabled()) {
        ...
        console.warn(
          "[ruiclaw ws inbound] invalid JSON",
    ...
    if (wsInboundDebugEnabled()) {
      console.log("[ruiclaw ws inbound]", summarizeInboundWsPayload(parsed));
    }
  ...
  private dispatch(chatId: string, ev: InboundEvent): void {
    ...
    q.push(ev);
    const over = q.length - RuiClawClient.PENDING_INBOUND_MAX;
    if (over > 0) {
      q.splice(0, over);
    }
  }
