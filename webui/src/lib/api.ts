...
async function request<T>(
  ...
  if (contentType && !contentType.toLowerCase().includes("application/json")) {
    ...
    throw new ApiError(
      res.status,
      isHtml
        ? "Gateway returned WebUI HTML instead of JSON. Restart ruiclaw gateway and try again."
...
export async function listRuns(token: string, limit = 50, base = ""): Promise<RunSummary[]> {
  const body = await request<{ runs: RunSummary[] }>(
    `${base}/api/webui/runs?limit=${encodeURIComponent(String(limit))}`,
    token,
    undefined,
    API_READ_TIMEOUT_MS,
  );
...
export async function getRun(token: string, runId: string, base = ""): Promise<RunDetail> {
  return request<RunDetail>(
    `${base}/api/webui/runs/${encodeURIComponent(runId)}`,
    token,
    undefined,
    API_READ_TIMEOUT_MS,
  );
...
export async function fetchRuiClawFeatures(
  token: string,
  base: string = "",
): Promise<RuiClawFeaturesPayload> {
  return request<RuiClawFeaturesPayload>(
    `${base}/api/settings/ruiclaw-features`,
    token,
    undefined,
    API_READ_TIMEOUT_MS,
  );
...
export async function enableRuiClawFeature(
  transport: WebUIMutationTransport,
  name: string,
  options: { instanceId?: string; installOnly?: boolean } = {},
): Promise<RuiClawFeaturesPayload> {
  return mutation<RuiClawFeaturesPayload>(
    transport,
...
export async function disableRuiClawFeature(
  transport: WebUIMutationTransport,
  name: string,
