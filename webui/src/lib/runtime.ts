...
const LOOPBACK_HOST_STORAGE_KEY = "ruiclaw-webui.native-host";
...
let loopbackHostApi: RuiClawHostApi | null = null;

declare global {
  interface Window {
    ruiclawHost?: RuiClawHostApi;
  }
}

function getHostApi(): RuiClawHostApi | null {
  if (typeof window === "undefined") return null;
  return window.ruiclawHost ?? loopbackHostApi;
}
...
function createLoopbackHostApi(config: LoopbackHostConfig): RuiClawHostApi {
  return {
    async pickFolder(): Promise<string | null> {
      ...
      try {
        ...
        throw new Error("Native folder picker is unavailable. Reopen RuiClaw and try again.");
      }
