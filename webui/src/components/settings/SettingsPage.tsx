...
export function SettingsPage({
  ...
  const renderSection = (section: SettingsSectionKey, embedded = false): ReactNode => {
    ...
    switch (section) {
      ...
      case "models":
        return (
          <div className="settings-stack">
            ...
            <ProvidersSettings
              settings={settings}
              ruiclawFeatures={ruiclawFeatures}
              featureAction={ruiclawFeatureAction}
              capabilityError={ruiclawFeaturesError}
        ...
        return (
          <div className="settings-stack">
            ...
            <WebSettings
              ...
              olostepInstalling={ruiclawFeatureAction === "enable:olostep"}
              capabilityError={ruiclawFeaturesError}
            />
          </div>
        );
      case "channels":
        return (
          <ChannelsSettings
            token={token}
            ruiclawFeatures={ruiclawFeatures}
            loading={ruiclawFeaturesLoading}
            actionKey={ruiclawFeatureAction}
            chatAppsDocsUrl={settings.docs?.chat_apps_url}
            showBrandLogos={localPrefs.brandLogos}
            error={ruiclawFeaturesError}
            requiresRestartPending={pendingRestartSections.runtime}
            onAction={handleRuiClawFeatureAction}
            onFeaturesUpdate={setRuiClawFeatures}
            onDismissStatus={() => {
        ...
        return (
          <div className="settings-stack">
            ...
            <RuntimeSettings
              ...
              capabilitiesLoading={ruiclawFeaturesLoading}
              capabilityAction={ruiclawFeatureAction}
              capabilityError={ruiclawFeaturesError}
      ...
      case "runs":
        return <RunInspectorSettings token={token} />;
      case "memory":
  ...
  return (
    ...
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden bg-settings-canvas lg:flex-row">
      ...
      <RuiClawFeatureInstallDialog
        feature={ruiclawFeatureConfirm?.feature ?? null}
        installOnly={ruiclawFeatureConfirm?.installOnly ?? false}
        installing={ruiclawFeatureAction === `${ruiclawFeatureConfirm?.installOnly ? "install" : "enable"}:${ruiclawFeatureConfirm?.feature.name ?? ""}`}
        onOpenChange={(open) => {
          if (!open) setRuiClawFeatureConfirm(null);
        }}
        onConfirm={(feature) => handleRuiClawFeatureAction("enable", feature.name, {
          confirmed: true,
          installOnly: ruiclawFeatureConfirm?.installOnly,
        })}
      />
