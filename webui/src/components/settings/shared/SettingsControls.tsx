...
import type { RuiClawFeatureInfo } from "@/lib/types";
...
export function RuiClawFeatureInstallDialog({
  feature,
  installOnly = false,
  installing,
  onOpenChange,
  ...
  feature: RuiClawFeatureInfo | null;
  installOnly?: boolean;
  installing: boolean;
  onOpenChange: (open: boolean) => void;
  onConfirm: (feature: RuiClawFeatureInfo) => void | Promise<void>;
}) {
      ...
      >
        <DialogHeader className="items-center space-y-0 text-center">
          <DialogTitle className="text-center text-[20px] font-semibold leading-tight tracking-[-0.02em] text-foreground">
            {tx("settings.ruiclawFeatures.installConfirmTitle", "Install support for {{name}}?", { name })}
          </DialogTitle>
          <DialogDescription className="mt-3 max-w-[20rem] text-center text-[14px] leading-6 text-muted-foreground">
            {installOnly
              ? tx(
                "settings.ruiclawFeatures.installOnlyConfirmDescription",
                "ruiclaw will install what {{name}} needs. You can connect it after installation.",
                { name },
              )
              : tx(
                "settings.ruiclawFeatures.installConfirmDescription",
                "ruiclaw will add what {{name}} needs, then turn it on. Continue?",
                { name },
              )}
          </DialogDescription>
        </DialogHeader>
        <DialogFooter className="mt-7 !grid grid-cols-1 gap-2 sm:grid-cols-2">
          ...
          >
            ...
            {installOnly
              ? tx("settings.ruiclawFeatures.installOnlyConfirmAction", "Install support")
              : tx("settings.ruiclawFeatures.installConfirmAction", "Install and enable")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
