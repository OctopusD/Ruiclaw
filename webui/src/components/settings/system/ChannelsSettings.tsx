...
import type { RuiClawFeaturesPayload } from "@/lib/types";
import { Dialog, DialogClose, DialogContent, DialogTitle } from "@/components/ui/dialog";

type ChannelFilter = "all" | "enabled";

export function ChannelsSettings({
  token, ruiclawFeatures, loading, actionKey, chatAppsDocsUrl, showBrandLogos,
  error, requiresRestartPending, onAction, onFeaturesUpdate, onDismissStatus,
  onRestart, isRestarting,
}: {
  token: string;
  ruiclawFeatures: RuiClawFeaturesPayload | null;
  loading: boolean;
  ...
  const channels = (ruiclawFeatures?.features ?? [])
  ...
  const restartRequired = requiresRestartPending || Boolean(ruiclawFeatures?.requires_restart);
  useLayoutEffect(() => {
    if (!ruiclawFeatures || filterInitializedRef.current) return;
    filterInitializedRef.current = true;
    setFilter(hasEnabledChannels ? "enabled" : "all");
  }, [hasEnabledChannels, ruiclawFeatures]);
