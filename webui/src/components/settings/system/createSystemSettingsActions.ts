...
import {
  cancelMcpOAuth,
  completeMcpOAuth,
  disableRuiClawFeature,
  enableRuiClawFeature,
  fetchRuiClawFeatures,
  fetchSettings,
  fetchMcpOAuthStatus,
  fetchMcpPresets,
  importMcpConfig,
...
import type { RuiClawClient } from "@/lib/ruiclaw-client";
import type {
...
interface SystemSettingsActionsOptions {
  ...
  featureCatalog: RuiClawFeatureInfo[];
  client: RuiClawClient;
  token: string;
  getToken: () => string;
  t: TFunction;
...
export function createSystemSettingsActions({
  ...
  const beginRuiClawFeatureAction = (key: string) => {
    if (ruiclawFeatureActionRef.current) return false;
    ruiclawFeatureActionRef.current = key;
    setRuiClawFeatureAction(key);
    return true;
  };

  const endRuiClawFeatureAction = (key: string) => {
    if (ruiclawFeatureActionRef.current !== key) return;
    ruiclawFeatureActionRef.current = null;
    setRuiClawFeatureAction(null);
  };
  ...
  const installCapabilities = async (names: string[]): Promise<boolean> => {
    ...
    if (!beginRuiClawFeatureAction(key)) return false;
    setRuiClawFeaturesError(null);
    try {
      let latest = ruiclawFeatures;
      for (const name of missing) {
        latest = await enableRuiClawFeature(client, name);
        if (latest.requires_restart) {
          setPendingRestartSections((prev) => ({ ...prev, runtime: true }));
        }
      }
      if (latest) setRuiClawFeatures(latest);
      return true;
    } catch (err) {
      setRuiClawFeaturesError((err as Error).message);
      return false;
    } finally {
      endRuiClawFeatureAction(key);
    }
  };
  ...
  const handleApiServiceAction = async (
    ...
    try {
      ...
      const refreshed = await fetchRuiClawFeatures(token);
      setRuiClawFeatures(refreshed);
  ...
  const handleRuiClawFeatureAction = async (
    ...
    if (
      ...
      setRuiClawFeaturesError(null);
      setRuiClawFeatureConfirm({ feature, installOnly: Boolean(options.installOnly) });
      return;
    }
    const key = `${action === "enable" && options.installOnly ? "install" : action}:${name}`;
    if (!beginRuiClawFeatureAction(key)) return;
    setRuiClawFeatureConfirm(null);
    setRuiClawFeaturesError(null);
    try {
      const payload = action === "enable"
        ? await enableRuiClawFeature(client, name, { installOnly: options.installOnly })
        : await disableRuiClawFeature(client, name);
      setRuiClawFeatures(payload);
      if (payload.requires_restart) {
        setPendingRestartSections((prev) => ({ ...prev, runtime: true }));
      }
    } catch (err) {
      console.error("ruiclaw feature action failed", {
        action,
        name,
        installOnly: Boolean(options.installOnly),
        error: err,
      });
      setRuiClawFeaturesError(err instanceof Error ? err.message : String(err));
    } finally {
      endRuiClawFeatureAction(key);
    }
  };
    ...
    if (flow.status === "authorized" && flow.hot_reload) {
      ...
      setMcpError(
        ...
        || t("settings.mcp.reloadFailed", {
          defaultValue: "Signed in, but ruiclaw could not connect the tools. Try restarting ruiclaw.",
        }),
      );
      return;
    }
