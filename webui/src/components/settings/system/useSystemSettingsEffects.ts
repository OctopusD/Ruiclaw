...
export function useSystemSettingsEffects({
  ...
  useEffect(() => {
    ...
    const refresh = async (showLoading = false): Promise<void> => {
      ...
      refreshing = true;
      if (showLoading) setRuiClawFeaturesLoading(true);
      try {
        const payload = await fetchRuiClawFeatures(getToken());
        if (!cancelled) {
        ...
        if (!cancelled && message !== "HTTP 404") setRuiClawFeaturesError(message);
      } finally {
        refreshing = false;
        if (!cancelled && showLoading) setRuiClawFeaturesLoading(false);
      }
    };
    void refresh(true);
