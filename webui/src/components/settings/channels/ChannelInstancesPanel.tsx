...
export type ChannelInstancesPanelCustomization = {
  toggleAriaLabel?: (instance: RuiClawChannelInstanceInfo) => string;
  configuredLabel?: string;
  needsSetupLabel?: string;
  renderInstanceSummary?: (instance: RuiClawChannelInstanceInfo) => ReactNode;
  renderInstanceAction?: (instance: RuiClawChannelInstanceInfo) => ReactNode;
  footer?: ReactNode;
};
...
export function ChannelInstancesPanel({
  ...
  instances?: RuiClawChannelInstanceInfo[];
  ...
  const toggleInstance = async (instance: RuiClawChannelInstanceInfo, checked: boolean) => {
    if (pendingToggle || savingFields) return;
    setPendingToggle({ id: instance.id, checked });
    setNotice(null);
    try {
      const payload = checked
        ? await enableRuiClawFeature(client, feature.name, { instanceId: instance.id })
        : await disableRuiClawFeature(client, feature.name, { instanceId: instance.id });
  ...
  const saveSelectedInstanceSettings = async () => {
    ...
    try {
      ...
      if (payload.ruiclaw_features) {
        onFeaturesUpdate(payload.ruiclaw_features);
      }
...
function channelInstanceDisplayName(instance: RuiClawChannelInstanceInfo): string {
...
function instanceToggleChecked(instance: RuiClawChannelInstanceInfo): boolean {
...
function ChannelInstanceStatusBadge({
  ...
  instance: RuiClawChannelInstanceInfo;
  configuredLabel?: string;
  needsSetupLabel?: string;
}) {
