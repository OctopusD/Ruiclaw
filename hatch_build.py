"""Hatch build hook that bundles the webui (Vite) into ruiclaw/web/dist.
...
- No-op when `webui/package.json` is absent (e.g. installing from an sdist that
  already contains a prebuilt `ruiclaw/web/dist/`).
- Skips when `RUICLAW_SKIP_WEBUI_BUILD=1` is set.
- Reuses `ruiclaw/web/dist/` only when it is already fresh, unless
  `RUICLAW_FORCE_WEBUI_BUILD=1` is set.
...
def _load_webui_build_module() -> ModuleType:
    from ruiclaw.webui import build as webui_build
...
class WebUIBuildHook(BuildHookInterface):
    ...
    def initialize(self, version: str, build_data: dict) -> None:  # noqa: D401
        ...
        dist_dir = root / "ruiclaw" / "web" / "dist"
        ...
        if os.environ.get("RUICLAW_SKIP_WEBUI_BUILD") == "1":
            self.app.display_info("[webui-build] skipped via RUICLAW_SKIP_WEBUI_BUILD=1")
            return

        if not package_json.is_file():
            self.app.display_info(
                "[webui-build] no webui/ source tree, assuming prebuilt ruiclaw/web/dist/"
            )
            return

        webui_build = _load_webui_build_module()
        status = webui_build.inspect_webui_bundle(source_dir=webui_dir, dist_dir=dist_dir)
        force = os.environ.get("RUICLAW_FORCE_WEBUI_BUILD") == "1"
        if not status.needs_build and not force:
            self.app.display_info(
                f"[webui-build] reusing existing build at {dist_dir} "
                "(already fresh; set RUICLAW_FORCE_WEBUI_BUILD=1 to rebuild)"
            )
            return
        ...
        try:
            ...
            raise RuntimeError(
                "[webui-build] "
                f"{exc}. Install `bun` or `npm`, or set RUICLAW_SKIP_WEBUI_BUILD=1 to bypass."
            ) from exc
