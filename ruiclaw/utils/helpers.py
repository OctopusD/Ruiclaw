...
_TOOL_RESULTS_DIR = ".ruiclaw/tool-results"
...
def sync_workspace_templates(workspace: Path, silent: bool = False) -> list[str]:
    ...
    try:
        tpl = pkg_files("ruiclaw") / "templates"
    except Exception:
        return []
    if not tpl.is_dir():
        return []
...
def load_bundled_template(template_name: str) -> str | None:
    """Read a bundled template file from the ruiclaw package."""
    from importlib.resources import files as pkg_files

    with suppress(Exception):
        tpl = pkg_files("ruiclaw") / "templates" / template_name
        if tpl.is_file():
