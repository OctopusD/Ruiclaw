...
from ruiclaw.webui.metadata import WEBUI_TURN_METADATA_KEY
from ruiclaw.webui.session_identity import webui_chat_id, webui_session_key
...
class RecoveryCoordinator:
    ...
    def _recovery_candidates(self) -> list[str]:
        ...
        try:
            ...
            from ruiclaw.webui.session_list_index import list_webui_sessions
    ...
    async def _recover_session(self, session: Session, chat_id: str) -> None:
        ...
        checkpoint_run_id = (
            checkpoint.get(RUN_ID_METADATA_KEY)
        ...
        await self.bus.publish_inbound(
            InboundMessage(
                ...
                metadata={
                    ...
                    **(
                        {PARENT_RUN_ID_METADATA_KEY: state["parent_run_id"]}
                        if isinstance(state.get("parent_run_id"), str)
    ...
    def _set_state(
        ...
        *,
        ...
        previous_value = cast(object, session.metadata.get(RECOVERY_METADATA_KEY))
        previous = (
        ...
        if parent_run_id is None:
            previous_parent = previous.get("parent_run_id")
            if isinstance(previous_parent, str) and previous_parent:
                parent_run_id = previous_parent
        state = {
            "status": status,
            "recovery_id": recovery_id,
    ...
    def _has_unfinished_webui_transcript(session_key: str) -> bool:
        ...
        """
        try:
            from ruiclaw.webui.transcript import has_unfinished_transcript_tail
