...
from ruiclaw.config.paths import get_legacy_sessions_dir, get_runtime_subdir
from ruiclaw.providers.base import ProviderConversationState
from ruiclaw.runtime_context import (
    RUNTIME_CONTEXT_HISTORY_META,
    public_history_message,
)
from ruiclaw.session.history_visibility import HIDDEN_HISTORY_META, is_hidden_history_message
...
from ruiclaw.utils.subagent_channel_display import scrub_subagent_announce_body
    ...
    def _read_workspace_id(cls, marker: Path) -> str:
        ...
        if not _WORKSPACE_ID_RE.fullmatch(value):
            raise RuntimeError(
                ...
                "restore its original 32-character identifier before starting ruiclaw"
            )
        return value
    ...
    def _find_workspace_namespace(cls, workspace: Path, root: Path) -> str | None:
        ...
        if len(matches) > 1:
            raise RuntimeError(
                ...
                "remove the stale namespace marker before starting ruiclaw"
            )
        ...
        try:
            with open(tmp_path, "x", encoding="utf-8") as f:
                metadata_line = {
                    ...
                    # Keep old ruiclaw releases able to read sessions written
...
class SessionManager:
    ...
    def _get_legacy_session_path(self, key: str) -> Path:
        """Legacy global session path (~/.ruiclaw/sessions/)."""
