...
def workspace_sandbox_status(
    *,
    ...
    return WorkspaceSandboxStatus(
        ...
        provider="none",
        provider_label=_provider_label("none"),
        summary="Workspace restriction uses ruiclaw application-level guards.",
    )
...
def _env_system_provider(environ: dict[str, str] | None = None) -> str | None:
    ...
    explicit_provider = env.get("RUICLAW_WORKSPACE_SANDBOX_PROVIDER")
    enforced = env.get("RUICLAW_WORKSPACE_SANDBOX_ENFORCED")
    compatibility = env.get("RUICLAW_SANDBOX_ENFORCED")
