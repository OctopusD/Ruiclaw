---
name: update-setup
description: "One-time setup wizard for the ruiclaw upgrade skill. Triggers: setup update, configure update, 设置更新, 初始化更新."
---

# Update Setup

Generate a personalized upgrade skill in RuiClaw's agent workspace.
...
Use `exec` to run `ruiclaw --version`. Tell the user the current version.

Then collect install clues with `exec`. These commands are best-effort; if one fails,
keep going and show the useful output:

```
command -v ruiclaw || true
python -m pip show ruiclaw-ai || true
pipx list | sed -n '/ruiclaw-ai/,+3p' || true
uv tool list | sed -n '/ruiclaw-ai/,+3p' || true
```
...
answer, stop and ask the user to rerun this setup when they know how ruiclaw was
installed.
...
`question: "Where is your ruiclaw source checkout? Enter an absolute path or a path relative to this workspace:"`.
...
| Method | Command |
|--------|---------|
| uv | `uv tool install "ruiclaw-ai[EXTRAS]" --force` |
| pipx | `pipx install --force "ruiclaw-ai[EXTRAS]"` |
| pip | `python -m pip install --upgrade "ruiclaw-ai[EXTRAS]"` |
...
---
name: update
description: "Upgrade ruiclaw to the latest version. Triggers: upgrade ruiclaw, update ruiclaw, 升级ruiclaw, 更新ruiclaw."
---
...
4. Use `exec` to verify: `ruiclaw --version`
5. Tell the user the new version. Say: "Run `/restart` to restart ruiclaw and apply the update. If `/restart` is unavailable in this channel, restart the ruiclaw process manually."
```

## Step 5: Confirm

Only after `write_file` succeeds, tell the user:
"Upgrade skill created. Say 'upgrade ruiclaw' when you want to update."
