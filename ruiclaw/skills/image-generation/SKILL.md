...
If the `generate_image` tool is not available in the current tool list, tell the user that image generation is not enabled for this ruiclaw instance.
...
The tool stores generated images as persistent artifacts under ruiclaw's media directory and returns structured metadata:
...
```text
generate_image(
  prompt="A minimal app icon for ruiclaw: friendly robot head, rounded square, soft blue and white palette, clean vector style, no text",
  aspect_ratio="1:1",
  image_size="1K"
)
```
...
generate_image(
  ...
  reference_images=["/home/user/.ruiclaw/media/generated/2026-05-08/img_ab12cd34ef56.png"],
  aspect_ratio="1:1",
  image_size="1K"
)
```
