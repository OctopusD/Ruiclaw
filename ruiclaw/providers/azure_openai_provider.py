...
helpers from :mod:`ruiclaw.providers.openai_responses`.
...
2. **Microsoft Entra ID (AAD)** — when ``api_key`` is empty the provider
   ...
   is an optional dependency installed via ``ruiclaw plugins enable azure``.
"""
    ...
    def __init__(self, scope: str = _AZURE_OPENAI_SCOPE) -> None:
        try:
            ...
            raise RuntimeError(
                ...
                "Run: ruiclaw plugins enable azure"
            ) from exc
