...
from ruiclaw.providers.base import LLMResponse, ProviderCallContext
...
TOKEN_APP_NAME = "ruiclaw"
USER_AGENT = "ruiclaw/0.1"
...
def login_github_copilot(
    ...
    client_id = _resolve("RUICLAW_GITHUB_COPILOT_CLIENT_ID", GITHUB_COPILOT_CLIENT_ID)
    device_code_url = _resolve("RUICLAW_GITHUB_DEVICE_CODE_URL", DEFAULT_GITHUB_DEVICE_CODE_URL)
    access_token_url = _resolve("RUICLAW_GITHUB_ACCESS_TOKEN_URL", DEFAULT_GITHUB_ACCESS_TOKEN_URL)
    user_url = _resolve("RUICLAW_GITHUB_USER_URL", DEFAULT_GITHUB_USER_URL)
...
class GitHubCopilotProvider(OpenAICompatProvider):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            api_key="no-key",
            api_base=_resolve("RUICLAW_COPILOT_BASE_URL", DEFAULT_COPILOT_BASE_URL),
    ...
    async def _get_copilot_access_token(self) -> str:
        ...
        async with self._copilot_token_lock:
            ...
            if not github_token or not github_token.access:
                raise RuntimeError(
                    "GitHub Copilot is not logged in. Run: ruiclaw provider login github-copilot"
                )
            ...
            async with httpx.AsyncClient(
                ...
                response = await client.get(
                    _resolve("RUICLAW_COPILOT_TOKEN_URL", DEFAULT_COPILOT_TOKEN_URL),
...
def get_github_copilot_model_catalog(
    ...
    cache_key = (
        ...
        f"{_resolve('RUICLAW_COPILOT_BASE_URL', DEFAULT_COPILOT_BASE_URL)}\0{proxy or ''}"
    )
