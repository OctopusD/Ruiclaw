    ...
    def __init__(
        ...
        *,
        ...
        self._server_thread = threading.Thread(
            ...
            name="ruiclaw-xai-grok-oauth-callback",
            daemon=True,
        )
    ...
    def wait(self, timeout_s: float) -> XAIToken:
        ...
        try:
            ...
            raise XAIOAuthError(
                ...
                "`ruiclaw provider login xai-grok` to try again."
            ) from exc
        return self._finish(callback)
...
def get_xai_oauth_token(
    *,
    ...
    if token is None:
        raise XAIOAuthError(
            "xAI is not signed in. Run `ruiclaw provider login xai-grok` first."
        )
    ...
    if not token.refresh:
        ...
        raise XAIOAuthError(
            ...
            "Run `ruiclaw provider login xai-grok` again."
        )
    ...
    class CallbackHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            if code and received_state and hmac.compare_digest(received_state, expected_state):
                ...
                message = "You can close this tab and return to ruiclaw."
            elif code:
                result = _CallbackResult(error="OAuth state mismatch")
                title = "Sign-in failed"
                message = "The sign-in response could not be verified. Return to ruiclaw and retry."
            else:
                result = _CallbackResult(error=error or "access denied")
                title = "Access denied"
                message = "Return to ruiclaw and try signing in again."
