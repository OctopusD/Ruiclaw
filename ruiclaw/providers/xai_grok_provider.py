...
from ruiclaw.providers.oauth_model_catalog import OAuthModelCatalog, OAuthModelCatalogSnapshot
...
from ruiclaw.providers.xai_oauth import (
    XAI_CLIENT_VERSION,
...
def _build_headers(token: str, model: str) -> dict[str, str]:
    ...
    return {
        ...
        "x-grok-client-identifier": "ruiclaw",
        ...
        "User-Agent": f"ruiclaw/{__version__} (python)",
...
def _friendly_error(status_code: int, response_body: str | None = None) -> str:
    if status_code == 401:
        message = "xAI rejected the login. Sign in again with `ruiclaw provider login xai-grok`."
    elif status_code == 403:
        message = "This xAI account or subscription cannot access the Grok subscription endpoint."
    elif status_code == 426:
        message = "xAI requires a newer Grok client version. Update ruiclaw and try again."
