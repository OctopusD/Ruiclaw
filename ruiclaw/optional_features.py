"""Optional ruiclaw feature discovery and enablement."""
...
def optional_dependency_groups_from_metadata() -> dict[str, list[str] | None]:
    ...
    try:
        extras = metadata("ruiclaw-ai").get_all("Provides-Extra") or []
        raw_requirements = requires("ruiclaw-ai") or []
    ...
    *,
    ...
    if name in _BUNDLED_FEATURE_ALIASES:
        payload = optional_features_payload(
            last_action={
                "ok": True,
                "message": f"Feature '{name}' is included with ruiclaw",
                "enabled": True,
            }
        )
