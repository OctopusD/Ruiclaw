...
- Use the built-in `cron` tool to create/list/remove jobs (do not call `ruiclaw cron` via `exec`).
...
`HEARTBEAT.md` is checked periodically by the protected heartbeat cron job that `ruiclaw gateway` registers when `gateway.heartbeat.enabled` is true. Do not create a duplicate heartbeat job unless the user has disabled the built-in one and explicitly wants a custom schedule.
