...
from ruiclaw.bus.events import INBOUND_META_USER_SHELL, OutboundMessage
from ruiclaw.command.router import CommandContext, CommandRouter, normalize_command_text
from ruiclaw.providers.base import LLMUsage
from ruiclaw.utils.helpers import build_status_content
from ruiclaw.utils.restart import set_restart_notice_to_env
from ruiclaw.utils.workspace_prompts import initialize_workspace_prompt

if TYPE_CHECKING:
    from ruiclaw.agent.loop import AgentLoop
    from ruiclaw.session.manager import Session
    from ruiclaw.utils.gitstore import CommitInfo
...
async def cmd_restart(ctx: CommandContext) -> OutboundMessage:
    ...
    async def _do_restart():
        ...
        argv = [sys.executable, "-m", "ruiclaw"] + sys.argv[1:]
...
async def cmd_status(ctx: CommandContext) -> OutboundMessage:
    ...
    with suppress(Exception):
        from ruiclaw.utils.searchusage import fetch_search_usage
    ...
    if args == "init":
        if not initialize_workspace_prompt(path, store.default_dream_prompt()):
            content = (
                ...
                "Edit that file, or delete/empty it to return to ruiclaw's default."
            )
        else:
            content = (
                f"Created Dream memory instructions at `{display_path}`.\n\n"
                "Edit that file to teach Dream how to organize memory. "
                "This fully replaces ruiclaw's default Dream guide for this workspace. "
                "Delete or empty it to return to ruiclaw's default."
            )
    elif args:
        ...
        content = (
            ...
            f"- Path: `{display_path}`\n"
            "- Delete or empty this file to return to ruiclaw's default."
        )
    else:
        content = (
            "Dream memory instructions: ruiclaw default\n\n"
...
async def cmd_evaluator_prompt(ctx: CommandContext) -> OutboundMessage:
    ...
    from ruiclaw.utils.evaluator import (
    ...
    if args == "init":
        ...
        content = (
            "Heartbeat evaluator prompt: ruiclaw default\n\n"
            f"- Editable file: `{display_path}`\n"
...
async def cmd_goal(ctx: CommandContext) -> OutboundMessage | None:
    ...
    from ruiclaw.agent.goal_permission import goal_mutation_permission
...
async def cmd_pairing(ctx: CommandContext) -> OutboundMessage:
    """List, approve, deny or revoke pairing requests."""
    from ruiclaw.pairing import PAIRING_COMMAND_META_KEY, handle_pairing_command
...
async def cmd_trigger(ctx: CommandContext) -> OutboundMessage:
    ...
    command = f'ruiclaw trigger {trigger.id} "message"'
    return OutboundMessage(
        channel=ctx.msg.channel,
        chat_id=ctx.msg.chat_id,
        content=(
...
async def cmd_user_shell(ctx: CommandContext) -> OutboundMessage:
    """Run a trusted local ``!command`` through ruiclaw's exec policy."""
    metadata = dict(ctx.msg.metadata or {})
    if (
