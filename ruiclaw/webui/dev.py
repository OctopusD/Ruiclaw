...
from ruiclaw.webui.build import default_webui_source_dir, pick_webui_build_runner
...
def start_webui_dev_server(
    *,
    ...
    if not (resolved_source / "package.json").is_file():
        raise WebUIDevError(
            "`ruiclaw webui --dev` requires a source checkout containing webui/package.json"
        )
    ...
    child_env["RUICLAW_API_URL"] = target_url
