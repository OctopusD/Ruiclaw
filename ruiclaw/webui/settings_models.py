...
from ruiclaw.config.loader import resolve_config_env_vars
from ruiclaw.config.schema import Config, FallbackCandidate, ModelPresetConfig, ProviderConfig
from ruiclaw.providers.image_generation import get_image_gen_provider
from ruiclaw.providers.oauth_guidance import OAUTH_CLI_KIT_MISSING_MESSAGE
from ruiclaw.providers.oauth_model_catalog import (
    get_oauth_model_catalog,
    invalidate_oauth_model_catalog,
)
from ruiclaw.providers.registry import PROVIDERS, create_dynamic_spec, find_by_name
...
if TYPE_CHECKING:
    from ruiclaw.webui.settings_services import (
...
def oauth_provider_status(spec: Any) -> dict[str, Any]:
    ...
    if spec.name == "github_copilot":
        try:
            from ruiclaw.providers.github_copilot_provider import get_github_copilot_login_status
        except Exception:
            return {
                "configured": False,
                "account": None,
    ...
    if spec.name == "xai_grok":
        try:
            from ruiclaw.providers.xai_oauth import get_xai_oauth_login_status
        except Exception:
            return {
                "configured": False,
                "account": None,
...
def login_oauth_provider(
    ...
    *,
    ...
    if spec.name == "openai_codex":
        try:
            from ruiclaw.providers.openai_codex_oauth import start_openai_codex_oauth_login
        except ImportError:
    ...
    if spec.name == "github_copilot":
        try:
            from ruiclaw.providers.github_copilot_provider import (
    ...
    if spec.name == "xai_grok":
        from ruiclaw.providers.xai_oauth import start_xai_oauth_login
...
def complete_oauth_provider(
    ...
    *,
    ...
    try:
        if spec.name == "openai_codex":
            from ruiclaw.providers.openai_codex_oauth import (
            ...
            from ruiclaw.providers.xai_oauth import complete_xai_oauth_login
...
def logout_oauth_provider(
    ...
    *,
    ...
    if spec.name == "openai_codex":
        ...
        from ruiclaw.providers.xai_oauth import logout_xai_oauth
