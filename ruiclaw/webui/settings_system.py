...
from ruiclaw.channels._setup import channel_setup_spec
from ruiclaw.channels.connect import ChannelConnectError
...
from ruiclaw.llm_usage import llm_usage_payload
from ruiclaw.optional_features import OptionalFeatureError, with_channel_runtime_status
from ruiclaw.security.workspace_access import workspace_sandbox_status
from ruiclaw.webui.settings_capabilities import network_safety_payload
...
class SystemSettingsOperations:
    ...
    ruiclaw_features_payload: SettingsOperation
    ruiclaw_features_action: SettingsOperation
    ruiclaw_feature_instance_target: SettingsOperation
...
_DOCS_LATEST_URL = "https://ruiclaw.wiki/docs/latest"
_SKIP_FIELD = object()
...
def docs_payload(version: str) -> dict[str, Any]:
    ...
    base_url = f"https://ruiclaw.wiki/docs/{selected_version}"
    return {
...
class SystemSettingsHandler:
    ...
    async def _features(
        ...
        try:
            payload = await asyncio.to_thread(
                operations.ruiclaw_features_payload,
                config_path=self.settings.config.path,
            )
        except Exception:
            self.logger.exception("failed to load ruiclaw features")
            return SettingsRouteResult.failure(500, "failed to load ruiclaw features")
        return SettingsRouteResult.success(
            self._with_channel_runtime_status(payload, operations)
        )

    def _ruiclaw_features_payload(
        self,
        operations: SystemSettingsOperations,
    ) -> dict[str, Any]:
        return operations.ruiclaw_features_payload(config_path=self.settings.config.path)
    ...
    def _ruiclaw_features_action(
        ...
        *,
        ...
        return self.settings.mutate(
            operations.ruiclaw_features_action,
            action,
            query,
    ...
    async def _apply_feature_runtime_change(
        ...
        if not name:
            return payload
        try:
            instance_id = operations.ruiclaw_feature_instance_target(query)
    ...
    async def _channel_configure(
        ...
        if not enable:
            features = await asyncio.to_thread(
                self._ruiclaw_features_payload,
                operations,
            )
            payload["ruiclaw_features"] = self._with_channel_runtime_status(
                features,
                operations,
            )
            return SettingsRouteResult.success(
                ...
                decorate_restart=True,
                restart_section="runtime",
                restart_payload_key="ruiclaw_features",
            )
    ...
    async def _with_channel_connect_success(
        ...
        try:
            ...
            features = self.feature_runtime_fallback(
                self._ruiclaw_features_payload(operations),
                message=(
        ...
        updated = dict(payload)
        updated["ruiclaw_features"] = self._with_channel_runtime_status(
            features,
            operations,
        )
        return updated
