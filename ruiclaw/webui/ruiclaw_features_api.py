"""RuiClaw optional feature helpers for WebUI Settings."""
...
from ruiclaw.webui.http_utils import query_first

QueryParams = dict[str, list[str]]


def ruiclaw_features_payload(*, config_path: Path | None = None) -> dict[str, Any]:
...
def ruiclaw_feature_instance_target(query: QueryParams) -> str | None:
...
def ruiclaw_features_action(
    ...
    *,
    ...
    instance_id = ruiclaw_feature_instance_target(query)
    ...
    if action == "disable":
        ...
        if plugin is not None and "always_enabled" in plugin.capabilities:
            raise OptionalFeatureError(
                ...
                f"Use `ruiclaw plugins disable {name}` from a terminal if you need to disable it.",
                status=400,
            )
