...
from ruiclaw.command.builtin import builtin_command_palette
from ruiclaw.cron.session_turns import is_bound_cron_job
from ruiclaw.cron.types import CronJob, CronSchedule
from ruiclaw.observability.run_inspector import RunInspector
from ruiclaw.security.workspace_access import WorkspaceScope
from ruiclaw.session.manager import SessionManager
from ruiclaw.session.recovery import RecoveryActionError
from ruiclaw.session.session_handles import (
    SessionHandleResolver,
)
from ruiclaw.triggers.local_types import LocalTrigger
from ruiclaw.webui.file_preview import (
    WebUIFilePreviewError,
    file_preview_availability_payload,
    file_preview_payload,
)
from ruiclaw.webui.gateway_tokens import GatewayTokenStore, token_response_payload
...
from ruiclaw.webui.http_utils import is_loopback_host as _is_loopback_host
from ruiclaw.webui.http_utils import (
...
from ruiclaw.webui.ingress_policy import WebUIIngressPolicy
from ruiclaw.webui.media_gateway import WebUIMediaGateway
from ruiclaw.webui.native_folder_picker import (
    NativeFolderPickerError,
    native_folder_picker_available,
    pick_native_folder,
)
from ruiclaw.webui.session_automations import (
    all_automations_payload,
    serialize_automation_jobs,
    session_automation_jobs,
    session_automations_payload,
)
from ruiclaw.webui.session_context import session_context_payload
from ruiclaw.webui.session_identity import is_webui_session_key
from ruiclaw.webui.session_list_index import (
    WEBUI_SESSION_INDEX_INTERNAL_FIELDS,
    indexed_workspace_scope,
    list_webui_sessions,
)
from ruiclaw.webui.sidebar_state import (
    read_webui_sidebar_state,
    write_webui_sidebar_state,
)
from ruiclaw.webui.skills_api import (
...
from ruiclaw.webui.skills_marketplace import (
...
from ruiclaw.webui.thread_disk import delete_webui_thread
from ruiclaw.webui.transcript import build_webui_thread_response
from ruiclaw.webui.workspaces import WebUIWorkspaceController

_SLOW_WEBUI_HTTP_LOG_MS = 1_000
_WEBUI_MUTATION_PAYLOAD_ATTR = "_ruiclaw_webui_mutation_payload"
_WEBUI_MUTATION_REQUEST_ATTR = "_ruiclaw_webui_mutation_request"
...
_WEBUI_MUTATION_PATHS = {
    ...
    "settings.feature.enable": "/api/settings/ruiclaw-features/enable",
    "settings.feature.disable": "/api/settings/ruiclaw-features/disable",
...
if TYPE_CHECKING:
    ...
    from ruiclaw.channels.websocket.runtime import WebSocketConfig
...
def _default_model_name_from_config(config_path: Path | None = None) -> str | None:
    try:
        from ruiclaw.config.loader import load_config
...
class GatewayHTTPHandler:
    ...
    def __init__(
        ...
        *,
        ...
        from ruiclaw.webui.settings_api import runtime_capabilities as _rc
    ...
    def check_api_token(self, request: WsRequest) -> bool:
        if getattr(request, "_ruiclaw_trusted_proxy_authenticated", False):
            return True
    ...
    async def dispatch(self, connection: Any, request: WsRequest) -> Any | None:
        ...
        setattr(
            request,
            "_ruiclaw_trusted_proxy_authenticated",
    ...
    async def dispatch_webui_mutation(
        ...
        setattr(request, "_ruiclaw_trusted_proxy_authenticated", True)
    ...
    def _mcp_oauth_redirect_uri(self, request: WsRequest) -> str:
        ...
        from ruiclaw.agent.tools.mcp_oauth import MCP_OAUTH_CALLBACK_PATH
    ...
    def _sessions_list_payload(self) -> dict[str, Any]:
        ...
        from ruiclaw.session.webui_turns import websocket_turn_wall_started_at
    ...
    def _handle_webui_thread_get(self, request: WsRequest, key: str) -> Response:
        ...
        from ruiclaw.session.webui_turns import (
            websocket_turn_id,
    ...
    async def _dispatch_misc_routes(
        ...
        if got == "/api/webui/runs":
            return self._handle_webui_runs(request)
        m = re.fullmatch(r"/api/webui/runs/([^/]+)", got)
        if m:
            return self._handle_webui_run_detail(request, m.group(1))
        return None
    ...
    def _handle_webui_runs(self, request: WsRequest) -> Response:
        if not self.check_api_token(request):
            return _http_error(401, "Unauthorized")
        raw_limit = _query_first(_parse_query(request.path), "limit") or "50"
        try:
            limit = int(raw_limit)
        except ValueError:
            return _http_error(400, "invalid run limit")
        rows = RunInspector(self.skills_workspace_path).list_runs(limit=limit)
        return _http_json_response({"runs": rows})

    def _handle_webui_run_detail(self, request: WsRequest, run_id: str) -> Response:
        if not self.check_api_token(request):
            return _http_error(401, "Unauthorized")
        detail = RunInspector(self.skills_workspace_path).get_run(unquote(run_id))
        if detail is None:
