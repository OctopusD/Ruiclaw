...
from ruiclaw.security.workspace_policy import WorkspaceBoundaryError, require_path_within
...
def delete_webui_skill(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix=".ruiclaw-delete-", dir=skills_root) as staging:
...
def _ruiclaw_metadata(metadata: dict[str, Any] | None) -> dict[str, Any]:
    if metadata is None:
        return {}
    ...
    payload = metadata_payload.get("ruiclaw", metadata_payload.get("openclaw", {}))
...
def _install_options(metadata: dict[str, Any] | None) -> list[dict[str, str]]:
    ...
    raw_install = _ruiclaw_metadata(metadata).get("install")
