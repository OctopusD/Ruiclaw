...
from ruiclaw.config.paths import get_media_dir, get_runtime_subdir
...
BASE_INFO: dict[str, str] = {
    ...
    "bot_agent": f"ruiclaw/{__version__} (python)",
}
...
class WeixinConfig(Base):
    ...
    state_dir: str = ""  # Default: ~/.ruiclaw/weixin/
...
class WeixinChannel(BaseChannel):
    ...
    def _persist_connect_credentials(self, *, token: str, base_url: str) -> None:
        ...
        """
        from ruiclaw.config.loader import get_config_path, load_config, save_config
    ...
    async def start(self) -> None:
        ...
        if self.config.token:
            ...
            if not await self._qr_login():
                self.logger.error("login failed. Run 'ruiclaw channels login weixin' to authenticate.")
                self._running = False
                return
    ...
    def _assert_session_active(self) -> None:
        if self._auth_required:
            raise WeixinAuthError(
                ...
                errmsg="bot token is stale; run 'ruiclaw channels login weixin --force'",
            )
        ...
        try:
            ...
            raise WeixinAuthError(
                ...
                errmsg=(
                    ...
                    "run 'ruiclaw channels login weixin --force'"
                ),
            ) from None
    ...
    async def _send_text(
        ...
        *,
        ...
        client_id = client_id or f"ruiclaw-{uuid.uuid4().hex[:12]}"
...
def _encrypt_aes_ecb(data: bytes, aes_key_b64: str) -> bytes:
    ...
    try:
        ...
        logger.warning(
            "Cannot encrypt media. Run `ruiclaw plugins enable weixin` "
...
def _decrypt_aes_ecb(data: bytes, aes_key_b64: str) -> bytes:
    ...
    if decrypted is None:
        try:
            ...
            logger.warning(
                "Cannot decrypt media. Run `ruiclaw plugins enable weixin` "
                "to install WeChat support."
            )
            return data
