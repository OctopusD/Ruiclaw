...
from ruiclaw.channels.weixin import runtime as weixin_mod
from ruiclaw.channels.weixin.runtime import (
    ITEM_IMAGE,
    ITEM_TEXT,
    MESSAGE_TYPE_BOT,
...
def _make_channel() -> tuple[WeixinChannel, MessageBus]:
    ...
    channel = WeixinChannel(
        WeixinConfig(
            enabled=True,
            allow_from=["*"],
            state_dir=tempfile.mkdtemp(prefix="ruiclaw-weixin-test-"),
        ),
        bus,
    )
    return channel, bus
...
async def test_manager_surfaces_actionable_weixin_auth_error_without_traceback(
    ...
    from ruiclaw.channels import manager as manager_mod
...
def test_missing_aes_dependency_recommends_weixin_plugin(monkeypatch) -> None:
    ...
    assert warnings == [
        "Cannot encrypt media. Run `ruiclaw plugins enable weixin` to install WeChat support.",
        "Cannot decrypt media. Run `ruiclaw plugins enable weixin` to install WeChat support.",
    ]
