...
from ruiclaw.channels.weixin.manifest import SETUP_SPEC
...
async def test_channel_manager_does_not_retry_permanent_weixin_error(monkeypatch) -> None:
    ...
    sleep = AsyncMock()
    monkeypatch.setattr("ruiclaw.channels.manager.asyncio.sleep", sleep)
...
async def test_weixin_http_clients_ignore_system_proxy(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("ruiclaw.channels.weixin.runtime.httpx.AsyncClient", make_client)
