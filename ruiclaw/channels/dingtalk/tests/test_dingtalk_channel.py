...
try:
    import ruiclaw.channels.dingtalk.runtime as dingtalk_module
...
from ruiclaw.channels.dingtalk.runtime import (
    DingTalkChannel,
    DingTalkConfig,
    RuiClawDingTalkHandler,
)
...
async def test_group_send_uses_group_messages_api() -> None:
    ...
    ok = await channel._send_batch_message(
        ...
        "group:conv123",
        "sampleMarkdown",
        {"text": "hello", "title": "RuiClaw Reply"},
    )
...
async def test_handler_uses_voice_recognition_text_when_text_is_empty(monkeypatch) -> None:
    ...
    handler = RuiClawDingTalkHandler(channel)
...
async def test_handler_processes_file_message(monkeypatch) -> None:
    ...
    async def fake_download(download_code, filename, sender_id):
        return f"/tmp/ruiclaw_dingtalk/{sender_id}/{filename}"
    ...
    assert "/tmp/ruiclaw_dingtalk/user1/report.xlsx" in msg.content
...
async def test_handler_does_not_spawn_message_task_after_stop_during_download(
    ...
    async def delayed_download(*_args):
        ...
        return "/tmp/ruiclaw_dingtalk/user1/report.xlsx"
...
async def test_download_dingtalk_file(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "ruiclaw.config.paths.get_media_dir",
