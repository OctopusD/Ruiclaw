...
from ruiclaw.security.network import validate_resolved_url, validate_url_target
...
class RuiClawDingTalkHandler(_CallbackHandlerBase):
    """
    Standard DingTalk Stream SDK Callback Handler.
    Parses incoming messages and forwards them to the RuiClaw channel.
    """
    ...
    async def process(self, message: Any) -> tuple[Any, str]:
        ...
        try:
            ...
            # Forward to RuiClaw via _on_message (non-blocking).
...
class DingTalkChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        try:
            if not DINGTALK_AVAILABLE:
                self.logger.error(
                    "Stream SDK not installed. Run: ruiclaw plugins enable dingtalk"
                )
                return
            ...
            handler = RuiClawDingTalkHandler(self)
    ...
    async def _send_markdown_text(self, token: str, chat_id: str, content: str) -> bool:
        return await self._send_batch_message(
            ...
            chat_id,
            "sampleMarkdown",
            {"text": content, "title": "RuiClaw Reply"},
        )
    ...
    async def _on_message(
        ...
        """Handle incoming message (called by RuiClawDingTalkHandler).
