...
from ruiclaw.channels.wecom.runtime import (
...
async def test_download_and_save_success() -> None:
    ...
    with patch("ruiclaw.channels.wecom.runtime.get_media_dir", return_value=Path(tempfile.gettempdir())):
...
async def test_download_and_save_sanitizes_sdk_fallback(tmp_path: Path) -> None:
    ...
    with patch("ruiclaw.channels.wecom.runtime.get_media_dir", return_value=tmp_path):
...
async def test_process_image_message() -> None:
    ...
    try:
        with patch("ruiclaw.channels.wecom.runtime.get_media_dir", return_value=Path(os.path.dirname(saved))):
            frame = _FakeFrame(body={
                "msgid": "msg_img_1",
                "chatid": "chat1",
                "from": {"userid": "user1"},
