...
from ruiclaw.channels.mattermost.manifest import SETUP_SPEC
from ruiclaw.channels.mattermost.runtime import (
    MATTERMOST_MAX_MESSAGE_LEN,
    MattermostChannel,
    MattermostConfig,
)
from ruiclaw.pairing import PAIRING_CODE_META_KEY
...
async def test_start_identifies_bot():
    ...
    assert channel._self_username == "ruiclaw"
...
async def test_posted_event_routes_to_handle_message():
    ...
    channel._self_username = "ruiclaw"
            ...
            with patch.object(channel, "_should_respond_in_channel", return_value=True):
                ws_msg = {
                    ...
                    "data": {
                        ...
                        "post": json.dumps({
                            "id": "p1", "user_id": "u1",
                            "channel_id": "c1", "message": "@ruiclaw hello there", "root_id": "",
                        }),
                    },
                    "broadcast": {},
                }
...
async def test_group_policy_mention():
    ...
    assert channel._should_respond_in_channel("@ruiclaw hello", "c1") is True
...
async def test_group_policy_in_thread_defaults_to_group_policy():
    ...
    assert channel._should_respond_in_channel("@ruiclaw hello", "c1", in_thread=False) is True
    ...
    assert channel._should_respond_in_channel("@ruiclaw hello", "c1", in_thread=True) is True
...
async def test_send_with_file_upload():
    ...
    with patch("ruiclaw.channels.mattermost.runtime.Path.exists", return_value=True):
        with patch("ruiclaw.channels.mattermost.runtime.Path.read_bytes", return_value=b"data"):
            msg = OutboundMessage(
                channel="mattermost",
                chat_id="chan_1",
                content="with file",
        ...
        with patch.object(channel, "_is_allowed", AsyncMock(return_value=True)):
            ws_msg = {
                ...
                "data": {
                    ...
                    "post": json.dumps({
                        "id": "post_1", "user_id": "u1",
                        "channel_id": "c1", "message": "@ruiclaw start thread",
                        "root_id": "",
                    }),
                },
                "broadcast": {},
...
async def test_dm_allowlist_accepts_pairing_approval():
    ...
    with patch("ruiclaw.channels.mattermost.runtime.is_approved", return_value=True):
...
def test_is_mentioned_exact():
    channel, fake = _make_channel({"groupPolicy": "mention"})
    channel._self_username = "ruiclaw"
    assert channel._is_mentioned("hello @ruiclaw how are you") is True
    assert channel._is_mentioned("hello @ruiclawty") is False
    assert channel._is_mentioned("@ruiclaw_extra") is False
...
def test_is_mentioned_no_username():
    ...
    assert channel._is_mentioned("hello @ruiclaw") is False
...
def test_message_splitting():
    from ruiclaw.utils.helpers import split_message
    short = "short message"
