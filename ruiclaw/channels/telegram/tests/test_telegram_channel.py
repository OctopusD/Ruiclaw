...
from ruiclaw.channels.telegram.runtime import (
...
from ruiclaw.events import ContextCompactionEvent
...
class _FakeBot:
    ...
    async def get_me(self):
        self.get_me_calls += 1
        return SimpleNamespace(id=999, username="ruiclaw_test")
...
async def test_start_creates_separate_pools_with_proxy(monkeypatch) -> None:
    ...
    monkeypatch.setattr("ruiclaw.channels.telegram.runtime.HTTPXRequest", _FakeHTTPXRequest)
    monkeypatch.setattr(
        "ruiclaw.channels.telegram.runtime.Application",
...
async def test_stalled_polling_triggers_pool_rebuild(monkeypatch) -> None:
    ...
    monkeypatch.setattr("ruiclaw.channels.telegram.runtime.POLL_STALE_SECONDS", -1.0)
    monkeypatch.setattr("ruiclaw.channels.telegram.runtime.POLL_WATCH_INTERVAL", 0.0)
...
async def test_startup_failure_retries_with_backoff(monkeypatch) -> None:
    ...
    monkeypatch.setattr("ruiclaw.channels.telegram.runtime.POLL_WATCH_INTERVAL", 0.0)
    monkeypatch.setattr("ruiclaw.channels.telegram.runtime.RESTART_BACKOFF_INITIAL_SECONDS", 0.0)
...
async def test_send_during_rebuild_fails_instead_of_dropping(monkeypatch) -> None:
    ...
    monkeypatch.setattr("ruiclaw.channels.telegram.runtime.APP_RESTART_SEND_WAIT_SECONDS", 0.0)
    config = TelegramConfig(enabled=True, token="123:abc", allow_from=["*"])
...
async def test_send_waits_for_rebuild_to_finish(monkeypatch) -> None:
    ...
    monkeypatch.setattr("ruiclaw.channels.telegram.runtime.APP_RESTART_SEND_WAIT_SECONDS", 5.0)
    config = TelegramConfig(enabled=True, token="123:abc", allow_from=["*"])
    channel = TelegramChannel(config, MessageBus())
...
async def test_liveness_tracked_request_stamps_on_round_trip() -> None:
    from ruiclaw.channels.telegram.runtime import _LivenessTrackedRequest
...
async def test_send_text_retries_on_timeout() -> None:
    ...
    import ruiclaw.channels.telegram.runtime as tg_mod
...
async def test_rich_stream_overflow_failure_does_not_drop_same_text_next_delta(
    ...
    monkeypatch.setattr("ruiclaw.channels.telegram.runtime.TELEGRAM_RICH_MAX_LEN", 4)
    channel = TelegramChannel(
        TelegramConfig(enabled=True, token="123:abc", allow_from=["*"], rich_messages=True),
        MessageBus(),
    )
...
async def test_send_delta_stream_end_does_not_fallback_on_network_timeout(
    ...
    _install_ready_app(channel)
    monkeypatch.setattr("ruiclaw.channels.telegram.runtime._SEND_RETRY_BASE_DELAY", 0)
...
async def test_send_delta_stream_end_html_expansion_does_not_overflow() -> None:
    ...
    """
    from ruiclaw.channels.telegram.runtime import _markdown_to_telegram_html
...
async def test_send_delta_incremental_html_expansion_does_not_overflow() -> None:
    ...
    from ruiclaw.channels.telegram.runtime import TELEGRAM_HTML_MAX_LEN
...
async def test_send_remote_media_url_after_security_validation(monkeypatch) -> None:
    ...
    _install_ready_app(channel)
    monkeypatch.setattr("ruiclaw.channels.telegram.runtime.validate_url_target", lambda url: (True, ""))
...
async def test_send_blocks_unsafe_remote_media_url(monkeypatch) -> None:
    ...
    _install_ready_app(channel)
    monkeypatch.setattr(
        "ruiclaw.channels.telegram.runtime.validate_url_target",
...
async def test_group_policy_mention_accepts_text_mention_and_caches_bot_identity() -> None:
    ...
    await channel._on_message(_make_telegram_update(text="@ruiclaw_test hi", entities=[mention]), None)
    await channel._on_message(_make_telegram_update(text="@ruiclaw_test again", entities=[mention]), None)
...
async def test_group_policy_mention_accepts_caption_mention() -> None:
    ...
    await channel._on_message(
        _make_telegram_update(caption="@ruiclaw_test photo", caption_entities=[mention]),
        None,
    )

    assert len(handled) == 1
    assert handled[0]["content"] == "@ruiclaw_test photo"
...
async def test_download_message_media_returns_path_when_download_succeeds(
    ...
    monkeypatch.setattr(
        "ruiclaw.channels.telegram.runtime.get_media_dir",
...
async def test_forward_command_pairs_unauthorized_private_user(monkeypatch) -> None:
    ...
    _install_ready_app(channel)
    monkeypatch.setattr(
        "ruiclaw.channels.base.generate_code", lambda _ch, _sid: "ABCD-EFGH"
    )
...
async def test_forward_command_preserves_dream_log_args_and_strips_bot_suffix() -> None:
    ...
    channel._handle_message = capture_handle
    update = _make_telegram_update(text="/dream-log@ruiclaw_test deadbeef", reply_to_message=None)
...
async def test_forward_command_normalizes_telegram_safe_dream_aliases() -> None:
    ...
    channel._handle_message = capture_handle
    update = _make_telegram_update(text="/dream_restore@ruiclaw_test deadbeef", reply_to_message=None)
    ...
    handled.clear()
    update = _make_telegram_update(text="/dream_prompt@ruiclaw_test init", reply_to_message=None)
...
def test_telegram_bus_slash_command_regex_matches_agent_loop_commands() -> None:
    ...
    assert pat.fullmatch("/skill@ruiclaw_bot")
    assert pat.fullmatch("/new@ruiclaw_bot")
    assert pat.fullmatch("/goal@ruiclaw_bot refine objective")
    assert pat.fullmatch("/trigger@ruiclaw_bot CI summary")
    assert pat.fullmatch("/compact@ruiclaw_bot")
    assert pat.fullmatch("/dream_log deadbeef")
    assert pat.fullmatch("/dream_restore deadbeef")
    assert pat.fullmatch("/dream_prompt init")
    assert pat.fullmatch("/evaluator_prompt@ruiclaw_bot init")
...
async def test_telegram_command_handlers_preserve_core_names(monkeypatch, command, underscore):
    ...
    try:
        ...
        text = f"/{spelling}@ruiclaw_test argument"
...
async def test_call_with_retry_accepts_timedelta_retry_after(
    ...
    monkeypatch.setattr(
        "ruiclaw.channels.telegram.runtime.asyncio.sleep",
        sleep,
    )
...
def test_strip_md_block_removes_inline_formatting() -> None:
    from ruiclaw.channels.telegram.runtime import _strip_md_block
...
def test_markdown_to_html_code_block_special_chars_language() -> None:
    from ruiclaw.channels.telegram.runtime import _markdown_to_telegram_html, _strip_md_block
