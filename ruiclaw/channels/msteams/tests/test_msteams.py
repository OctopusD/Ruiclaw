...
try:
    import ruiclaw.channels.msteams.runtime as msteams_module
...
if not MSTEAMS_AVAILABLE:
    pytest.skip(
        ...
        "Run: ruiclaw plugins enable msteams",
...
from ruiclaw.channels.msteams.runtime import ConversationRef, MSTeamsChannel
...
def make_channel(tmp_path, monkeypatch):
    monkeypatch.setattr("ruiclaw.channels.msteams.runtime.get_workspace_path", lambda: tmp_path)
...
def test_strip_possible_bot_mention_removes_generic_at_tags(make_channel):
    ...
    assert ch._strip_possible_bot_mention("<at>RuiClaw</at> hello") == "hello"
...
def test_sanitize_inbound_text_keeps_normal_inline_message(make_channel):
    ...
    activity = {
        "text": "<at>RuiClaw</at> normal inline message",
        "channelData": {},
    }
...
async def test_start_logs_install_hint_when_pyjwt_missing(make_channel, monkeypatch):
    ...
    assert errors == ["PyJWT not installed. Run: ruiclaw plugins enable msteams"]
