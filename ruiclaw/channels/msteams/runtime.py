...
class MSTeamsChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if not MSTEAMS_AVAILABLE:
            self.logger.error("PyJWT not installed. Run: ruiclaw plugins enable msteams")
            return
    ...
    async def _validate_inbound_auth(self, auth_header: str, activity: dict[str, Any]) -> None:
        ...
        if not MSTEAMS_AVAILABLE:
            raise RuntimeError("PyJWT not installed. Run: ruiclaw plugins enable msteams")
