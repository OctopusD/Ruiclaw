...
def has_channel_package(name: str) -> bool:
    ...
    if _CHANNEL_PACKAGE_NAME.fullmatch(name) is None:
        return False
    return files("ruiclaw.channels").joinpath(name, "manifest.py").is_file()
...
def load_channel_package(name: str) -> ChannelPlugin | None:
    ...
    module_name = f"ruiclaw.channels.{name}.manifest"
    module = importlib.import_module(module_name)
    ...
    package_name = f"ruiclaw.channels.{name}"
    package_root = files("ruiclaw.channels").joinpath(name)
    ...
    if plugin.webui is not None:
        webui_entry = files("ruiclaw.channels").joinpath(name, *plugin.webui.split("/"))
