...
import ruiclaw.channels.matrix.runtime as matrix_module
...
from ruiclaw.channels.matrix.runtime import (
    MATRIX_HTML_FORMAT,
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("ruiclaw.channels.matrix.runtime.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "ruiclaw.channels.matrix.runtime.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("ruiclaw.channels.matrix.runtime.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "ruiclaw.channels.matrix.runtime.asyncio.create_task", _fake_create_task
    )
...
async def test_fetch_media_rejects_missing_declared_size(monkeypatch, tmp_path) -> None:
    ...
    channel.client = client
    monkeypatch.setattr("ruiclaw.channels.matrix.runtime.get_media_dir", lambda _name: tmp_path)
