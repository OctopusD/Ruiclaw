...
from ruiclaw.bus.outbound_events import ContextCompactionEvent, ProgressEvent
from ruiclaw.bus.queue import MessageBus
from ruiclaw.channels.discord.runtime import (
    MAX_MESSAGE_LEN,
    DiscordBotClient,
    DiscordChannel,
    DiscordConfig,
)
from ruiclaw.command.builtin import build_help_text
...
async def test_start_returns_when_discord_dependency_missing(monkeypatch) -> None:
    ...
    monkeypatch.setattr("ruiclaw.channels.discord.runtime.DISCORD_AVAILABLE", False)
...
async def test_start_handles_client_construction_failure(monkeypatch) -> None:
    ...
    monkeypatch.setattr("ruiclaw.channels.discord.runtime.DiscordBotClient", _boom)
...
async def test_start_handles_client_start_failure(monkeypatch) -> None:
    ...
    monkeypatch.setattr("ruiclaw.channels.discord.runtime.DiscordBotClient", _FakeDiscordClient)
...
async def test_on_message_unauthorized_dm_sends_pairing_code(monkeypatch) -> None:
    ...
    channel._client = client
    channel._running = True
    monkeypatch.setattr("ruiclaw.channels.base.is_approved", lambda _ch, _sid: False)
    monkeypatch.setattr(
...
async def test_on_message_downloads_attachments(tmp_path, monkeypatch) -> None:
    ...
    channel._handle_message = capture_handle  # type: ignore[method-assign]
    monkeypatch.setattr("ruiclaw.channels.discord.runtime.get_media_dir", lambda _name: tmp_path)
...
async def test_send_delta_streams_by_editing_message(monkeypatch) -> None:
    ...
    monkeypatch.setattr("ruiclaw.channels.discord.runtime.time.monotonic", lambda: next(times, 5.0))
...
async def test_send_delta_stream_end_splits_oversized_reply(monkeypatch) -> None:
    ...
    times = iter([1.0, 3.0])
    monkeypatch.setattr("ruiclaw.channels.discord.runtime.time.monotonic", lambda: next(times, 3.0))
