...
from ruiclaw.channels.contracts import ChannelInstanceSpec
from ruiclaw.channels.feishu.config import FeishuConfig, feishu_default_config
from ruiclaw.channels.feishu.instances import (
    DEFAULT_INSTANCE_ID,
...
from ruiclaw.channels.feishu.websocket import get_feishu_ws_runner
from ruiclaw.command.router import normalize_command_text
from ruiclaw.config.paths import get_media_dir
from ruiclaw.pairing import clear_channel
from ruiclaw.utils.helpers import safe_filename
from ruiclaw.utils.logging_bridge import redirect_lib_logging
...
def save_registration_result(
    ...
    *,
    ...
    default_name = (
        ...
        else f"ruiclaw {effective_instance_id}"
    )
...
class FeishuChannel(BaseChannel):
    ...
    async def login(self, force: bool = False) -> bool:
        ...
        _LOGIN_CONSOLE.print("Authorize with the mobile app. ruiclaw will save the new bot credentials.\n")

        result = qr_register(initial_domain=self.config.domain or "feishu")
        if not result:
            _LOGIN_CONSOLE.print(
                "[yellow]Login was not completed.[/yellow] "
                "Run 'ruiclaw channels login feishu --force' to retry."
            )
            return False
    ...
    async def start(self) -> None:
        ...
        if not FEISHU_AVAILABLE:
            self.logger.error("SDK not installed. Run: ruiclaw plugins enable feishu")
            return

        if not self.config.app_id or not self.config.app_secret:
            self.logger.error(
                "app_id and app_secret not configured. "
                "Run 'ruiclaw channels login feishu' to set up via QR code."
            )
            return
