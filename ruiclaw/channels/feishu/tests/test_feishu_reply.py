...
try:
    from ruiclaw.channels.feishu import runtime as feishu
...
from ruiclaw.channels.feishu.runtime import FeishuChannel, FeishuConfig
...
async def test_on_message_keeps_longer_mention_key_that_shares_bot_prefix() -> None:
    ...
    assert len(captured) == 1
    assert captured[0]["content"] == "@Alice (ou_alice) /new @ruiclaw (ou_bot)"
...
async def test_on_message_audio_publishes_downloaded_path_and_transcription() -> None:
    ...
    channel._download_and_save_media = AsyncMock(
        return_value=(r"C:\\Users\\dodre\\.ruiclaw\\media\\feishu\\voice.ogg", "[audio: voice.ogg]")
    )
    ...
    channel.transcribe_audio.assert_awaited_once_with(r"C:\\Users\\dodre\\.ruiclaw\\media\\feishu\\voice.ogg")
    assert len(captured) == 1
    assert captured[0].media == [r"C:\\Users\\dodre\\.ruiclaw\\media\\feishu\\voice.ogg"]
