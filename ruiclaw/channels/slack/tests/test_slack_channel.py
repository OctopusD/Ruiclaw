...
from ruiclaw.channels.slack.runtime import SLACK_MAX_MESSAGE_LEN, SlackChannel, SlackConfig
...
def test_slack_download_failure_marker_is_actionable() -> None:
    ...
    assert "not available to ruiclaw" in marker
...
def _patch_download_transport(
    ...
    monkeypatch.setattr(
        "ruiclaw.channels.slack.runtime.PinnedDNSAsyncTransport",
        lambda: httpx.MockTransport(handler),
    )
    monkeypatch.setattr("ruiclaw.channels.slack.runtime.httpx_env_proxy_mounts", lambda: {})
...
def _patch_download_validation(
    ...
    monkeypatch.setattr("ruiclaw.channels.slack.runtime.validate_url_target", validate)
...
async def test_download_follows_safe_redirect(
    ...
    _patch_download_validation(monkeypatch, validated)
    monkeypatch.setattr(
        "ruiclaw.channels.slack.runtime.get_media_dir", lambda _channel=None: str(tmp_path)
    )
    channel = SlackChannel(SlackConfig(enabled=True, bot_token="xoxb-test"), MessageBus())
    url = "https://files.slack.com/files-pri/x"
