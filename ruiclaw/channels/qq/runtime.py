...
- Publish to RuiClaw bus via BaseChannel._handle_message()
...
def _make_bot_class(channel: QQChannel) -> type[Any]:
    ...
    class _Bot(botpy_sdk.Client):
        def __init__(self):
            # Disable botpy's file log — ruiclaw uses loguru; default "botpy.log" fails on read-only fs
...
class QQConfig(Base):
    ...
    # Optional: directory to save inbound attachments. If empty, use ruiclaw get_media_dir("qq").
    media_dir: str = ""
...
class QQChannel(BaseChannel):
    ...
    def _init_media_root(self) -> Path:
        ...
        if self.config.media_dir:
            ...
            try:
                ...
                root = Path.home() / ".ruiclaw" / "media" / "qq"
        else:
            root = Path.home() / ".ruiclaw" / "media" / "qq"
    ...
    async def start(self) -> None:
        ...
        if not QQ_AVAILABLE:
            self.logger.error("SDK not installed. Run: ruiclaw plugins enable qq")
            return
