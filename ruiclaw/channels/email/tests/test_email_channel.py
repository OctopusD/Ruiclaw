...
from ruiclaw.channels.email.runtime import EmailChannel, EmailConfig
...
def test_fetch_new_messages_parses_unseen_and_marks_seen(monkeypatch) -> None:
    ...
    monkeypatch.setattr("ruiclaw.channels.email.runtime.imaplib.IMAP4_SSL", lambda _h, _p: fake)
...
def test_fetch_new_messages_rejected_returns_skipped_uid(monkeypatch) -> None:
    raw = _make_raw_email(from_addr="RuiClaw <bot@example.com>", subject="Loop test")

    monkeypatch.setattr(
        "ruiclaw.channels.email.runtime.imaplib.IMAP4_SSL",
...
def test_fetch_new_messages_retries_once_when_imap_connection_goes_stale(monkeypatch) -> None:
    ...
    monkeypatch.setattr("ruiclaw.channels.email.runtime.imaplib.IMAP4_SSL", _factory)
...
def test_fetch_new_messages_keeps_messages_collected_before_stale_retry(monkeypatch) -> None:
    ...
    monkeypatch.setattr("ruiclaw.channels.email.runtime.imaplib.IMAP4_SSL", lambda _h, _p: FlakyIMAP())
...
async def test_send_uses_smtp_and_reply_subject(monkeypatch) -> None:
    ...
    monkeypatch.setattr("ruiclaw.channels.email.runtime.smtplib.SMTP", _smtp_factory)
...
def test_extract_attachments_saves_pdf(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("ruiclaw.channels.email.runtime.get_media_dir", lambda ch: tmp_path)
...
async def test_send_with_single_file_attachment(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("ruiclaw.channels.email.runtime.smtplib.SMTP", lambda h, p, timeout=30: FakeSMTP(h, p, timeout=timeout))
