...
from ruiclaw.runtime_context import RUNTIME_CONTEXT_INPUT_META
...
async def test_webui_message_projects_attachments_to_other_clients(tmp_path: Path) -> None:
    ...
    with patch("ruiclaw.webui.media_gateway.get_media_dir", return_value=tmp_path):
...
async def test_message_with_single_image_forwards_saved_path(tmp_path) -> None:
    ...
    with patch(
        "ruiclaw.webui.media_gateway.get_media_dir", return_value=tmp_path
    ):
        await channel._dispatch_envelope(mock_conn, "client-1", envelope)
