...
async def test_websocket_does_not_report_running_before_bind_succeeds(monkeypatch) -> None:
    ...
    monkeypatch.setattr("ruiclaw.channels.websocket.runtime.serve", fail_bind)
...
async def test_websocket_restarts_only_its_listener_after_serving_socket_is_lost(
    ...
    monkeypatch.setattr("ruiclaw.channels.websocket.runtime.serve", bind)
    monkeypatch.setattr(
        "ruiclaw.channels.websocket.runtime._LISTENER_CHECK_INTERVAL_S",
        0.01,
    )
    monkeypatch.setattr(
        "ruiclaw.channels.websocket.runtime._LISTENER_RESTART_BACKOFF_S",
        (0.05,),
    )
