...
from ruiclaw.channels.websocket.runtime import WebSocketChannel, WebSocketConfig
from ruiclaw.config.loader import load_config, save_config
from ruiclaw.cron.service import CronService
from ruiclaw.cron.types import CronJob, CronPayload, CronSchedule
from ruiclaw.optional_features import InstallResult
from ruiclaw.security.workspace_access import WORKSPACE_SCOPE_METADATA_KEY
from ruiclaw.session.keys import UNIFIED_SESSION_KEY
from ruiclaw.session.manager import Session, SessionManager
from ruiclaw.session.session_handles import SessionHandleResolver
from ruiclaw.triggers.local_store import LocalTriggerStore
from ruiclaw.webui.gateway_services import GatewayServices, build_gateway_services
...
def _isolate_runtime_data(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("ruiclaw.config.paths.get_data_dir", lambda: tmp_path)
...
async def test_sessions_list_and_thread_restore_transcript_without_canonical_file(
    ...
    from ruiclaw.webui.transcript import append_transcript_object
...
async def test_session_automations_route_lists_local_triggers(
    ...
    try:
        ...
        assert job["payload"]["command"] == f'ruiclaw trigger {trigger.id} "message"'
        assert job["state"]["pending"] is True
    finally:
        await channel.stop()
        await server_task
...
async def test_webui_skills_route_requires_token_and_hides_paths(
    ...
    (unavailable_skill / "SKILL.md").write_text(
        "\n".join([
            ...
            "    requires:",
            "      bins:",
            "        - definitely-missing-ruiclaw-skill-cli",
            "      env:",
            "        - DEFINITELY_MISSING_RUICLAW_SKILL_ENV",
            "---",
    ...
    try:
        ...
        assert unavailable["unavailable_reason"] == (
            "CLI: definitely-missing-ruiclaw-skill-cli, "
            "ENV: DEFINITELY_MISSING_RUICLAW_SKILL_ENV"
        )
        ...
        assert detail_body["requirements"] == {
            "bins": ["definitely-missing-ruiclaw-skill-cli"],
            "env": ["DEFINITELY_MISSING_RUICLAW_SKILL_ENV"],
            "missing_bins": ["definitely-missing-ruiclaw-skill-cli"],
            "missing_env": ["DEFINITELY_MISSING_RUICLAW_SKILL_ENV"],
        }
...
async def test_webui_skill_management_routes(
    ...
    monkeypatch.setattr("ruiclaw.webui.ws_http.set_webui_skill_enabled", set_enabled)
    monkeypatch.setattr("ruiclaw.webui.ws_http.delete_webui_skill", delete)
...
async def test_webui_skills_marketplace_routes_search_and_install(
    ...
    monkeypatch.setattr("ruiclaw.webui.ws_http.search_marketplace_skills", search)
    monkeypatch.setattr("ruiclaw.webui.ws_http.trending_marketplace_skills", trending)
    monkeypatch.setattr("ruiclaw.webui.ws_http.marketplace_skill_trends", trends)
    monkeypatch.setattr("ruiclaw.webui.ws_http.install_marketplace_skill", install_mock)
...
async def test_webui_skill_delete_remains_local_only(
    ...
    monkeypatch.setattr("ruiclaw.config.loader.load_config", lambda: policy)
    monkeypatch.setattr("ruiclaw.webui.ws_http.delete_webui_skill", delete)
    channel = _ch(
        bus,
        session_manager=_seed_session(tmp_path),
...
async def test_webui_skill_install_honors_remote_install_opt_in(
    ...
    monkeypatch.setattr(
        "ruiclaw.webui.ws_http.install_marketplace_skill",
...
async def test_cli_apps_routes_require_token_and_return_payload(
    ...
    monkeypatch.setattr(
        "ruiclaw.webui.settings_routes.cli_apps_payload",
        payload,
    )
    monkeypatch.setattr(
        "ruiclaw.webui.settings_routes.cli_apps_action",
...
@pytest.mark.asyncio
async def test_ruiclaw_feature_routes_require_token_and_enable(
    bus: MagicMock,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    server_task = asyncio.create_task(channel.start())
    try:
        deny = await _http_get("http://127.0.0.1:29916/api/settings/ruiclaw-features")
        assert deny.status_code == 401
        ...
        catalog = await _http_get(
            "http://127.0.0.1:29916/api/settings/ruiclaw-features",
            headers=auth,
        )
        assert catalog.status_code == 200
...
@pytest.mark.asyncio
async def test_ruiclaw_feature_route_reports_live_channel_failure(
    bus: MagicMock,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    try:
        ...
        response = await _http_get(
            "http://127.0.0.1:29946/api/settings/ruiclaw-features",
            headers={"Authorization": f"Bearer {token}"},
        )
...
async def test_pairing_routes_require_token_and_approve_or_deny(
    ...
    monkeypatch.setattr("ruiclaw.webui.settings_routes.list_pending", lambda: list(pending))
    monkeypatch.setattr(
        "ruiclaw.webui.settings_routes.approve_code",
        lambda code: approved.append(code) or ("feishu", "ou_123") if code == "ABCD-EFGH" else None,
    )
    monkeypatch.setattr(
        "ruiclaw.webui.settings_routes.deny_code",
...
@pytest.mark.asyncio
async def test_ruiclaw_feature_remote_install_requires_opt_in(
    bus: MagicMock,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.asyncio
async def test_ruiclaw_feature_local_install_allowed_by_default(
    bus: MagicMock,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.asyncio
async def test_ruiclaw_feature_channel_action_can_apply_without_restart(
    bus: MagicMock,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.asyncio
async def test_ruiclaw_feature_install_only_does_not_start_channel(
    bus: MagicMock,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    monkeypatch.setattr(
        "ruiclaw.webui.settings_routes.ruiclaw_features_action",
        feature_action,
    )
    channel = _ch(
        bus,
...
async def test_channel_connect_runtime_import_error_is_not_reported_as_unsupported(
    ...
    monkeypatch.setattr(
        "ruiclaw.webui.settings_routes.load_channel_plugin",
...
async def test_feishu_connect_routes_write_config_and_hot_reload(
    ...
    assert body["ruiclaw_features"]["requires_restart"] is False
...
def test_feishu_connect_create_appends_instance(
    ...
    from ruiclaw.channels.feishu import runtime as feishu_module
    from ruiclaw.channels.feishu.connect import FeishuConnectStore
    from ruiclaw.config import loader
...
async def test_channel_configure_route_saves_discord_config_and_hot_reloads(
    ...
    monkeypatch.setattr("ruiclaw.webui.settings_routes.ruiclaw_features_action", fake_feature_action)
    calls: list[tuple[str, str, str]] = []
...
async def test_channel_configure_route_preserves_existing_channel_values(
    ...
    discord = next(
        feature
        for feature in body["ruiclaw_features"]["features"]
...
@pytest.mark.asyncio
async def test_ruiclaw_feature_loopback_reverse_proxy_install_requires_opt_in(
    bus: MagicMock,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.asyncio
async def test_ruiclaw_feature_remote_enable_without_install_is_allowed(
    bus: MagicMock,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.asyncio
async def test_ruiclaw_feature_remote_disable_does_not_need_install_policy(
    bus: MagicMock,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
async def test_cli_apps_catalog_does_not_block_other_webui_http_routes(
    ...
    monkeypatch.setattr("ruiclaw.webui.settings_routes.cli_apps_payload", slow_payload)
...
async def test_cli_apps_route_supports_installed_only_payload(
    ...
    monkeypatch.setattr("ruiclaw.webui.settings_routes.cli_apps_payload", payload)
...
async def test_mcp_presets_routes_require_token_and_return_payload(
    ...
    monkeypatch.setattr(
        "ruiclaw.webui.mcp_presets_api.mcp_presets_payload",
        lambda **_kwargs: {
            "presets": [
                {
                    "name": "browserbase",
    ...
    monkeypatch.setattr(
        "ruiclaw.webui.mcp_presets_api.mcp_presets_action",
        _mcp_preset_action,
    )
    monkeypatch.setattr(
        "ruiclaw.webui.mcp_presets_api.custom_mcp_action",
        _custom_action,
    )
...
async def test_webui_automations_route_manages_local_triggers(
    ...
    try:
        ...
        assert by_id[trigger.id]["trigger"]["command"] == f'ruiclaw trigger {trigger.id} "message"'
...
async def test_webui_thread_resigns_assistant_media_urls(
    ...
    monkeypatch.setattr("ruiclaw.webui.media_gateway.get_media_dir", fake_media_dir)
...
async def test_workspace_folder_picker_is_local_authenticated_mutation(
    ...
    monkeypatch.setattr(
        "ruiclaw.webui.ws_http.native_folder_picker_available",
        lambda: True,
    )
    monkeypatch.setattr("ruiclaw.webui.ws_http.pick_native_folder", pick_folder)
    channel = _ch(bus)
...
def test_local_browser_request_requires_loopback_host_and_forwarded_origin() -> None:
    from ruiclaw.webui.http_utils import is_local_browser_request
    ...
    assert is_local_browser_request(_LOCAL, {"Host": "ruiclaw.example"}) is False
    assert (
        is_local_browser_request(
            _LOCAL,
    ...
    assert (
        is_local_browser_request(
            _LOCAL,
            {"Host": "127.0.0.1:8765", "X-Forwarded-Host": "ruiclaw.example"},
        )
        is False
    )
    assert (
        is_local_browser_request(
            _LOCAL,
            {"Host": "127.0.0.1:8765", "Forwarded": "for=203.0.113.42;host=ruiclaw.example"},
        )
        is False
    )
        ...
        _FakeReq(
            {
                "Host": "ruiclaw.example",
                "X-Forwarded-For": "203.0.113.42",
                "Forwarded": "for=203.0.113.42;host=ruiclaw.example",
                "X-Real-IP": "203.0.113.42",
                "X-Forwarded-Host": "ruiclaw.example",
...
def test_trusted_proxy_matches_ip_versions_and_mapped_peers(
    ...
    from ruiclaw.webui.http_utils import is_trusted_proxy_authenticated_request
...
def test_wildcard_ipv6_with_secret_is_valid(bus: MagicMock) -> None:
    ...
    resp = channel.gateway.http._handle_bootstrap(
        _REMOTE, _FakeReq({"X-RuiClaw-Auth": "s3cret"})
    )
    assert resp.status_code == 200
...
def test_bootstrap_ws_url_uses_forwarded_https_host(bus: MagicMock) -> None:
    ...
    assert resp.status_code == 200
    body = json.loads(resp.body)
    assert body["ws_url"] == "wss://ruiclaw.example/"
...
def test_bootstrap_without_auth_rejects_reverse_proxy_remote_headers(bus: MagicMock) -> None:
    ...
    resp = channel.gateway.http._handle_bootstrap(
        _LOCAL,
        _FakeReq({"Host": "ruiclaw.example", "X-Forwarded-For": "203.0.113.42"}),
    )
    assert resp.status_code == 403
...
def test_bootstrap_prefers_runtime_model_name(bus: MagicMock, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "ruiclaw.webui.ws_http._default_model_name_from_config",
...
def test_bootstrap_accepts_x_ruiclaw_auth_header(bus: MagicMock) -> None:
    channel = _ch(bus, host="0.0.0.0", tokenIssueSecret="s3cret")
    resp = channel.gateway.http._handle_bootstrap(
        _REMOTE, _FakeReq({"X-RuiClaw-Auth": "s3cret"})
    )
