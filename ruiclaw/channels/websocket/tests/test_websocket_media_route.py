...
from ruiclaw.webui.media_api import (
    b64url_decode,
    b64url_encode,
    sign_media_path,
)
...
def test_sign_media_path_rejects_paths_outside_media_root(
    ...
    with patch("ruiclaw.webui.media_gateway.get_media_dir", return_value=media):
...
def test_local_markdown_image_is_staged_and_rewritten(
    ...
    with patch("ruiclaw.webui.media_gateway.get_media_dir", side_effect=_fake_media_dir(media)):
...
def test_local_markdown_video_is_staged_and_rewritten(
    ...
    (workspace / "ruiclaw-intro.mp4").write_bytes(video_bytes)
    media = tmp_path / "media"
    channel = _ch(bus, workspace_path=workspace, port=0)

    with patch("ruiclaw.webui.media_gateway.get_media_dir", side_effect=_fake_media_dir(media)):
        rewritten = channel.gateway.media.rewrite_local_markdown_images(
            "The result:\n![ruiclaw-intro.mp4](ruiclaw-intro.mp4)"
        )

    assert "![ruiclaw-intro.mp4](/api/media/" in rewritten
    staged = list((media / "websocket").iterdir())
    assert len(staged) == 1
