...
from ruiclaw.bus.events import (
...
from ruiclaw.channels.websocket.runtime import (
    WebSocketChannel,
    WebSocketConfig,
    _is_valid_chat_id,
    _parse_envelope,
...
from ruiclaw.runtime_context import RUNTIME_CONTEXT_INPUT_META, WEBUI_QUOTE_SOURCE
from ruiclaw.security.workspace_access import WORKSPACE_SCOPE_METADATA_KEY
from ruiclaw.session import webui_turns as wth
from ruiclaw.session.manager import SessionManager
from ruiclaw.session.model_selection import SESSION_MODEL_PRESET_METADATA_KEY
from ruiclaw.session.session_handles import session_handle_for_name
from ruiclaw.webui.gateway_services import GatewayServices, build_gateway_services
from ruiclaw.webui.http_utils import (
...
from ruiclaw.webui.settings_api import settings_payload, update_provider_settings
from ruiclaw.webui.transcript import (
...
async def _webui_mutate(
    ...
    while True:
        ...
        return httpx.Response(
            status,
            json=body,
            request=httpx.Request("WS", "http://ruiclaw.local/webui-mutation"),
        )
...
async def test_start_extends_http_open_timeout_for_slow_settings_routes(
    ...
    import ruiclaw.channels.websocket.runtime as websocket_module
...
def isolate_webui_workspace_state(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "ruiclaw.webui.workspaces.get_webui_dir",
...
def test_issue_route_secret_matches_bearer_and_header() -> None:
    ...
    x_headers = Headers([("X-RuiClaw-Auth", "my-secret")])
...
async def test_webui_message_envelope_marks_inbound_metadata(bus: MagicMock) -> None:
    from ruiclaw.webui.transcript import read_transcript_lines
...
async def test_webui_message_envelope_persists_user_transcript_for_refresh(
    ...
    from ruiclaw.webui.transcript import build_webui_thread_response, read_transcript_lines
...
async def test_webui_user_transcript_append_failure_does_not_block_inbound(
    ...
    monkeypatch.setattr("ruiclaw.webui.transcript.append_transcript_object", fail_append)
    channel = _ch(bus)
    conn = AsyncMock()
    conn.remote_address = ("127.0.0.1", 50123)
...
def test_webui_request_cache_prunes_expired_completed_but_keeps_pending(
    ...
    import ruiclaw.webui.inbound_commands as websocket_module
...
async def test_remote_webui_scope_allows_access_reduction(
    ...
    monkeypatch.setattr("ruiclaw.webui.workspaces.get_webui_dir", lambda: tmp_path / "webui")
    default_workspace = tmp_path / "default"
    default_workspace.mkdir()
    sessions = SessionManager(tmp_path / "sessions")
    channel = WebSocketChannel(
...
async def test_stream_transcript_writes_once_per_completed_segment(monkeypatch) -> None:
    ...
    append = MagicMock()
    monkeypatch.setattr("ruiclaw.webui.transcript.append_transcript_object", append)
...
async def test_retry_status_is_transient_and_turn_scoped(
    ...
    monkeypatch.setattr("ruiclaw.webui.outbound_wire.time.time", lambda: 120.0)
    bus = MagicMock()
    channel = WebSocketChannel(
        {"enabled": True, "allowFrom": ["*"]},
        bus,
...
async def test_durable_incomplete_marker_stays_pending_without_safe_session_recovery(
    ...
    from ruiclaw.bus.events import InboundMessage
    from ruiclaw.webui.transcript import build_webui_thread_response
    ...
    monkeypatch.setattr("ruiclaw.webui.transcript.append_transcript_object", fail_answer)
...
async def test_non_webui_transcript_failure_does_not_block_idle_cleanup(
    ...
    await wth.publish_turn_run_status(bus, inbound, "running", started_at=1234.5)
    monkeypatch.setattr(
        "ruiclaw.webui.transcript.append_transcript_object",
...
def test_registry_discovers_websocket_channel() -> None:
    from ruiclaw.channels.registry import load_channel_class
...
async def test_settings_api_returns_safe_subset_and_updates_whitelist(
    ...
    monkeypatch.setattr(
        "ruiclaw.webui.settings_api._oauth_provider_status",
        lambda _spec: {
            "configured": False,
            "account": None,
            "expires_at": None,
    ...
    monkeypatch.setattr(
        "ruiclaw.webui.settings_routes.request_image_generation_reload",
        image_reload,
    )
    ...
    try:
        ...
        assert workspace_path.endswith("/.ruiclaw/workspace")
        ...
        assert saved.agents.defaults.bot_name == "ruiclaw"
...
async def test_bootstrap_exposes_native_surface(bus: MagicMock) -> None:
    ...
    try:
        response = await _http_get(
            ...
            headers={"X-RuiClaw-Auth": "native-secret"},
        )
        assert response.status_code == 200
        body = response.json()
...
def test_settings_payload_reports_workspace_sandbox(monkeypatch, tmp_path) -> None:
    ...
    save_config(config, config_path)
    monkeypatch.setattr("ruiclaw.config.loader._current_config_path", config_path)
    monkeypatch.setenv("RUICLAW_SANDBOX_ENFORCED", "macos_app_sandbox")
...
def test_sessions_list_includes_active_run_started_at(monkeypatch) -> None:
    ...
    from ruiclaw.session import webui_turns as wth
    from ruiclaw.webui import ws_http as ws_http_module
...
async def test_handle_session_context_get_reads_detached_session() -> None:
    ...
    from ruiclaw.session import Session
...
def test_handle_webui_thread_get_reports_registered_turn_as_pending(
    ...
    monkeypatch.setattr("ruiclaw.config.paths.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "ruiclaw.session.webui_turns.websocket_turn_wall_started_at",
        lambda chat_id: 1_700_000_000.0 if chat_id == "running" else None,
    )
    monkeypatch.setattr(
        "ruiclaw.session.webui_turns.websocket_turn_id",
...
def test_handle_file_preview_returns_workspace_file(tmp_path) -> None:
    ...
    workspace = tmp_path / "workspace"
    source = workspace / "ruiclaw" / "agent" / "hook.py"
    ...
    enc = quote(key, safe="")
    path = quote("ruiclaw/agent/hook.py:12", safe="")
    req = Request(
    ...
    assert resp.status_code == 200
    body = json.loads(resp.body.decode())
    assert body["display_path"] == "ruiclaw/agent/hook.py"
...
def test_file_preview_normalizes_windows_file_url() -> None:
    from ruiclaw.webui.file_preview import _clean_preview_path
...
def test_handle_webui_thread_get_does_not_backfill_cron_internal_prompt(
    ...
    from ruiclaw.cron.session_turns import CRON_HISTORY_META
    from ruiclaw.webui.transcript import append_transcript_object
...
def test_handle_webui_thread_get_does_not_backfill_trigger_internal_prompt(
    ...
    from ruiclaw.session.automation_turns import AUTOMATION_HISTORY_META
    from ruiclaw.webui.transcript import append_transcript_object
