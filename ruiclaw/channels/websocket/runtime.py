"""WebSocket server channel: ruiclaw acts as a WebSocket server and serves connected clients."""
...
from ruiclaw.webui.inbound_commands import WebUICommandRouter
from ruiclaw.webui.metadata import (
    WEBSOCKET_TURN_OWNER_METADATA_KEY,
    WEBUI_TURN_METADATA_KEY,
)
from ruiclaw.webui.outbound_projection import WebUIOutboundProjector
from ruiclaw.webui.outbound_wire import (
    WebUIWirePayload,
    WebUIWirePersistence,
    project_tool_events,
)
from ruiclaw.webui.session_identity import is_valid_webui_chat_id
from ruiclaw.webui.transcript import WEBUI_TRANSCRIPT_INCOMPLETE_KEY
from ruiclaw.webui.websocket_logging import websockets_server_logger
...
class WebSocketConfig(Base):
    ...
    - ``token_issue_path``: If non-empty, **GET** (HTTP/1.1) to this path returns JSON
      ...
      ruiclaw and shares the asyncio loop, use a thread or async HTTP client for GET—do not call
      blocking ``urllib`` or synchronous ``httpx`` from inside a coroutine.
    - ``token_issue_secret``: If non-empty, token requests must send ``Authorization: Bearer <secret>`` or
      ``X-RuiClaw-Auth: <secret>``.
