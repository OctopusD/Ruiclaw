...
from ruiclaw.config.timezone import detect_system_timezone
from ruiclaw.config_base import Base
from ruiclaw.cron.types import CronSchedule

if TYPE_CHECKING:
    from ruiclaw.agent.tools.cli_apps import CliAppsToolConfig
    from ruiclaw.agent.tools.filesystem import FileToolsConfig
    from ruiclaw.agent.tools.image_generation import ImageGenerationToolConfig
    from ruiclaw.agent.tools.self import MyToolConfig
    from ruiclaw.agent.tools.shell import ExecToolConfig
    from ruiclaw.agent.tools.web import WebToolsConfig
...
class TranscriptionConfig(Base):
    ...
    enabled: bool = True
    provider: str | None = None  # Validated by ruiclaw.audio.transcription_registry.
    model: str | None = None
...
class ModelPresetConfig(Base):
    ...
    def to_generation_settings(self) -> Any:
        from ruiclaw.providers.base import GenerationSettings
...
class AgentDefaults(Base):
    ...
    workspace: str = "~/.ruiclaw/workspace"
    ...
    bot_name: str = "ruiclaw"  # Display name shown in CLI prompts (e.g. "{name} is thinking...")
...
class ProviderConfig(Base):
    ...
    # ruiclaw/providers/openai_compat_provider.py. Kept duplicated here to
...
class ToolsConfig(Base):
    ...
    web: WebToolsConfig = Field(default_factory=lambda: _lazy_default("ruiclaw.agent.tools.web", "WebToolsConfig"))
    exec: ExecToolConfig = Field(default_factory=lambda: _lazy_default("ruiclaw.agent.tools.shell", "ExecToolConfig"))
    file: FileToolsConfig = Field(default_factory=lambda: _lazy_default("ruiclaw.agent.tools.filesystem", "FileToolsConfig"))
    cli_apps: CliAppsToolConfig = Field(default_factory=lambda: _lazy_default("ruiclaw.agent.tools.cli_apps", "CliAppsToolConfig"))
    my: MyToolConfig = Field(default_factory=lambda: _lazy_default("ruiclaw.agent.tools.self", "MyToolConfig"))
    image_generation: ImageGenerationToolConfig = Field(
        default_factory=lambda: _lazy_default("ruiclaw.agent.tools.image_generation", "ImageGenerationToolConfig"),
    )
...
class Config(BaseSettings):
    """Root configuration for ruiclaw."""
    ...
    def _match_provider(
        ...
        *,
        ...
        from ruiclaw.providers.registry import (
            PROVIDERS,
            find_by_name,
        )
...
def _resolve_tool_config_refs() -> None:
    ...
    like ``from ruiclaw.config.schema import ExecToolConfig`` continue to work.
    """
    import sys
    ...
    from ruiclaw.agent.tools.self import MyToolConfig
    from ruiclaw.agent.tools.shell import ExecToolConfig
    from ruiclaw.agent.tools.web import WebFetchConfig, WebSearchConfig, WebToolsConfig
