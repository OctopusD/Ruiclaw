"""Interactive onboarding questionnaire for ruiclaw."""
...
from ruiclaw.cli.models import (
    format_token_count,
...
def _show_main_menu_header() -> None:
    ...
    body.add_row(f"{__logo__} [bold {_UI_TEXT}]ruiclaw[/] [{_UI_MUTED}]v{__version__}[/]")
...
def _handle_fallback_models_field(
    ...
    from ruiclaw.config.schema import InlineFallbackConfig
...
def _handle_search_provider_field(
    ...
    from ruiclaw.agent.tools.web import SEARCH_PROVIDER_OPTIONS
...
def _try_auto_fill_context_window(model: BaseModel, new_model_name: str) -> None:
    ...
    Note:
        This function imports AgentDefaults from ruiclaw.config.schema to get
...
def _enable_quick_start_websocket_defaults(config: Config) -> bool:
    ...
    console.print(
        f"[{_UI_MUTED}]This lets the browser UI at http://127.0.0.1:8765 connect to ruiclaw.[/]"
    )
    console.print()
    while True:
