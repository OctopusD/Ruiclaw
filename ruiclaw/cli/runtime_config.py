...
def _print_runtime_config_validation_error(
    ...
    *,
    ...
    from ruiclaw.config.errors import ConfigIssue, ConfigLoadError, validation_issues
...
def _status_command(config_path: Path) -> str:
    return f'ruiclaw status --config "{config_path}"'
...
def _print_model_setup_steps(config_path: Path) -> None:
    ...
    console.print(
        f"  WebUI: run [cyan]ruiclaw webui {escape(config_arg)}[/cyan], "
        "then open Settings → Models"
    )
    console.print(f"  CLI:   run [cyan]ruiclaw onboard --wizard {escape(config_arg)}[/cyan]")
...
def _load_inspection_config(
    ...
    from ruiclaw.config.errors import ConfigLoadError
    from ruiclaw.config.loader import get_config_path, load_config, set_config_path
...
def _migrate_cron_store(config: "Config") -> None:
    ...
    from ruiclaw.config.paths import get_cron_dir
...
def _provider_setup_error(config: Config) -> str | None:
    ...
    from ruiclaw.providers.factory import validate_provider_setup
