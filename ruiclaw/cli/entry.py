...
from ruiclaw.cli.process_identity import set_cli_process_identity
...
def main() -> None:
    ...
    raw_args = sys.argv[1:]
    command_name = "ruiclaw" if Path(sys.argv[0]).stem == "ruiclaw" else "ruiclaw"
    # Installed completion scripts call ``ruiclaw`` without positional arguments
    ...
    shell_completion = bool(os.environ.get("_RUICLAW_COMPLETE"))
    ...
    if agent_args is not None and (
        ...
        prog_name = command_name if root_agent_alias else f"{command_name} agent"
