"""Give ruiclaw processes recognizable operating-system names."""
...
def set_cli_process_identity(args: list[str]) -> None:
    """Name this CLI process after the ruiclaw role it is running."""
    if os.name == "nt":
        # Windows process managers use the console launcher's executable name,
        # which packaging already generates as ``ruiclaw.exe``.
        return
    role = args[0] if args and args[0] in _ROLES else None
    _set_process_title(f"ruiclaw-{role}" if role else "ruiclaw")
