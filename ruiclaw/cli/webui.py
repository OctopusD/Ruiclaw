...
from ruiclaw.config.paths import get_workspace_path
from ruiclaw.utils.helpers import sync_workspace_templates
from ruiclaw.webui.dev import (
    WebUIDevError,
    WebUIDevServer,
...
def webui(
    ...
    background: bool = typer.Option(
        False,
        "--background",
        help="Deprecated; use `ruiclaw gateway --background`",
    ),
    ...
    from ruiclaw.config.loader import resolve_config_env_vars, save_config
    ...
    if background:
        ...
        command = ["ruiclaw", "gateway", "--background", "--config", str(config_path)]
        if workspace:
            command.extend(
                ["--workspace", str(Path(workspace).expanduser().resolve(strict=False))]
            )
        console.print(
            "[red]`ruiclaw webui --background` no longer owns gateway lifecycle.[/red]"
        )
        ...
        console.print(
            "  [cyan]ruiclaw webui --config " + shlex.quote(str(config_path)) + "[/cyan]"
        )
        raise typer.Exit(1)
    created_config = not config_path.exists()
    if created_config:
        console.print(f"[yellow]No config found at {config_path}.[/yellow]")
        _confirm_webui_action("Create a RuiClaw config and workspace now?", yes=yes)
    ...
    try:
        ...
        retry_command = f'ruiclaw webui --config "{config_path}"'
        _print_runtime_config_validation_error(
            exc,
            config_path=config_path,
