...
from ruiclaw.gateway.service import (
...
def create_gateway_app(
    *,
    ...
    gateway_app = typer.Typer(
        help="Start and manage the RuiClaw gateway.",
    ...
    def interactive_build_mode() -> BuildMode:
        # `ruiclaw gateway` is often launched by tests, supervisors, or service managers.
        # The higher-level `ruiclaw webui` command owns interactive first-run guidance.
        return "warn"
        ...
        if background:
            ...
            if (
                ...
                console.print(
                    ...
                    "`ruiclaw gateway --background`.[/dim]"
                )
            ...
            if (
                ...
                console.print(
                    "[dim]Stop it first, then rerun `ruiclaw gateway --background` "
            ...
            if result.message == "gateway_already_running":
                if result.promoted:
                    ...
                    console.print(
                        ...
                        "use `ruiclaw gateway stop` to stop it.[/dim]"
                    )
                else:
                    console.print(
    ...
    def gateway_restart(  # pyright: ignore[reportUnusedFunction]
        ...
        if result.message == "gateway_not_running":
            ...
            console.print(
                "[dim]Start a persistent gateway with `ruiclaw gateway --background`.[/dim]"
            )
            print_status(result.status)
            raise typer.Exit(1)
    ...
    def gateway_install_service(  # pyright: ignore[reportUnusedFunction]
        ...
        verbose: bool = typer.Option(False, "--verbose", "-v", help="Verbose output"),
        config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
        name: str = typer.Option("ruiclaw-gateway", "--name", help="Service name"),
