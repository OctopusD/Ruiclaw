"""Discover and attach to an already-running RuiClaw Desktop instance."""
...
def dispatch_bare_desktop_target(args: list[str]) -> int | None:
    ...
    if (
        args not in ([], ["webui"])
        or os.environ.get("_RUICLAW_COMPLETE")
    ...
    try:
        ...
        print("RuiClaw Desktop discovery could not be authenticated.", file=sys.stderr)
    ...
    try:
        ...
        print("RuiClaw Desktop is unavailable.", file=sys.stderr)
        _print_python_target()
        return None

    if status.state != "ready":
        print(f"RuiClaw Desktop is not ready ({status.state}).", file=sys.stderr)
        _print_python_target()
        return None
    ...
    if not args:
        from ruiclaw.cli.desktop_tui import launch_desktop_tui

        try:
            return launch_desktop_tui(target)
        except DesktopTargetError:
            _print_desktop_error("Desktop terminal attachment is unavailable or incompatible. Update Desktop and ruiclaw, then try again.")
            return 3
    ...
    try:
        ...
        _print_desktop_error("RuiClaw Desktop disconnected or returned an incompatible WebUI.")
        return 3
    from ruiclaw.cli.webui_support import _launch_browser
    ...
    try:
        ...
        opened = False
    if not opened:
        _print_desktop_error("The default browser could not open RuiClaw Desktop.")
        return 3
    print("Connected to RuiClaw Desktop. Closing the browser leaves Desktop running.")
    return 0
...
def _choose_target(status: DesktopReply) -> Literal["desktop", "python"]:
    ...
    try:
        answer = questionary.select(
            "RuiClaw Desktop is running. Choose a target for this invocation:",
            choices=[
                questionary.Choice(f"RuiClaw Desktop{detail}", value="desktop"),
                questionary.Choice(
...
def _desktop_terminal_directory() -> Path | None:
    if sys.platform == "darwin":
        root = os.environ.get("RUICLAW_DESKTOP_ROOT", "").strip()
        base = Path(root).expanduser() if root else Path.home() / "Library/Application Support/RuiClaw"
        return base.absolute() / "terminal"
    if sys.platform == "win32":
        override = os.environ.get("RUICLAW_DESKTOP_DATA_DIR", "").strip()
...
def _parse_descriptor(raw: bytes, *, directory: Path) -> DesktopTarget:
    ...
    if sys.platform == "darwin":
        ...
        expected = f"ruiclaw-desktop-v1-{instance_id}"
        if transport != "namedPipe" or address != expected:
            raise DesktopTargetError("Unsupported Desktop transport")
        selected_transport = "namedPipe"
    else:
        raise DesktopTargetError("RuiClaw Desktop is not supported on this platform")
