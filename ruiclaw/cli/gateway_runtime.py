...
from ruiclaw import __logo__, __version__
from ruiclaw.agent.hook import AgentHook, AgentRunHookContext
from ruiclaw.agent.hooks import create_file_edit_activity_hook
from ruiclaw.agent.loop import AgentLoop
from ruiclaw.agent.tools.mcp import MCPProvider
...
from ruiclaw.cli.webui_support import (
...
from ruiclaw.config.paths import is_default_workspace
from ruiclaw.config.schema import Config
from ruiclaw.gateway.runtime import GatewayInstance
from ruiclaw.security.network import is_loopback_host
from ruiclaw.session.keys import UNIFIED_SESSION_KEY, last_channel_from_metadata
from ruiclaw.utils.evaluator import evaluate_response, resolve_evaluator_prompt
from ruiclaw.utils.helpers import sync_workspace_templates
from ruiclaw.webui.build import BuildMode
from ruiclaw.webui.dev import WebUIDevError, WebUIDevServer
from ruiclaw.webui.sidebar_state import read_webui_sidebar_state
...
def _run_gateway(
    ...
    *,
    ...
    from ruiclaw.agent.model_presets import load_model_preset_catalog
    ...
    from ruiclaw.config.watcher import watch_config_file
    from ruiclaw.cron.bound_runner import run_bound_cron_job
    from ruiclaw.cron.service import CronJobSkippedError, CronService
    from ruiclaw.cron.session_turns import is_bound_cron_job
    from ruiclaw.cron.types import CronJob
    from ruiclaw.llm_usage import record_llm_call
    from ruiclaw.llm_usage.context import llm_usage_source
    from ruiclaw.providers.factory import (
        ProviderSnapshot,
        build_provider_snapshot,
        build_unconfigured_provider_snapshot,
        load_provider_snapshot,
    )
    from ruiclaw.providers.fallback_provider import FallbackProvider
    ...
    from ruiclaw.session.recovery import RecoveryCoordinator
    from ruiclaw.session.webui_turns import (
        WebuiTurnCoordinator,
        WebuiTurnRoutePolicy,
        build_webui_fallback_model_observer,
    )
    from ruiclaw.triggers.local_runner import run_local_trigger_queue
    ...
    console.print(f"{__logo__} Starting RuiClaw gateway version {__version__} on port {port}...")
    _prepare_webui_bundle_for_gateway(
        config,
    ...
    from ruiclaw.config.loader import get_config_path
    ...
    from ruiclaw.session.keys import session_key_for_channel
        ...
        try:
            ...
            tasks = [
                asyncio.create_task(
                    ...
                    name="ruiclaw-config-watcher",
                ),
                asyncio.create_task(_run_agent(), name="ruiclaw-agent-loop"),
                asyncio.create_task(channels.start_all(), name="ruiclaw-channels"),
                asyncio.create_task(
