...
from ruiclaw.webui.build import (
    BuildMode,
    WebUIBuildError,
    ensure_webui_bundle,
...
if TYPE_CHECKING:
    from ruiclaw.gateway.runtime import GatewayRuntime
...
def _launch_browser(url: str) -> bool:
    ...
    if sys.platform == "win32":
        from ruiclaw.cli.windows_browser import launch_browser
...
def _resolve_webui_config_path(config: str | None) -> Path:
    """Resolve the config path used by ``ruiclaw webui`` and bind loader state."""
...
def _validate_gateway_startup(config: Config) -> str | None:
    ...
    try:
        ...
        retry_command = f'ruiclaw gateway --config "{config_path}"'
        _print_runtime_config_validation_error(
            exc,
...
def _gateway_health_ready(host: str, port: int, *, timeout_s: float = 0.4) -> bool:
    """Return whether the ruiclaw gateway health endpoint responds OK."""
    import json
    import urllib.error
...
def _print_foreground_port_conflict(
    *,
    ...
    if gateway_running:
        console.print(
            "[yellow]A ruiclaw gateway is already running for this local instance.[/yellow]"
        )
    else:
        console.print(
            "[red]Error: ruiclaw cannot start because one of its local ports "
    ...
    if gateway_running:
        ...
        console.print("If this is an existing ruiclaw instance, use it or stop it first:")
    console.print("  [cyan]ruiclaw gateway status[/cyan]")
    console.print("  [cyan]ruiclaw gateway stop[/cyan]")
    console.print(
...
def _gateway_instance_command(
    ...
    *,
    ...
    parts = ["ruiclaw", "gateway", subcommand, "--config", str(config_path)]
    if workspace:
