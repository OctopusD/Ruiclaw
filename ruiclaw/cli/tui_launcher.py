...
from ruiclaw import __version__
from ruiclaw.cli.process_identity import named_executable
from ruiclaw.cli.runtime_config import _model_display
...
from ruiclaw.config.paths import get_data_dir
from ruiclaw.config.schema import Config
from ruiclaw.webui.session_identity import is_webui_session_key, webui_chat_id
...
_TUI_RELEASE_LIMITS = {
    ...
    "ruiclaw-tui-source.tar.gz": 20 * 1024 * 1024,
...
def launch_tui(
    ...
    *,
    ...
    try:
        env = os.environ.copy()
        env.pop("RUICLAW_TUI_WS_URL", None)
        env.pop("RUICLAW_TUI_API_TOKEN", None)
        env.pop("RUICLAW_TUI_DESKTOP_RESOLVER", None)
        env.pop("RUICLAW_TUI_DESKTOP_TARGET", None)
        env.update(
            {
                "RUICLAW_TUI_BOOTSTRAP_URL": f"{base_url}/webui/bootstrap",
                "RUICLAW_TUI_HEALTH_URL": _gateway_health_url(
                    config.gateway.host,
                    config.gateway.port,
                ),
                "RUICLAW_TUI_API_URL": base_url,
                "RUICLAW_TUI_MODEL": _model_display(config)[0],
                "RUICLAW_TUI_MODEL_PRESET": config.agents.defaults.model_preset or "default",
                "RUICLAW_TUI_WORKSPACE": str(tui_workspace),
                "RUICLAW_TUI_VERSION": __version__,
                "RUICLAW_TUI_ACCESS": (
                    "workspace access" if config.tools.restrict_to_workspace else "full access"
                ),
                "RUICLAW_TUI_THEME": theme,
                "RUICLAW_TUI_GATEWAY_STOP_COMMAND": _gateway_instance_command(
                    "stop",
        ...
        if bootstrap_secret:
            env["RUICLAW_TUI_BOOTSTRAP_SECRET"] = bootstrap_secret
        else:
            env.pop("RUICLAW_TUI_BOOTSTRAP_SECRET", None)
        if chat_id:
            env["RUICLAW_TUI_CHAT_ID"] = chat_id
        else:
            env.pop("RUICLAW_TUI_CHAT_ID", None)
        try:
...
def resolve_tui_command(*, data_dir: Path | None = None) -> list[str]:
    override = os.environ.get("RUICLAW_TUI_BIN", "").strip()
    if override:
        executable = Path(override).expanduser().resolve(strict=False)
        if not executable.is_file():
            raise TuiUnavailableError(f"RUICLAW_TUI_BIN does not exist: {executable}")
    ...
    asset = f"ruiclaw-tui-{system}-{machine}{suffix}"
    ...
    if source_dir is not None:
        ...
        if not bun:
            raise TuiUnavailableError(
                ...
                "install Bun, then run `ruiclaw agent` again"
            )
    ...
    raise TuiUnavailableError(
        f"no native TUI archive is published for ruiclaw {__version__} on this platform; "
        ...
        "`ruiclaw agent --classic` if intentional"
    )
...
def _download_release_tui(asset: str, *, data_dir: Path | None = None) -> Path | None:
    ...
    if os.environ.get("RUICLAW_TUI_NO_DOWNLOAD") == "1":
        return None
    ...
    base = f"https://github.com/OctopusD/Ruiclaw/releases/download/v{version}"
...
def _read_release_asset(url: str, *, max_bytes: int) -> bytes:
    request = urllib.request.Request(url, headers={"User-Agent": f"ruiclaw/{__version__}"})
        ...
        def wait_for_ready(log_path: object) -> _GatewayHandle:
            ...
            if current.running:
                raise TuiUnavailableError(
                    ...
                    "Run `ruiclaw gateway status` and inspect logs at "
                    f"{log_path}; if it remains degraded, run `ruiclaw gateway restart`."
                )
            raise TuiUnavailableError(
        ...
        if status.running:
            if status.port not in {None, config.gateway.port}:
                raise TuiUnavailableError(
                    ...
                    "restart it or use `ruiclaw agent --classic`"
                )
            ...
            raise TuiUnavailableError(
                "the configured gateway port belongs to a different ruiclaw instance; "
                "stop that instance or use `ruiclaw agent --classic`"
            )
