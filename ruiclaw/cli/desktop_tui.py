...
from ruiclaw.cli.desktop_target import (
    DesktopTarget,
    DesktopTargetError,
...
def launch_desktop_tui(target: DesktopTarget) -> int:
    from ruiclaw.cli.tui_launcher import TuiUnavailableError, resolve_tui_command

    process: subprocess.Popen[bytes] | None = None
    try:
        current = _connection(target)
        # Resolve/install only the matching terminal client, never a backend.
        cache = os.environ.get("RUICLAW_DESKTOP_CLIENT_CACHE")
        ...
        env = {key: value for key, value in os.environ.items() if not key.startswith("RUICLAW_TUI_")}
        ...
        env["RUICLAW_TUI_DESKTOP_RESOLVER"] = json.dumps([
            sys.executable, "-I", "-S", str(Path(__file__).resolve()), "--resolve",
        ])
        env["RUICLAW_TUI_DESKTOP_TARGET"] = json.dumps({
...
def main() -> None:
    ...
    if args == ["--protocol"]:
        ...
        return
    if args or os.environ.get("_RUICLAW_COMPLETE") or not sys.stdin.isatty() or not sys.stdout.isatty():
        raise SystemExit(2)
    try:
        target = discover_desktop_target()
        if target is None:
            raise DesktopTargetError("Desktop is absent")
        expected = os.environ.get("RUICLAW_DESKTOP_EXPECTED_INSTANCE")
