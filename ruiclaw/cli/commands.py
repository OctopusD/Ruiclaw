...
# Remove default handler and re-add with unified ruiclaw format
logger.remove()
...
from ruiclaw import __logo__, __version__  # noqa: E402
from ruiclaw import optional_features as feature_support  # noqa: E402
from ruiclaw.agent.hooks import create_file_edit_activity_hook  # noqa: E402
from ruiclaw.agent.loop import AgentLoop  # noqa: E402
from ruiclaw.agent.tools.mcp import MCPProvider  # noqa: E402
from ruiclaw.agent.tools.registry import ToolRegistry  # noqa: E402
from ruiclaw.cli import terminal as cli_terminal  # noqa: E402
from ruiclaw.cli.agent import agent  # noqa: E402
from ruiclaw.cli.bench import bench_app  # noqa: E402
from ruiclaw.cli.gateway import create_gateway_app  # noqa: E402
from ruiclaw.cli.gateway_runtime import _run_gateway  # noqa: E402
from ruiclaw.cli.log_control import _set_ruiclaw_logs  # noqa: E402
from ruiclaw.cli.process_identity import set_cli_process_identity  # noqa: E402
from ruiclaw.cli.provider import provider_app  # noqa: E402
from ruiclaw.cli.runs import runs_app  # noqa: E402
from ruiclaw.cli.runtime_config import (  # noqa: E402
...
from ruiclaw.cli.webui import webui  # noqa: E402
from ruiclaw.cli.webui_support import (  # noqa: E402
    _prepare_webui_bundle_for_gateway,
    _validate_gateway_startup,
)
from ruiclaw.config.paths import get_workspace_path  # noqa: E402
from ruiclaw.config.schema import Config  # noqa: E402
from ruiclaw.security.network import is_loopback_host  # noqa: E402
from ruiclaw.utils.helpers import sanitize_surrogates as _sanitize_surrogates  # noqa: E402,F401
from ruiclaw.utils.helpers import (  # noqa: E402
...
app = typer.Typer(
    ...
    help=f"{__logo__} RuiClaw - Personal AI Assistant",
    epilog=(
        "Run `ruiclaw` without a subcommand to start the terminal agent. "
        "Use `ruiclaw agent --help` for agent options."
    ),
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} RuiClaw v{__version__}")
        raise typer.Exit()
...
def main(
    ...
    if command is None:
        from ruiclaw.cli.entry import _run_agent

        _run_agent([], prog_name="ruiclaw")
...
def onboard(
    ...
    """Initialize RuiClaw configuration and workspace."""
    from ruiclaw.config.loader import get_config_path, load_config, save_config, set_config_path
    ...
    if wizard:
        from ruiclaw.cli.onboard import run_onboard
        ...
        try:
            ...
            console.print("[yellow]Please run 'ruiclaw onboard' again to complete setup.[/yellow]")
            raise typer.Exit(1)
    ...
    webui_cmd = "ruiclaw webui"
    if explicit_config:
        webui_cmd += f' -c "{config_path}"'

    typer.echo(f"\n✓ RuiClaw is ready. Run: {webui_cmd}")
...
def _onboard_plugins(config_path: Path) -> None:
    ...
    from ruiclaw.channels.contracts import channel_default_config
    from ruiclaw.channels.registry import discover_plugins
    from ruiclaw.config.loader import merge_missing_defaults
...
def serve(
    ...
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show RuiClaw runtime logs"),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="Workspace directory"),
    config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
):
    ...
    try:
        ...
        console.print("[red]aiohttp is required. Install with: ruiclaw plugins enable api[/red]")
        raise typer.Exit(1)

    from ruiclaw.api.server import create_app
...
def sessions_restore_workspace(
    ...
    """Copy sessions back into the workspace before downgrading ruiclaw."""
    from ruiclaw.session.manager import SessionManager
...
def channels_status(
    ...
    """Show channel status."""
    from ruiclaw.channels.registry import discover_all
...
plugins_app = typer.Typer(help="Manage optional ruiclaw features")
...
def plugins_list(
    ...
    """List optional ruiclaw features."""
    from ruiclaw.channels.registry import discover_plugins
    from ruiclaw.config.loader import load_config, set_config_path
...
def plugins_enable(
    ...
    """Enable a ruiclaw feature."""
    from ruiclaw.config.loader import get_config_path, set_config_path

    resolved_config_path = Path(config_path).expanduser().resolve() if config_path else None
    if resolved_config_path is not None:
        set_config_path(resolved_config_path)
    resolved_config_path = resolved_config_path or get_config_path()
    _set_ruiclaw_logs(logs)
...
def plugins_disable(
    ...
    """Disable a ruiclaw channel feature."""
    from ruiclaw.config.loader import get_config_path, set_config_path
...
def status(
    ...
    console.print(f"{__logo__} RuiClaw Status\n")
    ...
    if config_path.exists():
        from ruiclaw.config.errors import ConfigLoadError
        from ruiclaw.config.loader import resolve_config_env_vars, resolve_env_refs
        from ruiclaw.providers.registry import PROVIDERS
        ...
        if provider_ready:
            console.print()
            console.print('Next: [cyan]ruiclaw agent -m "Hello!"[/cyan]')
            console.print(
