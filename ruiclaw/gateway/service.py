...
from ruiclaw.gateway import GatewayStartOptions, build_gateway_command
...
class GatewayServiceInstaller:
    ...
    def _install_systemd(
        ...
        *,
        ...
        content = _systemd_unit_content(
            description=f"RuiClaw Gateway ({options.name})",
            command=command,
    ...
    def _install_launchd(
        ...
        *,
        ...
        stdout_path = self.home / ".ruiclaw" / "logs" / f"{log_stem}.launchd.log"
        stderr_path = self.home / ".ruiclaw" / "logs" / f"{log_stem}.launchd.err.log"
        payload = {
            "Label": label,
...
def _launchd_label(name: str) -> str:
    if name.startswith("ai.ruiclaw."):
        return name
    suffix = _safe_service_name(name).removeprefix("ruiclaw-").replace("-", ".")
    return f"ai.ruiclaw.{suffix}"
...
def _safe_service_name(name: str) -> str:
    ...
    value = value.strip(".-")
    return value or "ruiclaw-gateway"
