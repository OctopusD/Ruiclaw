...
from ruiclaw.agent import context as agent_context
from ruiclaw.agent import model_presets as preset_helpers
from ruiclaw.agent.autocompact import AutoCompact
...
from ruiclaw.agent.subagent import SubagentManager
from ruiclaw.agent.tools.context import RequestContext, bind_request_context, reset_request_context
from ruiclaw.agent.tools.exec_session import ExecSessionManager
from ruiclaw.agent.tools.file_state import FileStateStore, bind_file_states, reset_file_states
from ruiclaw.agent.tools.message import capture_message_deliveries
from ruiclaw.agent.tools.registry import ToolRegistry
from ruiclaw.agent.tools.runtime_control import AgentRuntimeControl
from ruiclaw.agent.turn_delivery import (
    TurnDelivery,
    TurnDeliveryFactory,
)
from ruiclaw.agent.turn_delivery import TurnRoute as TurnRoute
from ruiclaw.agent.turn_hooks import AgentTurnHookSpec, build_agent_turn_hook
from ruiclaw.agent.working_memory import WorkingMemoryHook
from ruiclaw.bus.events import INBOUND_META_USER_SHELL, InboundMessage, OutboundMessage
...
from ruiclaw.bus.runtime_events import (
    PARENT_RUN_ID_METADATA_KEY,
    RUN_ID_METADATA_KEY,
)
from ruiclaw.command import CommandContext, CommandRouter, register_builtin_commands
from ruiclaw.command.router import normalize_command_text
from ruiclaw.config.schema import AgentDefaults, ModelPresetConfig
from ruiclaw.events import NO_EVENTS, AgentEvent, EventSink
from ruiclaw.llm_usage import capture_llm_calls
from ruiclaw.llm_usage.context import source_from_request
from ruiclaw.observability.run_ledger import RunLedger, RunLedgerHook
from ruiclaw.providers.base import LLMProvider, LLMUsage, ProviderConversationState
from ruiclaw.providers.factory import ProviderSnapshot
from ruiclaw.runtime_context import (
...
from ruiclaw.security.workspace_access import (
    WorkspaceScopeResolver,
    bind_workspace_scope,
    reset_workspace_scope,
)
from ruiclaw.session import turn_continuation
from ruiclaw.session.automation_turns import automation_history_overrides
from ruiclaw.session.goal_state import goal_state_runtime_lines
from ruiclaw.session.history_visibility import HIDDEN_HISTORY_META
from ruiclaw.session.keys import UNIFIED_SESSION_KEY, remember_last_channel
from ruiclaw.session.manager import SESSION_CACHE_MAX_SIZE, Session, SessionManager
from ruiclaw.session.model_selection import (
...
from ruiclaw.session.summary import (
    SessionSummary,
    SessionSummaryCheckpoint,
)
from ruiclaw.triggers.local_turns import LocalTriggerTurnCoordinator
from ruiclaw.utils.cancellation import task_is_cancelling
from ruiclaw.utils.document import reference_non_image_attachments
from ruiclaw.utils.helpers import image_placeholder_text
from ruiclaw.utils.llm_runtime import LLMRuntime
from ruiclaw.utils.progress_events import output_events
from ruiclaw.utils.runtime import (
    EMPTY_FINAL_RESPONSE_MESSAGE,
)

if TYPE_CHECKING:
    from ruiclaw.config.schema import (
        ChannelsConfig,
        Config,
        ProviderConfig,
        ToolsConfig,
...
class AgentLoop:
    ...
    def __init__(
        ...
        self.tool_timeout_seconds = (
            ...
            else defaults.tool_timeout_seconds
        )
        ...
        # RUICLAW_MAX_CONCURRENT_REQUESTS: unset or <=0 means unlimited.
        _max = int(os.environ.get("RUICLAW_MAX_CONCURRENT_REQUESTS", "0"))
    ...
    def from_config(
        ...
        **extra: Any,
        ...
        """
        from ruiclaw.providers.factory import make_provider
    ...
    async def execute_user_shell_command(self, ctx: CommandContext) -> OutboundMessage:
        ...
        if tool is None:
            content = "Shell execution is disabled in this ruiclaw configuration."
        else:
            ...
            async def _to_user_message(pending_msg: InboundMessage) -> dict[str, Any]:
                ...
                if pending_msg.is_user_input:
                    ...
                    pending_request = RequestContext(
                        ...
                        parent_run_id=request_ctx.parent_run_id,
                    )
    ...
    async def _process_message(
        ...
        run_id = (
            inherited_run_id
            if is_internal_continuation
            and isinstance(inherited_run_id, str)
            and inherited_run_id
            else uuid4().hex
        )
        parent_run_id_value = msg.metadata.get(PARENT_RUN_ID_METADATA_KEY)
        parent_run_id = (
            parent_run_id_value
            if msg.sender_id == "system:recovery"
            and isinstance(parent_run_id_value, str)
            and parent_run_id_value
            else None
        )
        delivery.bind_run(
            turn_id=turn_id,
        ...
        if not ctx.ephemeral:
            scope = self.workspace_scopes.for_turn(
                channel=ctx.delivery.route.channel,
                message_metadata=ctx.msg.metadata,
                session_metadata=ctx.require_session().metadata,
            )
            ctx.run_ledger = RunLedger(
                runs_dir=scope.project_path / ".ruiclaw" / "runs",
                run_id=ctx.run_id,
                session_key=ctx.session_key,
                turn_id=ctx.turn_id,
                parent_run_id=ctx.parent_run_id,
                trigger=(
                    "resume"
                    if ctx.msg.sender_id == "system:recovery"
                    else "continuation"
                    if is_internal_continuation
                    else "user"
        ...
        try:
            ...
            await self._append_run_ledger(ctx, "context_built")
            ...
            await self._finish_run_ledger(ctx, "interrupted", "cancelled")
            raise
        except Exception:
            await self._finish_run_ledger(ctx, "failed", "internal_error")
            raise
        finally:
            if not turn_continuation.internal_continuation_pending(ctx.msg.metadata):
                status = "failed" if ctx.stop_reason in {"error", "tool_error"} else "succeeded"
                await self._finish_run_ledger(ctx, status, ctx.stop_reason or None)

    @staticmethod
    async def _append_run_ledger(ctx: TurnContext, event_type: str) -> None:
        if ctx.run_ledger is None:
            return
        try:
            await ctx.run_ledger.append(event_type)
        except Exception:
    ...
    async def _restore_turn(self, ctx: TurnContext) -> None:
        ...
        if ctx.session is None:
            ...
            elif ctx.ephemeral and not ctx.session_key.startswith("dream:"):
                ctx.session = self.sessions.get_or_create_transient(ctx.session_key)
            else:
    ...
    async def _run_turn(self, ctx: TurnContext) -> None:
        ...
        with capture_llm_calls(physical_calls), capture_message_deliveries() as message_sends:
