...
from ruiclaw.utils.helpers import _write_text_atomic  # pyright: ignore[reportPrivateUsage]
...
_CLIENT_URI = AnyHttpUrl("https://github.com/OctopusD/Ruiclaw")
_LOGO_URI = AnyHttpUrl(
    "https://raw.githubusercontent.com/OctopusD/Ruiclaw/main/"
    "webui/public/brand/ruiclaw_apple_touch.png"
)
...
class _RefreshingOAuthClientProvider(OAuthClientProvider):
    ...
    the upstream fix is available in ruiclaw's supported MCP version.
    """
    ...
    async def _initialize(self) -> None:
        ...
        self._apply_snapshot(await self._ruiclaw_storage.get_snapshot())
    ...
    async def _reload_after_lost_claim(self, lease: _RefreshLease) -> bool:
        snapshot = await self._ruiclaw_storage.get_snapshot()
    ...
    async def _handle_refresh_response(self, response: httpx.Response) -> bool:
        ...
        if response.status_code != 200:
            ...
            if error in {"invalid_grant", "invalid_client", "unauthorized_client"}:
                ...
                cleared = await self._ruiclaw_storage.finish_refresh(
                    lease,
                    None,
                    clear_client=clear_client,
                )
            ...
            if response.status_code == 404:
                await self._release_refresh_claim()
                await self._ruiclaw_storage.clear_oauth_metadata()
        ...
        committed = await self._ruiclaw_storage.finish_refresh(lease, token_response)
        await self._release_refresh_claim()
        if not committed:
            return await self._reload_after_lost_claim(lease)
    ...
    async def _refresh_token(self) -> httpx.Request:
        ...
        lock = self._ruiclaw_storage.refresh_lock()
        await lock.acquire()
        try:
            snapshot = await self._ruiclaw_storage.get_snapshot()
        ...
        if metadata is None or oauth_issuer is None or token_issuer != _normalize_issuer(oauth_issuer):
            try:
                await self._ruiclaw_storage.clear_tokens()
            finally:
                await lock.release()
            self.context.clear_tokens()
            self._token_issuer = None
    ...
    async def _perform_authorization(self) -> httpx.Request:
        ...
        if metadata is not None:
            ...
            await self._ruiclaw_storage.set_oauth_metadata(metadata, issuer=oauth_issuer)
        if self._refresh_after_discovery.get():
            ...
            await self._ruiclaw_storage.clear_tokens_and_client()
            self.context.clear_tokens()
            self.context.client_info = None
            self._token_issuer = None
            raise _RetryOAuthWithStoredCredentials
...
async def create_mcp_oauth_auth(
    ...
    metadata = OAuthClientMetadata(
        ...
        logo_uri=_LOGO_URI,
        software_id="https://github.com/OctopusD/Ruiclaw",
    )
