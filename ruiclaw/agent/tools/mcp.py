...
if TYPE_CHECKING:
    ...
    from ruiclaw.agent.tools.mcp_oauth import MCPOAuthHandlers
    from ruiclaw.config.schema import Config, MCPServerConfig
...
class MCPToolWrapper(_MCPWrapperBase):
    """Wraps a single MCP server tool as a ruiclaw Tool."""
    ...
    def _store_image_block(
        ...
        from ruiclaw.utils.artifacts import ArtifactError, store_generated_image_artifact
...
class MCPResourceWrapper(_MCPWrapperBase):
    """Wraps an MCP resource URI as a read-only ruiclaw Tool."""
...
class MCPPromptWrapper(_MCPWrapperBase):
    """Wraps an MCP prompt as a read-only ruiclaw Tool."""
        ...
        try:
            ...
            if cfg.auth == "oauth":
                if transport_type not in {"sse", "streamableHttp"}:
                    ...
                    return False
                from ruiclaw.agent.tools.mcp_oauth import (
...
def _configured_servers(config: Config) -> dict[str, MCPServerConfig]:
    from ruiclaw.agent.plugins import agent_plugin_mcp_servers
...
class MCPProvider:
    ...
    async def connect(self) -> None:
        ...
        async with self._lock:
            ...
            if oauth_servers:
                from ruiclaw.agent.tools.mcp_oauth import mcp_oauth_has_credentials
    ...
    async def reload(self) -> dict[str, Any]:
        ...
        async with self._lock:
            ...
            try:
                ...
                return {
                    "ok": False,
                    "message": "Could not reload MCP config. Restart ruiclaw to pick up changes.",
            ...
            if failed:
                ...
                message = "MCP connections refreshed without restarting ruiclaw."
            else:
                message = "MCP config reloaded without restarting ruiclaw."
