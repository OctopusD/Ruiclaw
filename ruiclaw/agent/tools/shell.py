...
from ruiclaw.agent.tools.base import Tool, ToolResult, tool_parameters
from ruiclaw.agent.tools.context import ToolContext, current_request_session_key
from ruiclaw.agent.tools.exec_session import (
...
from ruiclaw.agent.tools.sandbox import wrap_command
...
from ruiclaw.security.workspace_access import current_scope_allows_loopback, current_tool_workspace
from ruiclaw.security.workspace_policy import is_path_within

_IS_WINDOWS = sys.platform == "win32"
_PROCESS_TREE_OWNER_ATTR = "_ruiclaw_process_tree_owner"
...
class ExecTool(Tool):
    ...
    def __init__(
        ...
        self.deny_patterns = (deny_patterns or []) + [
            ...
            r":\(\)\s*\{.*\};\s*:",          # fork bomb
            # Block writes to ruiclaw internal state files (#2989).
    ...
    def _wrap_path_export(self, command: str, env: dict[str, str]) -> str:
        segments: list[str] = []
        if self.path_prepend:
            env["RUICLAW_PATH_PREPEND"] = self.path_prepend
            segments.append("$RUICLAW_PATH_PREPEND")
        segments.append("$PATH")
        if self.path_append:
            env["RUICLAW_PATH_APPEND"] = self.path_append
            segments.append("$RUICLAW_PATH_APPEND")
    ...
    def _create_windows_job() -> _ProcessTreeOwner:
        from ruiclaw.agent.tools._windows_job import WindowsJob
    ...
    def _guard_command(
        ...
        *,
        ...
        from ruiclaw.security.network import contains_internal_url
