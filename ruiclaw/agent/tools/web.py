...
from ruiclaw.utils.helpers import build_image_content_blocks
...
_VOLCENGINE_TRAFFIC_TAG = "ruiclaw"
...
def _resolve_url_safe(url: str) -> tuple[bool, str, tuple[str, ...]]:
    ...
    from ruiclaw.security.network import resolve_url_target
...
def _fetch_client_kwargs(proxy: str | None, timeout: float) -> dict[str, Any]:
    from ruiclaw.security.network import httpx_env_proxy_mounts
...
def _unsafe_url_request_error(exc: BaseException) -> str | None:
    from ruiclaw.security.network import UnsafeURLRequestError
...
class WebSearchTool(Tool):
    ...
    async def _search_olostep(self, query: str, n: int) -> str:
        try:
            ...
            return ToolResult.error(
                ...
                "Run `ruiclaw plugins enable olostep`."
            )
    ...
    async def _search_anysearch(self, query: str, n: int) -> str:
        ...
        headers = {
            "Content-Type": "application/json",
            "User-Agent": self.user_agent,
            "X-Anysearch-Client": "ruiclaw/1.0.0",
        }
        if api_key:
