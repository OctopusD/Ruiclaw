...
def _load_manifest(plugin_root: Path) -> AgentPlugin | None:
    ...
    ruiclaw_value = extension_payload.get("dev.ruiclaw")
    ruiclaw = cast(dict[str, object], ruiclaw_value) if isinstance(ruiclaw_value, dict) else {}
    return AgentPlugin(
        name=name,
        root=plugin_root,
        description=_string(payload.get("description")),
        repository=_string(payload.get("repository")),
        display_name=_string(ruiclaw.get("displayName")) or name,
        category=_string(ruiclaw.get("category")) or "Plugin",
        accent_color=_accent_color(ruiclaw.get("accentColor")),
        logo=_plugin_logo(ruiclaw.get("logo"), plugin_root),
        permissions=_string_tuple(ruiclaw.get("permissions")),
    )
...
def _plugin_logo(value: object, plugin_root: Path) -> str | None:
    """Resolve ruiclaw's optional packaged logo extension."""
    if value is None:
        return None
