...
from ruiclaw.agent.skills import SkillsLoader
from ruiclaw.agent.tools import image_generation as image_generation_tools
from ruiclaw.agent.tools import mcp as mcp_tools
from ruiclaw.agent.tools import sessions as session_tools
from ruiclaw.agent.tools.registry import ToolRegistry
from ruiclaw.agent.working_memory import WORKING_MEMORY_META, MemoryRetrieval, WorkingMemoryStore
from ruiclaw.apps.cli import utils as cli_app_utils
...
from ruiclaw.security.workspace_access import WorkspaceScopeResolver
from ruiclaw.session.keys import last_channel_from_metadata
from ruiclaw.session.manager import Session
from ruiclaw.session.summary import SessionSummary
from ruiclaw.utils.helpers import detect_image_mime, estimate_message_tokens, load_bundled_template
from ruiclaw.utils.prompt_templates import render_template

RUICLAW_CONTEXT_SOURCE_META = "ruiclaw_context_source"
RUICLAW_CONTEXT_SOURCE_WEIGHTS_META = "ruiclaw_context_source_weights"
_SYSTEM_SECTION_SEPARATOR = "\n\n---\n\n"
...
class ContextBuilder:
    ...
    @staticmethod
    def _system_source_weights(prompt: str) -> dict[str, int]:
        """Return content-only token weights for the system prompt's stable sections."""
        weights: dict[str, int] = {}
        for section in prompt.split(_SYSTEM_SECTION_SEPARATOR):
            if section.startswith("# Memory"):
                source = "memory"
            elif section.startswith("# Relevant Working Memory"):
                source = "working_memory"
            elif section.startswith(("# Active Skills", "# Skills")):
                source = "skills"
            elif section.startswith("[Archived Context Summary]"):
                source = "session_summary"
            else:
                source = "bootstrap"
            tokens = max(
                1,
                estimate_message_tokens({"role": "system", "content": section}) - 4,
            )
            weights[source] = weights.get(source, 0) + tokens
        return weights
    ...
    def __init__(
        ...
        self.working_memory_limit = max(0, working_memory_limit)
    ...
    def build_system_prompt(
        ...
        *,
        ...
        working_memory: MemoryRetrieval | None = None,
    ) -> str:
        ...
        if include_memory:
            ...
            if working_memory is not None and (rendered := working_memory.render()):
    ...
    def build_transcript(
        ...
        *,
        ...
        root = workspace or self.workspace
        retrieved_memory = (
            WorkingMemoryStore(root).retrieve(
                transcript.current_message,
                limit=self.working_memory_limit,
            )
            if include_memory
            and transcript.current_role == "user"
            and transcript.current_message
            else None
        )
        working_memory = (
            retrieved_memory
            ...
            and (
                retrieved_memory.candidate_count > 0
                or retrieved_memory.stale_rejected_count > 0
            )
            else None
        )
        ...
        messages: list[dict[str, Any]] = [
            {
                ...
                "_meta": {
                    RUICLAW_CONTEXT_SOURCE_WEIGHTS_META: self._system_source_weights(
                        system_prompt
                    ),
                    **(
                        {WORKING_MEMORY_META: working_memory.metadata()}
        ...
        internal_meta = dict(current.get("_meta") or {})
        internal_meta[RUICLAW_CONTEXT_SOURCE_META] = (
            "user_input" if transcript.current_role == "user" else "runtime"
        )
