...
from ruiclaw.bus.notification_delivery import notification_is_deliverable
...
from ruiclaw.bus.runtime_events import RuntimeEventPublisher
from ruiclaw.channels.notification_routes import notification_metadata
from ruiclaw.events import AgentEvent, EventSink
...
class TurnDelivery:
    ...
    turn_id: str | None = field(init=False, default=None)
    run_id: str | None = field(init=False, default=None)
    parent_run_id: str | None = field(init=False, default=None)
    ...
    def bind_run(
        ...
        *,
        ...
        """Bind the stable execution identity before lifecycle publication."""
        if self.run_id is not None:
            raise RuntimeError("turn delivery run identity is already bound")
