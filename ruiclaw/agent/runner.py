...
from ruiclaw.agent.context import (
    RUICLAW_CONTEXT_SOURCE_META,
    RUICLAW_CONTEXT_SOURCE_WEIGHTS_META,
    TranscriptInput,
)
from ruiclaw.agent.context_governance import (
...
from ruiclaw.agent.hook import (
    AgentHook,
    AgentHookContext,
    AgentRunHookContext,
    ModelRequestMetrics,
...
from ruiclaw.agent.working_memory import WORKING_MEMORY_META
from ruiclaw.events import NO_EVENTS, EventSink
from ruiclaw.llm_usage.context import (
    LLMUsageSource,
    bind_llm_usage_source,
    reset_llm_usage_source,
    source_from_session_key,
)
from ruiclaw.providers.base import (
    LLMProvider,
    LLMResponse,
    LLMUsage,
    ProviderConversationState,
)
from ruiclaw.providers.conversation_state import ProviderConversationStateController
from ruiclaw.runtime_context import (
    RUNTIME_CONTEXT_HISTORY_META,
    RUNTIME_CONTEXT_MESSAGE_META,
)
from ruiclaw.session.summary import SessionSummaryCheckpoint
from ruiclaw.utils.helpers import (
...
from ruiclaw.utils.prompt_templates import render_template
...
class AgentRunner:
    ...
    def _model_request_metrics(
        ...
        memory_stale_rejected_count = 0
        memory_selected: tuple[tuple[str, int, tuple[str, ...], str], ...] = ()
        for message in messages:
            role = message.get("role")
            bucket = role if role in role_tokens else "other"
            message_tokens = estimate_message_tokens(message)
            role_tokens[bucket] += message_tokens
            metadata_value = cast(object, message.get("_meta"))
            metadata = (
                cast(dict[str, Any], metadata_value)
                if isinstance(metadata_value, dict)
                else {}
            )
            source_value = cast(object, metadata.get(RUICLAW_CONTEXT_SOURCE_META))
            weights_value = cast(
                object,
                metadata.get(RUICLAW_CONTEXT_SOURCE_WEIGHTS_META),
            )
            retrieval_value = cast(object, metadata.get(WORKING_MEMORY_META))
            if role == "system" and isinstance(retrieval_value, dict):
                retrieval = cast(dict[object, object], retrieval_value)
                memory_candidate_count = self._non_negative_int(
                    retrieval.get("candidate_count")
                )
                memory_selected_count = self._non_negative_int(
                    retrieval.get("selected_count")
                )
                memory_stale_rejected_count = self._non_negative_int(
                    retrieval.get("stale_rejected_count")
                )
                memory_selected = self._memory_selections(retrieval.get("selected"))
            if role == "system" and isinstance(weights_value, dict):
                weights = {
                    source: weight
                    for source, weight in cast(dict[object, object], weights_value).items()
                    if isinstance(source, str)
                    and isinstance(weight, int)
                    and not isinstance(weight, bool)
                    and weight > 0
                }
                allocated = self._allocate_source_tokens(message_tokens, weights)
                for source, tokens in allocated.items():
                    source_tokens[source] = source_tokens.get(source, 0) + tokens
            else:
                source = (
                    source_value
                    if isinstance(source_value, str) and source_value
                    else self._default_context_source(role)
                )
                source_tokens[source] = source_tokens.get(source, 0) + message_tokens
            for key in (RUNTIME_CONTEXT_MESSAGE_META, RUNTIME_CONTEXT_HISTORY_META):
                marker_value = cast(object, metadata.get(key))
                if not isinstance(marker_value, dict):
                    continue
                marker = cast(dict[str, Any], marker_value)
                sources_value = cast(object, marker.get("sources"))
                if not isinstance(sources_value, list):
                    continue
                runtime_sources.update(
                    source
                    for source in cast(list[object], sources_value)
                    if isinstance(source, str) and source
                )
        tool_definition_tokens = 0
        tool_token_source = "none"
        if tool_definitions:
            tool_definition_tokens, tool_token_source = estimate_prompt_tokens_chain(
                object(),
                None,
                [],
                tool_definitions,
            )
        total_tokens = sum(role_tokens.values()) + tool_definition_tokens
        if tool_definition_tokens:
            source_tokens["tool_definitions"] = tool_definition_tokens
        token_source = f"message_estimate+{tool_token_source}"
        context_window = max(0, spec.runtime.context_window_tokens)
        input_budget = self.context_governor.input_budget(state.config)
        return ModelRequestMetrics(
            message_count=len(messages),
            system_tokens=role_tokens["system"],
            user_tokens=role_tokens["user"],
            assistant_tokens=role_tokens["assistant"],
            tool_result_tokens=role_tokens["tool"],
            other_tokens=role_tokens["other"],
            ...
            utilization_ratio=(
                round(total_tokens / context_window, 6)
                if context_window > 0
                else None
            ),
            compacted=state.last_request_compacted,
            runtime_context_sources=tuple(sorted(runtime_sources)),
            source_tokens=tuple(sorted(source_tokens.items())),
            memory_candidate_count=memory_candidate_count,
            memory_selected_count=memory_selected_count,
            memory_stale_rejected_count=memory_stale_rejected_count,
    ...
    @staticmethod
    def _memory_selections(
        value: object,
    ) -> tuple[tuple[str, int, tuple[str, ...], str], ...]:
        if not isinstance(value, list):
            return ()
        selections: list[tuple[str, int, tuple[str, ...], str]] = []
        for item_value in cast(list[object], value):
            ...
            if (
                ...
                continue
            reasons = tuple(
                reason for reason in cast(list[object], reasons_value)
                if isinstance(reason, str)
            )
            selections.append((memory_id, score, reasons, source))
        return tuple(selections)

    @staticmethod
    def _default_context_source(role: object) -> str:
    ...
    @staticmethod
    def _allocate_source_tokens(total: int, weights: dict[str, int]) -> dict[str, int]:
        """Allocate one message estimate proportionally while preserving its total."""
        weight_total = sum(weights.values())
        if not weights or weight_total <= 0:
            return {"bootstrap": total}
        allocations = {
            source: total * weight // weight_total
        ...
        for iteration in range(spec.max_iterations):
            ...
            if response.should_execute_tools:
                ...
                results, new_events = await execute_tool_calls(
                    ...
                    hook=hook,
                    context=context,
                    max_concurrency=spec.max_concurrent_tools,
                    timeout_s=spec.tool_timeout_seconds,
                )
    ...
    async def _request_model(
        ...
        *,
        ...
        context.model_request = self._model_request_metrics(
            spec,
            request_state,
            messages,
            tool_definitions,
        ...
        try:
            ...
            _pause_generation()
            await _close_native_reasoning()
            await hook.on_model_call_error(context, exc)
            raise
        except Exception as exc:
            _pause_generation()
            await _close_native_reasoning()
    ...
    async def _request_no_tools(
        ...
        *,
        ...
        assert request_state.messages is not None
        context.response = response
        context.usage = self._usage_or_estimate(
            spec,
            request_state.messages,
            response,
            tool_definitions=request_state.tool_definitions,
        )
        await hook.after_model_call(context)
        return response
