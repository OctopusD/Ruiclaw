...
from ruiclaw.agent.skills import parse_skill_metadata, valid_skill_metadata
from ruiclaw.apps.protocol import app_manifest, compact_dict
from ruiclaw.config.paths import get_runtime_subdir
from ruiclaw.security.workspace_policy import is_path_within
...
RUICLAW_EXTENSION_REGISTRY_URL = "https://raw.githubusercontent.com/Re-bin/ruiclaw-extension/main/registry.json"
RUICLAW_EXTENSION_RAW_BASE = "https://raw.githubusercontent.com/Re-bin/ruiclaw-extension/main"
_CATALOG_SOURCES = (
    ...
    ("extensions", RUICLAW_EXTENSION_REGISTRY_URL, RUICLAW_EXTENSION_RAW_BASE, False),
)
...
class CliAppManager:
    ...
    def _manifest_source(self, app: dict[str, Any]) -> str:
        ...
        if source == "extensions":
            return "ruiclaw-extension"
        return f"cli-anything:{source}"

    def _trust_registry(self, app: dict[str, Any]) -> str:
        return "ruiclaw-extension" if str(app.get("_source") or "") == "extensions" else "cli-anything"
        ...
        *,
        ...
        remove = compact_dict({
            ...
            "verification": (
                ...
                else ["ruiclaw_state_absent", "managed_paths_absent"]
            ),
        })
        return app_manifest(
            app_id=name,
    ...
    def _fallback_skill(self, app: dict[str, Any]) -> str:
        ...
        description = (_catalog_description(app) or f"Use {display} from ruiclaw.")[:1024]
        return f"""---
...
Use this skill when the user asks ruiclaw to operate {display} through its installed CLI app.
...
"""
    ...
    def _with_ruiclaw_skill_note(self, content: str, app: dict[str, Any]) -> str:
        name = str(app.get("name") or "unknown")
        skill_name = _skill_name(name)
...
"""
    ...
    def install_skill(self, app: dict[str, Any]) -> Path:
        ...
        content = self._with_ruiclaw_skill_note(content, app)
    ...
    def _record_installed(self, app: dict[str, Any]) -> dict[str, Any]:
        from ruiclaw.agent.plugins import set_agent_plugin_enabled
    ...
    def uninstall(self, name: str) -> dict[str, Any]:
        ...
        if strategy == "bundled" and still_available:
            message = (
                f"Removed {app['display_name']} from ruiclaw. {entry_point} "
                "is still available because it is managed outside ruiclaw."
            )
