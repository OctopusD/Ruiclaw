...
# re-exec as ruiclaw. Fail closed: if the privilege drop cannot be performed,
# exit rather than run the agent as root.
if [ "$(id -u)" = "0" ]; then
    chown -R ruiclaw:ruiclaw "$dir" 2>/dev/null || echo "[entrypoint] warning: chown $dir failed"
    if setpriv --reuid=ruiclaw --regid=ruiclaw --init-groups true 2>/dev/null; then
        echo "[entrypoint] dropping privileges to ruiclaw via setpriv"
        exec setpriv --reuid=ruiclaw --regid=ruiclaw --init-groups ruiclaw "$@"
    fi
...
Fix (pick one):
  Host:   sudo chown -R 1000:1000 ~/.ruiclaw
